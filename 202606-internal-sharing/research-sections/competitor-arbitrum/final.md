...
promotion_metadata:
  promoted_at: "2026-05-24T06:10:00+08:00"
  promoted_from_draft: "202606-internal-sharing/research-sections/competitor-arbitrum/drafts/round-1.md"
  approved_draft_commit: "9ee84721980cbec648a8226beabd3f7211078ca9"
  approval_evidence: "Review Verdict approve comment c3b659d5-4434-47c7-a928-3367c82805b4"
  final_promotion_changes:
    - "Documented exact GitHub crawl reproduction env: ARB_ACTIVITY_UNTIL=2026-05-23T21:32:40Z."
    - "Clarified that PR category counts are directional rule-based tags, not a precise taxonomy."
---
...
结论置信度：GitHub ranking / PR metric 为 **high**；代表 PR 分类为 **medium-high**（基于 title/body/recent PR manual spot-check，不是全量 diff review）；PR category totals 为 **directional**（规则标签受 PR template / checklist / test / release 词影响，不能当精确 taxonomy）；Stylus/Orbit/Timeboost/BoLD 官方叙事为 **high**；生态采用与竞争归因为 **medium**，因为公开 GitHub 和 docs 无法证明内部 staffing 或商业 pipeline。

## 2. Item Findings

### item-1: GitHub org universe discovery and inclusion rules

**执行口径。** 使用 GitHub REST API via `gh` token，对 `OffchainLabs` 与 `ArbitrumFoundation` 做 `/orgs/{org}/repos?per_page=100&type=all` 分页扫描，记录 repo metadata、topics、language、archived/fork/template、default branch、stars/forks、created/pushed/updated time。随后对 2026-02-24 之后 pushed 的非 archived / 非 fork repo 拉取 PR、commits、release 和 open backlog 指标。抓取脚本和原始结果已随本 final 存放在 `source-data/github-activity/`。[G1][G2]

**精确复现说明。** `metadata.json` 与本文使用的 crawl window end 是 `2026-05-23T21:32:40Z`。由于 `collect-github-activity.mjs` 的内置默认 `WINDOW_END` 早于该时点，精确复现本数据集时必须显式设置环境变量后运行：`ARB_ACTIVITY_UNTIL=2026-05-23T21:32:40Z node collect-github-activity.mjs`。
...
- `open_backlog_all_age`: 抓取时所有 open PR，不限创建时间。**该字段只报告 backlog，不进入 score**，以响应 review guardrail。
...
**分类口径 caveat。** 下表 category counts 是方向性标签，不是精确 taxonomy。规则分类会受 PR body/template 中的 checklist、test、release、workflow 等通用词影响；例如 docs PR 和 chain-sdk feature PR 可能被归到 `observability-infra-ci-release`。这会降低 category totals 的精度，但不影响 raw leaderboard、merge/open/backlog 指标，也不影响人工挑选的代表 PR 样本。
...
| src-9 competitor_sources | Met via existing internal competitor final sections plus official Optimism/ZKsync/Starknet docs checked during research |
...
| src-12 industry_commentary | Light; this section intentionally prioritizes primary docs/GitHub and existing internal research |
...
3. **Open PR status caveat**: BoLD/MEL/Timeboost-related PRs often appear as open, draft, or DO NOT MERGE. 本文 avoids shipped claims unless release/tag evidence exists.
...
- [G5] `source-data/github-activity/category-counts.tsv` - directional rule-based PR category tags; not a precise taxonomy.
...
| 1 | final-promotion-caveats-addressed | Documented exact `ARB_ACTIVITY_UNTIL=2026-05-23T21:32:40Z` reproduction command and clarified that category counts are directional tags. |
