...
final_metadata:
  ...
  promoted_at: "2026-05-23T11:36:00+08:00"
  mode: lightweight_final_promotion
  approved_draft_path: 202606-internal-sharing/research-sections/competitor-base/drafts/round-1.md
  approved_draft_round: 1
  approved_draft_commit: da0914e0fa5b8ff0c1826686855a2acb3b94df0e
  approval_evidence: "Multica comment 5923f17e-3504-43fd-b220-ac1a2dd76d21 (Review Verdict: draft-approved, severity: minor)"
  branch: research/202606-internal-sharing/competitor-base
  finalization_caveats_preserved:
    - "Source Coverage and Gap Analysis caveats retained, including capped PR sample and absence of official 5K TPS primary source."
    - "Beryl / Token Factory / Policy Registry remain merged-code behind activation gates, not launched capability."
    - "5K Peak TPS remains reported/benchmark/roadmap only and is separated from sustained mainnet TPS and Flashblocks latency."
    - "Live on-chain metrics are flagged as requiring refresh before live presentation use."
  promotion_notes:
...
Base 的近期 PR 可以按"功能成熟度 + 叙事价值"分类，而不是按标题关键词直接归因。下表是本 section 的分类矩阵。
...
本 section 未找到 primary source 可证明 "5K Peak TPS" 是 Base mainnet sustained TPS。可以证明的是 Base 近期围绕性能做了大量工程，以及既有内部研究显示 Base 主网用户交易吞吐曾显著高于 Mantle 样本。
...
1. **Outline status mismatch**: persisted outline frontmatter is `status: candidate`, while Orchestrator consensus approved the outline. This final section proceeds because the issue state transition controls the workflow; final promotion should not edit the outline unless Orchestrator asks.
