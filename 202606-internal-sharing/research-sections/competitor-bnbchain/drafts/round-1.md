...
draft_metadata:
  ...
  primary_data_source: "GitHub API via gh CLI, BNB Chain official blog, BEP full-text, web research"
  ...
  source_requirements_coverage: ["src-1: covered", "src-2: covered", "src-3: covered (BEP-673/BEP-658 全文确认)", "src-4: covered (含路线图/SDK主网公告)", "src-5: partially covered (web research 二手数据)", "src-6: partially"]
...
同期，**AI Agent** 赛道（`bnbagent-sdk` 30 PRs + `bnbchain-mcp` 46 PRs）呈现"叙事先行、工程跟进"特征：BNBAgent SDK 已于 2026 年 5 月在 BSC 主网上线，累计 150,000+ ERC-8004 agents 注册，Agent Survival Pack 联合 6 家合作伙伴推出。但代码成熟度仍有限（bnbchain-mcp 89% close 率）。**opBNB L2** 活跃度极低（`opbnb` 3 PRs + `op-geth` 8 PRs），Laplace 硬分叉仍处 open PR 阶段。**Greenfield** 存储层保持稳定维护（`greenfield` 8 PRs），但 `node-real` org 活跃度几乎为零，显示 NodeReal 已淡出一线开发。BNB Chain 2026 路线图未提及 Greenfield 和 opBNB 具体发展目标。

**重要勘误**：经 BEP-673 全文确认，Pasteur 硬分叉范围比此前预期**小得多**，仅包含 BEP-682（CometBFT 验证器唯一性）和 BEP-675（builder-proposed blocks）。BEP-670（250ms）、BEP-667（vote interval）、BEP-677（EIP-8056）等为独立 Draft BEP，不在 Pasteur 范围内。

BSC 生态链上指标：TVL ~$8.66B，DAU ~4M；opBNB TVL ~$19.2M，DAU ~1.5M。对 Mantle 的核心竞争压力来自 BSC 的性能口径（250ms 出块目标）、reth 双客户端战略的客户端多样性叙事、以及 Binance 交易所背景的生态导流能力（$8.66B TVL 中相当部分来自 CeFi 导流）。但 BSC L1 与 Mantle L2 架构根本不同（Parlia PoSA vs OP Stack），性能参数不可直接对标。
...
| **核心 BEP** | BEP-658（hardfork meta），包含 9 个子 BEP |
| **implementation_status** | **mainnet-active**（已于 2026-04-28 激活） |
| **evidence_confidence** | primary-verified（release notes + PR #3610 + BEPs #672 status update + BEP-658 全文） |

**Mendel 包含的 BEP 详表**：

| BEP/EIP | 名称 | 类型 | 说明 |
|---------|------|------|------|
| BEP-652 | MEV bid gas check / Hard Gas Ceiling | BSC 特有 | 硬编码 gas 上限 16,777,216，限制 MEV builder 的 gas 消耗 |
| BEP-655 | Bid block size check | BSC 特有 | 限制 builder block 大小 |
| BEP-657 | Limit blob tx by block number | BSC 特有 | 按区块号限制 blob 交易 |
| EIP-7692 | EOF 套件 | Ethereum Osaka 对齐 | EVM Object Format 相关 |
| EIP-7918 | Blob base fee capped at data gas limit | Ethereum Osaka 对齐 | **BSC 中禁用**（通过 BEP 参数跳过） |
| EIP-2537 | BLS12-381 precompile | Ethereum Osaka 对齐 | BLS 预编译 |
| EIP-7691 | Blob throughput increase | Ethereum Osaka 对齐 | Blob 吞吐量提升 |
| - | secp256r1 gas 成本调整 | BSC 特有 | 验证 gas 成本优化 |
| - | In-memory voting pool for fast finality | BSC 特有 | 快速最终性投票池内存化优化 |
...
| **BEP-673 明确包含的 BEP** | **仅 2 个**：BEP-682（硬分叉 required）、BEP-675（非硬分叉 optional） |
| **相关但未纳入 Pasteur 的 Draft BEP** | BEP-667、BEP-670、BEP-677、BEP-684 — 均为独立 Draft BEP，BEP-673 未将其列入 Pasteur 范围 |
| **计划时间线** | **尚未公布具体激活时间** |
| **implementation_status** | **spec-merged / 部分 open-pr** |
| **evidence_confidence** | primary-verified（BEP-673 全文确认仅列 BEP-682 和 BEP-675） |

**⚠️ 重要勘误**：BEP-673（Pasteur hardfork meta）全文仅列出 2 个 BEP：
- **硬分叉 Required**：BEP-682（Enforce Unique Validators in CometBFT Light Block Validator）
- **非硬分叉 Without Hard Fork**：BEP-675（Builder-Proposed Block with Validator Blind Signing）

BEP-667（Vote Interval）、BEP-670（250ms blocks）、BEP-677（EIP-8056）、BEP-684（Decouple voting）虽然作为独立 Draft BEP 存在，但**不在 BEP-673 的 Pasteur 范围内**。这些可能在未来硬分叉中纳入，但不应与 Pasteur 混淆。

**Pasteur BEP 实现进度表（严格按 BEP-673 范围）**：

| BEP | 名称 | 类型 | BEP 状态 | bsc 实现 PR | 实现状态 |
|-----|------|------|---------|------------|---------|
| BEP-682 | Unique Validators in Light Block | 硬分叉 Required | MERGED | #3623 (OPEN, Pasteur 标签) | **open-pr** |
| BEP-675 | Builder-Proposed Block with Validator Blind Signing | 非硬分叉 | MERGED | #3691 (OPEN) | **open-pr** |

**相关独立 Draft BEP（非 Pasteur 范围）**：

| BEP | 名称 | BEP 状态 | bsc 实现 PR | 实现状态 | 说明 |
|-----|------|---------|------------|---------|------|
| BEP-670 | 250ms blocks Phase 4 | MERGED | 无独立 PR | **spec-only** | 未列入任何 hardfork meta |
| BEP-677 | EIP-8056 Scaled UI Amount | MERGED | 无 bsc PR | **spec-only** | 未列入任何 hardfork meta |
| BEP-667 | Vote Interval | MERGED | #3589 (CLOSED) | **closed/deferred** | 实现 PR 已关闭 |
| BEP-684 | Decouple voting from execution | OPEN | 无 | **spec-draft** | BEP 本身仍为 draft |

**关键发现**：Pasteur 硬分叉范围比此前预期**小得多**，仅含 BEP-682 和 BEP-675。BEP-667 的 bsc 实现 PR #3589 于 2026-04-28 关闭，BEP-670（250ms blocks）无实现 PR。Pasteur 没有确定激活时间，实际客户端代码正在推进但范围有限。
...
### 9.4 BNBAgent SDK 主网上线与 Agent Survival Pack

**2026 年 5 月重大进展**：

| 事件 | 时间 | 说明 |
|------|------|------|
| BNBAgent SDK BSC 主网上线 | 2026-05 | SDK 已部署至 BSC 主网，包含 Identity（ERC-8004 agent 注册）、Payments、Memory 三大模块 |
| Agent Survival Pack 发布 | 2026-05-25 | 与 6 个生态伙伴联合推出的 AI Agent 开发者工具包 |
| ERC-8004 Agent 部署量 | 截至 2026-04 | 超过 150,000 个 ERC-8004 agents 已部署（BNB Chain 官方数据） |

**implementation_status**: **mainnet-active**（SDK 已在 BSC 主网可用）。

**判断修正**：BNBAgent SDK 已从测试网推进至主网部署，速度快于此前基于代码活动的预判。但 150,000 agents 部署量需审慎看待 — ERC-8004 agent 注册成本极低（单笔交易），高注册量不等于高使用量或高质量。Agent Survival Pack 作为生态推广活动，更多反映 BD/生态运营投入而非工程深度。

### 9.5 工程实质 vs 叙事驱动判断

**判断：AI Agent 赛道呈现"叙事先行、工程跟进"模式，已有主网部署但代码成熟度仍有限。**

| 维度 | 评价 | 证据 |
|------|------|------|
| 代码成熟度 | **低-中** | 频繁 breaking changes、合约重部署、仅 2-3 名开发者；但已推进至主网 |
| BEP 提案 | **活跃** | BAP-692 范围宏大但仍为 OPEN draft |
| 生产用例 | **早期主网** | SDK 已在 BSC 主网可用，150,000+ agents 注册，但使用深度未知 |
| 开发者采用 | **早期增长** | Agent Survival Pack 联合 6 家合作伙伴；外部 PR 仅 3 个 |
| 叙事声量 | **最高** | BNB Chain 官方推广 AI Agent 概念，Agent Survival Pack 为重点活动 |
| bnbchain-mcp 质量 | **极低** | 89% PR close 率，实际有效工作量极小 |

**narrative_signal**: narrative-heavy with early mainnet traction。叙事驱动特征仍然明显，但已超越纯叙事阶段。

**evidence_confidence**: primary-verified（代码活动）+ web-verified（SDK 主网上线、Agent Survival Pack）+ inferred（叙事判断）。
...
### 11.3 BSC 生态链上指标（参考数据）

| 指标 | BSC L1 | opBNB L2 | 数据来源 | 时间点 |
|------|--------|---------|---------|--------|
| TVL | ~$8.66B | ~$19.2M | DeFiLlama / web research | 2026-05 |
| DAU | ~4M | ~1.5M | BNB Chain 官方 / web research | 2026-05 |
| 峰值日交易数 | 31M（2025-10） | - | BNB Chain 官方数据 | 2025-10 |
| 稳定币市值 | ~$5.5B | - | web research | 2026-05 |

**⚠️ 数据限定**：以上链上指标来自 web research 二手数据，非直接抓取链上 RPC 或 Dune。TVL 数据波动较大，仅作量级参考。BSC L1 的 TVL 和 DAU 不可与 Mantle L2 直接对标 — 两者生态定位、用户来源（Binance CeFi 导流 vs 独立 DeFi 生态）存在根本差异。

### 11.4 BNB Chain 2026 路线目标

据 BNB Chain 官方 2026 路线图（web research）：

| 目标 | 时间框架 | 状态 | 说明 |
|------|---------|------|------|
| 20,000 TPS | 2026 | 目标 | BSC L1 吞吐量目标 |
| Sub-second finality | 2026 | 部分实现 | 当前 450ms 出块 + fast finality；250ms 为下一步 |
| reth 生产验证者 | 2026 | 推进中 | reth-bsc beta，尚未生产部署 |
| 下一代交易链（1M TPS） | 2026-2028 | 早期规划 | 超高性能链，细节未公布 |

**值得注意**：官方路线图中 **未提及 Greenfield 和 opBNB 的具体发展目标**，印证了这两个产品线在战略优先级中的下降。

### 11.5 代码活动 vs 叙事对齐度分析

最突出的不对齐是 **AI Agent**：官方叙事声量最高（Agent Survival Pack、150K agents 数据），代码成熟度有限但已推进至主网。相反，**reth 双客户端**工程投入最大（所有方向中人力最多），但叙事声量偏低 -- 这可能因为 reth 是基础设施升级，不如 AI Agent 有市场传播效果。

**Binance 交易所与 BSC 的生态绑定**仍是 BNB Chain 最大的结构性优势。BSC TVL $8.66B 和 4M DAU 中有显著部分来自 Binance CeFi 导流，这不是工程手段可以复制的资产，也意味着 BNB Chain 的用户/TVL 叙事不能直接归因于技术优势。
...
| AI Agent SDK 有成熟生产用例 | **partially supported** | SDK 已在 BSC 主网上线，150K+ agents 注册，但注册成本极低，使用深度和质量未知 |
| Greenfield 与 AI 集成 | **unsupported** | 无代码证据 |
| opBNB TVL/用户数据 | **partially supported** | web research 二手数据：opBNB TVL ~$19.2M, DAU ~1.5M；未直接链上验证 |
...
| src-3 | bep_proposals | **covered** | BEP-670, BEP-675, BEP-677, BEP-667, BEP-673（全文确认 Pasteur 仅含 BEP-682 + BEP-675）, BEP-682, BEP-684, BAP-692, BEP-658（Mendel meta 全文） |
| src-4 | official_bnbchain_docs | **covered** | Release notes for v1.7.1/v1.7.2/v1.7.3 via `gh release view`; 2026 路线图、BNBAgent SDK 主网公告、Agent Survival Pack 公告 via web research |
| src-5 | on_chain_data | **partially covered** | BSC TVL ~$8.66B, DAU ~4M, 峰值 31M txns/day; opBNB TVL ~$19.2M, DAU ~1.5M; 稳定币市值 ~$5.5B — 均来自 web research 二手数据，非直接链上抓取 |
...
|-----|----------|--------|-----------|
| BEP-673/BEP-658 全文已确认，其余 BEP 未逐一全文阅读 | low | BEP-673 Pasteur 范围已修正；其他 BEP 可能遗漏技术细节 | 在 review 后按需补充 |
| 链上数据为二手来源 (src-5) | medium | TVL/DAU/TPS 数据来自 web research 而非直接链上抓取，可能存在时间延迟或统计口径差异 | 建议在 review 后用 DeFiLlama API 或 Dune 验证关键数字 |
| 官方路线图为 web research 二手引用 | low | 2026 路线目标（20K TPS、下一代交易链）来自二次来源，可能不完整 | 可在修订轮验证官方公告 |
...
|-------|--------|---------|--------|
| 1 | initial draft + enhancement | 完成全部 13 个 outline items 的数据采集和分析；产出 8+ 个图/表；覆盖 src-1/src-2 fully, src-3/src-4 covered (BEP-673/BEP-658 全文、路线图、SDK 主网公告), src-5 partially covered (web research 二手链上数据), src-6 partially。增强内容：Pasteur BEP 范围勘误（BEP-673 全文确认仅含 BEP-682+BEP-675）、Mendel BEP 详表、BSC/opBNB 链上指标、BNBAgent SDK 主网上线 + Agent Survival Pack、2026 路线目标 | Orchestrator dispatch 1cc1f5ab-068d-4f0b-b9ee-e706ceaba592 |
