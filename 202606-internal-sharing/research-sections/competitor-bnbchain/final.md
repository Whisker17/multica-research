...
artifact_paths:
  ...
  draft: "202606-internal-sharing/research-sections/competitor-bnbchain/drafts/round-2.md"
...
| **核心 BEP（BEP-673 定义）** | BEP-682 (unique validators), BEP-675 (builder blocks) |
| **计划时间线** | **尚未公布具体激活时间** |
| **implementation_status** | **spec-merged / 部分 open-pr** |
| **evidence_confidence** | primary-verified（BEP-673 Meta-Pasteur 全文 + BEPs repo + bsc PRs） |

**Pasteur 核心 BEP 实现进度表**（依据 BEP-673 Meta-Pasteur 定义，仅含 BEP-682 和 BEP-675）：
...
**近期相关独立 draft BEP**（非 Pasteur 硬分叉组成部分，但与 BSC 演进路线相关）：

| BEP | 名称 | BEP 状态 | bsc 实现 PR | 实现状态 | 说明 |
|-----|------|---------|------------|---------|------|
| BEP-670 | 250ms blocks Phase 4 | MERGED | 无独立 PR | **spec-only** | 独立 draft 标准，短出块间隔路线 |
| BEP-677 | EIP-8056 Scaled UI Amount | MERGED | 无 bsc PR | **spec-only** | 独立 draft 标准，Token 显示优化 |
| BEP-684 | Decouple voting from execution | OPEN | 无 | **spec-draft** | 独立 draft 标准，快速最终性方向 |
| BEP-667 | Vote Interval | MERGED | #3589 (CLOSED) | **closed/deferred** | 快速最终性方向，实现已关闭 |

**关键发现**：BEP-673（Meta-Pasteur）明确定义 Pasteur 硬分叉仅包含 BEP-682 和 BEP-675 两项。BEP-670、BEP-677、BEP-684 等虽与 BSC 演进相关，但不属于 Pasteur 硬分叉范围。BEP-667 的 bsc 实现 PR #3589 于 2026-04-28 关闭，可能需要重新设计。Pasteur 硬分叉目前没有确定的激活时间，两项核心 BEP 的客户端实现均处于 open-pr 状态。
...
| **Phase 4** | **250ms** | **BEP-670** | **spec-merged** | 独立 draft 标准（非 Pasteur 硬分叉组成），**无 bsc 实现 PR** |
...
| BEP | 标题 | 状态 | 说明 |
|-----|------|------|------|
| BAP-692 | BNBAgent SDK: Identity, Commerce, Payment, Memory | OPEN (draft BAP，非 BEP) | open/draft BAP（bnb-chain/BEPs PR #692），范围宏大 |
| BEP-677 | EIP-8056 Scaled UI Amount | MERGED | Token 标准（独立 draft 标准，非 Pasteur 硬分叉组成） |
...
**Pasteur 硬分叉**正在形成类似链条但尚未闭合（依据 BEP-673 定义，Pasteur 仅含 BEP-682 + BEP-675）：
BEP 提案 (#673 meta, #682, #675) -> bsc 客户端 (#3623, #3691 进行中) -> 后续环节未启动
...
- [ ] BSC Pasteur 硬分叉（BEP-682 + BEP-675）：#3623 和 #3691 merge 状态
- [ ] BSC 短出块间隔：BEP-670 (250ms) 独立 draft 标准的实现进展
...
| 2 | revision (CONSTRAINT-B + C carry-forward) | **CONSTRAINT-B 修复**：依据 BEP-673 Meta-Pasteur 定义，将 Pasteur 核心 BEP 修正为仅 BEP-682 + BEP-675；将 BEP-657、BEP-670、BEP-677、BEP-684 重新分类为独立 draft BEP，从所有 Pasteur 列表、跨 repo 链条引用中移除。**CONSTRAINT-C carry-forward**：BAP-692 保持 open/draft (BAP) 标注。**未回归项**：CONSTRAINT-A Mendel 日期不变；PR 数据时间戳不变；feature status 分离不变；node-real 定量数据不变；BSC L1 vs Mantle L2 架构限定不变 | Orchestrator revision request 286aa514-dcc5-4196-a88c-6e733ca2757d |
