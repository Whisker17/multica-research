...
draft_metadata:
  ...
  tracker_coverage: "unavailable in issue/repo attachments at draft time; see Source Coverage and Gap Analysis"
---
...
可复现 GitHub Search 快照显示，`ethereum-optimism/optimism` 在窗口内约有 1195 个 created PR，其中 merged 约 779、open 约 188、closed 约 1007；`ethereum-optimism/op-geth` 只有 21 个 created PR，其中 merged 12、open 4、closed 17。后续复核时 GitHub API 已达到 rate limit，因此本 draft 保留这些已抓取查询结果，并把重复查询失败列为 source gap。PR Tracker 每日报告在 issue 与仓库上下文中没有可访问路径或附件，本 draft 按 dispatch caveat 标记为 unavailable，没有伪造 tracker 覆盖。
...
本 draft 使用三层口径：第一层是已抓取的 GitHub Search 总数；第二层是本地缓存 PR metadata 样本；第三层是对代表 PR 对应的 docs/code 文件做文件级验证。PR Tracker 每日报告未暴露路径，因此未纳入计数。

**PR baseline（window: 2026-02-23..2026-05-23，date_verified: 2026-05-23）**

| Repo | Created PR | Merged | Open | Closed | 口径与限制 |
|---|---:|---:|---:|---:|---|
| `ethereum-optimism/optimism` | 1195 | 779 | 188 | 1007 | 来自已执行 GitHub Search 查询；closed 包含 merged；后续重复查询因 API rate limit 失败 |
| `ethereum-optimism/op-geth` | 21 | 12 | 4 | 17 | 来自已执行 GitHub Search 查询；窗口内可人工枚举全部 21 个 PR |

**本地 PR metadata 样本**

| Repo sample | 样本规模 | State split | 说明 |
|---|---:|---|---|
| `optimism-prs-1000.json` | 1000 / 1195 | MERGED 627, CLOSED 205, OPEN 168 | 最近 1000 个 PR 样本，不等于全量；用于分类和代表 PR 选择 |
| `op-geth-prs.json` | 21 / 21 | MERGED 12, CLOSED 5, OPEN 4 | 覆盖窗口内全部 op-geth PR |
...
本 draft 选取代表 PR 时优先使用同时满足以下条件的 PR：标题明确指向协议/运行时/升级路径；能在 upstream docs 或当前代码路径中找到对应实现；对 Mantle 迁移/兼容/叙事有直接影响。没有把纯标题关键词当作功能完成证据。
...
|---|---|---|---|
| src-1 PR Tracker reports | **Unavailable** | Issue references PR Tracker daily reports, but no report path or attachment was exposed in Multica issue comments or repo context | Not fabricated. Draft relies on GitHub/API snapshots + cached metadata + code/docs review |
| src-2 GitHub PR API | **Partial / sufficient for standard draft** | Prior GitHub Search totals: optimism 1195 created / 779 merged / 188 open / 1007 closed; op-geth 21 created / 12 merged / 4 open / 17 closed. Local cache has 1000 optimism PR metadata and all 21 op-geth PRs | Repeat API calls on 2026-05-23 03:01 UTC hit GitHub rate limit; author stats in cache missing login field |
| src-3 Code analysis | **Partial** | `/tmp/optimism-competitor-research` files: `op-supernode/README.md`, docs audit-source paths, `op-core/interop/depset/depset.go`, op-node interop config; `/tmp/op-geth-competitor-research/superchain/chain.go`, post-exec tx / engine API files referenced by PR metadata | Did not review every representative PR diff; used merged-head code and docs audit-source for file-level verification |
...
| src-5 Governance proposals | **Partial** | Upgrade 19 notice, Security Council / governance implications from prior Stage 1 internal research, L2CM / OPCMv2 docs references | Did not crawl full Optimism governance forum in this draft; conclusions about governance are conservative |
...
| src-7 Industry commentary | **Limited** | Primarily official Optimism docs plus internal Base research; no broad third-party media crawl | Acceptable for decision-useful engineering draft; market narrative should be refreshed before public use |
...
The persisted outline file still has frontmatter `status: candidate`; however, Multica issue comment `Review Verdict: outline-approved` and the deep-draft dispatch explicitly approve it for Phase B. Per squad protocol, approval is tracked by Orchestrator state rather than file path. This draft did not modify the outline metadata.

## 5. Gap Analysis

1. **PR Tracker daily reports are unavailable**. This is the largest evidence gap relative to the outline. The draft explicitly avoids tracker-derived trend claims such as day-by-day velocity, missing days, or tracker/API reconciliation.
2. **GitHub API repeat queries are rate-limited**. The draft uses earlier captured totals and local metadata caches. A reviewer with fresh API quota should rerun the exact search queries before final promotion if numeric precision matters.
...
6. **Mantle codebase-specific compatibility remains a follow-up engineering task**. This draft identifies watchlist items, but it does not inspect Mantle's live fork line by line. The next useful artifact for Mantle engineering would be a compatibility matrix against Mantle's actual op-geth, op-node, op-contracts and bridge versions.
