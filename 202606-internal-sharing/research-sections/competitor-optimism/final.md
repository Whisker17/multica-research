...
final_metadata:
  ...
  promoted_at: "2026-05-23T23:39:08+08:00"
  ...
  approved_draft_commit: "ef11a0f1eb333d3f80ef68bd44545231226c5a35"
  approval_evidence: "Multica comment 1dcbe794-db56-4c4e-874c-9c4c0f3e9d2b (Review Verdict: approve, severity: minor)"
  branch: "research/202606-internal-sharing/competitor-optimism"
  reviewer_caveats_carried_forward:
    - "op-geth#791 is closed unmerged and must not be described as shipped op-geth behavior; op-geth/proxyd namespace migration remains a watch item."
    - "The interop namespace shift is real in the optimism monorepo via merged optimism#20778 / optimism#20905, while infra#623 remains open."
    - "Do not claim Base has left Superchain."
    - "Do not claim interop is mainnet-ready across all chains."
    - "Do not claim op-reth is a drop-in Mantle replacement without Mantle-specific validation."
  promotion_notes:
    - "Promoted approved round-1 draft to final.md."
    - "Applied the required op-geth/proxyd namespace wording correction from draft review."
draft_metadata:
...
3. **`op-geth` 仍活跃但更像 maintenance / compatibility surface，而不是新增战略重心。** 它在窗口内有 21 个 PR、12 个 merged PR、7 个 release；代表 PR 包括 engine API error refactor、superchain-registry bump、runtime images 等维护项。interop namespace shift 在 `optimism` monorepo 中是真实方向，但 `op-geth#791` 已 closed unmerged，当前 `op-geth` main 仍调用 `supervisor_checkAccessList`；因此 op-geth/proxyd namespace migration 应作为 watch item，而不是已 shipped 的 op-geth 行为。官方 docs notice 同时写明 `op-geth` / `op-program` 支持到 **2026-05-31**，并要求迁移到 `op-reth` / `cannon-kona`，这使 Mantle 必须把执行层迁移或自维护作为 P0 watch item。
...
| org / repo source | 状态 | 本 section 处理 |
|---|---|---|
...
|---|---|---|---|---|
| Superchain interop / dependency set | `optimism` PRs touch op-supernode/supervisor/interopgen; interop namespace shift is real in monorepo, but op-geth/proxyd namespace migration remains watch item; devnet configs | [`optimism#20778`](https://github.com/ethereum-optimism/optimism/pull/20778), [`optimism#20905`](https://github.com/ethereum-optimism/optimism/pull/20905), [`optimism#20991`](https://github.com/ethereum-optimism/optimism/pull/20991), [`op-geth#791`](https://github.com/ethereum-optimism/op-geth/pull/791), [`infra#623`](https://github.com/ethereum-optimism/infra/pull/623), [`devnets#333`](https://github.com/ethereum-optimism/devnets/pull/333), [`devnets#334`](https://github.com/ethereum-optimism/devnets/pull/334), [interop specs](https://specs.optimism.io/interop/overview.html) | `optimism#20778/#20905` merged; `op-geth#791` closed unmerged; `infra#623` open; devnet/config active; not all mainnet-ready | direct watch; affects RPC namespace, EL/CL assumptions, operator packaging |
...
| op-geth / execution maintenance | 21 PRs, 7 releases; engine API/refactor, registry bump, runtime images; namespace migration is watch item, not shipped op-geth behavior | [`op-geth#790`](https://github.com/ethereum-optimism/op-geth/pull/790), [`op-geth#788`](https://github.com/ethereum-optimism/op-geth/pull/788), [`op-geth#794`](https://github.com/ethereum-optimism/op-geth/pull/794), [`op-geth#791`](https://github.com/ethereum-optimism/op-geth/pull/791), [`op-geth releases`](https://github.com/ethereum-optimism/op-geth/releases) | maintenance-only / compatibility; `op-geth#791` closed unmerged | critical for Mantle if still op-geth-derived |
...
- `optimism` monorepo merged the interop namespace shift in [`optimism#20778`](https://github.com/ethereum-optimism/optimism/pull/20778) / [`optimism#20905`](https://github.com/ethereum-optimism/optimism/pull/20905). However, [`op-geth#791`](https://github.com/ethereum-optimism/op-geth/pull/791) is closed unmerged, current `op-geth` main still calls `supervisor_checkAccessList`, and [`infra#623`](https://github.com/ethereum-optimism/infra/pull/623) remains open. Treat op-geth/proxyd namespace migration as a watch item, not shipped op-geth behavior.
...
Interpretation: This is the strongest engineering-heavy direction. It is not just a blog narrative: code paths touch CL/EL coordination, access-list checks, engine-controller lifecycle, devnet configs, and operator workflows. However, many PRs are open, closed-unmerged, or devnet-only, so this final section avoids phrasing this as fully mainnet active across Superchain.

Mantle implication: track namespace/API changes (`interop_*` vs `supervisor_*`) as a pending compatibility risk, not a completed op-geth migration; also track dependency-set semantics, supervisor failure modes, and op-supernode packaging. Any Mantle interop narrative should be gated on devnet test harness and explicit user-facing product commitment.
...
- **Superchain interop:** specs pages exist, devnets are active, op-supernode/supervisor PRs are active, and `optimism` monorepo has merged interop namespace work. The op-geth/proxyd namespace migration is still a watch item because `op-geth#791` is closed unmerged and `infra#623` remains open.
...
- **Full interop product readiness:** devnet and code evidence is strong, but this section distinguishes devnet/testnet/mainnet and avoids over-promising user-facing readiness.

**Evidence map.**

| Narrative | GitHub evidence | Official/public evidence | Final judgment |
|---|---|---|---|
| Superchain interop | `optimism#20991`, merged `optimism#20778/#20905`, closed-unmerged `op-geth#791`, open `infra#623`, `devnets#333/#334`, specs interop pages | Optimism docs notices and specs | strong, active development; op-geth/proxyd namespace migration remains watch item |
...
| interop access-list / supervisor namespace | RPC/API and validation changes can break derived stacks | monorepo `interop_*` shift, current op-geth `supervisor_checkAccessList`, proxyd namespace PR status, dependency set, op-supernode failure modes |
...
| Base independence overstated | Final distinguishes technical/client independence from Superchain membership/governance |
...
| `op-geth` | execution maintenance | namespace migration watch item | `#790`, `#791`, `#788` | merged/open/closed mix; `#791` closed unmerged | critical |
| `devnets` | interop-jnt-v2 devnet status/config | op-deployer artifacts | `#333`, `#334` | merged | high for test harness |
| `infra` | proxyd / infra / CI | interop namespace watch item and consensus tracker | `#623`, `#622`, proxyd releases | `#623` open; mixed open+merged | medium-high |
...
timeline
    ...
    2026-05 W20-W21 : optimism monorepo PR peak, op-supernode/op-reth/op-deployer PRs
                    : op-geth/proxyd namespace migration watch item
...
flowchart LR
    ...
    RPC --> InteropFilter[interop_* / supervisor_* namespace compatibility watch]
...
|---|---|---|---|---|
| Must track | op-geth EOL, op-reth/kona releases, op-geth/proxyd interop namespace watch item, op-contracts v7/SCR | critical | high | protocol/client lead |
...
| Mantle production config not inspected in this section | high | Mantle recommendations are watch/POC/gate items, not implementation assertions |
