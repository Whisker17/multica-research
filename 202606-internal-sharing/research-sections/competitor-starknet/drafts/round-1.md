...
# StarkNet 近期开发与叙事分析 - Round 1 Draft
...
| ID | Source |
|---|---|
| S1 | GitHub GraphQL / gh activity crawl, 117 repo scoring set, window 2026-02-23..2026-05-23, fetched 2026-05-23; local scratch file `/tmp/starknet-research-graphql/graphql-metrics.json` and leaderboard TSV generated during this run |
...
1. **Full long-tail crawl gap**：完整 235-repo REST crawl 曾因网络重置中断，最终 ranking 用 117-repo candidate pass。它足以支持 Top active shortlist，但不满足审计级"每个 seed repo 指标全量粘贴"。若 final 需要更强可复核性，应把 raw JSON/TSV 作为 repo artifact 持久化或重跑完整分页。
