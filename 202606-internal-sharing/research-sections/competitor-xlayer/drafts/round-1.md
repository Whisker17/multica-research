...
artifact_paths:
  ...
  draft: "202606-internal-sharing/research-sections/competitor-xlayer/drafts/round-1.md"
...
draft_metadata:
  ...
  created_at: "2026-05-26T07:00:00+08:00"
  mode: initial_draft
  ...
  github_query_timestamp_utc: "2026-05-26T06:50:00Z"
  source_caveats:
    - "GitHub 数据仅覆盖公开仓库；OKX 内部私有仓库和 monorepo 不可见。"
    - "GitHub REST API per_page=100 限制，commit/PR 计数超过 100 的仓库为截断值（标注 100+）。"
    ...
    - "TVL 数据来自 L2BEAT TVS $10.23M（2026-05-26），不含 Aave/Uniswap 部署后的 DeFi TVL 增量（需 DeFiLlama 交叉验证）。"
  items_covered: [item-1, item-2, item-3, item-4, item-5, item-6]
  diagrams_produced: [diag-1, diag-2, diag-3, diag-4]
---

# X Layer 近期开发与叙事分析 — Draft Round 1
...
2. **GitHub 活动集中在基础设施 fork、AI/Agent 和支付协议三个方向。** `okx` org 近 3 个月最活跃仓库为 `optimism`（100+ commits/41 PRs）、`xlayer-reth`（88 commits/100 PRs）、`onchainos-skills`（100+ commits/26 PRs）、`op-succinct`（61 commits/10 PRs）、`xlayer-toolkit`（47 commits/54 PRs）和 `base-contracts`（44 commits，ERC-8183 实现）。`okx-xlayer` 作为独立 org 不存在，所有开发集中在 `okx` org 下。
...
4. **APP 协议将 Agent 支付从"单次转账"升级到"完整商业关系"。** 2026 年 4 月发布白皮书，定义四种支付意图（charge 即时转账、escrow 托管、session 流式支付、upto 预授权计量），通过 Broker 角色进行链下编排 + 链上结算，传输层与 HTTP 解耦（支持 IM/QR/离线），采用 MPP EVM 线格式并与 x402 结构对齐。ERC-8183 定义 Agent commerce 的 Job escrow 原语。

5. **OKX 生态整合展现了交易所 L2 的最强分发能力。** Uniswap 2026 年 1 月上线（零接口费）、Aave v3.6 2026 年 3 月上线（X Layer 第 21 条链）、ICE（NYSE 母公司）2026 年 3 月战略投资 OKX 估值 $25B（$200M 投资 + 董事会席位 + 代币化股票合作）、OKB 2025 年 8 月完成 6500 万代币一次性销毁固定供应量至 2100 万。
...
| 2026-01-15 | Uniswap 上线 X Layer | 零接口费，Preferred DEX 地位 |
...
**来源**: [X Layer Architecture Migration: From Polygon zkEVM to OP Stack](https://web3.okx.com/learn/x-layer-architecture-migration-from-polygon-zkevm-to-op-stack), [OKX PP Upgrade Announcement](https://www.okx.com/en-us/help/announcement-on-the-pp-upgrade-of-x-layer-and-optimisation-of-the-okb-gas), [Optimism Blog: OKX Migrates XLayer to the OP Stack](https://www.optimism.io/blog/okx-migrates-xlayer-to-the-op-stack)
...
扫描 `okx` GitHub Organization 下所有在 2026-02-26 之后有推送活动的公开仓库，并使用 GitHub REST API 获取近 3 个月的 commit 和 PR 计数。`okx-xlayer` 作为独立 Organization 不存在（返回 404），所有 X Layer 相关开发集中在 `okx` org 下。REST API 的 per_page=100 限制意味着超过 100 的计数值为截断值。
...
|------|-------|---------------|-----------|------|------|
| `optimism` | 5 | 100+ | 41 | Go | Optimism fork，X Layer 核心 OP Stack 实现 |
| `xlayer-reth` | 14 | 88 | 100+ | Rust | XLayer 定制 Reth 实现，针对 Optimism-based L2 优化 |
| `op-succinct` | 0 | 61 | 10 | Rust | SP1 ZK 证明引擎用于 OP Stack |
| `xlayer-toolkit` | 2 | 47 | 54 | Solidity | X Layer 开发与运维工具集 |
| `op-geth` | 7 | 100+ | 6 | Go | OP Stack 修改版 Geth |
| `reth` | 2 | 4 | 4 | Rust | Reth 上游 fork |

`optimism` 和 `op-geth` 的高 commit 活动反映了 OP Stack 迁移后的持续维护和同步上游工作。`xlayer-reth` 的 100+ PRs 表明 OKX 正在积极构建基于 Reth 的替代执行客户端（与 Base 的 `base-reth-node` 策略类似）。`op-succinct` 是 SP1 ZK 证明系统在 OP Stack 上的实现，表明 X Layer 虽已迁移到 Optimistic Rollup，但仍在探索 ZK 证明路径（Succinct 的 Production-Grade Proving Engine）。

**Tier 2: AI / Agent 方向**

| 仓库 | Stars | Commits (3mo) | PRs (3mo) | 语言 | 说明 |
|------|-------|---------------|-----------|------|------|
| `onchainos-skills` | 272 | 100+ | 26 | Rust | Onchain OS API 集成的 AI Agent Skills（钱包、代币发现、DEX swap、交易广播） |
...
AI/Agent 方向是 `okx` org 中社区关注度（stars）最高的类别。`onchainos-skills` 作为 Onchain OS 的核心 SDK，100+ commits 反映持续活跃开发。`agent-trade-kit`（312 stars）是 OKX 交易能力通过 MCP（Model Context Protocol）暴露给 AI Agent 的关键产品，代表了交易所 → AI Agent 的分发通道。
...
| 仓库 | Stars | Commits (3mo) | PRs (3mo) | 语言 | 说明 |
|------|-------|---------------|-----------|------|------|
| `xlayer-docs` | 962 | 12 | 21 | MDX | X Layer L2 文档（最高 stars 仓库） |
| `plugin-store` | 10 | 2 | 100+ | Rust | 插件商店 |
| `wallet-core` | 68 | 2 | 7 | — | 基于 EIP-7702 的 OKX Wallet Core 实现 |

`xlayer-docs`（962 stars）是 `okx` org 内与 X Layer 直接相关的最高 stars 仓库，21 PRs 反映文档持续更新。`wallet-core` 基于 EIP-7702 实现模块化、安全、gas 优化的智能合约钱包。
...
**关键观察：**
- 基础设施层活动最密集，反映了 OP Stack 迁移后的持续工程投入（包括 Reth 替代执行客户端和 ZK 证明探索）
- AI/Agent 方向社区关注度最高（stars 总和最大），但代码活跃度主要集中在 `onchainos-skills` 一个仓库
...
| 意图 | 使用场景 | 结算时机 | 争议支持 | 签名时金额已知 |
|------|----------|----------|----------|----------------|
| **charge** | 一次性转账（小费、固定价商品、单次 API 调用） | 即时 | — | 是 |
| **escrow** | 带交付验收的任务（自由职业、设计、代码） | 接受/争议解决后 | 是（可配置仲裁者） | 是 |
| **session** | 流式计量（LLM 按 token 计费、API 按调用计费） | 通道关闭时 | — | 否（单价已知） |
| **upto** | 有上限的预授权（"最多 5000 token"） | Seller 使用报告后 | — | 否（上限已知） |
...
- **定位**: x402 范围是单次 HTTP 402 往返，MPP 范围是 EVM 支付方法 wire format，APP 在相同基础上增加 escrow、争议解决和非 HTTP 传输
...
- 来源：[OKX: Why did OKB Burn?](https://www.okx.com/learn/why-did-okb-burn), [CryptoBriefing: OKB Tokenomics Upgrade](https://cryptobriefing.com/okb-tokenomics-upgrade-token-burn/), [KuCoin: OKX Burns $7.6B OKB](https://www.kucoin.com/news/articles/okx-burns-7-6b-worth-of-okb-slashing-supply-to-21m-as-x-layer-takes-center-stage)
...
X Layer 的 TVL 数据呈现出有趣的分层现象：
- L2BEAT TVS（Total Value Secured）仅 $10.23M
- CoinGecko 排名 X Layer 为第 32 大区块链（按 TVL）
- 根据报道，Aave 上线前 X Layer TVL 约为 $25M，Aave 部署带动了约 20% 的交易量增长

**注意**: L2BEAT 的 TVS 衡量桥接和原生铸造资产总和，DeFiLlama 的 TVL 衡量 DeFi 协议锁仓价值，两者口径不同。X Layer 的实际生态价值可能高于 L2BEAT TVS 所示。
...
graph TB
    ...
    subgraph Settlement["Settlement Layer"]
        ...
        S2["escrow<br/>审计托管合约<br/>dispute window"]
        S3["session<br/>流式支付通道<br/>累积 voucher"]
        S4["upto<br/>预授权上限<br/>双签名保护"]
    end
...
| src-2 | code_analysis | okx GitHub Organization 活跃 repo 的 PR/commit 数据扫描 | 6 | 15 repos scanned (optimism, xlayer-reth, onchainos-skills, op-succinct, xlayer-toolkit, base-contracts, op-geth, payments, mpp-specs, xlayer-docs, agent-trade-kit, agent-skills, wallet-core, plugin-store, reth) | ✅ 满足 |
...
| src-5 | on_chain_data | X Layer 网络数据（TVL、TPS、地址数）— L2BEAT、DeFiLlama | 2 | 2 (L2BEAT TVS $10.23M/UOPS, X Layer 官网 TPS/地址数/交易成本) | ✅ 满足（但 DeFiLlama 实时 TVL 未直接引用，标记为 gap） |
...
| gap-2 | DeFiLlama 实时 TVL 数据未直接引用，仅通过 L2BEAT TVS 和间接报道获得 | Minor | 直接抓取 DeFiLlama API 或页面数据 |
| gap-3 | Exchange OS 白皮书为 V1.0 愿景文档，合约接口、精确风控参数和参考实现不在范围内 | Minor | 等待 Exchange OS Protocol Specification 发布 |
| gap-4 | APP 白皮书同为 V1.0，wire-format schemas 和安全分析维护在独立 spec 中 | Minor | 等待 APP Specification 发布 |
| gap-5 | 私有仓库不可见，OKX 可能有大量未公开的 X Layer 工程活动 | Medium | 无法弥补，需在结论中注明 |
| gap-6 | Exchange OS 和 APP 均为近期发布（2026.04-05），尚未有主网上线后的实际运行数据 | Medium | Q3 Exchange OS 上线后可获取实际数据 |
| gap-7 | Mantle 侧数据未在本研究中深入收集，对比维度中 Mantle 信息较薄 | Minor | 参考同项目其他研究 section 补充 |
