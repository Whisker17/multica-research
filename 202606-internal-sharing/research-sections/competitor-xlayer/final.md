...
round: 2
status: final
promoted_at: "2026-05-26T09:20:00+08:00"
approved_draft_path: "202606-internal-sharing/research-sections/competitor-xlayer/drafts/round-2.md"
approved_draft_round: 2
approved_draft_commit: "b64cb96"
approval_evidence: "Review Verdict approve — comment 995bfc5b-bdb9-45c0-aa24-29084004a7da, Research Review Agent, severity: minor, 2026-05-26T09:12:11Z"
...
**扫描范围**: `okx` GitHub Organization 下在 2026-02-26 之后有推送活动的公开仓库。`okx-xlayer` 作为独立 Organization 不存在（返回 404），所有 X Layer 相关开发集中在 `okx` org 下。

> **仓库选取说明**: `okx` org 下活跃的公开仓库远不止本文所列的 14 个——org 中还包含 `sp1`、`sp1-cluster`、`ERCs` 等活跃仓库，但这些仓库与 X Layer / Onchain OS / APP 主题的关联性较低。本文 14 个仓库表为**与 X Layer 直接相关的精选子集**，筛选标准为：(1) X Layer 基础设施（OP Stack fork、执行客户端、运维工具）、(2) AI-Agent 工具（Onchain OS skills、Agent MCP server、Agentic Wallet）、(3) 支付协议（APP、ERC-8183、MPP）、(4) X Layer 文档与生态（xlayer-docs、plugin-store、wallet-core）。不在此四类中的通用 SDK、上游 fork 同步仓库和非 X Layer 链的工具不纳入分析范围。
...
#### 活跃仓库总览（X Layer / Onchain OS / APP 精选子集）

按功能方向分类，近 3 个月（2026-02-26 ~ 2026-05-26）`okx` org 下与 X Layer 直接相关的 14 个精选仓库如下：
...
| final | promotion | labeling | Final promotion：根据 Review Verdict approve（comment 995bfc5b，severity: minor）的建议，澄清 14-repo 表为 X Layer / Onchain OS / APP 相关精选子集而非 `okx` org 全部活跃仓库，明确筛选标准（基础设施、AI-Agent 工具、支付协议、文档生态） |
