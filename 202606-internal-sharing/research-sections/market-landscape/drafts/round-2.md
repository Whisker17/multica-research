...
artifact_paths:
  ...
  draft: "202606-internal-sharing/research-sections/market-landscape/drafts/round-2.md"
...
draft_metadata:
  ...
  original_draft_commit: "6e935f92df801f86266367867a472a7782346cbd"
  ...
  draft_gates:
    ...
    data_artifact_gate: "提供链特定 DuneSQL 查询（无占位符），committed .sql 文件，per-metric last-refreshed 时间戳，CSV-ready 图表数据（Mantle 行已填充或标注不可用原因），DAU/交易数据 bot/spam 过滤 caveat 紧邻图表"
...
在用户活跃度维度，分化更为剧烈：Base 以日均 38.3 万-66.3 万活跃地址和 1,289 万笔日交易领跑全部 L2；Arbitrum 约 13.3 万 DAU / 417 万笔日交易；OP Mainnet 约 8.2 万 DAU；zkSync Era 降至约 4,000 DAU / 1.96 万笔日交易。Mantle 日均约 5,000-6,000 活跃地址 / 80,000-85,000 日交易量（2025 Q4 Nansen 数据），在五链中排名第四，高于 zkSync Era 但远低于前三。
...
| Q4+Q5 | 新合约部署 + 活跃合约 | `queries/contracts-all-chains.sql` | 所有五链 |
...
│            │               │          │              │          │            │          │ confidence: med  │
├────────────┼───────────────┼──────────┼──────────────┼──────────┼────────────┼──────────┼──────────────────┤
│ Mantle     │ ~$1.15-1.2B   │ ~2-3%    │ ~5,000-6,000 │ ~80-85K  │ <$0.01     │ <$1K     │ DeFiLlama/L2Beat │
│            │               │          │              │          │ (MNT gas)  │ (est.)   │ Nansen Q4 rpt    │
│            │               │          │              │          │            │          │ confidence: med  │
├────────────┼───────────────┼──────────┼──────────────┼──────────┼────────────┼──────────┼──────────────────┤
...
Per-metric 数据来源与 last-refreshed:
┌────────────┬────────────────────────┬──────────────────────────────────────────────────────────┬─────────────────────┐
│ 指标       │ 数据来源                │ URL / 查询文件                                           │ Last refreshed (UTC)│
├────────────┼────────────────────────┼──────────────────────────────────────────────────────────┼─────────────────────┤
│ TVL        │ DefiLlama + L2Beat     │ https://defillama.com/chains ; https://l2beat.com/scaling/tvl │ 2026-05-25 18:00   │
│ DAU        │ Token Terminal / L2Beat │ https://tokenterminal.com/explorer/metrics/user-dau      │ 2026-05-25 20:00   │
│            │ + Nansen Q4 (Mantle)   │ https://nansen.ai/post/mantle-q4-2025-report             │ 2026-01-15 (Q4 rpt)│
│ 日交易量   │ L2Beat Activity        │ https://l2beat.com/scaling/activity                       │ 2026-05-25 20:00   │
│            │ + Nansen Q4 (Mantle)   │ + Messari State of Mantle Q4 2025                        │ 2026-01-15 (Q4 rpt)│
│ Avg Fee    │ DefiLlama Fees         │ https://defillama.com/fees                                │ 2026-05-25 18:00   │
│ 日收入     │ Token Terminal         │ https://tokenterminal.com/explorer/metrics/fees            │ 2026-05-25 20:00   │
└────────────┴────────────────────────┴──────────────────────────────────────────────────────────┴─────────────────────┘
...
⚠️ DAU 过滤说明 (适用于本图表):
  ...
  - **Mantle 特别说明**: DAU 数据来自 Nansen Q4 2025 报告 (5K-6K avg)，非实时 dashboard；Q2 2025 曾降至 12K，Q3 恢复至 53K，Q4 回落至 5K-6K
```
...
```csv
chain,dau_24h,daily_tx,avg_fee_usd,daily_revenue_usd,confidence,last_refreshed,source,filter_caveat
Base,382500,12890000,0.05,185000,high,2026-05-25T20:00:00Z,Token Terminal + L2Beat Activity,"系统地址已排除(OP Stack predeploys); 未过滤 bot/social protocol/MEV bot; Farcaster 等 social 贡献显著"
Arbitrum,132600,4170000,0.085,55000,high,2026-05-25T20:00:00Z,Token Terminal + L2Beat Activity,"系统地址已排除(ArbOS precompiles + batch submitter); 未过滤 DeFi arbitrage bot"
OP Mainnet,82000,422000,0.06,NULL,medium,2026-05-25T20:00:00Z,L2Beat Activity,"系统地址已排除(OP Stack predeploys); DAU 为估计值; 收入数据不可用"
Mantle,5500,82500,0.008,800,medium,2026-01-15T00:00:00Z,Nansen Q4 2025 Report,"系统地址排除(OP Stack predeploys); 数据为 Q4 2025 日均值非实时; 低费(<$0.01)导致 bot 风险; gas 以 MNT 计价"
zkSync Era,4000,19600,0.07,NULL,high,2026-05-25T20:00:00Z,L2Beat Activity,"系统地址已排除(zkSync system contracts 0x800x); 已过滤 bootloader; retail 活跃极低"
```

> **DAU per-metric last-refreshed**:
> - Base/Arbitrum/OP Mainnet/zkSync: 2026-05-25T20:00:00Z (Token Terminal / L2Beat Activity 实时)
> - **Mantle**: 2026-01-15T00:00:00Z (Nansen Q4 2025 Report 发布日期; 非实时数据)
>
> **Mantle 数据来源说明**: Mantle 未出现在 Token Terminal DAU 排名前列，主流 L2 dashboard (L2Beat Activity, Token Terminal) 的 Mantle DAU/tx 数据可见度有限。本表 Mantle 行使用 Nansen Q4 2025 季报的日均值：DAU ~5,000-6,000, daily tx ~80,000-85,000, 峰值 DAU 17,021 (2025-10-05)。avg_fee 基于 Mantle 极低 gas 定价 (0.001 Gwei MNT base fee) 估算。daily_revenue 基于 fee × tx 估算，置信度低。
>
> **Mantle 历史波动**: Q1 2025 DAU 37.8K → Q2 2025 DAU 12.2K (↓67.7%) → Q3 2025 DAU 53K (↑335%) → Q4 2025 DAU 5-6K (↓~90%)。波动幅度极大，单一快照无法代表趋势。

⚠️ **DAU 过滤 caveat（本图表）**: 系统地址排除详见上方图表注释。Bot/spam 未过滤——无跨链统一标准。Base 的 DAU 显著包含 social protocol + MEV bot 流量。Mantle 的极低 gas 费 (<$0.01/tx) 降低了 bot 成本门槛。**DAU ≠ 真实用户数**。
...
- Base DAU 约为 Arbitrum 的 3 倍、OP Mainnet 的 4.7 倍、Mantle 的 ~65 倍、zkSync Era 的 96 倍
- Base 日交易量约为 Arbitrum 的 3 倍、Mantle 的 ~156 倍
- Base 以 89 TPS（2026-04）领跑所有 Optimistic Rollup

**但 DAU 需谨慎解读**: Base 的高 DAU 中 social protocol（Farcaster 生态）、NFT mint、小额 retail 交易贡献显著比例。Arbitrum 的用户虽然较少，但平均交易价值更高（DeFi 为主）。OP Mainnet 的 DAU 数据较少被单独统计，其战略价值更多体现在 Superchain 网络层面。Mantle 的 DAU (~5-6K, Q4 2025 均值) 高于 zkSync Era (~4K)，但远低于前三链，反映用户基数和链上活跃度仍需提升。Mantle Q2 2025 的 DAU 67.7% 骤降和 Q3 的 335% 反弹显示活跃度波动性大。
...
| DAU (Mantle) | Nansen Q4 2025 Report | https://nansen.ai/post/mantle-q4-2025-report | 2026-01-15 |
...
**Confidence**: high (TVL, Base/Arbitrum DAU), medium (Mantle DAU/tx — Q4 2025 report, not real-time)
...
> Last refreshed: 2026-05-24, competitor research finals + official docs
...
│                │ [2026-05-25]     │               │              │                       │
│ DAU            │ ~5,000-6,000     │ ~37,816 (Q1)  │ 数据有限     │ Q4 2025 Nansen 日均值  │
│                │ [2026-01-15 rpt] │               │              │ 峰值 17,021 (10/05)   │
│ 日交易量       │ ~80,000-85,000   │ ~391,399 (Q1) │ 数据有限     │ Q4 2025 Nansen 日均值  │
│                │ [2026-01-15 rpt] │               │              │ 峰值 >120K            │
...
│ 合约部署       │ N/A (未量化)      │ N/A           │ N/A          │ Dune mantle.creation  │
│                │                  │               │              │ _traces 可查询        │
│ Gas/Fees       │ <$0.01/tx        │ <$0.01/tx     │ 稳定         │ MNT gas, 0.001 Gwei   │
│                │ [2026-05-25]     │               │              │ base fee default      │
└────────────────┴──────────────────┴───────────────┴──────────────┴───────────────────────┘

Per-metric last-refreshed:
  TVL: 2026-05-25T18:00:00Z (DefiLlama + L2Beat)
  DeFi TVL: 2026-05-25T18:00:00Z (DefiLlama)
  DAU: 2026-01-15 (Nansen Q4 2025 Report — 非实时, Q4 日均值)
  日交易量: 2026-01-15 (Nansen Q4 2025 Report — 非实时, Q4 日均值)
  mETH/cmETH TVL: 2026-05-25 (Mantle official + DefiLlama)
  mETH 持有人: 2026-05-25 (Etherscan + Mantlescan)
  稳定币供应: 2026-01-15 (Nansen Q4 2025 Report)
  Gas/Fees: 2026-05-25 (Mantlescan gas tracker)

⚠️ DAU 过滤 caveat (适用于本 dashboard):
  - Mantle DAU 数据来自 Nansen Q4 2025 季报，使用链上唯一交易发起地址计算
  ...
  - 波动极大: Q1→Q2 DAU ↓67.7%, Q2→Q3 DAU ↑335%, Q3→Q4 DAU ↓~90%。
    季度均值无法反映日级波动
...
| 链 | TVL 90d 趋势 | DAU 90d 趋势 | 交易量 90d 趋势 | 整体判断 | 数据来源 Last Refreshed |
|----|-------------|-------------|----------------|---------|------------------------|
| Base | ↑ 持续增长 | ↑ 持续增长 | ↑ 持续增长 | 全面领先 | 2026-05-25 (DefiLlama/L2Beat) |
| Arbitrum | → 稳定 | → 稳定 | → 稳定 | DeFi 基本盘稳固 | 2026-05-25 (DefiLlama/L2Beat) |
| OP Mainnet | → 稳定 | → 稳定 | → 稳定 | Superchain 价值在网络层 | 2026-05-25 (DefiLlama/L2Beat) |
| Mantle | ↑ Aave 效应 | ? 季报滞后 | ? 季报滞后 | TVL 短期提升，粘性存疑 | TVL: 2026-05-25; DAU/tx: 2026-01-15 (Q4 rpt) |
| zkSync Era | ↓ DeFi 萎缩 | ↓ 持续下降 | ↓ 持续下降 | 转型企业/机构定位 | 2026-05-25 (DefiLlama/L2Beat) |

> **Mantle 90d 数据局限说明**: Mantle 的 DAU 和 tx 90 天趋势无法通过主流 dashboard 实时获取。上表 Mantle DAU/tx 列标注 "?" 表示基于 Q4 2025 季报推断——Q4 均值 (DAU ~5-6K, tx ~80-85K) 可能已不能代表 2026 年 2-5 月的真实趋势（Aave V3 上线可能改变活跃度）。需要执行 `queries/dau-mantle.sql` 获取实时数据验证。
...
| 季度波动性大 | **高** | Q1→Q2 DAU ↓67.7%, Q2→Q3 ↑335%, Q3→Q4 ↓~90%。激励活动和市场周期驱动大幅波动 |

### 8.4 诊断结论

Mantle 的情况更准确地描述为**"相对竞争压力 + 激励依赖风险 + 高活跃度波动性"**而非简单的"活跃度下降"：

1. **TVL 绝对值有所回落**（从 $2.36B 峰值），但 Aave V3 上线后 DeFi TVL 恢复增长至 $543M+
2. **用户活跃度波动极大**: Q2 骤降 67.7% 后 Q3 反弹 335%，再到 Q4 回落至 5-6K DAU——这更像激励周期驱动的波动而非单向衰退
3. **相对于 Base/Arbitrum 的差距在扩大**：Base 的 DAU/tx 领先优势加速拉开（Mantle DAU 约为 Base 的 1/65）
...
**⚠️ Caveat**: 当前数据支持"结构变化 + 相对竞争压力"的结论，而非简单的"活跃度下降"。DAU 的季度波动性极大（67.7% 下降 → 335% 反弹 → 90% 回落），单一快照不具代表性。

**数据来源**:
- DefiLlama: https://defillama.com/chain/mantle | Last refreshed: 2026-05-25T18:00:00Z
- Nansen Q4 2025: https://nansen.ai/post/mantle-q4-2025-report | Last refreshed: 2026-01-15
- Messari Q1/Q2/Q3 2025: https://messari.io/report/state-of-mantle-q1-2025 等 | Last refreshed: 2025-Q3
...
- L2Beat: https://l2beat.com/scaling/tvl | Last refreshed: 2026-05-25T18:00:00Z

**Evidence level**: dashboard-data, media-reported, industry-quarterly-report
**Confidence**: medium (TVL trends: high; DAU/activity: medium — Q4 report data, not real-time)
...
- DAU/活跃度数据支持"高波动性 + 相对竞争压力"而非单向"活跃度下降"
- 不得为叙事需求强行下"活跃度下降"结论——当前数据更支持"TVL 从峰值回落 + 激励周期波动 + 结构转型期"
...
| Nansen Q4 2025 | Mantle DAU, tx, DeFi TVL, stablecoin, top entities | ✅ Met | 2026-01-15 |
...
| Nansen Research | Mantle Q4 2025 quarterly |
...
| GAP-2 (MITIGATED) | ~~Mantle DAU / 日交易量 / 合约部署缺乏数据~~ | ~~无法精确判断~~ | ~~Medium~~ | ✅ DAU/tx 已用 Nansen Q4 2025 + Messari Q1-Q3 数据填充 (5-6K DAU, 80-85K tx)。合约部署仍为 N/A，已注明原因 (Dune mantle.creation_traces 可查询但未执行) |
...
| GAP-7 | Mantle DAU/tx 数据为 Q4 2025 季报值，非 2026 年 5 月实时 | 可能无法反映 Aave V3 上线后的活跃度变化 | Medium | 需执行 queries/dau-mantle.sql 获取最新数据 |
