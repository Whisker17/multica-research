...
draft_metadata:
  ...
  diagrams_produced:
    ...
    - diag-8
promotion_metadata:
  promoted_from: "202606-internal-sharing/research-sections/narrative-agentfi/drafts/round-1.md"
  approved_draft_round: 1
  approved_draft_commit: "3f8e9a69d58f02969a1064fac1fa0223c95c9256"
  approval_evidence: "comment 97050729-d3a2-4cc9-a54c-2b28d5650ac8 (Research Review Agent — approve, severity: minor)"
  promoted_at: "2026-05-26"
  promoted_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  caveats_applied:
    - "市场数据刷新: 标注三桶分离和刷新日期要求；不将 token 市值换算为 TAM"
    - "Mantle 链上证据: AA/paymaster/stablecoin 相关论断标注为所需建设而非已有生产优势"
    - "安全部分来源: 安全威胁模型标注为设计风险分析（非事故驱动证据），增加具体参考方向"
---

# AgentFi 叙事方向技术分析

> **数据时效声明**：本文所有市场/API 数据快照取自 2026-05-26，在 2026-06-05 内部分享前须再次刷新。Token 市值快照、协议/支付使用量、开发者活跃度三类指标严格分桶，**不将 token 市值换算为 TAM 或 agent commerce 真实规模**。
...
AgentFi 的市场规模必须拆成三类：token 资产快照、协议/支付使用快照、开发者活动快照。三者不能互相替代，**不应将 token 市值直接换算为 TAM 或 agent commerce 真实市场规模**。以下所有数据均需在 2026-06-05 内部分享前刷新至最新快照。
...
Mantle 适配（**所需建设，非已有生产优势**）：Mantle 官方文档已列出 Etherspot 与 Particle Network 账户抽象集成路径。Particle 文档说明 Mantle Mainnet/Testnet 兼容，并可通过 SimpleAccount/Biconomy V2/CyberConnect 等方式执行 gasless sponsored transaction；Etherspot 教程使用 Mantle chainId 5000。这些是第三方 provider 的路由层证据，表明技术兼容性存在，**但不等于 Mantle 具备官方第一方 agent wallet/paymaster SLA、已验证的稳定币 gas abstraction 经济模型或可引用的 production 部署数据**。补齐路线需要：官方 paymaster provider partnership 与 SLA、sponsored gas 经济模型与 abuse control、Mantle 资产/TVL/stablecoin 发行与提供方数据。在此之前，AA/paymaster 相关 AgentFi 能力应被定位为 "可建设方向" 而非 Mantle 当前竞争优势。

安全模型必须单独处理（**⚠️ 以下为设计风险分析，基于威胁建模而非事故驱动证据；来源覆盖偏少，后续应补充具体参考**）：

- prompt injection 诱导 agent 调用高权限 tool — 参考方向：OWASP LLM Top 10 中 LLM01 Prompt Injection 条目、Anthropic/OpenAI 官方 agent safety guidelines 中关于 tool-use guardrails 的建议；
- tool-call hijack 或恶意 API response 替换交易参数；
- private-key leakage / MPC policy bypass；
- 无限授权、跨链 replay、session key 未撤销 — 参考方向：ERC-7715 wallet permission request 规范中的 scoped permission/revocation 设计、ZeroDev/Pimlico 等 AA provider 的 session key policy 文档；
- agent collusion / evaluator collusion；
- paymaster abuse（恶意 gas sponsorship 消耗）— 参考方向：Biconomy/Alchemy AA paymaster 文档中的 rate limit、spending cap 和 abuse control 机制；
- 策略失败后的 chargeback、赔偿与审计责任。

> **来源覆盖说明**：本节威胁模型基于 agent wallet/permission/payment 的架构推导，而非已发生安全事故的事后分析。上述参考方向为建议性补充路线，在最终分享前应核实并引用具体文档版本。当前应将此安全分析视为 **设计风险框架**，而非经事故验证的安全评估。
...
| AA / paymaster | Etherspot、Particle docs 路由层兼容；**第三方路径可用但无官方 production SLA（所需建设）** | Base Account/CDP wallet 更强；Sui/Solana gasless UX 更直接 | 第一方 paymaster provider partnership、SLA、sponsored gas 经济模型、abuse control；补充 Mantle 官方 asset/TVL/stablecoin 及提供方数据 | 0-1 季度 | Paymaster economics、provider SLA 与 abuse control 均未证实 |
...
| DeFi/yield/liquidity | Mantle 有 mETH/cmETH/yield-bearing 生态叙事基础；**稳定币/收益资产的具体 TVL、提供方和 SLA 数据待补充（所需建设）** | Base/Solana app density 更强；Arc/Tempo stablecoin positioning 更尖锐 | Agent treasury、portfolio rebalancing、yield policy vault demo；补充 Mantle 官方 stablecoin（USDT0/AUSD/USDy/USDe 等）发行量、TVL 和提供方数据 | 0-2 季度 | 当前叙事基于生态定位而非已验证的 production 数据；需要官方资产/流动性数据补证 |
...
| 稳定币/支付伙伴证据不足（所需建设） | x402/API commerce 更偏 stablecoin；当前 Mantle 稳定币/收益相关论断为路由层证据 | 补齐官方稳定币发行量/TVL/提供方/SLA、CCTP/bridge、paymaster 经济模型和 merchant/API partner 数据 |
...
| Security / wallet risk | 标注为设计风险分析（非事故驱动证据）；增加了 OWASP LLM Top 10、ERC-7715 scoped permission/revocation、AA paymaster abuse control 参考方向 | 最终分享前需核实具体文档版本并引用；如发现相关安全事故案例应补充 | low → medium（参考方向已标注，具体引用待核实） |
...
- **Mantle stablecoin/yield gap（已标注所需建设）**：本稿使用 Mantle yield/liquidity 作为路线判断，相关论断已在正文中标注为"所需建设"而非已有生产优势。仍需补最新官方 stablecoin/native assets（USDT0/AUSD/USDy/USDe 等发行量）、CCTP/bridge、mETH/cmETH liquidity 与 DeFi TVL 数据、以及提供方/SLA 信息。
- **AA/paymaster production gap（已标注所需建设）**：Etherspot/Particle docs 证明第三方路由层兼容性，已在正文中标注为"所需建设"而非已有生产优势。需要官方 paymaster provider partnership、SLA、rate limit、sponsorship economics、abuse controls 与 Mantle 官方 asset/TVL/stablecoin 及提供方数据。
- **Security source gap（已部分补齐）**：安全模型已标注为设计风险分析（非事故驱动证据），并增加了 OWASP LLM Top 10（prompt injection）、ERC-7715 scoped permission/revocation、AA paymaster abuse control 机制的参考方向。最终分享前需核实具体文档版本并补充引用链接；如发现 wallet-permission/session-key/paymaster/prompt-injection 相关安全事故案例应进一步补充。
...
| 1→final | Promoted to final | Applied 3 minor caveats from adversarial review (comment 97050729): (1) 市场数据刷新 — 添加数据时效声明，强化三桶分离和 2026-06-05 刷新要求，明确不将 token 市值换算为 TAM；(2) Mantle 链上证据 — AA/paymaster/stablecoin 相关论断标注为"所需建设"而非已有生产优势，补充补齐路线说明；(3) 安全部分来源 — 威胁模型标注为设计风险分析（非事故驱动证据），增加 OWASP LLM Top 10、ERC-7715、AA paymaster abuse control 参考方向。 |
