...
artifact_paths:
  ...
  draft: 202606-internal-sharing/research-sections/payment-ark/drafts/round-1.md
...
draft_metadata:
  ...
  created_at: "2026-05-22T15:32:00Z"
...
支付场景上，Ark 更适合 BTC 持有者的低额 P2P、钱包内转账、商户 BTC 收款体验改善、Lightning 边缘互操作和 BTC liquidity entry；Tempo 更适合稳定币单位计价、企业/商户结算、EVM 开发者、机器支付和赞助 gas 的 Web2-like 支付体验。Mantle 目前未发现与 Ark 的直接官方合作或集成证据；Mantle 相关性应被表达为间接机会：BTC liquidity / FBTC / Mantle L2 / mETH / stablecoin settlement 之间的资产表达与路由设计，而不是既成生态绑定。
...
**high_level_summary**: Ark 与 Tempo 代表两条不同支付路线。Ark 以 BTC 为资产和安全锚，牺牲通用智能合约与稳定币原生能力来改善 Bitcoin 支付 UX；Tempo 是 stablecoin payment-optimized L1，保留 EVM 熟悉度，同时把 fee、TIP-20、memo、sponsorship、scheduled/batched transactions 和 payment lanes 做成协议叙事。二者更互补而非正面替代：Ark 解决 BTC-native holders 的支付入口，Tempo 解决企业/商户/机器支付的稳定币结算层。

**comparison_to_tempo**:

| 维度 | Ark / Arkade | Tempo |
|---|---|---|
| 结算资产 | BTC 原生；潜在 Arkade assets | 稳定币优先；gas 可用 USD stablecoins |
...
| 最终性 | 即时 preconfirmation 与 L1 settlement 分层 | 面向低延迟确定性；官方 README 称 normal conditions sub-second finality |
...
| 商户 reconciliation | 需要应用层处理 invoices/settlement | TIP-20 memo / commitment patterns / policy registry 更贴近商户账务 |
...
- Public testnet Moderato 已公开 RPC、chain ID 42431、explorer 与 faucet；不应把它写成已成熟主网，除非后续找到官方 mainnet launch 证据。[S13]

**payment_fit**:

| 产品路线 | Ark 优先 | Tempo 优先 |
|---|---|---|
| BTC holder wallet | 是 | 否 |
...
- Tempo 的主网时间、validator 去中心化、stablecoin issuer partnerships 和 production risk 需以最新官方发布核验。
- Arkade assets 是否会发展出足够强的 stablecoin path，目前只能作为可能性，不能当成既有能力。

### item-6: 与 Mantle 生态的潜在关联和可行动观察点

**high_level_summary**: 未发现 Ark 与 Mantle 的直接官方合作、投资、技术集成或正式提及。Mantle 相关性主要来自间接资产与支付栈互补：Ark 可能作为 BTC-native payment/liquidity entry，Mantle L2 / FBTC / mETH / DeFi 可作为 EVM 资产表达、收益和 stablecoin settlement 层。这个判断必须带风险边界：Bitcoin-to-EVM bridge / custody / compliance / UX complexity 是核心障碍。
...
4. **Stablecoin settlement**: Mantle 若关注支付赛道，可把 Ark 视为 BTC inflow / wallet routing 入口，把 Tempo 或 EVM stablecoin rails 视为 settlement / merchant payout 层。
...
- Mantle 是否发布 Bitcoin liquidity / payment strategy，明确把 FBTC、mETH、stablecoins 和 L2 商户结算纳入同一产品叙事。
...
- FBTC 的 trust model 是 custodial/offchain + smart contracts，与 Ark 的 Bitcoin-native VTXO 安全模型不同，不能混称为同一 BTC trustless path。
...
- Tempo repo README 比 Arkade 更明确面向 public testnet / developer onboarding；两者成熟度指标不可直接用同一标尺比较，因为一个是 BTC offchain implementation，一个是 payment L1 stack。[S13]
...
**high_level_summary**: 2026 支付叙事更可能分化为两层：BTC-native offchain payments 服务 Bitcoin holders 的支付可用性，stablecoin payment chains 服务商户、企业、机器和跨境结算。Ark 的战略价值在于保留 BTC 资产属性并改善 UX；Tempo 的战略价值在于把支付所需的低费、赞助、memo、compliance 和 EVM tooling 做成默认能力。Mantle 应更关注两者在钱包路由、BTC liquidity、stablecoin settlement 和 DeFi treasury 层的互补，而非押注单一协议替代全部支付需求。

**comparison_to_tempo**:

- **BTC as money vs stablecoin as unit of account**: Ark 强化 BTC 支付体验，但 BTC 波动性和税务/会计处理仍限制商户采用；Tempo 以稳定币作为费用和转账单位，更贴近商户和企业账务。
- **Offchain scalability vs dedicated L1 scalability**: Ark 把大量支付留在链下，最终回到 Bitcoin；Tempo 把支付吞吐做进 L1 execution / consensus / blockspace design。
- **Privacy vs compliance**: Ark 对链上观察者有隐私优势，但 operator 可见；Tempo 默认更适合合规和 reconciliation，但隐私 token still coming soon。
- **Wallet UX**: Ark wallet 需要处理 VTXO state、renewal、recovery、operator selection；Tempo wallet 更接近 EVM familiar UX，并支持 sponsorship/passkeys 等抽象。

**payment_fit trend judgment**:

| 趋势 | 受益路线 | Mantle 应关注 |
|---|---|---|
| BTC holders 希望不用卖 BTC 也能支付 | Ark | BTC entry、FBTC mint/redeem、wallet routing |
...
| DeFi 希望 BTC liquidity 进入 EVM | Mantle / FBTC | Ark-to-FBTC / BTC liquidity path，但需要信任桥 |
| 机器支付/API payments | Tempo | Programmable stablecoin payments、scheduled/batched transfers |

**open_questions**:

- 稳定币支付链能否获得 issuer、PSP、merchant 和监管认可，是 Tempo 路线的关键。
...
flowchart TB
  ...
  subgraph Tempo["Tempo: payment-optimized stablecoin L1"]
    T1[Settlement asset: stablecoins]
    T2[Execution: EVM + TIP-20]
    T3[Coordinator: validator set / consensus]
    T4[Finality: low-latency chain finality]
    T5[UX: stablecoin gas, sponsorship, batching]
    T6[Best fit: merchants, PSPs, machine payments]
  end
...
flowchart LR
  ...
  STABLE -.payment L1 alternative.-> TEMPO[Tempo stablecoin payment chain]
...
| src-4 tempo_official_docs | 5 | 5 | Met | Tempo README/docs: TIP-20, stablecoin gas/Fee AMM, Tempo Transactions, performance/finality, public testnet |
...
5. **Tempo mainnet status gap**: Tempo public testnet details are clear from the repo/docs. The draft avoids claiming production mainnet maturity without a directly verified official mainnet launch source.
