...
**发展阶段（date_verified: 2026-05-23）**：2025 年 8 月发布 litepaper、2025 年 10 月公测上线（100+ 机构参与）、2026 年 5 月发布 ARC 代币白皮书并完成 $222M 预售（a16z 领投 $75M，FDV $3B）、主网预计 2026 年夏季上线。截至 2026 年 5 月 5 日，测试网已处理超 2.44 亿笔交易（evidence_confidence: official-announcement，注：不同来源在不同时间点报告的数字存在差异，Circle 2026 产品愿景博客提及 "150+ million transactions"，白皮书发布时引用 244.1M）。
...
Circle 是 USDC 的发行方，2024 年在纽交所上市（代码 CRCL）。Q1 2026 财务数据（evidence_confidence: verified-primary, source: Circle Q1 2026 earnings）：
...
| 2025.10-2026.05 | 测试网运行，处理 244.1M 交易 | ARC 白皮书（截至 2026.05.05）；Circle 2026 产品愿景博客引用 "150+ million transactions"（截至博客发布时） | official-announcement; 不同来源时间点不同，数字差异属正常 |
| 2026.05 | ARC 代币白皮书发布 | Arc 博客 | verified-primary |
| 2026.05.11 | $222M ARC 代币预售完成 | CNBC、Bitcoin.com、多家媒体 | verified-primary |
| 2026 夏季（预计） | 主网 beta 上线 | Circle 公告；具体日期未确认 | official-announcement |
| 2028.05.08 | PoS 过渡截止/投资者回购权触发日 | 预售条款 | verified-primary |

#### 战略背景：GENIUS Act 后的竞争防御

GENIUS Act（2025 年 7 月通过）是美国首部联邦稳定币监管框架，要求 Permitted Payment Stablecoin Issuers (PPSIs) 满足 1:1 流动资产储备、赎回保障、风控和运营监管要求。OCC 于 2026 年 2 月 25 日发布实施细则征求意见稿，Circle 于 2026 年 5 月 1 日提交详细评论信（evidence_confidence: verified-primary, source: Circle GENIUS Act page, Circle OCC comment）。
...
Arc 采用渐进式去中心化路线（evidence_confidence: verified-primary, source: CoinGecko、Circle 公告）：

- **初期：PoA（许可制验证者）**。验证者由 Circle 根据运营稳定性、地理多样性和监管合规性选择。
- **未来：PoS 过渡**。逐步向更广泛的金融机构开放验证者参与。
- **双层安全模型**：身份层（KYC/合规资质）+ 经济层（ARC 代币质押）。

⚠️ **PoA → PoS 过渡时间表尚未确定。预售条款设定 2028.05.08 为 PoS 过渡的最后期限（触发投资者回购权）。**
...
CCTP（Cross-Chain Transfer Protocol）是 Circle 的原生跨链 USDC 转账协议，通过原生 burn-and-mint 机制实现 1:1 USDC 跨链（evidence_confidence: verified-primary, source: Circle infrastructure report）：

| 指标 | 数值 | Date verified |
|---|---|---|
| Q3 2025 季度交易量 | $31B | 2026-05-23 |
| YoY 增长率 | 740% | 2026-05-23 |
| 覆盖网络数 | 18+ 区块链（CCTP）；30 个区块链（USDC 原生） | 2026-05-23 |
...
（evidence_confidence: verified-primary, source: ARC 白皮书通过多家媒体报道、Bitcoin.com、CNBC）

| 参数 | 数值 |
|---|---|
| 代币名称 | ARC |
| 初始总量 | 100 亿（10 billion） |
| 分配：生态系统 | 60%（网络参与者和贡献者） |
| 分配：Circle | 25%（验证者基础设施运营和质押收入） |
| 分配：长期储备 | 15% |
| 预售数量 | 7.4 亿代币（740M tokens） |
| 预售价格 | $0.30/token |
| 预售总额 | $222M |
| 全稀释估值（FDV） | $3B |

⚠️ **白皮书中的代币经济学参数可能标注为 "indicative" 或 "preliminary"，最终参数以主网上线时为准。**
...
所有交易费用（无论以 USDC、其他稳定币还是 ARC 支付）在协议层统一转换为 ARC，然后分为两个方向（evidence_confidence: official-announcement, source: 白皮书摘要）：
...
采用"共享责任制"和"渐进去中心化"路线。五个决策域的初始治理分工由白皮书定义。

#### 投资者保护条款

预售投资者享有以下保护（evidence_confidence: verified-primary, source: Bitcoin.com）：
...
- **首家上市公司代币预售**：Circle 称此为 "the first time a publicly listed company has conducted a token presale in this type of compliant structure"。
...
| 公司类型 | 上市公司（2024 NYSE IPO） | 私有公司（Stripe 未上市） |
| 主网状态 | 测试网运行中；主网预计 2026 夏季 | **已上线**（2026.03.18 mainnet/Presto launch）⚠️ Tempo final.md caveat: 真实交易量/活跃商户/生产伙伴深度未充分验证 |
...
| 跨链 | CCTP V2（原生 1:1 USDC 跨链，18+ 网络） | 无内置跨链协议 |
...
| 集成 | 开发者指南/黑客松/参考实现阶段 | **已上线**：Visa AI Agent 支付集成（evidence_confidence: verified-primary, source: 多家媒体） |
...
| USDC 状态 | **关键事实（date_verified: 2026-05-23）**：Mantle 上的 USDC 为桥接版本（非 Circle 原生发行）。Mantle **未** 被纳入 CCTP 支持链列表。Mantle 未出现在 USDC 原生发行的 28 条链名单中。Mantle 目前优先发展 USDT0（与 Bybit/Ethena 合作）而非 USDC/CCTP |
...
| CCTP/跨链稳定币流动性 | 原生 USDC + CCTP V2 | 评估 CCTP 集成优先级（当前 Mantle USDC 为桥接版本） | 中（需 Circle 合作） |
...
| 路径 | 描述 | 可行性 |
|---|---|---|
| CCTP 跨链桥 | Arc ↔ Mantle USDC 流动性互通 | 中（需 Circle 将 Mantle 纳入 CCTP 支持链；当前 Mantle 不在名单中） |
...
| 风险 | 评估 |
|---|---|
| 主网未上线 | 预计 2026 夏季，但具体日期未确认。测试网数据不代表主网表现 |
...
| 投资者保护条款 | 2028.05 回购权如果被触发，可能对项目造成重大影响 |
...
flowchart TB
    ...
    subgraph L3["Protocol Services"]
        PS1[CCTP V2<br/>跨链 USDC 转账<br/>18+ 网络]
...
gantt
    ...
    section GENIUS Act
    参议院通过 (68-30)          :milestone, genius1, 2025-07-01, 0d
    ...
    测试网运行                  :active, arc_test, 2025-10-28, 2026-07-31
    ARC 白皮书发布              :milestone, arc3, 2026-05-05, 0d
    $222M 代币预售              :milestone, arc4, 2026-05-11, 0d
    主网 beta (预计)            :milestone, arc5, 2026-07-31, 0d
...
| src-2 whitepaper (ARC 代币白皮书, min 1) | ⚠️ 白皮书 PDF 无法直接解析；通过多家媒体报道（CNBC、Bitcoin.com、Bitget、PANews）间接引用白皮书内容。代币参数和经济学数据可交叉验证 |
...
| src-5 industry_reports ($222M 预售报道, min 2) | ✅ 使用 Bitcoin.com、CNBC（403 但 search 结果完整）、CoinGecko 分析、多家加密媒体 |
...
| src-7 official_docs (Tempo 官方资料, min 2) | ✅ 通过 Tempo final.md 引用的 Tempo 官方站点、Paradigm 公告、公开代码 repo 间接覆盖 |
| src-8 official_announcement (Circle 基础设施报告, min 1) | ✅ 使用 Circle Internet Financial System Report（CCTP 数据、生态概览） |
...
1. **ARC 白皮书 PDF 未直接解析**：白皮书 PDF 内容通过多家媒体交叉验证间接引用，但未能直接从 PDF 提取原始文本。代币经济学细节（如五个决策域的初始治理分工、具体通胀参数曲线）可能有遗漏。建议后续补充白皮书全文分析。
...
6. **Mantle 当前代码路径未审计**：与 Tempo 研究类似，本 section 对 Mantle 的建议基于 Mantle 常识和 OP Stack 架构约束，未直接审计 Mantle 当前 repo。工程设计阶段应另开 Mantle-specific POC。
