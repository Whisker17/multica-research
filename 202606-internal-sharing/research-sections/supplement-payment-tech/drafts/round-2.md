...
这种双网络隔离的设计意义在于：执行层的负载（大型合约调用、DeFi 交易）不会直接拖慢共识延迟。对支付场景，这意味着降低执行负载对共识路径的直接影响；拥堵下 SLA 仍需实测。
...
**机构验证者架构**：Arc 初期采用 PoA 许可制验证者，由 Circle 按运营稳定性、地理多样性和监管合规性筛选。安全设计为双层模型：身份层（KYC/合规资质）+ 经济层（ARC 代币质押）。这种"合规准入 + 经济约束"的架构天然适配受监管机构接入——公测阶段已有 100+ 机构（含 BlackRock、Goldman Sachs、Mastercard 等）参与测试网，验证了该模型对传统金融的吸引力。未来将渐进过渡至 PoS，向更广泛的金融机构开放验证者参与。

（来源：`payment-ark/final.md` item-2、item-7）
...
| 2 | Addressed adversarial feedback (major + minor caveat). Fix 1 (Major): Added institutional validator architecture paragraph to Section 4.4 — covers Arc's initial PoA design, dual-layer security model (identity + economic), and why these choices support regulated institutional onboarding (100+ testnet institutions). Sourced from `payment-ark/final.md` item-2 and item-7. Fix 2 (Minor): Softened Tempo dual-network overclaim in Section 3.1 — replaced "支付交易的终局时间不受通用计算拥堵的影响" with "降低执行负载对共识路径的直接影响；拥堵下 SLA 仍需实测" to match source-supported design intent without implying confirmed production SLA. |
