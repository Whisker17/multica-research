...
final_metadata:
  ...
  promoted_at: "2026-05-17T05:00:00Z"
  approved_draft_path: "base-azul-upgrade/research-sections/base-strategy-azul-overview/drafts/round-2.md"
  approved_draft_round: 2
  approved_draft_commit: "7f2ff187c8c08de27de2bdec324fb4d0a6f03365"
  approval_evidence: "Review Verdict 9a65bf60 (round-2 approve)"
  orchestrator_minor_caveat_incorporated: "OP Stack op-geth support is transitional — Karst/Upgrade 19 ends op-geth support after 2026-05-31. The Azul-era 'three documented OP Stack execution clients' statement is a snapshot of optimism @ d905be1e (2026-05-17) and not a permanent OP Stack architectural feature."
...
> **Snapshot note for OP Stack comparisons in this section**: All Base-vs-OP-Stack comparisons in this document are evaluated against `ethereum-optimism/optimism @ d905be1e` (snapshot taken 2026-05-17). In particular, the statement that OP Stack's node-operator docs document **three execution clients (op-geth / op-reth / Nethermind)** is a snapshot fact, **not** a permanent architectural feature: **op-geth support is on a transitional schedule** — per Orchestrator context, OP Stack's Karst / Upgrade 19 line ends op-geth support after 2026-05-31. Downstream sections (`mantle-impact-assessment`, any future Superchain client-architecture work) should re-verify OP Stack's execution-client matrix against a later snapshot when making forward-looking claims, rather than treating the three-EL snapshot in this section as enduring.
...
Azul 之后的 Base Stack 可以概括为「base-reth-node（基于 paradigmxyz/reth）+ base-consensus（自研、derivation pipeline 沿用 OP Stack 风格）+ Multiproof（TEE + ZK）+ Flashblocks payload + 标准 OP 风格 settlement 合约」。OP Stack 在同期（`ethereum-optimism/optimism @ d905be1e03df0e30112dc382d3b9b74d0d65aaa3`，2026-05-17 快照）则是「op-node + 多执行客户端（op-geth / op-reth / Nethermind）+ 交互式 Fault Proof + Interop 多链桥接 + 标准 OP 桥」。**注意 OP Stack 执行客户端清单本身在演化**：op-geth 处于过渡支持状态，按 Orchestrator 提供的上下文，Karst / Upgrade 19 之后（2026-05-31 起）OP Stack 将停止支持 op-geth；因此本节的「三执行客户端」清单是 d905be1e 快照下的事实，不应被引用为 OP Stack 的永久架构特征。
...
|---|---|---|
| 执行客户端 | 仅 `base-reth-node`（依赖 `paradigmxyz/reth v1.11.4`，见 `base/base` `Cargo.toml`） | 官方 node-operator 文档（d905be1e 快照）列出三种：`op-geth`（**过渡支持**，见本节顶部 snapshot note 与下方 footnote）、`op-reth`、`Nethermind`（`ethereum-optimism/optimism` `docs/public-docs/node-operators/guides/configuration/execution-clients.mdx`） |
...
[FOOTNOTE — op-geth transitional status]：上表的 OP Stack「三执行客户端」是 `optimism @ d905be1e`（2026-05-17 快照）当时的 node-operator 文档状态。按 Orchestrator 在 final-promotion 调度中提供的上下文，**op-geth 在 OP Stack 内的支持是过渡性的**：Karst / Upgrade 19 之后（2026-05-31 起）op-geth 将不再被官方支持。因此「OP Stack 拥有三种执行客户端」是一个时间窗口内的快照事实，**不是 OP Stack 的永久架构特征**。下游课题（`mantle-impact-assessment`、未来的 Superchain client-architecture 子课题）如需对 OP Stack 的执行客户端矩阵做面向未来的判断，应以更晚的 commit 快照重新核验，而不是引用本节的三 EL 快照作为长期事实。本研究未在 `d905be1e` 仓库内独立核验 Karst / Upgrade 19 的命名与时间点（Orchestrator 上下文输入），因此该 footnote 的对外引用应回到 OP Stack 官方公告作为原始来源。

[VERIFIED]：表中所有「OP Stack 同期」列的论断在 commit `d905be1e` 中均能找到对应源码或官方文档（不含上面 footnote 涉及的 Karst / Upgrade 19 时间点，属 Orchestrator 提供的外部上下文）。
...
- Karst / Upgrade 19 终止 op-geth 支持的具体时间点：Orchestrator 上下文输入；本研究未在 `d905be1e` 仓库或本轮抓取的官方公告中独立核验，下游课题应回到 OP Stack 官方公告 / release notes 作为原始来源。
...
- OP Stack 执行客户端清单的「时间敏感性」：本节 d905be1e 快照下的三 EL 清单不应被视为永久；尤其 op-geth 的过渡性支持（Karst / Upgrade 19 之后 2026-05-31 起停止支持）意味着 Base 与 OP Stack 在「客户端多样性 vs 收敛」轴上的相对位置在 Azul 激活窗口内本身就在变化。对客户端多样性的 high-level 比较应被理解为一个动态过程，而非两点之间的静态对照。
...
**finality 语言一致性**：本研究在全文统一使用「当 TEE 和 ZK 两种证明一致时可实现 1 天快速路径; 否则仍需 7 天长窗口」的表述。1 day 不是新的默认提款窗口，而是双证一致时的条件分支；任一证明缺失或冲突走 7 day 长窗口。`DelayedWETH` 的「1-day withdrawal」标记也是同一条件性结论的体现。
...
OP Stack 自家文档明确以「multi-client and multi-proof」自我定位（`faq-sec-model.mdx`），且 node-operator 文档（`d905be1e` 快照）列出 `op-geth` / `op-reth` / `Nethermind` 三种执行客户端实现（`docs/public-docs/node-operators/guides/configuration/execution-clients.mdx`，commit `d905be1e`）。这是一个时间敏感的快照：按 Orchestrator 提供的上下文，OP Stack 的 op-geth 支持是过渡性的，Karst / Upgrade 19 之后（2026-05-31 起）op-geth 将不再被官方支持，OP Stack 自身也在向更窄的执行客户端集合收敛。Base 在 Azul 选择以单一 reth-based 客户端换取迭代速度与硬件路径优化；与同窗口下 OP Stack 的多客户端定位形成相反的工程哲学，但「客户端多样性收敛」本身是双方共同的方向，仅幅度与节奏不同。
...
- `docs/guides/UPGRADES.md`（运营者升级路径的描述入口；具体细节由后续课题按需阅读）
...
Base 与 OP Stack（Optimism mainnet）的 hardfork 时刻表，自 Azul 起在客户端层完全解耦。Optimism 自身的下一 hardfork 节奏、是否对齐 Osaka EIP 集合，本研究未在 `optimism @ d905be1e` 仓库中按子目录核验具体计划，归为 unverified（待 Superchain 子课题核验）。值得注意的是 OP Stack 内部的升级时间表本身就在变化 — 例如 Orchestrator 提供的上下文显示 Karst / Upgrade 19 会在 2026-05-31 之后停止 op-geth 支持，与 Base 自身的 Azul mainnet code-set 时间戳（2026-05-28）几乎相邻。两条升级路径在 2026 年 5 月底集中重叠的事实意味着，「Base 自有升级节奏 vs OP Stack 升级节奏」这一对比不是 Base 慢 / OP 快或反之，而是两条独立时间轴在同一历史窗口中各自演进。
...
- Mainnet 激活时间在文档（spec overview 写 TBD）与代码（`config.rs` 写 `1_779_991_200`）之间存在表述差异；这与 blog 抓取摘要给出的「May 13, 2026」也存在矛盾。**工作判断**：mainnet 激活日在公开 governance / spec 重新公布之前**不应视为已承诺日期**；code 端 `1_779_991_200` 仅描述「客户端二进制内置的目标时间戳」而非「公开承诺的生效日」。下游研究在引用「Azul mainnet 激活时间」时应使用同样的双口径表述。
...
- OP Stack 侧 Karst / Upgrade 19 终止 op-geth 支持的时间点（Orchestrator 提供 2026-05-31 后）与 Base mainnet code-set 激活窗口（2026-05-28）几乎同时落在 2026 年 5 月底；下游 `mantle-impact-assessment` 与潜在 Superchain 课题应关注两条独立升级路径在同一窗口的协调 / 冲突。
...
flowchart TB
  ...
  subgraph OPS["OP Stack (snapshot @ d905be1e, 2026-05-17; client matrix is time-sensitive)"]
    direction TB
    OCL["op-node (CL; op-node/README.md)<br/>+ rust/kona (Rust impl)"]
    OEL["op-geth (transitional; Karst/Upgrade 19<br/>ends support after 2026-05-31)<br/>op-reth / Nethermind<br/>(execution-clients.mdx)"]
...
flowchart LR
  ...
  subgraph OP_STACK["Contemporary OP Stack (snapshot @ d905be1e, 2026-05-17; matrix is time-sensitive)"]
    OPNODE["op-node"]
    OPGETH["op-geth<br/>(transitional: Karst/Upgrade 19<br/>ends support after 2026-05-31)"]
    OPRETH["op-reth"]
...
gantt
    ...
    section OP Stack window
    op-geth support ends (Karst/Upg19)   :crit, opgeth, 2026-05-31, 1d
...
| src-6 | orchestrator_context | Orchestrator 提供的外部上下文 | — | 1 | Karst / Upgrade 19 在 2026-05-31 之后终止 OP Stack op-geth 支持 — 本研究未在 `optimism @ d905be1e` 仓库内独立核验，按上下文标注以避免把「OP Stack 三 EL」当作永久特征；下游课题应回到 OP Stack 官方公告 / release notes 作为原始来源 |
...
| OP Stack op-geth 支持的过渡性 | minor | 本研究的 OP Stack 比较以 `d905be1e`（2026-05-17）为快照；op-geth 在该快照下作为三 EL 之一被官方支持，但按 Orchestrator 上下文 Karst / Upgrade 19 之后（2026-05-31 起）op-geth 将停止被支持。本研究未在 `d905be1e` 仓库内独立核验 Karst / Upgrade 19 的命名与时间锚。 | 已在 §2.2 footnote、§3.1 / §3.4 / §3.5 diagram、§5 此条 gap 中显式标注「快照事实」而非「永久特征」；下游 Superchain / client-architecture 子课题应回到 OP Stack 官方公告 / release notes 核验时间点 |
...
| final | 2026-05-17 | agent:research-agent (Deep Research Agent) | Promotion from approved round-2 (commit `7f2ff187c8c08de27de2bdec324fb4d0a6f03365`; approval evidence: Review Verdict 9a65bf60). Incorporated Orchestrator minor caveat (final-polish): OP Stack op-geth support is **transitional** — Karst / Upgrade 19 ends op-geth support after 2026-05-31. Added a top-of-document **snapshot note**, a §2.2 footnote on op-geth's transitional status with [UNVERIFIED] tag and downstream re-verification guidance, an updated §2.2 `risks_open_questions` bullet on the "time-sensitivity" of the OP Stack EL matrix, a §2.6 `comparison_to_op_stack` rewrite acknowledging that "client-diversity convergence" is a shared direction at differing scales/paces, a §2.8 `comparison_to_op_stack` and `risks_open_questions` note on the 2026-05-28 / 2026-05-31 same-month overlap, a new `op-geth (transitional)` annotation in diag-1 / diag-4 / diag-5, a new `src-6 orchestrator_context` row in Source Coverage, and a new Gap Analysis row (`OP Stack op-geth 支持的过渡性`, severity minor). No body claim was strengthened; the polish is purely additive context to prevent readers from treating the d905be1e snapshot as a permanent OP Stack feature. |
