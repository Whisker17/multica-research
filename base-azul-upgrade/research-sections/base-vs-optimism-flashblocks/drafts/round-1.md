...
draft_metadata:
  ...
  trigger_comment_id: "3d904a0c-e91a-43b4-bafd-5bb613e56929"
  ...
  produced_at: "2026-05-18T04:30:00Z"
  code_pins:
...
# Round 1 Draft — Base vs Optimism Flashblocks 机制与设计差异对比
...
- `eth_subscribe("newFlashblocks")`：在 op-reth 现行 codebase 中**待确认行号级实现**；
  本 round 经初步 grep 未定位到与 base/base 等价的命名 subscription，Phase B 标记为 OQ-4.2。
...
- **Spec**：`specs/flashblocks_p2p.md`（193 行）—— 完整设计文档。
- **Code 实现状态**：本 round 通过 grep `flblk\|Authorization\|StartPublish\|StopPublish` 在
  pinned 三处仓库内未发现对应 devp2p 子协议的实现源码；P2P 路径在 pinned commit 下处于
  **spec-only / 无 PoC** 状态。Phase B 应通过 GitHub issue + PR 搜索确认是否有 in-progress
  branch（如 `flashbots/rollup-boost#?` 开发分支）。
...
##### P2P Authorization 失败转移流程（spec）

1. 主 builder 发布 `StopPublish` 消息（带 Authorization 签名），通过 RLPx 广播。
2. P2P 网络收到 `StopPublish` 后停止对该 publisher 的 flashblock 进一步接受。
3. 新 builder 在 sequencer 切换时通过 `StartPublish` 通告身份；通过历史 flashblock 的 RLPx
   接收，新 builder 已知上一 builder 已发布的 flashblock 0..k，从 flashblock k+1 继续构建。
4. 下游 consumer 通过 Authorization 双签名校验消息真实性；无需信任 rollup-boost 中介。
5. **关键收益**：失败转移不丢失已发布 flashblock；`pending` tag 单调推进，无回退。
6. RTO 取决于 P2P propagation + sequencer leader election；理论上低于 WS 路径（无需 WS 重连）。
...
| 链 | WS HA | P2P HA |
|---|---|---|
| Base | 已部署 op-conductor + WS proxy；canonical 配置在 `base/base` 运维侧 | spec-only |
| Optimism Mainnet | 已部署 op-conductor + WS proxy | spec-only |
| Unichain | 已部署 op-conductor + WS proxy | spec-only |

- **Base TDD 描述（公开 Notion 文档）**：Base 详细规划了 op-conductor 与 rollup-boost 的串联，
  并已上线；本 round 引用为间接证据，不做代码 commit 级核对。
...
- WS HA：生产中，但被一致认为是临时方案；spec issue 与 #321（v2 议程）讨论以 P2P 替代。
- P2P HA：仅 spec，无代码；演进窗口未明确。
...
- **OQ-6.1**：op-conductor 在 rollup-boost stack 下的 RTO 实测值（Base / Unichain 公开数据）。
...
- **风险 R-6.1**：P2P 路径在缺失代码实现的情况下，任何"承诺 Authorization HA"的设计都是
  spec-bet；Mantle 不应将 P2P 列入近期 commitment。
...
flowchart TB
    ...
    subgraph P2P["P2P 路径 (spec-only, specs/flashblocks_p2p.md)"]
      direction TB
      PSEQ["sequencer<br/>+ rollup-boost"]
      PBLD["builder<br/>(publisher)"]
      PAUTH["Authorization<br/>= sequencer_sig + builder_sig"]
      PMSG["Authorized Message<br/>(StartPublish / StopPublish / payload)"]
      P2NET["devp2p 'flblk/1' mesh<br/>(RLPx)"]
      PCONS["consumer (RLPx peer)"]
      PSEQ --> PAUTH
      PBLD --> PAUTH
      PAUTH --> PMSG --> P2NET --> PCONS
      PNOTE["信任模型: Authorization 双签名;<br/>无需 rollup-boost 同步验证<br/>HA: StopPublish → StartPublish 无缝 publisher 切换<br/>RTO: 取决于 P2P propagation"]
      P2NET -.-> PNOTE
    end
```
...
flowchart LR
    ...
    subgraph P2PHA["P2P Authorization (spec-only)"]
      direction TB
      P1["主 builder StopPublish (Authorization 签名)"]
      P2["P2P mesh 接收, 停止接受该 publisher 后续 flashblock"]
      P3["新 builder StartPublish (Authorization 签名)"]
      P4["新 builder 通过 RLPx 已知 flashblock 0..k"]
      P5["从 flashblock k+1 继续构建"]
      P6["consumer 'pending' 单调推进, 无回退"]
      P7["RTO ≈ P2P propagation + leader election<br/>(理论低于 WS)"]
...
classDiagram
    ...
    class op_reth_flashblocks_optimism {
      ...
      eth_subscribe_newFlashblocks: unverified
...
| Requirement ID | Min Count | Actual | Items Citing | Resources |
|---|---|---|---|---|
| **src-1** official_docs | 4 | 4 | item-1, 2, 5, 7 | (1) `flashbots/rollup-boost/specs/flashblocks.md` (1071 行) — items 1, 2, 5, 7; (2) `specs/flashblocks_p2p.md` (193 行) — items 1, 5, 6; (3) `specs/flashtestations.md` (839 行) — item 7; (4) Flashbots 官方 RPC docs https://rollup-boost.flashbots.net/developers/flashblocks-rpc.html — item 4 (背景，未做 fetch；列为 OQ-4.3) |
| **src-2** code_analysis rollup-boost | 6 | 6 | item-1, 2, 5, 6, 7 | (1) `crates/rollup-boost/src/server.rs` (Engine API multiplexer); (2) `src/proxy.rs`; (3) `src/selection.rs:6-37` (full); (4) `src/flashblocks/{outbound.rs:58-70, inbound.rs::FlashblocksReceiverService, args.rs, launcher.rs, service.rs}`; (5) `crates/rollup-boost-types/src/{flashblocks.rs:76-88, payload.rs}`; (6) `crates/websocket-proxy/src/{auth.rs, client.rs, rate_limit.rs ~1505 LoC, registry.rs, server.rs, subscriber.rs, metrics.rs}` |
| **src-3** code_analysis base/base | 5 | 5 | item-4, 5 | (1) `crates/execution/flashblocks/src/lib.rs` (re-exports); (2) `crates/execution/flashblocks/src/validation.rs:1-189` (enums); (3) `crates/execution/flashblocks/src/rpc/{eth.rs ~634, pubsub.rs ~351, types.rs ~289}`; (4) `crates/execution/flashblocks/src/{pending_blocks.rs, processor.rs, receipt_builder.rs, state.rs, state_builder.rs, subscription.rs, traits.rs, block_assembler.rs, cache.rs, config.rs}`; (5) `crates/execution/flashblocks-node/src/{extension.rs::FlashblocksExtension::apply, lib.rs, test_harness.rs}` |
| **src-4** code_analysis optimism reth | 4 | 4 | item-3, 4, 7 | (1) `rust/op-reth/crates/flashblocks/src/{lib.rs, service.rs, worker.rs, validation.rs, sequence.rs, cache.rs, consensus.rs, payload.rs, tx_cache.rs}`; (2) `rust/op-reth/crates/flashblocks/src/pending_state.rs:1-388` (PendingStateRegistry); (3) `rust/op-reth/crates/flashblocks/src/ws/*`; (4) `rust/op-reth/crates/rpc/src/eth/pending_block.rs:1-119` (LoadPendingBlock); (附) `rust/rollup-boost/crates/flashblocks-rpc/src/{lib.rs, bin/main.rs, cache, rpc, flashblocks, metrics}` (vendored) |
| **src-5** code_analysis op-rbuilder | 2 | 2 | item-3 | (1) `rust/op-rbuilder/crates/op-rbuilder/src/builders/flashblocks/config.rs` (interval 默认 250ms); (2) `rust/op-rbuilder/crates/op-rbuilder/src/builders/flashblocks/payload.rs:~439` (gas_per_batch) + `:~797` (target_gas_for_batch 累加) + ws.rs (outbound publisher) |
| **src-6** governance_proposals | 4 | 6 | item-3, 4, 7 | (1) reth #17982 (新增 reth-optimism-flashblocks); (2) reth #21532 / commit `372802d06` (迁移 op-reth); (3) reth #17858 (native op flashblocks); (4) rollup-boost #321 (v2 议程); (5) rollup-boost #455 (压缩演进); (6) rollup-boost #456 / commit `f6d1246` (移除 flashblocks-rpc) |
| **src-7** expert_commentary | 3 | 3 | item-1, 5, 6 | (1) Flashbots blog "Introducing Rollup-Boost — Launching on Unichain"; (2) Base TDD "Rollup Boost Integration with HA Sequencer" (Notion，间接引用); (3) `base/flashblocks-websocket-proxy` README (公共 repo，本 round 未行号级核对——列为 OQ) |
| **src-8** cross_reference | 1 | 1 | item-2 | `flashblocks-network-changes` (WHI-30) final.md：Azul payload 简化（移除 `new_account_balances` / `receipts`、保留 `access_list` 但不填充） |

**总览**：8 项 source 要求全部满足或超额满足。
...
- **OQ-2.2**：`version` / `parent_flash_hash` 字段在 v2 spec 中的地位。
- **OQ-3.1**：Base / Unichain / Optimism Mainnet 部署的 F 值与 interval 实测确认。
- **OQ-3.2**：op-rbuilder MEV-aware 排序是否实现（payload.rs 行号级核对）。
- **OQ-4.1**：op-reth flashblocks (b) 是否提供 `eth_subscribe("newFlashblocks")` 等价 subscription。
- **OQ-4.2**：`base/base` 与 op-reth 在 receipts 处理 (`UnifiedReceiptBuilder` vs reth-native)
  对 indexer/wallet 的可观察差异。
- **OQ-4.3**：Flashbots 官方文档 https://rollup-boost.flashbots.net/developers/flashblocks-rpc.html
  是否仍指向 paradigmxyz/reth（应已迁移至 ethereum-optimism/optimism）。
- **OQ-5.1**：P2P spec 是否已有 PoC 实现分支（GitHub issue / PR 搜索）。
- **OQ-5.2**：`base/flashblocks-websocket-proxy` 与 `crates/websocket-proxy/` 的代码同源 diff
  规模。
- **OQ-6.1**：op-conductor 在 rollup-boost stack 下的 RTO 实测值。
- **OQ-6.2**：WS subscriber 重连后是否有 `pending` 索引 gating 防回退。
- **OQ-7.1**：v2 spec draft 是否已合入 `specs/`。
...
### Items Not Fully Resolved（needs round 2 if flagged adversarial）

- **item-1 OQ-1.1**：V5 行号级核对未完成。
- **item-3 OQ-3.2**：op-rbuilder MEV 排序未完成行号级核对。
- **item-4 OQ-4.1**：op-reth `newFlashblocks` subscription 等价性未确认。
- **item-5 OQ-5.1**：P2P PoC 分支搜索未做。
- **item-7 OQ-7.1**：v2 spec draft 状态未做。

其余 item 已达到 outline 要求的 field 全覆盖；如 adversarial round 不 flag，可进入 final
promotion 阶段。
