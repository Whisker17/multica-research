...
draft_metadata:
  outline_commit: "9bf5b0c"
  outline_round: 1
  prior_draft_commit: "186c2ecebcd810819a7c7d2b0c12651dc7bc9a2c"
  review_verdict_comment_id: "98120b59-370e-41cf-86a2-4cecce0fe78a"
  trigger_comment_id: "b4541ad4-8de9-48a0-876f-ccef90c5f535"
  multica_issue_id: "66010217-a7d4-4892-b8b0-fe9703520612"
  mode: "single-issue lightweight (no report_issue_id)"
  produced_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  produced_at: "2026-05-18T04:05:00Z"
  code_pins:
  ...
  must_verify_resolutions:
    op_supportedCapabilities: "spec-only / unimplemented — verified 0 matches in rollup-boost crates, base/base crates, ethereum-optimism/optimism rust/ subtree"
    danyalprout_reth_flashblocks: "transferred (HTTP 301) — github.com/danyalprout/reth-flashblocks → github.com/base/base"
  round_2_fixes:
    p2p_status: "Replaced 'spec-only / no PoC' framing with a 5-row P2P implementation status matrix anchored on rollup-boost-types/src/authorization.rs:15, crates/rollup-boost/src/client/rpc.rs:138/414, crates/rollup-boost/src/flashblocks/args.rs:120, optimism rust/op-rbuilder/crates/p2p/, and rust/op-rbuilder/crates/op-rbuilder/src/builders/flashblocks/{p2p.rs:8-9, payload_handler.rs, service.rs}. Items 5/6 and diagrams 3/5/6 updated."
    source_coverage: "Downgraded from 'complete (8/8)' to 'partial (5 verified / 3 unverified or stale)'. Moved Flashbots RPC doc, Base TDD, base/flashblocks-websocket-proxy (archived per GitHub API, pushed_at=2025-05-08) and other external claims into a 'Not Established / Unverified' subsection."
    op_reth_newflashblocks: "Resolved OQ-4.1: op-reth pinned at d905be1e exposes only internal Rust API (subscribe_received_flashblocks, subscribe_flashblock_sequence at rust/op-reth/crates/rpc/src/eth/mod.rs:126,131); no public eth_subscribe('newFlashblocks') RPC method found. Marked as 'not found in pinned code' in Consumer matrix and diag-6."
---

# Round 2 Draft — Base vs Optimism Flashblocks 机制与设计差异对比

> **Round 2 scope**: Revision targeted at three findings from draft-review round-1
> (verdict `98120b59`, needs-attention/major): (1) P2P status from spec-only to nuanced
> matrix; (2) source coverage from complete to partial; (3) op-reth public
> `newFlashblocks` from open question to "not found in pinned code". Unflagged sections
> from round-1 are preserved unchanged unless directly referenced by the three fixes.
...
- `op_supportedCapabilities` JSON-RPC 方法在 `flashbots/rollup-boost` spec
  ...
  base `crates/`、ethereum-optimism/optimism `rust/`）中**零匹配**。本 draft 全程将其标注为
...
**Phase B 额外发现（adversarial round 必读，可能引发 spec ↔ code drift 二次评审）**：
...
**Round 2 修正后的 P2P 状态摘要（详见 item-5 / item-6 与 diag-3、diag-5、diag-6）**：
P2P 路径**并非完全 spec-only**。`flashbots/rollup-boost@6cf697e` 已实现 `Authorization` 委派结构
体（rollup-boost-types/src/authorization.rs:15，单签者 `authorizer_sig` 由 sequencer 签发，授权
特定 builder 公钥），并通过 `FlasblocksP2PRpcClient`（client/rpc.rs:138、:414 的
`flashblocks_fork_choice_updated_v3`）把 Authorization 注入 Engine API。`ethereum-optimism/optimism@d905be1e`
在 `rust/op-rbuilder/crates/p2p/` 提供通用 libp2p 框架，在
`rust/op-rbuilder/crates/op-rbuilder/src/builders/flashblocks/{p2p.rs:8-9, payload_handler.rs, service.rs}`
中以 `/flashblocks/1.0.0` 作为 libp2p `StreamProtocol` 名实现 builder ↔ builder 的 `OpBuiltPayload`
同步通道（注意：此通道携带**完整 OpBlock** 用于 HA syncing，而非 200ms 粒度的 `FlashblocksPayloadV1`
广播）。**`StartPublish` / `StopPublish` 仅在 spec 中定义**（`specs/flashblocks_p2p.md:90-127`，0 匹
配 in `crates/`），**op-reth consumer 不订阅 `flblk/1`**（op-reth 仍只支持 WebSocket inbound 通过
`WsConnect`），且本研究未观察到任何链上的 P2P 路径**生产部署**证据。

**Round 2 修正后的源覆盖摘要（详见 Source Coverage 与 Not Established 节）**：源覆盖从"完整 (8/8)"
**下调为"部分 (5 verified / 3 partial-or-unverified)"**。`base/flashblocks-websocket-proxy` 已通过
GitHub API 确认为 archived (`archived: true`, `pushed_at: 2025-05-08`)；Flashbots RPC docs 站点、
Base TDD Notion、op-reth `newFlashblocks` 公共订阅、P2P PoC 搜索状态、v2 issue (#321) 状态、
op-rbuilder MEV 排序行号级证据均**未在本轮独立核验**，已迁出 Source Coverage 主表、归入下方
"Not Established / Unverified" 子节。
...
- **CLI / 环境变量差异**：rollup-boost 的差异化注入点在 CLI flags（`--builder-url`,
  ...
  际部署值不在代码中暴露，本 round 仅断言"代码无 chain-specific patch"，未对部署配置做断言。
...
- **OQ-1.1**：rollup-boost 主线对 `engine_getPayloadV5` 的支持完整度尚未在本 round 做行号级核
  对；OP Stack Azul 的 V5 引入了 `executionRequests` 字段（pectra 风格），需在 adversarial round
  验证 server.rs 已落实。
...
- **types crate API**：`use rollup_boost_types::flashblocks::FlashblocksPayloadV1;` 为 consumer 端
  ...
  共同依赖（后者通过 cargo workspace 与外部 git 依赖；具体方式由 op-reth Cargo.toml 决定，本
  round 未做精确依赖图 dump）。
...
重写；spec 文件本 round pinned 版本为 6cf697e3 HEAD。
...
- **OQ-2.1**：spec 的 SSZ 表述何时与 code 同步？已知现状但未确认 upstream 修正进度。建议在
  adversarial round 通过 GitHub issue 搜索 `rollup-boost SSZ encoding` 给出明确状态。
...
- **Repo 物理位置**：`flashbots/op-rbuilder` 的源码在本 dispatch 的 nested 工作树中以
...
- **配置层差异**：F 值与 interval 在 Base / Unichain 部署中可能不同（运维参数，非代码常量）。
  ...
  flashblock = F 取 4（1s L2 block / 250ms）。**本 round 无法以代码 grep 证实部署 F 值**——这是配
  置层差异。建议 adversarial round 通过 chain config（op-conductor / Notion TDD / blog post）
  做最终核对。
...
- 外部：Base 与 Unichain 的公开发布博客（Base "Sub-second blocks for Base Mainnet"、Flashbots
  "Introducing Rollup-Boost — Launching on Unichain"）——本 round 仅作背景，未引代码 commit。
...
- **`eth_subscribe("newFlashblocks")` 等价的公开 JSON-RPC 订阅在 pinned commit 下 not found**：
  ...
  可由 JSON-RPC 客户端通过 `eth_subscribe("newFlashblocks")` 调用的公共订阅 endpoint。本 round
...
- Flashbots 官方文档：https://rollup-boost.flashbots.net/developers/flashblocks-rpc.html
  （建议 adversarial round 复核此页是否仍指向 paradigmxyz/reth 而非 ethereum-optimism/optimism）
...
- **OQ-4.1**：op-reth flashblocks (b) 是否提供 `eth_subscribe("newFlashblocks")` 等价语义？base/base
  侧明确有，op-reth 侧本 round grep 未定位等价命名。Phase B 应给出确认。
...
- **OQ-4.3**：Flashbots 官方文档 (rollup-boost.flashbots.net/developers/flashblocks-rpc.html)
  历史指向 paradigmxyz/reth；在 reth #21532 后是否已更新到 ethereum-optimism/optimism？本 round
  未做 fetch 核对（建议 adversarial round 通过 WebFetch 验证）。
...
- **Base canonical proxy**：`base/flashblocks-websocket-proxy` repo（独立维护，提供 Base
  公共 WS 端点 https://...flashblocks.base.org/...）。Phase B 通过 README 确认其与
  `rollup-boost/crates/websocket-proxy/` 是**同源代码不同部署**——Base 维护时间更长，含部分
  Base-specific 运维 patch；不在本 pin 工作树内做行号级核对。
...
- **Code 实现状态（round-2 重新核验）**：P2P 路径**并非完全 spec-only**——本 round 在 pinned
  commits (`rollup-boost@6cf697e`, `optimism@d905be1e`) 下实测发现以下三层非空 scaffolding：
...
| **op-reth flashblocks P2P consumer** | n/a | ❌ **未实现** | op-reth 消费 flashblock 仅走 WebSocket（`rust/op-reth/crates/flashblocks/src/ws/`），未订阅 `flblk/1` libp2p 流；本 round 在 `rust/op-reth/` 子树 0 匹配 `flblk`、`/flashblocks/1.0.0`、`libp2p` |
...
- **关键区分**：上表把"spec-defined `flblk/1` + HA `StartPublish`/`StopPublish` 未完整实现"与
  "builder-side P2P scaffolding 已存在"**显式分离**。本 round-2 修正了 round-1 把 P2P 一律标为
  "spec-only / 无 PoC" 的过度简化结论。
...
- **wire 协议**：devp2p 子协议 `flblk/1`。
- **核心消息**：
  - `Authorization`（双签名结构：sequencer pubkey + signature; builder pubkey + signature）
  - `Authorized Message`（包 Authorization + payload）
  - `StartPublish` / `StopPublish`（HA 切换消息——通告 builder 角色变更）
  - `Builder Public Key` / `Publisher` 概念
- **信任模型**：rollup-boost 同步验证被**取消**——所有 flashblock 通过 Authorization 双签名建
  立信任链，RLPx 节点单独验证签名而无需运行完整 sequencer 验证逻辑。
- **多 builder 协调**：通过 `StartPublish` / `StopPublish` 实现 publisher 角色平滑迁移。

#### chain_specific_diffs

- **WebSocket 部署**：
  - Base：使用 `base/flashblocks-websocket-proxy` canonical proxy（独立 repo，公共端点）；与
    base/base 节点强耦合。
  - Optimism Mainnet / Unichain：直接使用 `crates/websocket-proxy/` 部署；端点由各自运维方
    提供（Alchemy / QuickNode / 自建）。
- **P2P 部署**：两链均处于 spec-only 阶段，无实质部署差异。
...
- **P2P 路径**：spec 已合并入 `flashbots/rollup-boost/specs/flashblocks_p2p.md`；代码层未启动；
  HA 模型详见 item-6。
...
- 外部：`base/flashblocks-websocket-proxy` README（GitHub 公共 repo，本 round 未做行号核对）

#### mantle_integration_implications

- **WS 起步**：低成本（部署 `crates/websocket-proxy/` 实例或直接复用 rollup-boost 内置 publisher）；
  但单点故障 + fan-out 拓扑限制 horizontal scaling。
- **P2P 演进**：在 P2P 实现完成前不能提前下注；Mantle 应在 WS 起步基础上预留 P2P 切换点（
  在 reth 节点上保留对 `flblk/1` capability 的接入空间）。
- **运维负担**：Redis cluster + Brotli 解压客户端 = Mantle 需自建或采购 RPC provider 集成。

#### open_questions_and_risks

- **OQ-5.1**：P2P spec 是否有 PoC 实现分支？本 round 未在 pinned commit 内发现，建议 adversarial
  round 在 flashbots/rollup-boost issue tracker 搜索 `p2p` `flblk`。
...
- **op-conductor 集成（Base TDD 描述，代码不在 pinned commits 范围内）**：op-conductor 仓库本
  round 未直接 checkout；以 Base TDD（Notion: "TDD: Rollup Boost Integration with HA
  Sequencer"）+ Flashbots blog 作为间接证据。
- **P2P HA 模型**：spec-only（specs/flashblocks_p2p.md §"Publisher Lifecycle"）。
...
##### P2P Authorization 失败转移流程（**部分实现 + spec**，round-2 修正）
...
4. 下游 consumer 通过 Authorization 双签名校验消息真实性 —— **设计上"双签名"在 spec，
   round-2 实测 `authorization.rs:15` 仅含单签者 `authorizer_sig`（sequencer 签 builder pubkey）**，
...
7. RTO 取决于 P2P propagation + sequencer leader election；理论上低于 WS 路径（无需 WS 重连）；
   本 round 未观察到任何生产环境的 P2P HA 部署数据。

#### chain_specific_diffs

- **当前 HA 部署状态（round-2 修正）**：
...
- **Base TDD 描述（公开 Notion 文档）**：Base 详细规划了 op-conductor 与 rollup-boost 的串联，
  本 round 引用为**间接证据**——本研究**未独立访问** Base TDD Notion 链接，亦未在
...
- **中期**：P2P Authorization 成熟后切换；切换面在 builder + consumer 端，rollup-boost sidecar
  侧需评估是否保留。
...
- **OQ-6.1**：op-conductor 在 rollup-boost stack 下的 RTO 实测值（Base / Unichain 公开数据）—— 本
  round 仅有 Flashbots blog "Introducing Rollup-Boost — Launching on Unichain" 的定性陈述；
  实测数据未获取，已移入 Source Coverage → Not Established。
...
- **Flashblocks v2**：rollup-boost issue #321（v2 议程，公开 issue，本 round 不在工作树内）；
...
- 时序差异：Base 与 Optimism 的部署节奏可能不同（Base 可能更激进追新；Optimism 可能更保守对齐
  OP Stack hardfork 节奏）。本 round 不做主观判断。
...
- **OQ-7.1**：v2 spec draft 是否已合入 specs/ 目录？本 round pinned spec/ 下未发现 v2 文件，
  Phase B 建议在 rollup-boost issue #321 内核对最新状态。
...
| HA | op-conductor + WS HA（生产） | op-conductor + WS HA | op-conductor + WS HA | **路径相同**；P2P 均为 spec-only |
...
| Propagation | WS 起步 → 等 P2P 实现 → 切换 | 直接押注 P2P（不推荐，spec-only） |
...
flowchart TB
    ...
    subgraph P2P["P2P 路径 (部分实现 — round-2 修正; spec @specs/flashblocks_p2p.md)"]
      direction TB
      ...
      PNOTE["实际状态 (round-2):<br/>• rollup-boost-side Authorization 注入 ✅<br/>• op-rbuilder libp2p 框架 ✅ 仅 sealed block sync<br/>• StartPublish/StopPublish HA 控制 ❌ spec-only<br/>• consumer-side P2P 订阅 ❌ 未实现<br/>• 生产部署 ❌ 0 公开证据"]
      P2NET -.-> PNOTE
    end
```
...
flowchart LR
    ...
    subgraph P2PHA["P2P Authorization (部分实现 — round-2 修正)"]
      direction TB
...
classDiagram
    ...
    class op_reth_flashblocks_optimism {
      ...
      eth_subscribe_newFlashblocks: not_found_public_rpc (round-2)
...
> **Round-2 修正**：源覆盖从 round-1 的"完整 (8/8)" **下调为"部分 (5 verified / 3
> partial-or-unverified)"**。下方主表仅保留**本研究在 pinned commits 内独立完成行号级核对**的
> 源；外部文档（Flashbots blog、Base TDD Notion、`base/flashblocks-websocket-proxy` README、
> Flashbots RPC docs 网站）与若干 sub-claim（RTO 实测值、F 值生产值等）已迁出，归入下方
> "Not Established / Unverified" 子节。
...
|---|---|---|---|---|---|
| **src-2** code_analysis rollup-boost | 6 | 6 | item-1, 2, 5, 6, 7 | ✅ verified | (1) `crates/rollup-boost/src/server.rs` (Engine API multiplexer); (2) `src/proxy.rs`; (3) `src/selection.rs:6-37` (full); (4) `src/flashblocks/{outbound.rs:58-70, inbound.rs::FlashblocksReceiverService, args.rs, launcher.rs, service.rs}` + `src/flashblocks/args.rs:120-146` (FlashblocksP2PArgs round-2 新增) + `src/client/rpc.rs:138,414-472` (FlasblocksP2PRpcClient round-2 新增); (5) `crates/rollup-boost-types/src/{flashblocks.rs:76-88, payload.rs, authorization.rs:15 round-2 新增}`; (6) `crates/websocket-proxy/src/{auth.rs, client.rs, rate_limit.rs ~1505 LoC, registry.rs, server.rs, subscriber.rs, metrics.rs}` |
...
| **src-6** governance_proposals | 4 | 3 hard-pinned + 3 referenced | ⚠️ partial | **Hard pinned（commit/PR 号级别）**：(1) reth #21532 / commit `372802d06` (迁移 op-reth)；(2) rollup-boost #456 / commit `f6d1246` (移除 flashblocks-rpc)；(3) rollup-boost #477 (本 round 的 pin "fix health check"). **Referenced 但未在本研究中独立打开 PR/issue 验证内容**：reth #17982、reth #17858、rollup-boost #321 (v2 议程)、rollup-boost #455 (压缩演进)。详见 Not Established → gov-1。|
...
**总览（round-2）**：5/8 requirement 由本研究在 pinned commits 内独立 verified；3/8 partial 或包含
unverified external claims。Round 2 不再断言"完整覆盖"，并把 Mantle 接入决策依赖的外部 claims
明确移入 Not Established 子节。

### Not Established / Unverified（round-2 新增）

以下条目在 round-1 中以"verified" / "Open Question" 等口径混合提及，round-2 显式归入"未确立"
分类，**不在本研究的结论性陈述链路上**，需在后续 round 或 Mantle 实施阶段独立核对：

| ID | Subject | Round-1 处理 | Round-2 状态 | What's needed to verify |
|---|---|---|---|---|
| **docs-1** | Flashbots 官方 RPC docs (`rollup-boost.flashbots.net/developers/flashblocks-rpc.html`) 当前指向何 reth fork | 列为 OQ-4.3 | **未独立 fetch**；不确定页面是否已从 paradigmxyz/reth 更新到 ethereum-optimism/optimism | 直接 WebFetch 该 URL 并比对 |
| **tdd-1** | Base TDD "Rollup Boost Integration with HA Sequencer" Notion 文档实际内容（HA 流程、op-conductor 配置细节） | 列为 expert_commentary src-7-(2) | **本研究未访问 Notion 链接**；间接引用风险 | 获得 Notion 文档访问 / Base 公开 RFC 链接 |
| **wsproxy-1** | `base/flashblocks-websocket-proxy` 当前维护状态与代码同源度 | 列为 expert_commentary src-7-(3) + OQ-5.2 | **GitHub API 已确认 archived (`archived: true`, `pushed_at: 2025-05-08`, `fork: false`)**；与 `rollup-boost/crates/websocket-proxy/` 的 diff 规模未做 | git diff 两个 codebases 的 LoC 级别比对；判定是否已被 rollup-boost 上游替代 |
| **p2p-poc-1** | P2P spec `flblk/1` 路径是否有独立 PoC 分支 / WIP PR | 列为 OQ-5.1 | **本研究未在 flashbots/rollup-boost issue tracker 与 PR 中独立搜索** "p2p" / "flblk" 关键字 | gh issue search + 在 rollup-boost 仓库 branch list 内 grep `p2p` |
| **v2-issue-1** | rollup-boost issue #321 (v2 议程) 的当前进度与 v2 spec draft 是否已合入 specs/ | 列为 OQ-2.2 / OQ-7.1 | **本研究未独立打开 issue #321 内容核对** | `gh issue view 321` 或 WebFetch |
| **deploy-f-1** | Base (F=10 / 200ms)、Unichain (F=4 / 250ms)、Optimism Mainnet 的实际部署 F 值 | 在 item-3 / item-8 / Executive Summary 中作 "公开博客论断" | **未做 chain config / op-conductor 配置级核对**；仅依据 Flashbots blog 与 Base 发布稿的定性 | 访问 op-conductor 配置文件 或 chain genesis spec |
| **rto-1** | op-conductor + WS HA 的实测 RTO（Base / Unichain 公开数据） | 列为 OQ-6.1 | **未获取**；item-6 给出 "5-15s 典型" 是工程经验范围，非实测 | 公开 incident report、SRE 复盘文档、监控指标 |
| **mev-1** | op-rbuilder 是否支持 priority-fee 之外的 MEV-aware 排序（bundle-aware 等）行号级证据 | 列为 OQ-3.2 | **未做** `payload.rs` 全文逐行核对 | 完整阅读 `rust/op-rbuilder/crates/op-rbuilder/src/builders/flashblocks/payload.rs` 全文 |
| **op-reth-newfb-resolved** | op-reth public `eth_subscribe("newFlashblocks")` JSON-RPC | round-1 列为 OQ-4.1 | **✅ round-2 已 resolve**：`rust/op-reth/crates/rpc/src/eth/mod.rs:126,131` 仅暴露内部 Rust API (`subscribe_received_flashblocks`、`subscribe_flashblock_sequence`)；本 round 在 `rust/op-reth/` 子树未发现等价 public `eth_subscribe` kind 注册 | 已完成 |
...
- ✅ **`op_supportedCapabilities`** — spec 定义但代码未实现。Phase B 通过 grep 在 rollup-boost
...
### Newly Surfaced Spec ↔ Code Drift（adversarial round 必读）
...
这两处 drift 影响所有 consumer 与第三方实现，且与 issue #321（v2 议程）潜在相关——adversarial
round 应在 v2 spec draft 内核实"是否补齐字段"。

### Open Questions（round-2 re-clustered）

> **Round-2 修正**：(a) OQ-4.1 由 round-2 直接 resolved（详见下方）；(b) 凡 OQ 实质上是
> "未独立核对的外部源" 的，已迁出 OQ 列表、归入 Source Coverage → Not Established 子节，避免
> "OQ 看起来很多但其实大多是同一根因（未访问外部 doc）"造成 noise。

#### Resolved in Round 2

- **OQ-4.1 (resolved)**：op-reth flashblocks (b) **不提供**等价于 `eth_subscribe("newFlashblocks")`
  的公共 JSON-RPC 订阅。在 pinned commit `d905be1e` 下，`rust/op-reth/crates/rpc/src/eth/mod.rs:126`
  与 `:131` 仅暴露内部 Rust API 方法 `subscribe_received_flashblocks()` 与
  `subscribe_flashblock_sequence()`（返回 `tokio::sync::broadcast::Receiver`，供集成方代码消费）；
  在 `rust/op-reth/` 子树内对 `eth_subscribe`、`SubscriptionKind`、`new_flashblocks`、
  `newFlashblockTransactions` 字面量 grep 仅在 exex 示例中匹配，未在 `crates/rpc/` 主体的 PubSub
  处理代码内发现等价 subscription kind 注册。Consumer 矩阵（diag-6）与 item-4 (b) 已同步标注。

#### Still open (本研究范围内，非外部源依赖)
...
- **OQ-2.1**：spec ↔ code wire-format drift（SSZ vs JSON）的上游修正进度（GitHub issue 搜索）。
- **OQ-2.2**：`version` / `parent_flash_hash` 字段在 v2 spec 中的地位（与 Not Established →
  v2-issue-1 同根，但本身是规范层问题，保留 OQ）。
...
- **OQ-8.1**：Mantle 自身 chain config 的产品决策（block time、F、JWT）——非本 round 范围。
- **OQ-8.2**：op-reth flashblocks crate 与 reth `c4c690f7` 主线（alloy 1.6.0）的兼容性。

#### Moved to Source Coverage → Not Established（非 OQ，是源核对缺口）

以下原 OQ 已迁入 Not Established 子节，因为它们的解决路径是"独立核对外部资源"而非"本研究继续深挖
pinned commit"：

- 原 OQ-3.1 → **deploy-f-1**（部署 F 值）
- 原 OQ-3.2 → **mev-1**（op-rbuilder MEV 排序）
- 原 OQ-4.3 → **docs-1**（Flashbots RPC docs 站点）
- 原 OQ-5.1 → **p2p-poc-1**（P2P PoC 分支搜索）
- 原 OQ-5.2 → **wsproxy-1**（base/flashblocks-websocket-proxy diff；额外信息：已 archived）
- 原 OQ-6.1 → **rto-1**（op-conductor RTO 实测）
- 原 OQ-7.1 → **v2-issue-1**（v2 spec draft 状态）
...
- **R-5.2**：spec 与 code 在 wire 编码上的不一致（SSZ vs JSON / RLP）对"按 spec 实现"的第三方
  立即不兼容。
- **R-6.1**：P2P 路径仍仅 spec，不应纳入近期 commitment。
- **R-7.1**：协议演进窗口与 OP Stack hardfork 不对齐风险。
- **R-8.1**：Mantle 路径切换（(c) 起步 → (b) 深度集成）的 pending 语义切换用户感知。
- **R-8.2**：双 roadmap 并行跟随成本。

### Items Not Fully Resolved（round-2 状态）

**Round-2 resolved**:
- ✅ **item-4 OQ-4.1**：op-reth public `newFlashblocks` subscription 已确认 "not found in pinned
  code"（仅内部 Rust API，详见上方 Resolved in Round 2 与 Source Coverage → op-reth-newfb-resolved）。
- ✅ **item-5 P2P 实现状态**：由 round-1 的 "spec-only / no PoC" 修正为 5-row 实现状态矩阵
  （详见 item-5 §"Code 实现状态"）。

**Still open within research scope（可在后续 round 内继续）**:
- **item-1 OQ-1.1**：V5 行号级核对未完成。
- **item-2 OQ-2.1**：spec ↔ code wire-format drift 上游修正进度未做（依赖 v2-issue-1 外部源）。
- **item-4 OQ-4.2**：receipts builder 差异对 indexer/wallet 的影响未做。
- **item-6 OQ-6.2**：WS subscriber 重连 gating 推测未确认。

**Not Established 子节包含的源覆盖缺口**（迁出 Items Not Fully Resolved；处理路径不同）：
docs-1（Flashbots RPC docs 站点）、tdd-1（Base TDD Notion）、wsproxy-1
（base/flashblocks-websocket-proxy 已 archived per GitHub API，diff 未做）、p2p-poc-1（PoC 分支
搜索）、v2-issue-1（v2 spec draft 状态）、deploy-f-1（部署 F 值）、rto-1（HA RTO 实测）、mev-1
（op-rbuilder MEV 排序）。

如 adversarial round 不 flag 上述未解决项，可进入 final promotion 阶段；本研究的核心 commit-pinned
结论（rollup-boost / op-rbuilder / op-reth flashblocks / base/base flashblocks-node 的代码层
共享与差异）不依赖任何 Not Established 条目。
...
| Round | Action | Notes |
|---|---|---|
| 1 | Initial draft | First pass against approved outline (outline commit `9bf5b0c`, advanced from `candidate` to `approved` via Orchestrator Review Verdict comment `1d567755`). Verified the two must-verify items (`op_supportedCapabilities`, `danyalprout/reth-flashblocks`) and surfaced two additional spec ↔ code drifts (wire format SSZ vs JSON; `FlashblocksPayloadV1` field set discrepancy). 8 items covered with all 8 fields; 7 Mermaid diagrams produced. Mode: single-issue lightweight (no `report_issue_id`). Prior draft commit: `186c2ec`. |
| 2 | Targeted revision | Addressed three findings from draft-review round-1 (verdict comment `98120b59`, needs-attention/major) under trigger comment `b4541ad4`. **Fix #1 (Major, P2P status)**: replaced "spec-only / no PoC" framing with a 5-row P2P implementation status matrix in item-5 §"P2P 路径" (`rollup-boost-types/src/authorization.rs:15`, `crates/rollup-boost/src/client/rpc.rs:138,414-472`, `crates/rollup-boost/src/flashblocks/args.rs:120-146`, `optimism/rust/op-rbuilder/crates/p2p/`, `optimism/rust/op-rbuilder/crates/op-rbuilder/src/builders/flashblocks/{p2p.rs:8-9, payload_handler.rs}` ✅; `StartPublish/StopPublish` + op-reth consumer + production deployment ❌); updated item-6 HA flow, diag-3 (WS vs P2P), diag-5 (HA failover), and diag-6 (Consumer matrix) accordingly. **Fix #2 (Major, source coverage)**: downgraded from "complete (8/8)" to "partial (5 verified / 3 partial-or-unverified)"; created Not Established / Unverified subsection enumerating Flashbots RPC docs (docs-1), Base TDD Notion (tdd-1), `base/flashblocks-websocket-proxy` (wsproxy-1, confirmed `archived: true` / `pushed_at: 2025-05-08` via GitHub API), P2P PoC search (p2p-poc-1), v2 issue #321 state (v2-issue-1), deployment F values (deploy-f-1), HA RTO (rto-1), op-rbuilder MEV ordering (mev-1). **Fix #3 (Minor, op-reth `eth_subscribe("newFlashblocks")`)**: resolved OQ-4.1 — `rust/op-reth/crates/rpc/src/eth/mod.rs:126,131` exposes only internal Rust API (`subscribe_received_flashblocks`, `subscribe_flashblock_sequence` returning `tokio::sync::broadcast::Receiver`); no public `eth_subscribe` JSON-RPC kind found in `rust/op-reth/crates/rpc/` PubSub code. Marked as "not_found_public_rpc" in diag-6 and item-4 (b). Round-1 sections not directly touched by these three fixes are preserved unchanged. |
