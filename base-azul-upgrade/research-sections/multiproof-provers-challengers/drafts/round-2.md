...
artifact_paths:
  ...
  draft: base-azul-upgrade/research-sections/multiproof-provers-challengers/drafts/round-2.md
...
`process_invalid_proposal()` → `initiate_proof()` 的双路径分发（driver.rs L353-L403 +
L486-L557）只服务 Path 1 / 3 / 4：

- `InvalidTeeProposal` → `try_tee_first = true`，`intent = Challenge`；TEE-first 成功 →
  `submit_dispute(challenge, ...)`；TEE-first timeout / 失败 → 立即 ZK fallback，
  fallback 也走 `intent = Challenge`。
- `InvalidDualProposal` → `try_tee_first = true`，`intent = Nullify`；同上路径，但
  ZK fallback `intent = Nullify`。
...
flowchart TD
    ...
    F -->|InvalidTeeProposal<br/>tee=1 zk=0 ci=0| G1[process_invalid_proposal<br/>try_tee_first=true<br/>intent=Challenge<br/>driver.rs L275-L403]
    ...
    G1 --> K[initiate_proof TEE-first<br/>driver.rs L486-L557]
    G2 --> K
    G3 --> Qd[initiate_zk_proof direct<br/>driver.rs L556-L557]
    ...
    O -->|yes| P[submit_dispute<br/>PROOF_TYPE_TEE&#124;&#124;sig]
    O -->|timeout / err| Q[ZK fallback<br/>build_zk_request<br/>driver.rs L515-L528]
    ...
    V -->|invariant ok| W[submit_dispute<br/>PROOF_TYPE_ZK&#124;&#124;groth16<br/>nullify or challenge]
