...
status: final
order: 5
dependencies: [multiproof-architecture]
multica_issue_id: 1914fec7-71af-486c-a7b3-729f89218216

artifact_paths:
  outline: base-azul-upgrade/outlines/multiproof-provers-challengers.md
  draft: base-azul-upgrade/research-sections/multiproof-provers-challengers/drafts/round-3.md
...
promotion_metadata:
  approved_draft_path: base-azul-upgrade/research-sections/multiproof-provers-challengers/drafts/round-3.md
  approved_draft_round: 3
  approved_draft_commit: 627466d031fc0726997e8be1fa2494b19074779b
  approval_evidence_comment: 5bf80eed-f0ac-4e74-970a-942a43c838e4
  approval_verdict: approve
...
2. **Challenger**（`crates/proof/challenge`）—— 扫描 `IN_PROGRESS` games 并按
   `scanner.rs::classify()` 把它们分入四种合法 GameCategory（`InvalidTeeProposal`、
   `InvalidDualProposal`、`InvalidZkProposal`、`FraudulentZkChallenge`），对错误的
   TEE / dual 提案走 TEE-first + ZK fallback、对错误的 ZK 单证走直接 ZK，对针对
   TEE 提案的伪造 ZK 挑战（Path 2）走 **直接 ZK Nullify**（无 TEE-first）。
...
- **ASR snapshot block 语义（round-2 修订）**：`anchor_snapshot()` 直接调
  `provider.get_block_number()` 取 latest L1 block，并把 `getAnchorRoot()` 与
  `anchorGame()` 同步 pin 到该 block（`anchor_state_registry.rs` L132-L150，
  `futures::try_join!`）。这与 proposer 的 finalized 设置 **完全独立**。
  `DriverConfig.allow_non_finalized` 仅影响 proposal planning 阶段使用 `safe_l2`
  还是 `finalized_l2`（`pipeline.rs` L983-L990），不会改变 ASR snapshot 取哪个
  L1 高度。`game_count` 仍是另一次独立的 L1 RPC 读，因此与 ASR snapshot 不属于
  同一 L1 快照；这一 race 由 GameAlreadyExists 处理路径补偿。
...
- Scanner 输出：对每个游戏返回 `(teeProver, zkProver, counteredByIntermediateRootIndexPlusOne)`
  三元组，并分类为（严格对齐 `scanner.rs::classify()` L514-L577）：

| `(has_tee, has_zk, countered_index)` | GameCategory / 处理 |
|---|---|
| `(true, false, 0)` | `InvalidTeeProposal` —— TEE 单证未被挑战；TEE-first 路径，ZK 兜底 |
| `(true, true, 0)` | `InvalidDualProposal` —— TEE + ZK 双证未被挑战；TEE-first 路径，ZK 兜底 |
| `(true, true, ci > 0)` | `FraudulentZkChallenge { challenged_index: ci - 1 }` —— TEE 提案被 ZK 挑战；若 challenger 重算 canonical 与 game 公开的 intermediate root 一致，则 ZK 挑战为伪 |
| `(false, true, 0)` | `InvalidZkProposal` —— ZK 单证未被挑战；只能走 ZK 路径（无 TEE-first） |
| `(true, false, ci > 0)` | 异常 —— `error!` 后 skip；`ci > 0` 必由 `challenge()` 触发（同时设置 `zkProver`），单独出现疑似合约 bug |
| `(false, true, ci > 0)` | 异常 —— `warn!` 后 skip；仅在全局 `TEE_VERIFIER.nullify()` 把 TEE 证据从一个被挑战的 game 上删除后才可能出现（即 TEE soundness 失效 / 密钥泄露） |
| `(false, false, _)` | fully nullified —— scanner 在 `evaluate_game` 阶段会先过滤掉这种状态，不会进入 `classify`；若到达，`warn!` 后 skip |
...
loop {
    ...
    for candidate in candidates {
        match candidate.category {
            InvalidTeeProposal | InvalidDualProposal
                => process_invalid_proposal(...)   // try_tee_first=true; ZK fallback
            InvalidZkProposal
                => process_invalid_proposal(...)   // try_tee_first=false; direct ZK
            FraudulentZkChallenge { challenged_index }
                => process_fraudulent_zk_challenge(...) // direct ZK Nullify, no TEE-first
            Anomaly                                     => log + skip
        }
    }
...
`process_invalid_proposal()` → `initiate_proof()` 的双路径分发（driver.rs L341-L403 +
L486-L558）只服务 Path 1 / 3 / 4：

> **关键不变量（TEE proofs 永远 nullify）**：`initiate_proof()` 的 docstring（driver.rs
> L477-L478）显式声明 "TEE proofs always use `nullify()` regardless of `intent`"。
> 实现层由 `PendingProof::ready_tee()`（`pending.rs` L158-L178）强制保证 —— TEE-first
> 成功路径在构造 `ReadyToSubmit` entry 时把 `intent` **覆盖** 为
> `DisputeIntent::Nullify`，submit intent 因此与传入 `initiate_proof()` 的 `intent`
> 参数解耦。`intent` 参数的真实语义是 **"如果走 ZK fallback / 直接 ZK，最终 calldata
> 用 `challenge()` 还是 `nullify()`"**，它通过 `zk_fallback_intent`（driver.rs
> L516-L528）以及 `initiate_zk_proof()` fall-through 调用（driver.rs L556-L557）
> 传递到 ZK 路径。

- `InvalidTeeProposal` → `try_tee_first = true`，`intent = Challenge`。
  - **TEE-first 成功**：driver.rs L513-L538 拿到 `proof_bytes` 后构造
    `PendingProof::ready_tee(proof_bytes, invalid_index, expected_root,
    zk_fallback_request = Some(req), zk_fallback_intent = Some(Challenge))`；
    `ready_tee` 把该 entry 的 submit intent 写为 `Nullify`（pending.rs L177）。
    后续 `poll_or_submit()` → `submit_dispute(..., Nullify)` →
    `encode_nullify_calldata(PROOF_TYPE_TEE || sig, invalid_index, expected_root)`。
    **Path 1 的 TEE-first 成功 on-chain 调用 `nullify()`**，不是 `challenge()`：
    单证 TEE proposal 在 challenger 视角下被一个有效 TEE proof 直接置零即可，
    无需把额外的 ZK 证据安装到 game 上。
  - **TEE-first timeout / 提交失败 → ZK fallback**：`initiate_proof()`
    L556-L557 fall-through 到 `initiate_zk_proof(.., DisputeIntent::Challenge)`；
    ZK proof 拿到后 `submit_dispute(..., Challenge)` →
    `encode_challenge_calldata(PROOF_TYPE_ZK || groth16, invalid_index,
    expected_root)`。**Path 1 的 ZK fallback 才真正走 `challenge()`**：
    此时 game 尚未设置 `zkProver`，challenger 需要 `challenge()` 同时安装自己的
    ZK 证据、置位 `counteredByIntermediateRootIndex` 并 slash TEE proposer 的 bond。
- `InvalidDualProposal` → `try_tee_first = true`，`intent = Nullify`。
  - **TEE-first 成功**：同样落到 `ready_tee(...)`，submit intent 仍是 `Nullify`
    （与 `intent` 参数一致，无需覆盖语义） → `submit_dispute(..., Nullify)` →
    `nullify()`。
  - **TEE-first 失败 → ZK fallback**：`initiate_zk_proof(.., Nullify)` →
    `submit_dispute(..., Nullify)` → `nullify()`。Path 4 game 上已同时存在
    TEE + ZK 双证；任一证据被 `nullify()` 即可置零对应槽位。TEE 槽位 nullify
    成功后，game 会被 challenger 在下一轮 scan 重新归类为 Path 3（`InvalidZkProposal`），
    再用 ZK proof 把 ZK 槽位 nullify。
- `InvalidZkProposal` → `try_tee_first = false`；直接走 `initiate_zk_proof(...,
  DisputeIntent::Nullify)` —— 没有 TEE-first。
- `FraudulentZkChallenge` 类别在 `process_invalid_proposal` 入口被显式拒绝
  (`driver.rs` L385-L399 `error!` + `debug_assert!` + `return Err`)，必须走专属
  `process_fraudulent_zk_challenge()` 入口。
- ZK fallback / 直接 ZK 都走 polling 模式（`build_zk_request()` →
  `initiate_zk_proof()` → `PendingProofs::poll()`），最多 3 次完整 retry
  （`MAX_PROOF_RETRIES`）。

`process_fraudulent_zk_challenge()`（driver.rs L412-L473，Path 2 专用）只对
`counteredByIntermediateRootIndex - 1` 那一个 checkpoint 做 canonical 验证：

1. `verifier_client.intermediate_output_root(game, challenged_index)` 拉取 on-chain 根；
2. `candidate.checkpoint_start_block(challenged_index + 1)` 算出对应 L2 高度；
3. `validator.compute_output_root(checkpoint_block)` 重算 canonical 根；
4. on-chain 根 == canonical 根 → 该 ZK 挑战是欺诈 → **直接** 调用
   `self.initiate_zk_proof(candidate, challenged_index, on_chain_root,
   DisputeIntent::Nullify)`，**不走 TEE-first**。这是因为：

   - Path 2 的"原 TEE 提案"本身已被 challenger 验证为正确（隐含 challenger 信任的
     canonical root 与 TEE 提案的 intermediate root 在 `challenged_index` 一致），
     此时再用 TEE proof nullify 该 ZK challenge 在密码学上是冗余的；
   - 更重要的是，Nullify 的目标槽位是 `zkProver`/`countered_index` 而非 TEE 槽位；
     `PendingProof` 文档（`pending.rs` L58-L63）明确："Path 2 nullifies a fraudulent
     ZK challenge with a ZK proof"，submitter 编码 `PROOF_TYPE_ZK || groth16` 后
     `AggregateVerifier.nullify()` 才能在合约侧路由到 `_proofRefutedUpdate(ZK)`
     分支，把 `zkProver` 槽位清零。

   反之，TEE-first 仅对应 `InvalidTeeProposal` / `InvalidDualProposal` 路径
   （driver.rs L372-L403 显式枚举），不会被 `FraudulentZkChallenge` 走到。
...
4. PendingProofs poll → `GetProof(OnChainSnark)` → **变长** `SP1ProofWithPublicValues::bytes()`
   （Groth16 verifier selector `0x0e78f4db` 开头；测试仅约束其 length 小于 bincode
   编码的整个 SNARK receipt 字节数，详见 `zk/service/src/server/get_proof.rs`
   L24-L37 + L221-L241）+ `PROOF_TYPE_ZK` 前缀 → submitter `challenge(proofBytes,
   intermediateIndex, intermediateRoot)`。65 byte 是 TEE ECDSA signature 大小
   （`encode_dispute_proof_bytes` 输出 `proofType(1) + signature(65) = 66` 字节，
   `proof_encoder.rs` L88-L115），不是 ZK proof 的固定大小。
5. `AggregateVerifier.challenge()` 切换 game 到 ZK 路径；TEE 提案 bond 被 slash，
   challenger 进入 bond claim lifecycle（item-4）。

**scenario 3 — Fraudulent ZK challenge (ZK-powered nullification)**

1. 一个 TEE 提案先到位（state = `(tee≠0, zk=0, ci=0)`）。另一 challenger 对该 TEE
   提案执行 `challenge()`，把 `zkProver` 写为自己的地址、`countered_index = ci > 0`
   —— game 状态变为 `(teeProver != 0, zkProver != 0, countered_index > 0)`。
2. 本 challenger scanner 看到 `(true, true, ci > 0)`；按 `scanner.rs::classify()`
   L549-L553 分类为 `FraudulentZkChallenge { challenged_index: ci - 1 }`。**注意**
   伪造 ZK challenge 必须建立在已有 TEE 提案之上（`challenge()` 入口要求
   `teeProver` 已 set 才能把 `zkProver` 写进去），所以 `tee=1 zk=1 ci>0` 才是正确
   状态；旧 draft 写成 `tee=0 zk=1 ci>0` 与代码不符。
3. 本 challenger 进入 `process_fraudulent_zk_challenge()`（driver.rs L412-L473）：
   - `verifier_client.intermediate_output_root(game, challenged_index)` 取 on-chain
     根；
   - `validator.compute_output_root(checkpoint_block)` 重算 canonical 根，
     `checkpoint_block = candidate.checkpoint_start_block(challenged_index + 1)`；
   - 两者一致 → ZK challenge 是欺诈。
4. **直接** 调用 `initiate_zk_proof(candidate, challenged_index, on_chain_root,
   DisputeIntent::Nullify)` —— 不走 TEE-first（driver.rs L471-L472 显式如此），
   因为 Nullify 的目标是 `zkProver` 槽位，必须用 ZK 证据触发合约的
   `_proofRefutedUpdate(ZK)` 分支；`PendingProof` 文档（`pending.rs` L58-L63）也
   把这条路径明确为 "ZK proof nullifying a fraudulent ZK challenge"。
5. ZK 服务返回 `OnChainSnark` receipt 后，submitter
   `encode_nullify_calldata(PROOF_TYPE_ZK || proof_with_pv.bytes(), challenged_index,
   on_chain_root)` 上链；`AggregateVerifier.nullify()` 把伪 ZK challenge 槽位置零，
   伪 challenger bond 被 slash，进入 DelayedWETH unlock 阶段。

> 反例：TEE-first 仅适用于 `InvalidTeeProposal` / `InvalidDualProposal`
> （driver.rs L372-L403 在 `process_invalid_proposal` 中显式枚举），不会被
> `FraudulentZkChallenge` 这条路径使用。
...
flowchart TD
    ...
    F -->|InvalidTeeProposal<br/>tee=1 zk=0 ci=0| G1[process_invalid_proposal<br/>try_tee_first=true<br/>intent=Challenge<br/>governs ZK fallback ONLY<br/>driver.rs L341-L403]
    F -->|InvalidDualProposal<br/>tee=1 zk=1 ci=0| G2[process_invalid_proposal<br/>try_tee_first=true<br/>intent=Nullify]
    F -->|InvalidZkProposal<br/>tee=0 zk=1 ci=0| G3[process_invalid_proposal<br/>try_tee_first=false<br/>intent=Nullify<br/>direct ZK]
    F -->|FraudulentZkChallenge<br/>tee=1 zk=1 ci>0| H[process_fraudulent_zk_challenge<br/>driver.rs L412-L473]
    F -->|tee=1 zk=0 ci>0<br/>tee=0 zk=1 ci>0<br/>tee=0 zk=0 *| J[anomaly<br/>error!/warn! + skip]
    G1 --> K[initiate_proof TEE-first<br/>driver.rs L486-L558]
    G2 --> K
    G3 --> Qd[initiate_zk_proof direct<br/>driver.rs L556-L557<br/>intent=Nullify]
    H -->|checkpoint canonical valid| Hzk[initiate_zk_proof<br/>intent=Nullify<br/>driver.rs L471-L472]
    H -->|checkpoint canonical invalid| N[skip — challenge is legitimate]
    K --> O{TEE prove succeeded<br/>within request_timeout?}
    O -->|yes| Ptee[ready_tee FORCES submit intent=Nullify<br/>pending.rs L158-L178<br/>poll_or_submit → submit_dispute<br/>encode_nullify_calldata<br/>PROOF_TYPE_TEE&#124;&#124;sig<br/>on-chain: nullify<br/>applies to BOTH Path 1 and Path 4]
    O -->|timeout / err| Q[ZK fallback<br/>initiate_zk_proof intent=incoming<br/>Path 1: Challenge / Path 4: Nullify<br/>driver.rs L516-L557]
    Q --> R[PendingProofs poll<br/>derive_session_id<br/>uuid_v5 game&#124;&#124;invalid_index]
    Qd --> R
    Hzk --> R
    R --> S{poll result}
    ...
    V -->|invariant ok / Path 1 ZK fallback| W1[submit_dispute Challenge<br/>encode_challenge_calldata<br/>PROOF_TYPE_ZK&#124;&#124;groth16<br/>on-chain: challenge<br/>installs zkProver + ci]
    V -->|invariant ok / Path 2 / 3 / 4 ZK| W2[submit_dispute Nullify<br/>encode_nullify_calldata<br/>PROOF_TYPE_ZK&#124;&#124;groth16<br/>on-chain: nullify]
    V -->|already nullified| U
    W1 --> X[bond.rs lifecycle<br/>NeedsResolve → NeedsUnlock → AwaitingDelay → NeedsWithdraw]
    W2 --> X
    Ptee --> X
```

> **Diagram note on Path 1 TEE-first intent**：边 `G1 → K → O → Ptee` 是 Path 1 的
> TEE-first **成功** 路径，最终 on-chain 调用 `nullify()`（由 `PendingProof::ready_tee`
> 在 pending.rs L177 强制 `intent = DisputeIntent::Nullify`）。`G1` 节点上的
> `intent=Challenge` 仅在 TEE-first 失败 / 超时后由 `K → Q → R → T → V → W1`
> 落到 ZK fallback 时才生效，那一笔 tx 才是 `challenge()`。如果 game 上还没有
> `zkProver`，Path 1 的 ZK fallback 必须用 `challenge()` 而非 `nullify()`，因为
> `nullify()` 路径要求 ZK 证据已经被安装到 game 上。
...
sequenceDiagram
    ...
    Note over C,ZK: Scenario 3 — fraudulent ZK challenge nullified by ZK proof
    Note over L1: fake challenger calls challenge() on a TEE proposal<br/>state = (tee≠0, zk≠0, ci>0)
    C->>C: classify FraudulentZkChallenge { challenged_index = ci-1 }
    C->>L1: verifier_client.intermediate_output_root(game, ci-1)
    C->>C: validator.compute_output_root(checkpoint_block)<br/>checkpoint_block = start + interval * ci
    Note over C: on-chain root == canonical root<br/>=> ZK challenge is fraudulent (Path 2)
    C->>ZK: initiate_zk_proof(candidate, ci-1, on_chain_root, Nullify)<br/>session_id = v5(game || invalid_index_be)<br/>(no TEE-first; driver.rs L471-L472)
    ZK-->>C: GetProof(OnChainSnark)<br/>SP1ProofWithPublicValues::bytes() (variable size)
    C->>L1: AggregateVerifier.nullify(PROOF_TYPE_ZK||groth16, ci-1, on_chain_root)
    L1-->>C: fake challenger bond slashed + DelayedWETH unlock
    end
...
8. **`AnchorStateRegistry` snapshot block 语义（round-2 修订）**：`anchor_snapshot()`
   在 `anchor_state_registry.rs` L132-L150 内直接调 `provider.get_block_number()`
   取 **latest** L1 block，然后用 `futures::try_join!` 把 `getAnchorRoot()` 与
   `anchorGame()` 同步 pin 到该 block。**这个 block 选择与 `DriverConfig.allow_non_finalized`
   无关** —— 后者只在 `pipeline.rs` L983-L990 决定 proposal planning 使用 `safe_l2`
   还是 `finalized_l2`，不会改写 ASR 调用的 L1 block 高度。`game_count` 仍是与
   ASR snapshot 独立的另一次 L1 RPC 读，二者不属于同一 L1 快照；GameAlreadyExists
   处理路径（`pipeline.rs` L545-L580）补偿了这条 race。spec 段对此未明确文档化。
...
| 3 | 2026-05-17 | Deep Research Agent (`13a888db-49bb-4a19-9906-827729e156d9`) | 应用 adversarial draft review (round 2, needs-attention, severity: major) 的单条 narrow patch directive：**item-3 lifecycle + diag-2 Path 1 TEE-first 成功的 on-chain action 修正**。round-2 错误描述 Path 1 TEE-first 成功 → `submit_dispute(challenge, ...)`；实际上 `process_invalid_proposal` 把 `intent = DisputeIntent::Challenge` 传给 `initiate_proof()`，但 driver.rs L513-L538 在拿到 TEE `proof_bytes` 后调用 `PendingProof::ready_tee(...)`，而 `ready_tee` 在 `pending.rs` L158-L178 强制把 entry 的 submit intent 覆盖为 `DisputeIntent::Nullify`（docstring："TEE proofs always target `nullify()`"），所以 Path 1 的 TEE-first 成功 on-chain 实际调用 `nullify(PROOF_TYPE_TEE || sig, ...)`，**不是** `challenge()`。`challenge()` 仅在 TEE-first timeout / 提交失败后 fall-through 到 `initiate_zk_proof(.., Challenge)` 的 ZK fallback 路径才发生 —— 此时 game 尚未设置 `zkProver`，需要 `challenge()` 同时安装 ZK 证据 + 置位 `counteredByIntermediateRootIndex` + slash TEE bond。修订点：(a) item-3 `state_machine_or_lifecycle` 段补充 "关键不变量" 说明 + 重写 Path 1 / Path 4 双子句明确 "TEE-first 成功 → `nullify()`" vs "ZK fallback → `challenge()` (Path 1) / `nullify()` (Path 4)"；(b) diag-2 把 G1 节点 `intent=Challenge` 标注为 "governs ZK fallback ONLY"、把原 `P[submit_dispute PROOF_TYPE_TEE&#124;&#124;sig]` 改为 `Ptee[ready_tee FORCES submit intent=Nullify ... on-chain: nullify ... applies to BOTH Path 1 and Path 4]`、把原 `V → W` 单分支拆为 `V → W1 (Path 1 ZK fallback, on-chain: challenge)` 与 `V → W2 (Path 2 / 3 / 4 ZK, on-chain: nullify)`、追加 diagram note 总结 Path 1 双分支。round-1 的 4 条 patch directives（GameCategory 表与异常状态、`FraudulentZkChallenge` 直接 ZK Nullify 路由、scenario 3 ZK-powered nullification、scenario 2 SP1 proof bytes 变长 + ASR snapshot block 语义）round-2 已正确处理且 round-2 review 已验证通过，本轮 **未** 改动。 |
| 2 | 2026-05-17 | Deep Research Agent (`13a888db-49bb-4a19-9906-827729e156d9`) | 应用 adversarial draft review (round 1, needs-attention, severity: major) 的 4 条 patch directives：(1) **item-3 GameCategory 表 / scenario 3 / 相关描述**：严格对齐 `scanner.rs::classify()` L514-L577 —— `InvalidTeeProposal=(1,0,0)`、`InvalidDualProposal=(1,1,0)`、`FraudulentZkChallenge=(1,1,ci>0)`、`InvalidZkProposal=(0,1,0)`；`(1,0,ci>0)` 与 `(0,1,ci>0)` 为合约异常态 skip + log。修订 item-3 lifecycle pseudo-code + `process_invalid_proposal` / `initiate_proof` / `process_fraudulent_zk_challenge` 描述，明确 `try_tee_first` 仅对 Path 1/4 启用，Path 2 / Path 3 直接走 ZK。 (2) **diag-2**：修正 `FraudulentZkChallenge` 标签为 `tee=1 zk=1 ci>0`、`InvalidZkProposal` 标签为 `tee=0 zk=1 ci=0`；把 fraudulent ZK challenge 路由从 "TEE-first Nullify" 改为 `initiate_zk_proof` 直接 ZK Nullify；显式枚举三类 anomaly skip 状态。 (3) **item-8 scenario 3 + diag-5**：重写为 ZK-powered nullification —— fraudulent ZK challenge 必须建立在 TEE 提案之上（`challenge()` 入口要求 `teeProver` 已 set 才能写 `zkProver`），所以状态是 `(tee≠0, zk≠0, ci>0)`；`process_fraudulent_zk_challenge()` 在 driver.rs L471-L472 直接调用 `initiate_zk_proof(.., Nullify)`，submitter 编码 `PROOF_TYPE_ZK || groth16`；TEE-first 仅适用于 `InvalidTeeProposal` / `InvalidDualProposal`。 (4) **gap analysis + scenario 2**：修正 ASR snapshot block 语义为 `provider.get_block_number()` (latest L1)，明确 `DriverConfig.allow_non_finalized` 在 `pipeline.rs` L983-L990 只控制 proposal planning 用 `safe_l2` 还是 `finalized_l2`，与 ASR snapshot 解耦；移除 scenario 2 中"65 byte `proof_with_pv.bytes()`"的错误固定大小说法，改为变长 `SP1ProofWithPublicValues::bytes()`（测试仅约束 length < bincode SNARK、Groth16 selector `0x0e78f4db` 开头，详见 `zk/service/src/server/get_proof.rs` L24-L37 + L221-L241）；65 bytes 是 TEE ECDSA signature 大小（`proof_encoder.rs` L88-L115）。Verification notes：item-1 同一 snapshot 语义、item-7 signer attestation 顺序、Registrar `step()`、diag-4 已 round-1 正确处理，本轮未修改。 |
