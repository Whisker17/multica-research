...
artifact_paths:
  ...
  draft: base-azul-upgrade/research-sections/osaka-evm-changes/drafts/round-1.md
...
| `op-geth/core/vm/contracts.go:131, 152-178, 651-671` | `PrecompiledContractsOsaka` 注册 `bigModExp{eip2565: true, eip7823: true, eip7883: true}`；运行时检查 `if c.eip7823 && (inputLenOverflow || max(baseLen, expLen, modLen) > 1024) { return errors.New(...) }` |
| `op-geth/core/vm/contracts_test.go` | Osaka MODEXP 测试，含 oversize 拒绝 case |
...
| 测试 | 断言 |
|---|---|
| `test_modexp_eip7823_boundary` | 1024 字节字段成功，1025 失败 |
| `test_modexp_eip7823_each_field_rejects` | 任一字段超限即拒绝 |
| `test_modexp_eip7823_all_fields_at_limit` | 三字段均为 1024 时通过 |
...
| 源 | 位置 | 证据 |
|---|---|---|
| **op-geth EL** `v1.101702.2-rc.3` | `core/vm/contracts.go:131, 651-671` | `PrecompiledContractsOsaka` 标注 `eip7823: true`；oversize 检查就地实现 |
| 同上 | `core/vm/contracts_test.go` | Osaka MODEXP fork-aware 测试矩阵 |
...
| `op-geth/core/vm/contracts.go:651-671` | `osakaModexpGas`、`osakaMultComplexity` |
...
| 源 | 位置 | 证据 |
|---|---|---|
| **op-geth EL** `v1.101702.2-rc.3` | `core/vm/contracts.go:651-671` | `osakaModexpGas` 完整实现 |
| 同上 | `core/vm/contracts.go:131` | `PrecompiledContractsOsaka` 注册 `eip7883: true` |
| 同上 | `core/vm/contracts_test.go` | EIP-7883 公式回归测试 |
...
| `op-geth/core/vm/contracts.go:131, 738-739, 1697-1735` | `PrecompiledContractsOsaka` 注册 `0x100: &p256Verify{}`；`PrecompiledContractsFjord` 注册 `0x100: &p256VerifyFjord{}`；`(p256Verify).RequiredGas` 返回 `params.P256VerifyGas` (6,900) |
| `op-geth/params/protocol_params.go:183-184` | `P256VerifyGasFjord = 3450`；`P256VerifyGas = 6900` |
| Base Azul spec | `base/upgrades/azul/exec-engine.md` ≈ L37–43 | 同时提及 3,450 与 6,900（见 source_conflicts_and_resolution） |
...
| 源 | 位置 | 证据 |
|---|---|---|
| **op-geth EL** `v1.101702.2-rc.3` | `core/vm/contracts.go:131` | `PrecompiledContractsOsaka` 注册 `0x100: &p256Verify{}` |
| 同上 | `core/vm/contracts.go:738-739` | `(p256Verify).RequiredGas` 返回 `params.P256VerifyGas` |
| 同上 | `core/vm/contracts.go:1697-1735` | Fjord 版 `p256VerifyFjord` 与 Osaka 版 `p256Verify` 并存 |
...
Base Azul `exec-engine.md`（约 L37–43）同一节中同时出现：
...
两条数值陈述并存，未经熟悉历史的读者可能误以为 Azul 之后仍为 3,450、或两者并存。
...
- `3,450` 是 **Fjord/RIP-7212** 时期的历史 gas 值，对应 op-geth `P256VerifyGasFjord` 与 revm `P256VERIFY_BASE_GAS_FEE`。Base Azul spec 在 L37–43 提及该值时是在描述**Fjord 已有部署**，作为升级前对照；
...
建议 Base spec 团队在后续修订时显式标注两值的时间段（例如 "Fjord (3,450) → Azul (6,900)，2×"），但 draft 此处以代码 + 测试为最终权威。
...
| `op-geth/core/vm/contracts.go:131, 651-671, 738-739, 1697-1735` | Osaka precompile 集合 |
...
| EIP-7823 | `core/vm/contracts.go:131, 651-671` (`bigModExp{eip7823: true}`) | go.mod pin；全仓 `rg "7823"` = 0 hits（justified absence） |
| EIP-7883 | `core/vm/contracts.go:131, 651-671` (`osakaModexpGas`) | 同上；`rg "7883"` = 0 hits |
...
| EIP-7951 | `core/vm/contracts.go:131, 738-739, 1697-1735`、`params/protocol_params.go:183-184` (`P256VerifyGasFjord/P256VerifyGas`) | 同上；`rg "P256Verify\|7951\|7212"` = 0 hits |
...
flowchart TD
    ...
    subgraph Conflict["Spec text conflict in base/upgrades/azul/exec-engine.md (~L37-43)"]
        S1["statement 1:\np256 verify gas is 3,450"]
        S2["statement 2:\nAzul raises the cost to 6,900 to match L1"]
...
| ID | URL | 引用位置 |
|---|---|---|
| Base Azul exec-engine | https://github.com/base/base/blob/main/upgrades/azul/exec-engine.md | item-5 source_conflicts_and_resolution（含冲突原文） |
| Base Azul overview | https://github.com/base/base/blob/main/upgrades/azul/overview.md | Executive Summary |
...
| EIP-7823 | `core/vm/contracts.go:131, 651-671` (`bigModExp{eip7823: true}`) | item-2, item-6 |
| EIP-7883 | `core/vm/contracts.go:131, 651-671` (`osakaModexpGas`), `params/protocol_params.go` modexp 常量 | item-3, item-6 |
...
| EIP-7951 | `core/vm/contracts.go:131, 738-739, 1697-1735`, `params/protocol_params.go:183-184` | item-5, item-6 |
...
未发现影响 draft 结论正确性的硬 gap；所有 gap 均为"证据补强"类。
