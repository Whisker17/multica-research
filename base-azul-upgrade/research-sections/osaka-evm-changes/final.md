...
final_metadata:
  ...
  promoted_at: "2026-05-17T11:00:00Z"
  promoted_from: base-azul-upgrade/research-sections/osaka-evm-changes/drafts/round-2.md
  promoted_from_commit: b736c59eca2ff165fd8ccd8228f78fd0be51ada9
  outline_commit: bd080f626091b410162b9b85b34c73cca3044775
  outline_round: 2
  adversarial_approval_comment: b48af0bb-e909-4cb8-a568-4c4d5525ea1e
  adversarial_approval_confidence: 0.82
  cleanup_notes_applied:
    - "item-2 op_stack_upstream_mapping: corrected stale `Gap G-4 / 上游覆盖缺口` cross-reference to `Gap G-6` (EIP-7823 上游测试覆盖缺口)"
    - "src-6 expert commentary: downgraded coverage from 1/1 to 0/1 (deferred); the conceptual placeholder is no longer counted, gap remains transparently disclosed as G-1"
...
| 同上 | `core/vm/contracts_test.go:49-82` (`allPrecompiles`) | 仅含 `eip7883: true` fork 切片；**未发现**显式 EIP-7823 oversize 拒绝回归测试（Gap G-6 / 上游覆盖缺口） |
...
建议 Base spec 团队在后续修订时显式标注两值的时间段（例如 "Fjord (3,450) → Azul (6,900)，2×"），并修正 L37–38 把 `3,450` 归因于 EIP-7951 的表述。本 section 以代码 + 测试为最终权威。
...
### src-6: 专家评论 (min 1 / actual 0 — deferred)

本节最初准备引用 Ethereum Magicians "Osaka EVM changes" 讨论或 EIP-7883 RSA 影响分析帖作为外部互证，但 round-1/2 检索阶段未锁定单一 named 公开来源（具名博客、审计、OP Labs 工程师评估）能够独立支撑 item-3/item-5 backward_compatibility_impact 中的具体量化结论。为避免把概念占位计为已满足来源，该项**显式降级为未满足**，并保留为 [Gap G-1](#gap-analysis) 中的证据补强项。

| 引用 | 状态 | 备注 |
|---|---|---|
| Ethereum Magicians "Osaka EVM changes" 讨论汇总 / EIP-7883 RSA 影响分析帖 | deferred | 仅作为研究方向占位；不计入 actual 覆盖。建议后续在 TW final-report 或 Mantle replication 阶段补充具名来源。 |

**对结论的影响**：item-3（MODEXP 计价）与 item-5（P256 翻倍）的代码层与测试层结论**完全由 src-1 (EIP 原文)、src-3 (base/base 源码与单元测试)、src-5 (revm 常量)、src-7 (op-geth EL 实现) 互证支撑**——src-6 仅为外部生态视角强化项，缺失不影响代码与计价正确性结论。Mantle 团队在自家影响评估中应自行补充链上量化与生态访谈以填补这一外部视角。
...
| Gap | 描述 | 影响 | 缓解建议 |
|---|---|---|---|
| G-1 | src-6 专家评论保持未引用具名公开来源；final section 已将 actual 覆盖从 `1/1` 降级为 `0/1 (deferred)`，不再把概念占位计为已满足 | 削弱 backward_compatibility_impact 在 item-3/item-5 中的**外部生态视角**互证；**不影响**代码层与计价结论（src-1/3/5/7 已四方互证） | 在 TW final-report 聚合阶段或 Mantle replication 评估中补充具名来源（Optimism/OP Labs 关于 P256 翻倍的 4337 影响估算、Ethereum Magicians Osaka tag 中具体讨论帖、第三方 RSA/zk 协议影响审计） |
| G-2 | revm/reth 上游路径（src-5）以"概念位置"形式引用（`crates/precompile/src/secp256r1.rs` 等），未给出 commit-pinned 行号 | 行号在 revm 主线 commits 之间可能漂移；本 section 依赖 base/base 的 re-export 链 | 后续在 TW final-report 或 Mantle replication 阶段补 revm pinned commit 与精确行号；可从 base/base `Cargo.lock` 取 revm 版本 |
| G-3 | item-5 source-conflict resolution 引用了 base/base spec 第 37–43 行，但未直接 quote 完整段落 | 读者需自行打开 spec 文件验证；不影响结论正确性（已由 op-geth `params.P256VerifyGas` 与 base/base 测试三方互证） | TW final-report 或 Mantle replication 阶段可粘贴 spec 原文段落（≤ 5 行）并标注 commit hash 增强独立可验证性 |
| G-4 | op-geth `core/vm/contracts_test.go` 中 Osaka MODEXP/p256 测试名未一一枚举 | test_coverage 矩阵在 Go 侧不完整；不影响实现锚点正确性 | Mantle replication 阶段或上游补丁可跑 `grep -E "TestModExp.*Osaka\|TestP256.*Osaka" core/vm/contracts_test.go` 并把命中函数名补入表格 |
| G-5 | optimism monorepo 中"无 AzulTime"的 absence 仅基于本地 `rg`，未引用具体 PR/issue 讨论 | 弱化 justified absence 的论证强度；不影响 op-geth pin 关系结论 | TW final-report 阶段可检索 OP Stack governance 讨论或 PR 关于"Osaka 等价 fork"加入 monorepo 的进度 |
...
未发现影响本 section 结论正确性的硬 gap；G-6 是一项新发现的**上游测试覆盖缺口**，不影响 base/base 在 Azul 上的行为正确性（Rust 侧覆盖充分），但 Mantle 在复制 op-geth fork 时应正面应对。其余 gap 均为"证据补强"类。
...
| final | agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9) | 2026-05-17T11:00:00Z | 从已批准 round-2 draft（commit `b736c59e`，adversarial approve comment `b48af0bb`，confidence 0.82）晋升为 final section。两处 minor cleanup：(1) item-2 `op_stack_upstream_mapping` 表中 EIP-7823 上游覆盖缺口的交叉引用从 `Gap G-4` 修正为 `Gap G-6`（与 Gap Analysis 一致）。(2) src-6 专家评论显式降级——从 actual `1/1` 改为 `0/1 (deferred)`，不再把概念占位计为已满足来源；frontmatter `source_requirements_coverage.src-6` 同步更新，G-1 描述与缓解建议改写以反映新状态。其余主体（spec 摘要、code paths、code excerpts、diagrams、所有其他 items 与 source requirements）保持不变；frontmatter 由 `draft_metadata` 切换为 `final_metadata`，`status` 改为 `final`，`round` 改为 `final`，并记录 `promoted_from_commit`、`adversarial_approval_comment` 与 `cleanup_notes_applied`。 |
