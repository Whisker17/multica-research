...
promoted_from:
  ...
  draft_round: 1
  draft_commit: "58a6f6c1cb15995906bc9d79cca6a0bc8015ed22"
  approval_evidence: "dc1fd8ef-682e-4c22-a8b7-39a0d62fb236"
  review_verdict: "approve (minor severity)"
draft_metadata:
  code_baseline:
...
1. ~~**B20 官方 spec 文档**~~（`docs.base.org/base-chain/specs/upgrades/beryl/b20`）：**已关闭**。adversarial review 阶段独立确认官方 B20 spec 与本文 draft 在以下关键面一致：B20Factory 创建流程、PolicyRegistry 策略模型、7 角色 AccessControl 模型、initCalls 特权窗口、Asset variant `OPERATOR_ROLE`、Stablecoin 固定 6 位精度。Base 官方公告（[Introducing Base Beryl](https://blog.base.dev/introducing-base-beryl)）也证实 B20 为预编译级 ERC-20 兼容标准，支持两个变体（Asset 可配置精度 + Stablecoin 六位精度），包含角色权限、mint/burn 控制、转账限制和冻结/回收能力。Gap 等级：**closed**。
...
| final | 从 round-1 draft 提升为 final — 关闭 spec 交叉验证 gap（review 阶段独立确认 spec 一致性），更新 frontmatter 为 final 状态 |
