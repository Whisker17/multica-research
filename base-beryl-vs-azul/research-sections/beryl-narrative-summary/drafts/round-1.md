...
artifact_paths:
  ...
  draft: base-beryl-vs-azul/research-sections/beryl-narrative-summary/drafts/round-1.md
...
Beryl 的核心贡献在两个方向：**协议层合规原生化**——通过 B20 预编译级 Token 标准，将合规能力（策略引擎、角色权限、功能门控）从应用层下沉至协议层；**资本效率提升**——将 single-proof 提款最终确认窗口从 7 天缩短至 5 天，同时通过 Reth v2.2.0→v2.3.0 增量升级获得 +8.1% 执行吞吐提升。
...
- **资本效率**：Single-proof 提款窗口从 7 天缩短至 5 天（源码已确认，部署随 Beryl mainnet 激活），直接降低桥接 LP 资本成本约 28.6%。
...
flowchart LR
    ...
    subgraph beryl["Beryl 时代"]
        ...
        B2["💰 资本效率<br/>提款窗口 7→5 天<br/>Reth v2.3.0 (+8.1%)"]
    end
...
Single-proof 提款最终确认窗口从 7 天缩短至 5 天，直接释放桥接流动性提供者的资本。以年化收益率框架衡量，锁仓期缩短约 28.6% 意味着单次桥接的 LP 资本成本同比例下降，最终传导为用户桥接费率的下行压力。Dual-proof 快路径（1 天）保持不变，因 ZK 证明生成成本高，实际使用率低。
...
Beryl 的所有新增预编译通过 `>= BaseUpgrade::Beryl` 门控安装，对 Azul 及更早的标准预编译零影响。Blast radius 分析结论明确：标准以太坊预编译地址范围的风险为零，B20 结构编码的地址空间匹配残余概率极低（2^{-87}）。Reth v2.2.0→v2.3.0 是增量升级，不触及已在 Azul 稳定运行的 Storage V2 / Proof V2 核心架构。提款窗口缩短以 Multiproof 双证明架构为安全网——即使 single-proof 路径出现争议，TEE+ZK 双证明仍可在 1 天内完成确认（参见 beryl-precompile-infra/final.md §2、protocol-reth-withdrawal/final.md §4）。
...
| **提款窗口**（single-proof） | 7 天（604,800 秒） | 5 天（432,000 秒）— 源码已确认，部署随 Beryl mainnet 激活 | 桥接 LP 资本成本降低约 28.6% |
| **提款窗口**（dual-proof 快路径） | 1 天（86,400 秒） | 不变 | 安全冗余保持 |
...
Beryl 将 single-proof 提款窗口从 7 天缩短至 5 天。根据 L2Beat 数据，Mantle 当前的挑战期（challenge period）为 7 天。Base 官方声明"窗口将继续缩短"——这意味着资本效率差距可能进一步拉大。对 Mantle 的 fast-bridge 和流动性策略而言，需关注：(a) Mantle 自身是否有缩短挑战窗口的路线图；(b) 如果 Base 持续缩短窗口，对两链之间桥接流动性的竞争格局有何影响。Mantle 已通过 OP Succinct 引入 ZK 证明（partially live），这为未来缩短窗口提供了技术基础（参见 mantle-impact-assessment/final.md §1）。

### 战略要点

1. **合规能力差距正在形成**：Base 已在协议层嵌入合规 Token 标准（B20）。如果 Mantle 追求 RWA/机构化，需在下一个升级周期内评估是否需要等价的协议层合规能力，或者应用层方案是否足以满足目标客户需求。
2. **资本效率竞争已启动**：Base 提款窗口已从 7 天缩短至 5 天，且声明将继续缩短。Mantle 需评估自身挑战窗口缩短的技术可行性和时间线，尤其是在 ZK 证明能力已部分就绪的情况下。
...
|-----|----------|-------------|
| `beryl_mainnet_not_yet_activated` | Info | Beryl mainnet 计划 2026-06-25 激活，截至本研究时间（2026-06-21）尚未激活。提款窗口 7→5 天的变更已在源码（base/contracts @ v8.2.0）确认，但 L1 部署（新 AggregateVerifier 合约）待激活后完成。本叙事性总结中标注为"源码已确认，部署随 Beryl mainnet 激活"。 |
