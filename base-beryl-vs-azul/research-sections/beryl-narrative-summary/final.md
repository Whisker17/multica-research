...
promotion_metadata:
  approved_draft_path: base-beryl-vs-azul/research-sections/beryl-narrative-summary/drafts/round-2.md
  approved_draft_round: 2
  approved_draft_commit: e842255cd2254c17bee85dae0cf919d370cda5ac
  approval_evidence: "https://multica.ai/issue/2f090be1-2460-4d8c-bc9e-379cb9a358eb#5866cce0-b5b0-4be8-b2a7-2a62ea91eee1"
  final_polish_applied:
    - "tighten shipped/scheduled phrasing (lines 87, 195, 207) to not conflate Beryl code readiness with mainnet activation"
draft_metadata:
  outline_round: 2
...
计划于 2026 年 6 月 25 日激活的 Beryl 硬分叉标志着 Base 从基础设施建设转向协议层产品化（参见 beryl-scope-inventory/final.md §1）：
...
Beryl 通过 B20 在协议层引入合规 Token 标准（随 mainnet 激活生效），意味着 RWA（真实世界资产）Token 化可在 L2 层原生实现。Mantle 目前在合规 Token 领域采用应用层方案。两种路径各有权衡：协议层方案提供更强的一致性保证和更低的 DApp 集成成本，但牺牲了灵活性并增加了协议升级复杂度；应用层方案更灵活且不增加协议负担，但合规执行依赖各 DApp 自行实现，覆盖率和一致性难以保证。如果 Mantle 追求 RWA/机构化战略，需评估是否需要类似的协议层合规能力，以及在 OP Stack 兼容架构下实现的可行性。
...
1. **合规能力差距正在形成**：Base 通过 Beryl 在协议层引入合规 Token 标准 B20（代码已就绪，随 mainnet 激活生效）。如果 Mantle 追求 RWA/机构化，需在下一个升级周期内评估是否需要等价的协议层合规能力，或者应用层方案是否足以满足目标客户需求。
...
- **Round 2** (revision): 修复两项 adversarial review 反馈：(1) 部署状态一致性——全文统一标注提款窗口 7→5 天为"源码已确认的计划变更"。(2) 双证明安全回退误述——明确 1 天快路径需 proofCount ≥ 2、因 ZK 成本高实际触发率低、不应视为 single-proof 争议时的保底回退机制。
- **Final promotion** (from round 2 @ `e842255`): 应用最终润色——将"2026 年 6 月部署的 Beryl"改为"计划于 2026 年 6 月 25 日激活的 Beryl"（§1）；将"Base 已在协议层嵌入合规 Token 标准"改为"Base 通过 Beryl 在协议层引入合规 Token 标准 B20（代码已就绪，随 mainnet 激活生效）"（§4 战略要点）；将 §4 合规对标中"Base 通过 B20 在协议层嵌入"改为"Beryl 通过 B20 在协议层引入（随 mainnet 激活生效）"。确保全文不将 Beryl 代码就绪状态与 mainnet 已激活状态混淆。
