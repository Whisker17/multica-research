...
- 以太坊 EOA 地址由 `keccak256(public_key)` 的后 20 字节派生，前缀恰好为 `0xb2 00 00 00 00 00 00 00 00 00 {00|01}` 的概率为 2^{-87}（10 字节固定 + 1 字节 ∈ {0,1}，即 2^{-80} × 2^{-8} × 2 = 2^{-87}） — 每个 EOA 约 6.5 × 10^{-27} 的概率
...
2. 理论上满足 B20 结构编码的既有 EOA/contract 地址：概率为 2^{-87}（约 6.5 × 10^{-27}），密码学上可忽略
...
| 满足 B20 结构编码的非 factory 创建地址 | **理论非零，实际可忽略** | 匹配概率 2^{-87}（两个 variant 判别值）；被拦截后执行 B20 precompile 逻辑（非 EVM 空账户行为）；仅影响 Beryl 激活后的交易 |
...
| `alloy-evm` 版本锚定 | Resolved | v1.1.1 `Cargo.lock` 确认 `alloy-evm = 0.36.0`（checksum `a6e494...`）；`PrecompilesMap::get()` 的 static-first-then-lookup 行为在该版本中与源码分析一致 |
...
| 2 | 2026-06-21 | **§6 revised** per adversarial review: (1) Added §6.1 dispatch precedence proof from `PrecompilesMap::get()` static-first-then-lookup; (2) Replaced "mathematical impossibility" / "zero blast radius" with layered analysis — zero for standard ETH precompile addresses (proven), bounded residual for B20 structural encoding matches; (3) Documented B20 shadowing residual risk: `BerylLookup` uses pure structural matching without deployment check, `already_deployed` guard is creation-path only; (4) Added probability space analysis (2^{-87} per address, two variant discriminants); (5) Revised Diagram 3 to show dispatch flow with residual risk zone; (6) Updated Source Coverage with `alloy-evm`, `variant.rs`, `storage.rs`; (7) Updated Gap Analysis |
| final | 2026-06-21 | **Promoted to final**: (1) Corrected residual probability from 2^{-88} to 2^{-87} — `from_address()` accepts byte 10 ∈ {0,1} (two variants), giving 2^{-80} × 2^{-8} × 2 = 2^{-87}; (2) Replaced alloy-evm gap note with Cargo.lock anchor citation (alloy-evm=0.36.0, checksum `a6e494...`, verified in v1.1.1 Cargo.lock) |
