...
artifact_paths:
  ...
  draft: base-beryl-vs-azul/research-sections/protocol-reth-withdrawal/drafts/round-1.md
...
draft_metadata:
  ...
  source_coverage:
    primary:
      - "base/contracts @ main (local): src/L1/proofs/AggregateVerifier.sol"
...
- **当前 L1 已部署的 AggregateVerifier（Azul 时期）仍为 7 天**；Beryl mainnet 激活（2026-06-25）时将部署包含 5 天常量的新实现合约。
...
**参数位置**：`base/contracts` 仓库 `src/L1/proofs/AggregateVerifier.sol` L51

**参数类型**：**Solidity `constant`**（非 immutable，非 storage variable）

```solidity
// base/contracts @ main, src/L1/proofs/AggregateVerifier.sol L51
...
**证据来源**：`base/contracts` 本地仓库 @ `main` 分支，`src/L1/proofs/AggregateVerifier.sol` L48-54。完整 constant 声明块：

```solidity
// L48-68, AggregateVerifier.sol
////////////////////////////////////////////////////////////////
//                         Constants                          //
////////////////////////////////////////////////////////////////
...
| 参数 | Before (Azul) | After (Beryl) | 单位 | 确认来源 |
|------|--------------|---------------|------|----------|
| `SLOW_FINALIZATION_DELAY` | 7 days = 604,800 秒 | 5 days = 432,000 秒 | `uint64` (秒) | L1 已部署合约 + 代码变更 |
| `FAST_FINALIZATION_DELAY` | 1 day = 86,400 秒 | 1 day = 86,400 秒（不变） | `uint64` (秒) | L1 已部署合约 + 代码确认 |
| `PROOF_THRESHOLD` | 1 | 1（不变） | `uint256` | L1 已部署合约 + 代码确认 |
...
2. **base/contracts git history**：commit `3a25c8cf` (2026-05-18, `Update finalization delay constants (#293)`, author: roger-bai-coinbase) 的 diff 明确显示：

```diff
// base/contracts @ commit 3a25c8cf, src/L1/proofs/AggregateVerifier.sol
...
1. **base/contracts 当前代码**：`src/L1/proofs/AggregateVerifier.sol` L51 = `5 days`（本地仓库 @ main）。
2. **commit `3a25c8cf`** (PR #293) 是唯一修改此常量的提交，位于 `releases/v8.2.0` 分支。
...
**结论：已确认覆盖（confirmed override）——Base 部署的合约覆盖了 Optimism specs 中的默认值。**

| 层级 | 值 | 来源 |
|------|------|------|
| **Optimism specs 默认** | 7 days | OP Stack fault-dispute-game.md：标准 challenge window = 7 days |
| **Azul 已部署** | 7 days (604,800s) | L2BEAT 链上数据：`0xeEcb8A5944...D259` |
| **Beryl 代码** | 5 days (432,000s) | `base/contracts` AggregateVerifier.sol L51 |
| **Beryl 官方声明** | 5 days | blog.base.dev/introducing-base-beryl |
...
4. **Beryl overview 声明与代码一致**：官方文档声称 "single-proof window reduced to 5 days"，代码 commit `3a25c8cf` 确认 `SLOW_FINALIZATION_DELAY` 从 `7 days` 改为 `5 days`，两者完全一致。
...
| Source | 访问状态 | 使用情况 |
|--------|---------|---------|
| `base/contracts` @ main (local) | 已访问 | AggregateVerifier.sol 完整读取；git log 追踪 commit `3a25c8cf` |
...
| 字段 | Gap | 原因 | 置信度影响 |
|------|-----|------|-----------|
| `deployment_tx_or_config_source` | Beryl 新 AggregateVerifier 合约的部署交易 hash 不可获取 | **时序性限制**：Beryl mainnet 尚未激活（2026-06-25），截至研究时间（2026-06-20）部署尚未发生 | **低影响** — 合约源码变更证据充分（commit `3a25c8cf`），部署机制清晰（新合约部署 + DisputeGameFactory 注册），仅缺最终交易 hash |
