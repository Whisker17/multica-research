...
| artifact_paths.draft | `base-beryl-vs-azul/research-sections/sepolia-window-confidence/drafts/round-1.md` |
...
3. **架构低风险**：WHI-247 结论——对标准以太坊 precompile 地址空间 blast radius 为零；`beryl()` = `azul()` 静态集不变；所有 Beryl 新增均通过 fork 门控 + ActivationRegistry 动态安装，附加不修改。
...
| Sepolia 激活 | 2026-06-18 18:00 | Beryl fork 在 Sepolia 激活 | timestamp `1781805600` | 公网 fork 激活 |
...
| Mainnet 激活 | 2026-06-25 18:00 | Beryl fork 在 Mainnet 激活 | `4e84ba3d1` (#3627), timestamp `1782410400` | 主网上线 |
...
1. **变更性质不同**：Azul 涉及从 OP Stack 脱离、Multiproof 架构、Osaka EVM spec 等深层协议变更；Beryl 为纯附加式 precompile 层 + 已有 Reth V2 升级，不触及共识/derivation/sequencer 核心路径（WHI-245 scope inventory: 143 个 commit 中 141 个集中在 precompile 附加层）。
2. **Beryl 有 Azul 不具备的前置测试阶段**：Devnet E2E + Zeronet 激活与负载测试为 Beryl 提供了 Sepolia 之前约 13 天的验证时间（2026-06-05 ~ 2026-06-18）。
3. **架构风险等级不同**：Beryl blast radius = 零（WHI-247），Azul 则修改了 EVM handler 和 derivation 管线。
...
- **不修改既有路径**：WHI-245 scope inventory 确认 143 个 Beryl commit 中 141 个集中在 precompile 附加层，不触及共识、derivation、sequencer 核心路径。
...
|---------|---------|---------|-------------|-------------|
| 架构隔离（blast radius = 0） | **强** | 代码分析 + WHI-247 | BerylLookup 碰撞理论概率 | 低优先 |
...
**支持上线的因素**（证据充分）：
- 架构设计保证既有功能不受影响（blast radius = 0）
- 变更范围高度聚焦（143 commit 全部在 precompile 附加层）
- 发布工程高度成熟（21+4 RC，3-commit delta）
- 附加式变更 + ActivationRegistry 提供回退能力
- 多环境测试管线提供 23 天总测试覆盖
...
**核心洞察**：7 天窗口的信心基础并非来自 Sepolia 公网测试本身，而是来自「附加式架构 + 多环境管线 + 高密度审计 + 回退能力」的组合。7 天 Sepolia 窗口更像是部署管线的最终确认步骤，而非独立的测试阶段。这一策略的合理性建立在 Beryl 变更的附加性质之上——如果 Beryl 涉及共识或 derivation 层修改，同样的窗口将不具备充分的信心基础。
...
| status.base.org | 激活时间确认 | ⚠️ 未访问 | 次要来源；激活时间已由 code timestamp 确认 |
...
| status.base.org 未访问 | 低：激活时间已由 code timestamp 双重验证 | 次要来源，不影响核心论证 |
