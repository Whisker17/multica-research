...
因此 phased batch 是 remote-pr-diff verified，不是 local baseline fully verified。local baseline 只证明 wire model 预留了 nested phases。
...
| D6 batch atomicity | 4 | `Vec<Vec<Call>>` phases, PR #3696 phased execution。 | medium | exact phase revert semantics need final code/test review。 |
