...
本 draft 的结论是：ERC-4337 是当前最成熟、最可用的应用层账户抽象方案，但它的“不改协议”路线把复杂性从客户端/协议层转移到了 EntryPoint 合约、bundler、alt-mempool、paymaster、SDK 和钱包集成层。它不是失败方案；更准确地说，它在 gas overhead、链外基础设施、EOA 原地址兼容、版本碎片化和可观测性上有结构性代价。这些代价解释了为什么 Base 可能会探索 EIP-8130/native AA，但不足以单独证明 Mantle 必须转向 native AA。
...
5. **Mantle 4337 采用不能只看绝对量。** 同一 Dune 快照显示 Mantle YTD 只有 11,479 个 UserOps，远低于 Base 的 1,388,890；但除以总交易数后，Mantle 约 0.0821 UserOps / 100 tx，Base 约 0.0817，Arbitrum 约 0.0613。绝对量说明 Mantle AA 生态小，归一化占比不支持“显著弱于 Base”的单点结论；仍需钱包/SDK、应用场景、活跃地址质量和 sponsor 成本一起判断。
...
结论：4337 gas overhead 是真实机制成本，但严格量化必须用 paired benchmark；本 draft 的 Dune aggregate 只证明 4337 UserOp 的 gas 分布和 sponsor/成功率，不证明相对 EOA 的精确溢价。
...
时间窗：`2026-01-01 <= evt_block_date < 2026-06-27` UTC。Ethereum、Polygon、Optimism 在相同 bounded query 中未返回 rows；这可能是该时间窗/decoded table coverage/chain activity 组合导致，不能写成这些链没有 4337。
...
Dune query `7821129` divides UserOps by canonical total transactions over the same date window:
...
| proxy metric | Evidence in this draft | Assessment |
|---|---|---|
...
```text
2023        ERC-4337 Final ERC; EntryPoint v0.6 family widely used
2024        v0.7 introduces PackedUserOperation and revised gas/paymaster packing
2025        EIP-7702 ships in Pectra; 4337 ecosystem starts integrating EOA delegation flows
2026        v0.8 source includes Eip7702Support; provider/wallet support matrix fragments by EntryPoint version and chain
```
...
| src-3 on_chain_data >= 4 | Partially met | Dune decoded tables for Base, Arbitrum, Mantle; total tx denominators for Base, Arbitrum, Mantle. Ethereum/Polygon/Optimism rows not returned in bounded YTD query and are a gap. |
...
| src-5 infrastructure_docs >= 4 | Met enough for draft | bundler-spec, Pimlico docs, Biconomy docs, Alchemy docs, Coinbase/CDP docs |
...
| src-7 expert_commentary >= 3 | Not emphasized | Avoided using third-party commentary for core claims; can be added in final if reviewer wants more ecosystem narrative. |
| src-8 project_context >= 2 | Met | WHI-275 final framework; WHI-277 issue description and Orchestrator/reviewer comments |

## Gap Analysis

1. **Exact 4337 vs EOA gas premium remains unmeasured.** This draft identifies overhead sources and includes a real UserOp event sample, but does not execute paired EOA-vs-4337 traces. That is the right scope per reviewer F2; however, final Mantle decisions should not cite a precise percentage premium from this draft alone.
...
4. **Ethereum/Polygon/Optimism YTD rows missing in the bounded aggregate query.** The decoded tables exist in Dune search results, but the temporary aggregate returned rows only for Base, Arbitrum, and Mantle. This is reported as a query/data coverage gap, not evidence of no activity.
...
6. **EntryPoint address/version mapping should be expanded before final.** This draft notes top EntryPoint addresses from Dune but does not fully map each address to v0.6/v0.7/v0.8 deployments across all chains.
7. **Expert commentary requirement was intentionally de-emphasized.** The outline requested three expert-commentary sources, but this draft relies on primary specs, code, and Dune data for core claims. If the review wants more ecosystem narrative, add Ethereum Magicians / AA working group / provider engineering commentary in the next round, clearly marked as secondary interpretation.
