...
artifact_paths:
  ...
  draft: "base-eip8130-native-aa/research-sections/post7702-native-aa-landscape/drafts/round-1.md"
draft_metadata:
...
| 分级 | 含义 | 本文用法 |
|---|---|---|
| `explicit-public-statement` | 规范、设计文档、PR 描述、公开评论直接陈述。 | EIP/RIP status、OP Stack 采用 8130 proposal、#378 评论中 8130 vs 8141 对比。 |
| `implementation-signal` | Base PR/commit 显示实际投入，但没有直接说“选择理由”。 | PR wave、Cobalt/devnet/RPC/txpool/receipt/estimateGas/post-snapshot fixes。 |
...
5. **Base 的 implementation-signal 很强。** WHI-276 snapshot 后，`base/base` 继续合并或打开 EIP-8130 RPC、txpool、receipt、estimateGas、span batch codec、Sepolia addresses、counterfactual create、pending-state admission、auto-delegation/TOCTOU、end-to-end block executor test 等 PR。这说明 Base 已经把 8130 当作具体产品/协议 pipeline，而不是纸面研究。
...
| `explicit-public-statement` | OP design-docs PR #378 Summary：提议 OP Stack 以 hardfork feature 采用 8130，目标是尽量与 upstream EIP semantic compatibility，且不需要 OP Stack-specific divergence，因为 no EVM changes/opcodes。 | OP 侧存在公开“采用 8130 on OP Stack”的设计提案。 | https://github.com/ethereum-optimism/design-docs/pull/378 |
| `explicit-public-statement` | OP design doc Problem Statement：ERC-4337 需要 specialized mempools、bundlers、validation of wallet behavior；8130 把 authentication 从 wallet logic 中分离，tx 显式识别 sender/payer/auth path，sequencer/node 对 validation cost 可见。 | OP design doc 明确把 8130 与 4337 的 infra burden/validation visibility 对比。 | https://github.com/ethereum-optimism/design-docs/pull/378/files |
...
| `explicit-public-statement` | OP FMA：8130 主要风险包括 consensus bug、account takeover、mempool DoS/sequencer validation overload、receipt phase status、cross-chain owner changes；缓解包括 bounded canonical verifier set、mempool limits、defer nonceless mode。 | OP 采用 8130 时关注的主要安全边界与 rollout gate。 | https://github.com/ethereum-optimism/design-docs/pull/378/files |
| `explicit-public-statement` | EIP-8081 Hegota meta：EIP-8141 是 Considered for Inclusion，不是 Scheduled for Inclusion。 | 8141 在 L1 hardfork 方向上仍未定稿/未 scheduled。 | https://github.com/ethereum/EIPs/blob/master/EIPS/eip-8081.md |

### 3.3 Base 实现信号：WHI-276 清单重验与 post-snapshot 新增
...
| PR | 当前状态 | merge commit / state | selection signal |
|---|---|---|---|
| #3534 AccountConfiguration storage reader | merged 2026-06-16 | `396cf9873521dce364ab452e957691ae7cc3afec` | Account Configuration 是 8130 路线核心，而不是 4337/7702/RIP-7560 通用组件。 |
| #3535 actor authorization | merged 2026-06-16 | `1e4a6a6320054ee99577dba119d680c718f36717` | actor/authenticator stateful authenticate step 落地。 |
| #3540 tx actor authorization | merged 2026-06-16 | `0f629f69d45cf6e6932c75e3ff664c2e33b1cad7` | sender/payer scope gating，直接服务 8130 actor scope。 |
| #3585 2D nonce validation | merged 2026-06-17 | `da4e524313607db66ee2c257152f9ff1a8eda62e` | 8130 channel nonce/nonce-free 路径，和 RIP-7712 类似目标但在 Base 8130 pipeline 中实现。 |
| #3589 authorization orchestrator | merged 2026-06-19 | `f2a193ff242015ef1dbe967bba1c2cf95835ad65` | post-snapshot：统一 sender/payer/auth orchestration。 |
| #3651 account changes | merged 2026-06-22 | `d7825c65b06b1f96796252b955dd405a16a31c15` | create/config-change/delegation 进入 execution。 |
| #3653 EVM integration foundation | merged 2026-06-23 | `257c4121cb90c5c0b10c69f921aa57775542091c` | 8130 不只是 tx type，而是 EVM integration。 |
| #3680 pre-call execution pipeline | merged 2026-06-23 | `268820b38cb722e6a87fa424911720baff9daa20` | account-management txns 进入 block execution pipeline。 |
| #3720 txpool validation and RPC gating | merged 2026-06-24 | `a7c7fb10c9743340e99d5d4a79fe23df833c9edf` | txpool structural/stateful validation、Cobalt gate、payer funding、nonce/replay。 |
| #3649 RPC extensions | merged 2026-06-23 | `a81bdb6462b447dad407ae5b7d6f6ab6c71ef457` | `eth_getTransactionCount` 扩展 `nonce_key`，说明 tooling path 已开始。 |
| #3749 span batch codec | merged 2026-06-24 | `4509b0089f7b374376e98a65e82bfc06345ef537` | L2 batch/derivation path 支持 type `0x7B`。 |
| #3753 account-abstraction receipt | merged 2026-06-25 | `5b047190bf4b0a5d500c0cf6a8b6a6c03007e7b8` | receipt shape/tooling 已跟进，不只是 consensus validation。 |
| #3754 operator fee admission | merged 2026-06-24 | `d08ac8251a7f9353f0ab20f2d2d48d116e289180` | txpool admission gas/operator-fee edge case。 |
| #3755 estimateGas for 8130 | merged 2026-06-25 | `836ae417052d10cbacc92928acf47843b2286d5e` | RPC simulation/estimateGas 路径成熟化。 |
| #3723 local devnet Cobalt schedule | merged 2026-06-25 | `5bfda00b80eb6458d7bf858687817badc4dc5240` | devnet activation signal。 |
| #3763 Sepolia addresses | merged 2026-06-25 | `f6a4a2fbd1698efedd1ab3d83975d3bcc8d480d0` | Base Sepolia deployment/address pinning。 |
| #3766 counterfactual smart-account creates | merged 2026-06-26 | `50e568c8e2204780465018bb596656071baeeb68` | create path edge cases still active after WHI-276 snapshot。 |
| #3752 pending-state admission | open as of 2026-06-27 | open | txpool should validate against pending state and retry on lag；表明 mempool liveness 仍在调优。 |
| #3775 auto-delegate/TOCTOU | open as of 2026-06-27 | open | code-less sender auto-delegation、mempool gas budgeting TOCTOU；表明 EOA compatibility/gas safety still in flight。 |
| #3698 end-to-end inclusion test | open as of 2026-06-27 | open | BaseBlockExecutor e2e typed receipt inclusion test；验证 payload building/block import critical path。 |
...
| D | 预填 | 证据等级 |
|---|---|---|
| D1 | 协议原生 native AA，面向 rollup/L2，也借鉴 L1 AA。 | spec-cited |
| D2 | 新 EIP-2718 `AA_TX_TYPE`，新增 AA EntryPoint/predeploy semantics、receipt shape、gas accounting、validation/execution phases。 | spec-cited |
| D3 | 需要 txpool/builder 支持 AA tx；public mempool 需要 RIP-7711/ERC-7562-like rules；仍需 wallet/paymaster/deployer ecosystem。 | spec-cited |
| D4 | Smart Contract Account 自定义 validation；sender validation frame 决定 validity。 | spec-cited |
| D5 | paymaster first-class：`paymaster`、paymaster validation、postOp gas limits、gas precharge from sender/paymaster。 | spec-cited |
| D6 | execution frame + optional deployer/paymaster frames；更接近 4337 `handleOps` lifecycle 的 native 化。 | spec-cited |
| D7 | RIP-7712 引入 `nonceKey`/`nonceSequence` 与 `AA_NONCE_MANAGER`，支持 2D nonce。 | spec-cited |
| D8 | requires EIP-7702；可兼容 7702 authorizationList，但 EOA 原地址迁移仍取决于 wallet/account path。 | spec-cited/inferred |
| D9 | 任意 account validation code，签名算法灵活但 mempool 更难 bounded。 | spec-cited |
| D10 | Draft RIP；与 ERC-4337 经验兼容，但未发现 Base/OP 实现投入。 | spec-cited/implementation-signal |
| D11 | 主要风险是 arbitrary validation DoS、cross-validation invalidation、paymaster griefing；RIP-7711/ERC-7562-like sandbox 是核心缓解。 | spec-cited |
| D12 | 对 Mantle 适配成本高：execution client、txpool、batch/derivation、RPC/receipt、wallet/paymaster 都要改；若复用 4337 ecosystem，产品迁移较顺。 | inferred |
| D13 | 适合已有 4337 wallet/paymaster 生态、gasless onboarding、SCA 平滑 native 化。 | inferred |

### 4.2 EIP-8141

| D | 预填 | 证据等级 |
|---|---|---|
| D1 | 协议原生 native AA / frame abstraction。 | spec-cited |
| D2 | 新 `FRAME_TX_TYPE=0x06`，frames、signature list、`APPROVE`、TX/FRAME/SIG introspection 指令、receipt encoding、mempool rules。 | spec-cited |
| D3 | 普通节点/txpool/public mempool 需要 validation-prefix simulation 和 trace rules；wallet/account code 要按 frames 编排。 | spec-cited |
| D4 | 账户 code 可通过 VERIFY frames approval 自定义 validation；default code 支持无 code account 的 secp256k1 path。 | spec-cited |
| D5 | `APPROVE_PAYMENT` / `APPROVE_EXECUTION_AND_PAYMENT` 让 payer 在 frame 中确定；付款者是 approval context。 | spec-cited |
| D6 | 多 frames、atomic batch flag、`SENDER` mode；unused frame gas refund 到 payer。 | spec-cited |
| D7 | 外层 nonce 是 sender nonce；EIP-8250 Keyed Nonces for Frame Transactions 是 companion/proposed item，本文未展开。 | spec-cited/unknown |
| D8 | requires EIP-7702；default code 与 7702 delegation indicator 支持 EOA/code-empty 路径，但迁移体验依赖 wallet code。 | spec-cited |
| D9 | secp256k1/P256 signature list；Motivation 包含 PQ off-ramp；future aggregation 预留。 | spec-cited |
| D10 | Draft Core；EIP-8081 列为 Hegota Considered for Inclusion，未 scheduled。 | spec-cited |
| D11 | 最大风险是 public mempool validation complexity：validation prefix、banned opcodes、mutable state dependencies、`MAX_VERIFY_GAS`、paymaster exceptions。 | spec-cited |
| D12 | Mantle 适配成本高到极高：新 tx type、frame execution、mempool tracing、opcodes/predeploys/RPC/receipt/tooling，且 L1 spec 仍变动。 | inferred |
| D13 | 适合长期 L1-aligned、PQ/key rotation、generic programmable AA；短期 L2 product rollout 风险高。 | inferred |

### 4.3 EIP-7701 condensed delta row

| D | 预填 | 证据等级 |
|---|---|---|
| D1 | 历史 native AA 草案；不再作为独立候选。 | spec-cited |
| D2 | 新 transaction type + opcode family 的旧路线；已被 8141 frame abstraction 取代。 | spec-cited |
| D3 | 与 8141 类似需要 client/mempool/execution 改动，但具体机制已过期。 | spec-cited/inferred |
| D4 | 通过 validation steps 自定义 authorization。 | spec-cited |
| D5 | 区分 authorization 与 gas fee payment，允许另一合约付费。 | spec-cited |
| D6 | validation/execution/post-operation 分段。 | spec-cited |
| D7 | 旧 spec 细节不应沿用；按 8141/8250 重新评估。 | unknown |
| D8 | 非当前迁移方案；被 8141 取代。 | spec-cited |
| D9 | 自定义 validation 方向，但旧 opcode design 已被更新。 | spec-cited/inferred |
| D10 | Withdrawn；withdrawal reason: superseded by EIP-8141。 | spec-cited |
| D11 | 任意 validation/mempool DoS 问题需要新方案处理。 | inferred |
| D12 | 不建议 Mantle 采用；若评估，应评估 8141。 | inferred |
| D13 | 只作为历史背景和 8141 delta。 | inferred |

### 4.4 EIP-2938 historical row

| D | 预填 | 证据等级 |
|---|---|---|
| D1 | 早期协议原生 AA。 | spec-cited |
| D2 | 新 AA transaction type、`PAYGAS` opcode、`NONCE` opcode、verification/execution phase。 | spec-cited |
| D3 | miner/node strategy 需要执行 validation until `PAYGAS`，限制外部 state/opcode access。 | spec-cited |
| D4 | contract account code 定义 transaction validity。 | spec-cited |
| D5 | `PAYGAS` 让合约支付 gas。 | spec-cited |
| D6 | top-level AA frame，PAYGAS 前验证、PAYGAS 后执行。 | spec-cited |
| D7 | 单 tenant 仍使用 target nonce；multi-tenant nonce/replay 仍需研究。 | spec-cited |
| D8 | 不是 EOA 原地址迁移方案。 | inferred |
| D9 | 支持非 ECDSA 等自定义 wallet validation。 | spec-cited |
| D10 | Withdrawn；withdrawal reason: very out of date, needs rewrite。 | spec-cited |
| D11 | 外部 mutable state invalidation、verification gas cap、DoS 是核心问题。 | spec-cited |
| D12 | 不建议 Mantle 采用；只用于理解 native AA 问题史。 | inferred |
| D13 | 历史理论基线。 | inferred |
...
| D | 预填 | 证据等级 |
|---|---|---|
| D1 | EOA 增强，不是完整 native AA。 | spec-cited |
| D2 | 新 EVM opcodes `AUTH`/`AUTHCALL`。 | spec-cited |
| D3 | 需要 invoker contract ecosystem 与 wallet signing UX。 | spec-cited/inferred |
| D4 | EOA secp256k1 签名授权 invoker；无 onchain account config。 | spec-cited |
| D5 | sponsor pattern 可通过 invoker/relayer 实现，不是 paymaster protocol。 | spec-cited/inferred |
| D6 | invoker contract 内 batching。 | spec-cited |
| D7 | 依赖 EOA nonce in AUTH preimage。 | spec-cited |
| D8 | 原 EOA 地址强兼容。 | spec-cited |
| D9 | 主要是 secp256k1 EOA authorization，非多算法 native AA。 | spec-cited |
| D10 | Withdrawn；superseded by EIP-7702。 | spec-cited |
| D11 | invoker bug/恶意授权、签名 UI、sponsor griefing。 | spec-cited/inferred |
| D12 | 不建议 Mantle 新采用；7702 是当前替代。 | spec-cited/inferred |
| D13 | 历史参照：解释 7702/8130 为什么更偏 transaction/account-config 路线。 | inferred |

### 4.6 EIP-7851 adjacent row

| D | 预填 | 证据等级 |
|---|---|---|
| D1 | EOA delegation lifecycle 增强，不是完整 native AA。 | spec-cited |
| D2 | 扩展 7702，新增 `0xef0101` ECDSA-disabled delegation prefix 与 `SETSELFDELEGATE`。 | spec-cited |
| D3 | wallet code、txpool/account-code read、ecRecover companion 需要支持。 | spec-cited |
| D4 | delegated wallet code 控制后续 delegation，可永久禁用 residual ECDSA authority。 | spec-cited |
| D5 | 不定义 native payer/paymaster。 | spec-cited |
| D6 | 不定义 native batching，只由 delegate code 实现。 | spec-cited |
| D7 | 继承 7702/EOA nonce；无完整 AA nonce model。 | spec-cited/inferred |
| D8 | 很强：专为 7702 delegated EOA 迁移后烧掉原 ECDSA 权限。 | spec-cited |
| D9 | 通过 wallet code 间接支持 passkeys/multisig，但 protocol 只处理 ECDSA-disabled code state。 | inferred |
| D10 | Draft；EIP-8081 Proposed for Hegota。 | spec-cited |
| D11 | irreversible ECDSA disable、wallet UX、contracts direct `ecrecover` 不自动感知。 | spec-cited |
| D12 | 若 Mantle 已支持 7702，适配成本中等；但不是 8130 替代方案。 | inferred |
| D13 | 适合“从 7702 过渡到 wallet-code controlled account”场景。 | inferred |
