...
artifact_paths:
  ...
  draft: base-perf-analysis/research-sections/batcher-sequencer-backpressure/drafts/round-1.md
...
draft_metadata:
  ...
  source_requirements_covered: [src-1, src-2, src-3, src-4, src-5, src-6, src-7, src-8, src-9, src-10, src-11, src-12]
  gaps: []
  caveats_addressed:
    - id: caveat-1
...
1. **Mantle 当前处于"无有效背压"状态**。两种设计中的背压机制均不可用：`SequencerMaxSafeLag` 的 CLI 默认值为 0（禁用），仓库中无任何部署配置将其设为非零值（`runtime_configuration_evidence: cli_default`）；DA Throttling 因 Mantle op-geth fork 移除了 `miner_setMaxDASize` RPC 而完全失效（证据：`throttling_enabled_test.go` 明确记录此行为）。这意味着 Mantle 的 batcher-sequencer 交互目前缺乏**任何**主动背压控制——既无二值阻断，也无渐进调节。
...
3. **Mantle 恢复 DA Throttling 是最高优先级的架构修复**。op-batcher 中已有完整的 4 策略控制器代码（Step/Linear/Quadratic/PID），默认 Quadratic 控制器（lower=3.2MB, upper=12.8MB），但因 op-geth 缺失 RPC 接收端而休眠。恢复路径清晰：在 Mantle op-geth 中实现 `miner_setMaxDASize` RPC 即可激活已有代码。

4. **"5k TPS" 声明需要 ~5.3× 的 bytes/UOP 压缩改进**才能在 DA-finalized 层面达到。该声明可能指 pre-confirmation TPS（Flashblocks 层面），而非 DA-finalized TPS。Flashblocks 改善用户感知延迟（从 ~2s 降至 ~250ms），但不改变链最终吞吐。
...
| 字段 | Mantle | Base |
|------|--------|------|
| **backpressure_type** | explicit-batcher (不可用) | explicit-batcher (完整可用) |
...
| **trigger_condition** | `UnsafeDABytes() > LowerThreshold` (3.2MB default) | `da_backlog_bytes > threshold_bytes` (1MB default) |
| **control_granularity** | continuous (4 策略: Step/Linear/Quadratic/PID) — 但不可用 | continuous (3 模式: Off/Step/Linear) |
| **signal_direction** | batcher → sequencer/builder (via `miner_setMaxDASize` RPC) | batcher → builder (via `miner_setMaxDASize` RPC) |
| **runtime_configuration_evidence** | `cli_default`: `DefaultThrottleLowerThreshold = 3_200_000` (3.2MB), `DefaultThrottleUpperThreshold = 12_800_000` (12.8MB), `DefaultThrottleControllerType = "quadratic"` (`throttle_flags.go:10-27`). **⚠ 不可用**: `miner_setMaxDASize` RPC 在 Mantle op-geth 中已移除（`throttling_enabled_test.go:23-37` 明确记录）。`LowerThreshold=0` 禁用 throttling loop（`driver.go:180-185`）。 | `cli_default`: `threshold_bytes=1_000_000` (1MB), `block_size_lower_limit=2_000`, `block_size_upper_limit=130_000`, `tx_size_lower_limit=150`, `tx_size_upper_limit=20_000`（`bin/batcher/src/cli.rs:162-179`）. **完整可用**: `MinerApiExt` trait（`execution/rpc/src/miner.rs:12-27`）暴露 `setMaxDASize`, `getMaxDASize`, `setGasLimit` 三个方法。 |

**Mantle DA Throttling 不可用的关键证据**：

`throttling_enabled_test.go:23-37`:
> "TestThrottlingEnabledFailure verifies that when throttling is enabled (LowerThreshold > 0), the batcher fails to start because op-geth does not support the miner_setMaxDASize RPC method."
...
`driver.go:626-638`: 当 RPC 返回 "method not found" 时，`singleEndpointThrottler` 调用 `l.StopBatchSubmitting(ctx)` — batcher 主动关闭。

`throttling_disabled_test.go:32-41` 确认默认运行模式：`LowerThreshold = 0` 完全跳过 throttling loop。
...
graph TB
    subgraph Mantle["Mantle 数据流与背压"]
        ...
        M_BAT -.->|"❌ DA Throttling<br/>miner_setMaxDASize REMOVED<br/>from op-geth"| M_SEQ
...
**关键差异总结**：Mantle 的两条显式背压路径均不可用（MaxSafeLag 默认禁用 + DA Throttling RPC 已移除），仅剩 engine 侧 500MB 内存保护作为最后防线。Base 拥有完整的 DA Throttling 作为主动背压通道，且额外提供 `miner_setGasLimit` 作为更精细的控制手段。
...
| 场景 | Span 增长速率 | 控制机制 |
|------|-------------|----------|
| L1 blob fee 飙升 | Batcher tx 被延迟/替换 → span 以 1 block/2s 增长 | Mantle: 无控制（MaxSafeLag 默认禁用，DA throttling 不可用）；Base: DA throttling 激活 |
...
sequenceDiagram
    ...
    Note over SEQ: Mantle: NO backpressure (MaxSafeLag=0, DA throttle removed)
...
- `DefaultThrottleUpperThreshold = 12_800_000` (12.8MB = 4 × lower)
...
| Linear | 从 threshold 到 2×threshold 线性插值 |
...
ratio = (excess / range).min(1.0)  // 0 at threshold, 1.0 at 2×threshold
...
- **Mantle 无对应 RPC** — 仅有 L1 `SystemConfig.sol` 合约层面的 gas limit governance
...
| `miner_setMaxDASize` | **RPC 已移除** — 不可用 | 完整实现 + AtomicU64 存储 |
| `miner_setGasLimit` | 不存在 | 完整实现 — 策略 D 的现成基础设施 |
| **可用状态** | **❌ 休眠** (LowerThreshold=0) | **✅ 活跃** |
...
graph LR
    subgraph Mantle["Mantle: 背压控制 (当前状态)"]
        ...
        M_DAT["DA Throttling<br/>❌ miner_setMaxDASize<br/>RPC REMOVED"]
        ...
        M_DAT -->|"NOT available"| M_NOOP["No Effect"]
        ...
        style M_NOOP fill:#868e96,color:#fff
    end
...
**demand_activation_threshold**: 因果链 A 在 Mantle 当前配置下不会被激活（MaxSafeLag=0）。若未来启用（如设为 1000），则当 batcher cadence > 2000s (1000 blocks × 2s) 时触发。
...
**Mantle 当前状态**：DA Throttling 不可用 → 因果链 B 中的 throttling 控制环路**断开**。UnsafeDABytes 增长不会触发任何限制。但恶性循环的 L1 fee 部分仍可发生。

**demand_activation_threshold**：
- Base: `da_backlog_bytes > 1MB` 时 DA throttling 激活
- Mantle: 永不激活（throttling 不可用）。但 `UnsafeDABytes` 仍被计算和记录为 metric，可用于监控告警。

5b `da-bandwidth-throughput-ceiling` 结论：Mantle 有 ~1,480× DA headroom（1.18 TPS vs ~1,749 TPS DA ceiling），因此 DA 带宽本身不是瓶颈。**但 EIP-7918 blob fee 指数增长模型（+50% sustained demand → ~7× in 5min）意味着即使有充足带宽，费用螺旋仍可发生。**
...
**demand_activation_threshold**：此因果链在当前 demand 下已激活。Mantle 当前 ~1.18 TPS 已产生 ~224 block unsafe span。若 demand 增长 10×（~12 TPS），batcher 在 5a Quick Wins 前无法跟上（5a 估算当前 batcher 饱和容量 ~36 TPS），span 会持续增长。
...
| B (fee spiral) | ⚠ Throttling 断路，fee spiral 可发生但无自动缓解 | ✅ DA throttling 提供自动缓解 | Base 有渐进控制，Mantle 无 |
| C (block size reduction) | ❌ Throttling 不可用 | ✅ Active — block size 被动态调整 | Base 独有的用户体验优化 |
| D (derivation lag) | ✅ 始终活跃 (~224 block span) | ✅ 始终活跃 (~25 block span) | Mantle span 9× 大于 Base |
...
- 达到 5k TPS 需要 bytes/UOP 降至 ~29B（~5.3× 压缩改进）

**可能的 "5k TPS" 定义**：
1. **Pre-confirmation TPS**（最可能）：Flashblocks 8× sub-block 将用户延迟从 ≤2s 压至 ≤250ms，相当于用户感知的"响应速度"提升，但不改变链的吞吐能力
...
**tps_definition**: "5k TPS" 最可能指 pre-confirmation 层面的用户感知速率，或 gas throughput equivalent。DA-finalized 层面在当前 bytes/UOP 下无法达到 5k — 需 ~5.3× 压缩改进。

### 5.3 Flashblocks 解耦效果

**机制**（`cross_topic_reference`: Section 2 `block-builder-flashblocks-throughput` final.md）：
- 将 2s L2 block 拆为 8 × 250ms sub-block
- 用户延迟从 ≤2s 压至 ≤250ms
...
**前提**：Mantle 当前**完全缺失 DA throttling** — 仅有默认禁用的 binary stall 作为背压（且该 stall 在默认配置下不激活）。
...
2. **第二步**：激活 op-batcher 已有的 throttler 代码
   - 设置 `LowerThreshold > 0`（如默认 3.2MB）
...
| 维度 | 评估 |
|------|------|
| TPS 改善预估 | 理论线性扩展 batcher 吞吐。2 个 batcher → ~2× 提交频率 |
...
**思路**：Sequencer 根据 batcher 积压情况动态调整 L2 gas limit。
...
**Mantle 缺失**：无 `miner_setGasLimit` RPC。仅有 L1 `SystemConfig.sol` 合约层面的 gas limit governance。
...
| 实现复杂度 | **中**（~3-5 人周）。需在 op-geth 中实现 `miner_setGasLimit` RPC + batcher 侧 gas limit 控制逻辑 |
| 风险等级 | **中**。Gas limit 调整需与 EIP-1559 gas pricing 协调（Section 3 `gas-protocol-perf-config`）|
| 可逆性 | **高**。设 gas limit = 0（使用默认）可立即回退 |
| 时间线 | 6-8 周 |
...
| **D: 自适应 Gas Limit** | 中（精细控制） | 中（3-5 人周） | 中 | 高 | 6-8 周 | **P1** |
...
3. **DA Throttling 参数调优不可行**
   - `miner_setMaxDASize` RPC 在 op-geth 中不存在
   - 任何非零 `LowerThreshold` → batcher 检测到缺失 RPC → 主动关闭
   - 此路径被 op-geth fork 阻断

**预计 TPS 改善**：batcher 侧吞吐显著提升（5a 估算 ~30×），但背压控制能力不变。
...
2. 设置 `LowerThreshold` > 0 激活 throttling loop
3. 使用已有的 Quadratic controller（默认配置 lower=3.2MB, upper=12.8MB）
4. 配置 `SequencerMaxSafeLag` 为安全阀值

**预计 TPS 改善**：从 binary stall（或无控制）→ continuous throttle，用户体验显著改善。TPS 不直接提升，但允许更激进的 batcher 吞吐参数。
...
- Throttling loop 以非零 threshold 运行
...
| 方案 | 前置条件 | 投入 | 价值 |
|------|---------|------|------|
| 策略 D (自适应 gas limit) | 阶段 1 完成，op-geth 已有 RPC 基础 | 3-5 人周 | 精细化背压控制 |
...
**推荐优先序**：
1. 策略 D（自适应 gas limit）— 与阶段 1 RPC 工作共享基础设施（`miner_setGasLimit`）
2. 策略 C（Flashblocks）— 长期投资，改善用户体验
3. 策略 B（多 batcher）— 仅在 demand 接近单 batcher 极限时考虑

**与其他课题的关系**：
- 5a Quick Wins 是阶段 0 的前置条件（`cross_topic_reference`: `batcher-pipeline-architecture` final.md §6.1-§6.4）
- 5b 确认 DA 非约束 → 阶段 0/1 可独立于 DA 层改进（`cross_topic_reference`: `da-bandwidth-throughput-ceiling` final.md §3.2）
- Section 4 的 actor model 分析是阶段 2 架构决策的基础（`cross_topic_reference`: `sequencer-consensus-pipeline-perf` final.md §7）
- Section 2 的 Flashblocks 分析是策略 C 评估的基础（`cross_topic_reference`: `block-builder-flashblocks-throughput` final.md §2）
...
**5b EIP-7918 模型**：+50% sustained demand → ~7× blob fee in 5 minutes。

**Mantle 当前风险**：DA throttling 不可用 → 无法通过降低 block size 缓解积压。但 Mantle DA headroom ~1,480× → 正常操作中 blob fee 风险极低。风险主要在 L1 全网 blob demand spike 场景。
...
| L1 blob fee spiral | Medium | 低（~1,480× DA headroom） | 提交成本骤增 | 恢复 DA throttling + fee 监控 | `l1_blob_base_fee` |
...
### src-1: `mantle-v2/op-node/rollup/sequencing/sequencer.go` — SequencerMaxSafeLag 实现 ✅
...
### src-2: `mantle-v2/op-batcher/batcher/` — DA Throttling 实现链 ✅
...
- 默认配置：`flags/throttle_flags.go:10-27`

### src-3: `base/base` batcher throttling 实现 ✅
...
### src-4: `base/base` sequencer unsafe span 控制 ✅
...
### src-5: `mantle-v2/op-node/rollup/engine/engine_controller.go` ✅
...
### src-6: 课题 5a（batcher-pipeline-architecture）final.md ✅
...
- §6.6: "sequencer 出块速度 ~10× batcher 提交速度" 耦合度量

### src-7: 课题 5b（da-bandwidth-throughput-ceiling）final.md ✅

引用数据点：
- DA 非约束：Mantle 1.18 TPS demand vs ~1,749 TPS DA ceiling (~1,480× headroom)
- BPO2: target=14, max=21 blobs/block
- Base bytes/UOP = 153.03 B → 5k TPS 需 ~29B (~5.3× 压缩改进)
- EIP-7918: +50% sustained demand → ~7× blob fee in 5min

### src-8: Section 2（block-builder-flashblocks-throughput）final.md ✅
...
### src-9: Section 4（sequencer-consensus-pipeline-perf）final.md ✅
...
- **DA throttling threshold**：仓库中无 deployed_config 证据。默认值 `LowerThreshold=0`（禁用）。测试代码（`throttling_disabled_test.go`）确认默认运行模式为禁用。
