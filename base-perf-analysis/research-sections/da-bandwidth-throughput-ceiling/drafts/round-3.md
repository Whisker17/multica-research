...
draft_metadata:
  ...
  prior_draft_commit: "https://github.com/Whisker17/multica-research/commit/c080e5e"
  outline_rounds: 2
...
-- NOTE: avg_fill_rate cannot be derived from raw on-chain blob bytes alone
-- (the KZG blob is always 131,072 B zero-padded; actual payload length is
-- embedded in the OP Stack field-element encoding header and requires
-- off-chain decoding). Compute fill_rate as:
...
JOIN ethereum.blocks AS b
  ON pb.block_number = b.number;
-- NOTE: avg_fill_rate = L2BEAT_total_data_posted / (total_blobs × 130,044)
...
All required types present at or above `Min Count`. Round-3 updated Base bytes/UOP
to current L2BEAT data-posted window (153.03 B), rewrote both Dune SQL artifacts
with internally consistent CTEs, and separated diag-2 Mantle gap nodes from
observed styling. Remaining gap: window-specific fill rate + P95/P99 requires
Dune execution (SQL artifacts now self-consistent and ready to run).
