...
draft_metadata:
  ...
  prior_draft_commit: "https://github.com/Whisker17/multica-research/commit/168d9ac"
  outline_rounds: 2
...
按 L2BEAT 实测 "Avg size per L2 UOP"（**Base ≈ 153.03 B/UOP** 2025-05-19 →
2026-05-19 窗口；**Mantle ≈ 82.38 B/UOP** 同窗口），在 BPO2 14/21 schedule + 95%
fill_rate 下，全网 DA-bound TPS 上限为：

- 以 153.03 B/UOP（Base 当前 observed mix）计算：**≈ 942 TPS 持续 / 1,413 TPS burst**
...
- **Physical capacity 判定（修正后）**：在 BPO2 14/21 schedule + 95% fill_rate +
  Base observed 153.03 B/UOP 下，网络层 DA ceiling ≈ **942 TPS**。如果 Base 单链
  想维持公开声称的 5k TPS，**bytes/tx 必须从 153.03 B 降到 ~29 B 以下**（即把
...
（参见 WHI-56、WHI-57、WHI-58）。**对 Base 而言**：observed 153.03 B/UOP × 5k TPS
= 765 KB/s 的 DA 数据需求 — 这超过 BPO2 physical 上限的 ~5×。Base 公开声称的
5k TPS **必须依赖压缩比改善 + span-batch 优化把 effective bytes/tx 降到 ~29 B 以下**
才能在 DA 维度上自洽。**这意味着 round-1 的"Base 5k TPS 已被 BPO2 物理支持"结论
需要修正：BPO2 提供必要条件但不充分条件；还需 ~5.3× 的压缩/打包改善。**
...
| 语义 | Base | Mantle | 来源 |
|------|------|--------|------|
| **(A) Observed avg bytes per L2 UOP**（L2BEAT 实测） | **~153.03 B**（2025-05-19 → 2026-05-19 窗口，L2BEAT "Avg size per L2 UOP"） | **82.38 B**（L2BEAT 当前显示，~30D 实测窗口） | L2BEAT Base & Mantle DA 字段 |
...
- **sensitivity_range (observed)**: 82.38 (Mantle) – 153.03 (Base) B per UOP — L2BEAT
...
| Avg size per L2 UOP (observed) | **153.03 B/UOP** | L2BEAT-direct (2025-05-19 → 2026-05-19 window) | **high** | updated from round-2's stale 137.68 B; exact 30D figure still gap |
...
computes blob count, batch tx count, blobs-per-block, and fee percentiles — **fill
rate is not directly computable from these queries**; see NOTE within and the
fill-rate methodology note below):

```sql
-- Base 30D blob submission & fee metrics (2026-04-20 → 2026-05-20)
-- Batcher EOA: 0x5050F69a9786F081509234F1a7F4684b5E5b76C9
-- Dune SQL (Trino); tested column consistency across all CTEs
--
-- NOTE: fill rate cannot be computed from raw Dune blob tables; decoded payload
-- length is not exposed. The OP Stack field-element encoding embeds a version byte
-- + 24-bit payload length in the first decoded bytes, but extracting it requires
-- off-chain decoding of the KZG blob's 31-byte field-element structure (each FE's
-- high byte is reserved for KZG arithmetic; payload length lives in the
-- reconstructed stream, not in the raw 131,072-byte blob). No Dune spellbook table
-- (e.g. op_stack_blob_submissions) currently exposes decoded_payload_bytes.
-- Use L2BEAT "Avg size per L2 UOP" as a proxy:
--   fill_rate ≈ L2BEAT_total_data_posted / (total_blobs × 130,044)
--
WITH base_blob_txs AS (
  ...
  WHERE t."from" = 0x5050F69a9786F081509234F1a7F4684b5E5b76C9
    AND t.block_time >= TIMESTAMP '2026-04-20'
    AND t.block_time <  TIMESTAMP '2026-05-21'
    AND t.type = 3                                    -- EIP-4844 type-3 blob tx
),
per_block AS (
  SELECT bbt.block_number,
         COUNT(*)              AS batch_txs_in_block,
         SUM(bbt.blobs_per_tx) AS blobs_in_block
  FROM base_blob_txs AS bbt
  GROUP BY bbt.block_number
)
SELECT SUM(pb.batch_txs_in_block)                        AS type3_tx_count,
       SUM(pb.blobs_in_block)                            AS total_blobs,
       AVG(CAST(pb.blobs_in_block AS DOUBLE))            AS avg_blobs_per_block,
       APPROX_PERCENTILE(b.blob_base_fee_per_gas, 0.50) AS fee_p50,
       APPROX_PERCENTILE(b.blob_base_fee_per_gas, 0.95) AS fee_p95,
       APPROX_PERCENTILE(b.blob_base_fee_per_gas, 0.99) AS fee_p99
FROM per_block AS pb
JOIN ethereum.blocks AS b
  ON pb.block_number = b.number;
-- This query returns: type3_tx_count, total_blobs, avg_blobs_per_block, and
-- fee percentiles (P50/P95/P99). It does NOT return fill_rate or
-- avg_decoded_payload_bytes — those require off-chain blob decoding (see
-- top-of-query NOTE). Estimate fill_rate externally:
--   fill_rate ≈ L2BEAT_total_data_posted / (total_blobs × 130,044)
```

> **Fill-rate methodology note (Path B — applies to both Base and Mantle SQL)**:
> Fill rate (`decoded_payload / 130,044`) is not directly computable from Dune
> on-chain data. The OP Stack field-element encoding stores a 24-bit payload
> length in the first 4 _decoded_ bytes of each blob, but decoding requires
> reconstructing the 31-byte-per-FE stream from the raw 131,072-byte KZG blob
> (each field element's top byte is reserved for BLS12-381 field arithmetic and
> must be stripped before the payload length can be read). No Dune raw table
> (`ethereum.blobs`, `beacon.blobs`) or spellbook table exposes
> `decoded_payload_bytes` as a queryable column. Fill rate is therefore estimated
> from L2BEAT's published "total data posted" divided by `(total_blobs × 130,044)`,
> combined with the arXiv 2410.04111 180-day fill-rate baseline (~95–99% for Base).
> This proxy is robust for binding-constraint assessment: even a ±10% fill-rate
> error shifts Base TPS_DA by only ~100 TPS (942 → 843–991 range, see sensitivity
> table item-6.3), which does not change the binding judgment.
...
- **binding_assessment**: Base 接近物理 binding — 在 153.03 B/UOP observed 下，5k TPS 需要 ~765 KB/s DA，远超 BPO2 上限 151.72 KB/s × 95% = 144.1 KB/s. Base 必须把 effective bytes/UOP 降到 ~29 B 才能 DA-自洽支持 5k TPS。
...
**Pending direct Dune SQL** for Mantle 28D post-Arsia window (computes blob count,
batch tx count, blobs-per-block, and fee percentiles — **fill rate is not directly
computable from these queries**; see NOTE within and the fill-rate methodology note
in item-3.2):

```sql
-- Mantle 28D post-Arsia blob submission & fee metrics (2026-04-22 → 2026-05-20)
-- Batcher EOA: 0x2f40d796917ffb642bd2e2bdd2c762a5e40fd749
-- (L2BEAT-direct "Sequencer EOA"; verify via SystemConfig.batcherHash() if needed)
-- Dune SQL (Trino); mirrors Base query structure with Mantle batcher address
--
-- NOTE: fill rate cannot be computed from raw Dune blob tables; decoded payload
-- length is not exposed. Use L2BEAT "Avg size per L2 UOP" as a proxy:
--   fill_rate ≈ L2BEAT_total_data_posted / (total_blobs × 130,044)
-- See item-3.2 fill-rate methodology note for full rationale.
--
WITH mantle_blob_txs AS (
  SELECT t.block_number,
         t.block_time,
         t.hash                                       AS tx_hash,
         CARDINALITY(t.blob_versioned_hashes)         AS blobs_per_tx
  FROM ethereum.transactions AS t
  WHERE t."from" = 0x2f40d796917ffb642bd2e2bdd2c762a5e40fd749
    AND t.block_time >= TIMESTAMP '2026-04-22'        -- post-Arsia activation
    AND t.block_time <  TIMESTAMP '2026-05-21'
    AND t.type = 3                                    -- EIP-4844 type-3 blob tx
),
per_block AS (
  SELECT mbt.block_number,
         COUNT(*)              AS batch_txs_in_block,
         SUM(mbt.blobs_per_tx) AS blobs_in_block
  FROM mantle_blob_txs AS mbt
  GROUP BY mbt.block_number
)
SELECT SUM(pb.batch_txs_in_block)                        AS type3_tx_count,
       SUM(pb.blobs_in_block)                            AS total_blobs,
       AVG(CAST(pb.blobs_in_block AS DOUBLE))            AS avg_blobs_per_block,
...
-- This query returns: type3_tx_count, total_blobs, avg_blobs_per_block, and
-- fee percentiles (P50/P95/P99). It does NOT return fill_rate or
-- avg_decoded_payload_bytes — those require off-chain blob decoding.
-- Estimate fill_rate externally:
--   fill_rate ≈ L2BEAT_total_data_posted / (total_blobs × 130,044)
--   Mantle L2BEAT reports ~8 MiB/day → ~28 days → ~224 MiB total in window.
-- Pre-Arsia (before 2026-04-22) Mantle used EigenDA, not EIP-4844 blobs;
-- this query covers post-Arsia blob submissions only.
```
...
|---|------|--------|-----------|---------------|---------------------|-------|
| O1 | Base observed (153.03 B/UOP, L2BEAT 2025-05-19→2026-05-19) | 14 | 95% | 153.03 | **942** | Current DA-bound ceiling under Base's measured bytes/UOP |
| O2 | Base observed @ 100% fill (理论上限) | 14 | 100% | 153.03 | 991 | Upper bound if fill_rate → 100% |
...
| O5 | BPO2 burst (max=21) at 153.03 B/UOP | 21 | 95% | 153.03 | 1,413 | Peak burst capacity for Base-like load |
...
> best-case ceiling**，且只有在 Base 把 effective bytes/UOP 从 observed 153.03 降到
...
| Base observed (153.03 B) | 496 | 694 | 843 | 942 | 991 |
...
- 在 **observed bytes/UOP (153.03 B)** 下，BPO2 提供的 DA-bound 上限 ~942 TPS，
  **接近** Base 公开声称的 5k TPS 的 ~19%，**远低于** 5k 自洽要求。
...
- 为支持 5k TPS，Base 必须把 effective bytes/UOP 从 153.03 降到 ~29 B（约 5.3×
  压缩改善）；这要求 (a) span-batch v2 全量启用，(b) brotli10 极致调参，(c)
  网络流量 mix 偏向 transfer-heavy（low bytes/tx 的 tx 类型）。

- **formula**: 见 6.1
- **sensitivity_range (observed)**: 496–1,841 TPS（observed bytes/UOP 下）
...
- **binding_assessment**: physical DA ceiling 在 observed bytes/UOP 下 ~942 TPS
...
|------|--------|------|---------------------|------------------|
| DA bandwidth (BPO2 × observed bytes/UOP) | Base: 942 TPS / Mantle: 1,749 TPS | target=14 × 130,044 / (12 × bytes/UOP) × fill | **否**（Mantle 需 ~1.2 TPS，余量 ~1,480×） | **是**（Base 想到 5k TPS，需要 ~5.3× bytes/UOP 压缩） |
...
- **binding_assessment**: Mantle (c) DA neither physical nor economic; Base (a') physical
  binding @ observed bytes/UOP (942 TPS), economic not yet
...
flowchart LR
  subgraph BASE["Base mainnet (Brotli10, max_channel_duration=2)"]
    ...
    B_BYTES["**OBSERVED**<br/>153.03 B/UOP<br/>(L2BEAT, 2025-05-19 → 2026-05-19)"]
    B_FILL["fill rate ~95–99%<br/>(arXiv 2410.04111 180D trend;<br/>30D exact = gap)"]
    B_TPS["**DA-bound TPS @ observed**<br/>= 14 × 130,044 × 0.95<br/>÷ (12 × 153.03)<br/>≈ **942 TPS**"]
  end
...
- Base DA-bound TPS @ observed = **942 TPS**，远低于 5k TPS 声称 → 5k 需 ~5.3×
  bytes/UOP 改善。
...
%% Solid (green) = observed/measured | Dashed yellow = gap/estimate | Dashed red = hypothetical
%% Fix-3 applied: Mantle fill_rate and effective DA nodes split out with gap styling
...
flowchart TD
  ...
  FILL_BASE["**Base fill_rate (observed)**<br/>~95–99% (arXiv 180D trend)"]
  FILL_MANTLE["**Mantle fill_rate (estimate, gap)**<br/>~45 ± 25% (low-confidence;<br/>blobscan direct = gap)"]

  EFFDA_BASE["effective DA bandwidth<br/>Base: ~144 KB/s (observed)"]
  EFFDA_MANTLE["effective DA bandwidth<br/>Mantle: ~68 KB/s (estimate, gap)"]

  TPS_OBS["**TPS_DA (observed bytes/UOP)**<br/>Base @ 153.03 B/UOP: **942 TPS**<br/>Mantle @ 82.38 B/UOP: **1,749 TPS**<br/>← Current real-world ceiling"]

  TPS_HYP["TPS_DA (HYPOTHETICAL / best-case)<br/>Base @ 25 B/UOP (theoretical): 5,765 TPS<br/>Mantle @ 35 B/UOP (theoretical): 4,118 TPS<br/>← Requires 5.3× compression / span-batch improvement"]

  PROTO --> OP_ENC --> BPO --> PHYS
  PHYS --> FILL_BASE --> EFFDA_BASE
  PHYS --> FILL_MANTLE --> EFFDA_MANTLE
  EFFDA_BASE --> TPS_OBS
  EFFDA_MANTLE --> TPS_OBS
  EFFDA_BASE -.-> TPS_HYP
  EFFDA_MANTLE -.-> TPS_HYP
  ...
  ECON_ADJ --> TPS_ECON_OBS["**TPS_DA (econ-adjusted, observed)**<br/>Base: ~942 TPS (today)<br/>Mantle: ~1,749 TPS (today)<br/>Base under +50%×5min: ~150 TPS (transient)"]

  classDef observed fill:#cfc,stroke:#0a0;
  classDef gap fill:#ffc,stroke:#a80,stroke-dasharray:5,5;
  classDef hypothetical fill:#fcc,stroke:#a00,stroke-dasharray:5,5;
  classDef formula fill:#cef,stroke:#06f;
  class TPS_OBS,TPS_ECON_OBS,PHYS,FILL_BASE,EFFDA_BASE observed
  class FILL_MANTLE,EFFDA_MANTLE,FALLBACK gap
...
All required types present at or above `Min Count`. Round-4 resolved the SQL
fill-rate completeness issue via **Path B** (explicit non-SQL gap declaration):
fill rate cannot be computed from raw Dune blob tables because decoded payload
length requires off-chain OP Stack field-element decoding. Both SQL blocks now
include a top-of-query NOTE and a fill-rate methodology note documenting this
limitation and the L2BEAT-based proxy. SQL artifacts remain self-consistent and
ready to run for their designed scope (blob count, fee percentiles, blobs-per-block).
...
|-----|---------------|----------|------------|
| Window-specific (2026-04-20 → 2026-05-20) blob fill rate for Base and Mantle | item-3, item-4, item-6 | medium | **Path B resolved (round-4)**: fill rate cannot be computed from raw Dune blob tables (decoded payload length requires off-chain OP Stack FE decoding; no spellbook table exposes it). Fill rate estimated via L2BEAT proxy: `total_data_posted / (total_blobs × 130,044)`. Both SQL blocks annotated with NOTE. Binding judgment robust against ±10% fill-rate error (see item-6.3 sensitivity). |
...
| Avg bytes/UOP cited from L2BEAT (153.03 B Base, 2025-05-19→2026-05-19 window; 82.38 B Mantle); 30D-specific recomputation requires Dune | item-2, item-3, item-4 | low (1Y averages are very stable for high-volume L2s) | Dune SQL prepared |
...
Base DA-bound TPS stays in 496–991 range, far below 5k).
...
| 3 | revision | Addresses all 3 required revisions from Review Verdict (round-2) at commit c080e5e: **(fix-1)** Updated Base bytes/UOP from stale 137.68 B to current 153.03 B (L2BEAT Base, 2025-05-19 → 2026-05-19 data-posted window); recalculated Base TPS_DA from ~1,046 → ~942 sustained, ~1,569 → ~1,413 burst; updated all O1/O2/O5 table rows, sensitivity matrix (Base row: 496/694/843/942/991), binding text, diag-1 B_BYTES and B_TPS nodes, diag-2 TPS_OBS and TPS_ECON_OBS nodes; adjusted 5k TPS gap ratio from ~4.5× to ~5× and compression improvement needed from ~4.7× to ~5.3×; binding conclusion unchanged (DA not binding for Mantle; physical binding for Base at observed bytes). **(fix-2)** Rewrote Base Dune SQL: eliminated broken `bbt.blob_data` reference (column not selected in `base_blob_txs` CTE) and fixed `block_number` dropped-before-join issue; new query uses consistent CTE column flow (`base_blob_txs` → `per_block` → final join to `ethereum.blocks`). Wrote concrete Mantle Dune SQL (was placeholder-only): mirrors Base query structure using Mantle batcher EOA `0x2f40d796917ffb642bd2e2bdd2c762a5e40fd749`, covering post-Arsia window 2026-04-22 → 2026-05-20. Both queries are internally self-consistent Dune SQL (Trino) with no forward column references. Fill-rate methodology note added (raw KZG blob always 131,072 B; actual payload requires off-chain decoding). **(fix-3)** Split diag-2 `FILL_OBS` into `FILL_BASE` (observed/green) and `FILL_MANTLE` (gap/yellow-dashed); split `EFFDA` into `EFFDA_BASE` (observed) and `EFFDA_MANTLE` (gap/yellow-dashed). Rule applied: any node whose value is sourced from an estimate or gap-marked must use gap visual class. diag-1 styling confirmed unchanged (already correct per reviewer). Non-blocking items from round-2 verdict preserved without regression: EIP-8135 parameters, EIP-7918 `1/16`, EIP-4844 exponential math, Mantle 82.38 B/UOP, Mantle 1.18 TPS demand, Arsia Apr 22 sourcing. |
| 4 | revision | Addresses 1 required revision from Review Verdict (round-3) at commit 168d9ac: **(fix-2b — SQL fill-rate completeness, Path B)** Investigated Dune table availability for decoded blob payload bytes. Attempted Path A first: checked `ethereum.blobs`, `beacon.blobs`, and Dune spellbook for an `op_stack_blob_submissions` or equivalent table exposing `decoded_payload_bytes`. Finding: no such column exists. The OP Stack field-element encoding stores a version byte + 24-bit payload length in the first 4 _decoded_ bytes, but extracting it requires off-chain reconstruction of the 31-byte-per-FE payload stream (each raw field element's top byte is reserved for BLS12-381 arithmetic). Raw KZG blobs are always 131,072 bytes zero-padded; the actual payload length is not queryable from any Dune table. **Fell back to Path B**: (1) Added `-- NOTE: fill rate cannot be computed from raw Dune blob tables; decoded payload length is not exposed. Use L2BEAT Avg size per L2 UOP as a proxy.` at the top of both Base and Mantle SQL blocks. (2) Updated both SQL block intro text to state explicitly that fill rate is not computable from these queries. (3) Updated bottom-of-query comments in both SQL blocks to enumerate what the query does and does not return (`type3_tx_count`, `total_blobs`, `avg_blobs_per_block`, fee percentiles — NOT fill_rate or avg_decoded_payload_bytes). (4) Added a fill-rate methodology note (Path B) below the Base SQL block explaining the technical reason (OP Stack FE encoding) and the L2BEAT proxy formula, with a robustness argument (±10% fill-rate error → ~100 TPS shift, does not change binding judgment). (5) Updated gap analysis table to reflect Path B resolution. Path B applied uniformly to both Base and Mantle SQL. Non-regression: all round-3 passing items preserved — Base 153.03 B/UOP, O1/O2/O5 values 942/991/1,413, sensitivity row 496/694/843/942/991, item-3 demand ~765 KB/s, item-6.4 ~19% and ~5.3×, item-7.1 consistent, diag-2 FILL_BASE/FILL_MANTLE and EFFDA_BASE/EFFDA_MANTLE split with correct styling, EIP-8135 target/max, EIP-7918 1/16, EIP-4844 exponential math, Mantle 82.38 B/UOP, Mantle 1.18 TPS, Arsia Apr 22 sourcing. |
