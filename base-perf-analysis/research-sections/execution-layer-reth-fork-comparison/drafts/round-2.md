...
# Phase B Round 2 Draft — 执行层性能架构对比：Base Reth Fork vs Mantle Reth Fork
...
2. **真正的性能差异是「两种不同的缓存架构」，不是「缓存有无」**。两个 fork **都**在执行热路径
   ...
   - **架构对比**：Mantle 的 Tier B 缓存是"按 block / 子块为单位的 bundle-state 复用"，主要受益于
     flashblocks 子块串行的特征；Base 的 Tier C 是"执行 provider + precompile + receipt root 多个
     独立缓存/并发原语的组合"，覆盖面更细。下游性能差异体现在 (a) precompile 重复输入命中率
     (Base 有 Mantle 无)、(b) receipt root 是否在关键路径 (Base 异步 / Mantle 同步)、(c) state-root
     与 execute 是否并发 (Base 三档 / Mantle 默认 sync)，而**不是**整体"缓存 vs 无缓存"。
