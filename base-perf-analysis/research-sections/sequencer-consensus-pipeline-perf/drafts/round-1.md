...
artifact_paths:
  ...
  draft: base-perf-analysis/research-sections/sequencer-consensus-pipeline-perf/drafts/round-1.md
...
# Sequencer Pipeline 与共识层优化分析（Draft Round 1）
...
3. **Engine API 调用模式（item-3）**：Base 的 `EngineActorRequest` 共有 **7 个变体**
   （`actors/engine/request.rs:46-64`）：`BuildRequest`、`GetPayloadRequest`、
   ...
   HTTP/JWT RPC（`engine_controller.go:453, 560, 664, 1090; payload_process.go:onPayloadProcess;
   payload_success.go:onPayloadSuccess; build_seal.go:onBuildSeal`）。
...
- `OnEvent(ctx, ev)` switch 大致处理事件：
  ...
  - `PayloadSuccessEvent` → `engine.onPayloadSuccess`（payload_success.go）：依次调用
    `tryUpdateUnsafe` → `tryUpdatePendingSafe` → `tryUpdateLocalSafe` → `tryUpdateEngineInternal`
    （`ForkchoiceUpdate` 同步 RPC，无 attributes）
...
|---|---|---|---|---|---|
| build_start | `PayloadBuilder.build_on()` → `start_build_block` 发送 `EngineActorRequest::BuildRequest` | `startBuildingBlock` 发射 `BuildStartEvent` 触发 `engine.onBuildStart`（`engine_forkchoiceUpdatedV3` with attributes 同步 RPC） | `awaited_via_oneshot`（mpsc result_tx for `PayloadId`） | `direct_sync_call`（FCU+attrs HTTP/JWT） | Base：sequencer await `PayloadId`；Mantle：sequencer event 同步等 ForkchoiceUpdate 返回 |
| build_seal (get_payload) | `engine_client.get_payload(payload_id).await` 发送 `GetPayloadRequest` | `engine.onBuildSeal` 调用 `e.engine.GetPayload(rpcCtx, ev.Info)`（`engine_getPayloadV3` 同步 RPC） | `awaited_via_oneshot` | `direct_sync_call` | 同上 |
...
| 调用点 | 端点 | 同步性 | 文件:行号 |
|---|---|---|---|
| `EngineController.onBuildStart` (FCU+attrs) | `engine_forkchoiceUpdatedV{2,3}` with attrs | `direct_sync_call_with_event_trigger` | `engine_controller.go:453` (示意，具体方法在 onBuildStart 内) |
| `EngineController.onBuildSeal` (GetPayload) | `engine_getPayloadV{2,3,4}` | `direct_sync_call` | `mantle-v2/op-node/rollup/engine/build_seal.go:onBuildSeal` |
| `EngineController.onPayloadProcess` (NewPayload) | `engine_newPayloadV{2,3,4}` | `direct_sync_call`，超时 10s | `mantle-v2/op-node/rollup/engine/payload_process.go:onPayloadProcess` |
| `EngineController.onPayloadSuccess`（tryUpdateUnsafe → ... → tryUpdateEngineInternal） | `engine_forkchoiceUpdatedV{2,3}` (without attrs) | `direct_sync_call` 串行 | `mantle-v2/op-node/rollup/engine/payload_success.go:onPayloadSuccess` |
| `EngineController.InsertUnsafePayload` | `engine_newPayloadV{2,3,4}` + `engine_forkchoiceUpdatedV{2,3}` 串行 | 两个同步 RPC 顺序执行 | `engine_controller.go:497` |
| `EngineController.tryUpdateEngineInternal` | `engine_forkchoiceUpdatedV{2,3}` | 直接 `e.engine.ForkchoiceUpdate(ctx, &fc, nil)` | `engine_controller.go:560, 664, 1090` |

⚠ Mantle 端**没有"engine task"层** — 所有 RPC 由 `EngineController` 在事件回调内直接对
EL 同步执行；线程模型上来自 op-node `event` bus 的同一 OnEvent 调用栈。

#### frequency_class

| 端点 | per_block | per_l1_epoch | per_safe_head | per_finalized | per_reset |
|---|---|---|---|---|---|
| `engine_forkchoiceUpdatedV{2,3}` with attrs | 1 / block (build_start) | — | — | — | — |
| `engine_forkchoiceUpdatedV{2,3}` w/o attrs | 多次 / block (tryUpdateUnsafe + tryUpdatePendingSafe + tryUpdateLocalSafe + tryUpdateEngineInternal) | — | — | 触发 finalized 更新 | 1 / reset |
...
Mantle 端 onPayloadSuccess 里**四次 FCU 串行**（unsafe → pending_safe → local_safe → internal/finalize）
是 sequencer per-block 的额外 FCU 调用，远高于 OP 标准的单次 FCU。这是显式的 N×RPC 折点
（见 item-6 perf_impact_estimate）。
...
| 声明 | 数值 | 单位 | scenario | evidence_grade | additivity_class | tier |
|---|---|---|---|---|---|---|
| Mantle 每 block FCU 调用数（含 onPayloadSuccess 4 次 + onBuildStart 1 次） | 5 | calls/block | 静态代码（payload_success.go 中 4 个 tryUpdate*），non-reorg 场景 | measured (代码) | upper_bound_only | B+D |
| Base 每 block FCU 调用数 | 2-3 | calls/block | engine actor 内对应 EngineState 更新（attrs + 推进），具体次数视 reorg/sync | inferred (代码) | upper_bound_only | A+C |
...
- B：op-node 同步 RPC 直调模式；FCU 多次串行调用模式
- C：Base 把 `ProcessUnsafeL2BlockRequest` 与 `ProcessLocalUnsafeL2BlockRequest` 分变体（消除路径混淆）
- D：Mantle 在 onPayloadSuccess 中的 4 次 tryUpdate 行为；MetaTx / operator-fee 注入
...
| insert (NewPayload + FCU 串行) | 60-150 | 60-150 + 多次 FCU 累加 (Mantle onPayloadSuccess 串行 4 次 tryUpdate*) | ms/block | 同 | reported / inferred | A vs B+D |
...
|---|---|---|---|
| upper bound | ~150-330 ms | ~200-450 ms (event-loop 同步开销 + onPayloadSuccess 多次 FCU) | Base 7.5-16.5% / Mantle 10-22.5% |

⚠ **强约束**：上表数字是**单一场景 upper bound**，不可与 EL 执行耗时相加得到 "总 block
time"。EL 执行（reth / op-geth）耗时占大头（约 30-60%），共识层占比是 OP Stack 标准
共识开销的近似值。"Mantle 比 Base 多耗 50-120 ms/block" 这个差距具体由：

1. event-loop 串行 vs cross-task 并行带来的 derivation/sequencer 抢占（5-50 ms/block，仅
   非边界 tick）和 50-200 ms/block（L1 epoch 边界 tick，概率 ~1/6）；
2. onPayloadSuccess 多次 FCU 串行（多 2-3 次 FCU，每次 5-15 ms，~10-45 ms/block）；
...
|---|---|---|---|
| **onPayloadSuccess 4 次串行 FCU** | B+D | 10-45 ms | 是 |
| **derivation 与 sequencer 同 event loop** | B+D | 5-200 ms（视 L1 epoch 边界） | 是 |
| **EngineController 直接持有 RPC client 同步调用** | B | 与上面 1, 2 互锁，体现在主 event loop 阻塞 | 是 |
| sealingDuration=50ms 固定常量 | B+D | 0-30 ms（边界场景） | 是 |
...
- B：op-node event loop 串行 + EngineController 直调
- C：Base 动态 schedule、EngineRpcProcessor 隔离
- D：Mantle 4 次 FCU 串行 / MetaTx / Eigen DA / operator-fee 额外耗时
- E：n/a
...
| 减少 onPayloadSuccess 多 FCU 串行（若改为合并 RPC 或并行 fire-and-batch） | 10-30 | 同上 | inferred | non_additive | A+C → B+D |
...
| 改造项 | 预期 ms/block 收益 | scenario | evidence_grade | additivity | tier 来源 |
|---|---|---|---|---|---|
| lever-1 + lever-2 + lever-3 + lever-4 同时落地 | 累积场景 upper bound 30-300 ms/block | non-additive 合并报告 | inferred | non_additive | E → A |
...
|---|---|---|---|
| lever-1 | 15-45 | non_additive with 2/3 | 是 |
...
| lever-7 | 30-300 (合 lever-1+2+3+4) | non_additive | 是 |

⚠ **合并护栏**：`lever-1 + lever-2/3 + lever-4` 的合成 upper bound 不能简单相加；正确报告
方式是"在 L1 epoch boundary tick 上 lever-2/3 主导（最多 200 ms/block），在非边界 tick 上
lever-1 + lever-4 主导（最多 45 + 30 = 75 ms/block，但 lever-1 与 lever-4 部分重叠，单一
场景 upper bound 约 50 ms/block）"。
...
##### Rec-2：把 EngineController 改造为 actor + task queue（lever-1）

- **来源差距**：item-1 actor_or_module_map / item-3 Mantle 直接同步调用 / item-6 onPayloadSuccess
  4 次 FCU 串行
- **改造内容**：在 mantle op-node 内引入一个独立 goroutine（"engine worker"）持有
  ...
  await 本地 insert 完成（**not fire-and-forget on local sequencer insert**）。可选择把
  onPayloadSuccess 中 4 次 tryUpdate* FCU 合并 / 流水线化 / 并行（仅 unsafe / pending_safe /
  local_safe 之间可能可并行）。
- **预期收益**：15-45 ms/block (single scenario upper bound, evidence_grade = inferred,
  additivity = non_additive with Rec-3/4)
...
- **预期收益（5b）**：30-300 ms/block (non_additive 合并 upper bound)
...
| Rec-2 (lever-1 EngineController actor+queue) | 6-10 | 15-45 | medium-high | **优先 #4**（中长期） |
| Rec-5b (lever-7 kona-node 迁移) | 18-30 | 30-300 (复合) | high | **优先 #5**（战略，依赖 Rec-2/3 经验积累） |
...
sequenceDiagram
    ...
    EL-->>ENG: VALID
    ENG->>EL: engine_forkchoiceUpdatedV3 (no attrs) [direct_sync_call]<br/>tryUpdateUnsafe
    EL-->>ENG: VALID
    ENG->>EL: engine_forkchoiceUpdatedV3 (no attrs)<br/>tryUpdatePendingSafe
    EL-->>ENG: VALID
    ENG->>EL: engine_forkchoiceUpdatedV3 (no attrs)<br/>tryUpdateLocalSafe
    EL-->>ENG: VALID
    ENG->>EL: engine_forkchoiceUpdatedV3 (no attrs)<br/>tryUpdateEngineInternal
    EL-->>ENG: VALID
...
> onPayloadSuccess 含 **4 次串行 FCU**（unsafe / pending_safe / local_safe / engine_internal）
> 是显式开销点。
...
sequenceDiagram
    ...
    Note over SEQ,EL: Lane 3 — Mantle EngineController → EL（direct_sync_call_with_event_trigger）
    SEQ->>ENG_M: BuildStartEvent / BuildSealEvent / PayloadProcessEvent
    ENG_M->>EL: engine_forkchoiceUpdatedV3 (with attrs) [sync RPC]
    EL-->>ENG_M: PayloadId
    ENG_M->>EL: engine_getPayloadV3 [sync]
    EL-->>ENG_M: envelope
    ENG_M->>EL: engine_newPayloadV3 [sync, 10s timeout]
    EL-->>ENG_M: VALID
    loop 4x tryUpdate* (onPayloadSuccess)
    ENG_M->>EL: engine_forkchoiceUpdatedV3 (no attrs) [direct_sync_call]
    EL-->>ENG_M: VALID
    end
    Note over ENG_M: 全程在 driver eventLoop 同一调用栈，不释放主循环给其他事件
    end
```

> **强制 3 lane 表达**：(1) 本地 insert 用 **双向箭头** 明确 await；(2) 外部 unsafe path
> 用 **单向箭头**，无返回标识；(3) Mantle EngineController **同一 lane 内 sync_await + 4 次 FCU loop**。
...
以下缺口在 round-1 标注，留待 round-2 补充：

1. **op-rs/kona 上游 commit lag 量化（src-3）**：当前仅基于 mantle-xyz/kona 同构推断；
   round-2 应 checkout op-rs/kona main + 最新 release tag 直接对照 `crates/node/` 与 Base
...
2. **ethereum-optimism/optimism op-node Tier B baseline（src-3b）**：未直接 checkout 上游 OP
   monorepo；round-2 补充对照 onPayloadSuccess 4 次 FCU 是 OP 继承（Tier B）还是 Mantle 改动（Tier D），
   避免误归属。
3. **官方 Engine API specs（src-5/6）**：round-2 应拉取 specs.optimism.io 与 specs.base.org
...
5. **专家评论 / 设计讨论（src-7）**：round-2 应补充 op-rs/kona GitHub issues 与 op-node
   design docs 链接（如 EventDispatcher 重构 PR、kona-node release notes）。
6. **Mantle 主网 onPayloadSuccess 4 次 FCU 的实测延迟分布**：当前估算 10-45 ms/block 是
   reported + inferred；round-2 应通过 op-node 日志或 trace 工具采样实测分布。
7. **PoolActivation 与空块比例**：lever-6 的 effective TPS 收益依赖 Base/Mantle 主网空块比例
   数据，与 sibling 主题 block-builder-flashblocks-throughput 共享；round-2 协调单独引用一次。
8. **kona-node 在线 consensus 当前生产就绪度**：op-rs/kona kona-node binary 是否已在
   Base 主网 / 测试网在线运行；round-2 应基于 op-rs/kona release / Base 官方公告做出判断。
