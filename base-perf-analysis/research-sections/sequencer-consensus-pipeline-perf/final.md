...
sequenceDiagram
    ...
    EL-->>ENG_M: VALID
    Note over ENG_M: drained UnsafeUpdateEvent → tryUpdateEngine → needFCUCall=false (line 432-434),<br/>ErrNoFCUNeeded early-return → no extra FCU<br/>derived path: drained LocalSafeUpdateEvent → PromoteSafe → SetSafeHead (needFCUCall=true) → +1 FCU<br/>(conditional on the derived/concluding path; normal local sequencer path has no extra FCU)
