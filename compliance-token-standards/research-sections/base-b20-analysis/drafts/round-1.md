...
artifact_paths:
  ...
  draft: compliance-token-standards/research-sections/base-b20-analysis/drafts/round-1.md
draft_metadata:
  outline_round: 2
...
IB20 是所有 B20 变体继承的共享接口，通过 Rust `sol!` 宏定义。完整接口包含 **44 个函数**、**17 个事件**和 **19 个错误类型**。
...
| `PolicyUpdated(updater, scope, oldId, newId)` | — | 策略绑定更新 |
| `SupplyCapUpdated(updater, oldCap, newCap)` | — | 供给上限更新 |
| `NameUpdated(updater, newName)` | — | 名称更新 |
| `SymbolUpdated(updater, newSymbol)` | — | 符号更新 |
| `ContractURIUpdated()` | — | ERC-7572 URI 更新 |
| `EIP712DomainChanged()` | — | ERC-5267 域变更（名称更新触发） |

**错误类型**（19 个）：

包括 `AccessControlUnauthorizedAccount`、`Unauthorized`、`ContractPaused(PausableFeature)`、`InsufficientAllowance`、`InsufficientBalance`、`InvalidSender`、`InvalidReceiver`、`InvalidApprover`、`InvalidSpender`、`InvalidAmount`、`EmptyFeatureSet`、`InvalidSupplyCap`、`SupplyCapExceeded`、`PolicyForbids(scope, policyId)`、`PolicyNotFound`、`UnsupportedPolicyType`、`AccountNotBlocked`、`ExpiredSignature`、`InvalidSigner` 等。
...
步骤 5: 变体特定验证 → Asset: decimals ∈ [0, 18]; Stablecoin: currency A-Z
...
> 源码引用: `crates/common/precompiles/src/b20_factory/precompile.rs` — `create_b20()` 方法 @ [`8e87672`](https://github.com/base/base/tree/8e8767281d7c8768f6a0aed9124779cd4ed030ae)
