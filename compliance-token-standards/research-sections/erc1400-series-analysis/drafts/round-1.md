...
artifact_paths:
  ...
  draft: "compliance-token-standards/research-sections/erc1400-series-analysis/drafts/round-1.md"
...
adversarial_caveats_applied:
  - caveat: "ERC-1594 _data parameter flexibility"
    resolution: "Section 2 明确区分标准规范（允许 on-chain rules / off-chain certificate / hybrid）与实现模式（certificate 模式是 implementation pattern）。不再将 off-chain certificate 描述为 mandatory behavior。"
  - caveat: "ERC-20 fallback framing"
    resolution: "Section 3 将 fallback 风险定性为 implementation/integration hazard，引用 ConsenSys UniversalToken 的 _transferByDefaultPartitions 路由作为正确实现范例，区分标准设计风险与实现缺陷。"
  - caveat: "Source classification"
    resolution: "所有历史/采用 claims 标注证据类型：official-doc / github-confirmed / third-party-analysis / inferred。'40% of 31 requirements' 明确标注为 third-party-analysis (Oraclizer Research, 2026-03)。"
---
...
**当前状态**：ERC-1400 作为标准已被 ERC-3643 在生态位上取代。Polymath 已转向 Polymesh 专用链（official-doc），ConsenSys UniversalToken 仓库于 2025 年 3 月归档（github-confirmed），Securitize 采用自有 DS Protocol + 多链策略（official-doc）。但 ERC-1400 的设计遗产仍在延续——ERC-7518 (DyCIST) 继承了 partition 思想，Polymath 从应用层到协议层的转向与 B20/TIP-20 的设计选择形成呼应。
...
在 ConsenSys UniversalToken 实现中，ERC-20 `transfer` 调用 `_transferByDefaultPartitions`，后者调用 `_transferByPartition`，确保 ERC-20 路径也通过 partition 逻辑（github-confirmed：[ERC1400.sol](https://github.com/Consensys/UniversalToken/blob/master/contracts/ERC1400.sol)）。默认 partition 在构造函数中通过 `defaultPartitions` 参数配置。
...
### 3.1 风险定性：Implementation/Integration Hazard
...
**重要澄清（对应 adversarial caveat #2）**：这不是 ERC-1400 标准本身的"universal bypass"。ERC-1594 要求 `canTransfer` 覆盖空 `_data` 场景；标准的 ERC-20 兼容性设计意图是通过默认 partition 路由所有 ERC-20 调用。问题出在：(a) 实现是否正确 override 了 ERC-20 `transfer/transferFrom` 以通过 partition 和合规检查；(b) 集成方是否理解 ERC-1400 token 的合规约束。

### 3.2 正确实现范例：ConsenSys UniversalToken

ConsenSys UniversalToken 通过以下路径处理 ERC-20 `transfer` 调用（github-confirmed）：
...
ERC-20 transfer(to, amount)
  → ERC1400ERC20._transferByDefaultPartitions(msg.sender, msg.sender, to, amount, "")
    → ERC1410._transferByPartition(defaultPartition, operator, from, to, amount, "", "")
      → validator hook 检查（同 transferByPartition 路径）
```

这意味着在正确实现中，ERC-20 `transfer` 会：
1. 使用 `defaultPartitions`（部署时配置）
2. 通过同一个 validator hook 执行合规检查
3. 空 `_data` 仍触发 on-chain 规则检查（whitelist/blocklist/pause/lockup 等）

ConsenSys Diligence 审计也确认了此路由机制，但指出了一个相关风险：允许 token 持有者通过 `setDefaultPartitions` 自行设置默认 partition 可能引入意外行为，建议使用 token 级统一的默认 partition（third-party-analysis：[Codefi ERC1400 Assessment, ConsenSys Diligence, 2020-06](https://consensys.net/diligence/audits/2020/06/codefi-erc1400-assessment/)）。

### 3.3 风险场景（Implementation/Integration Level）

尽管正确实现可以避免 ERC-20 fallback 绕过，以下场景仍构成实际风险：

**场景一：不完整实现**

如果 ERC-1400 实现仅在 `transferByPartition` 中添加合规检查，但未 override ERC-20 `transfer/transferFrom` 以路由到 partition 逻辑，则 ERC-20 路径完全无保护。由于 ERC-1400 标准本身不强制特定 `transfer` 实现（各子标准独立提出），不同实现的安全质量参差不齐。
...
```mermaid
graph TB
    subgraph "正确实现（ConsenSys UniversalToken）"
        T_OK[ERC-20 transfer] --> DP[_transferByDefaultPartitions]
        DP --> TP[_transferByPartition]
        TP --> VH[Validator Hook<br/>合规检查]
        VH --> |"pass"| EX[Execute]
        VH --> |"fail"| RV[Revert]
    end
    ...
    subgraph "不完整实现（风险场景）"
        ...
        DIRECT --> EX2[Execute<br/>合规控制被绕过]
    end

    subgraph "DeFi 集成风险"
        DEFI[DeFi Protocol] --> |"调用 ERC-20 transferFrom"| T_DEFI[ERC-1400 Token]
        T_DEFI --> VH2[Validator Hook]
        VH2 --> |"合约地址不在 whitelist"| RV2[Revert<br/>不兼容而非绕过]
        VH2 --> |"需要 _data 但 DeFi 无法提供"| RV3[Revert<br/>certificate 模式不兼容]
    end

    style DIRECT fill:#e63946,color:#fff
    style EX2 fill:#e63946,color:#fff
    style RV2 fill:#f4a261,color:#000
    style RV3 fill:#f4a261,color:#000
```
...
**根因分析**：ERC-1400 的安全风险根源不在 ERC-20 兼容性本身，而在标准碎片化——四个独立子标准没有强制统一的合规检查入口。ERC-1594 的 `canTransfer` 应该是统一入口，但由于子标准独立性，具体如何将 ERC-20 调用路由到 `canTransfer` 由各实现自行决定。

> **证据分类**：security_risk | github-confirmed（ConsenSys UniversalToken）+ third-party-analysis（ConsenSys Diligence audit, Zealynx, QuillAudits）
...
| **Securitize** | Active（市场领导者） | 早期 ERC-1400 参与者之一，现使用自有 DS Protocol + ERC-20 + 多链策略。AUM $4B+；与 NYSE 合作代币化证券；SEC 注册 broker-dealer + transfer agent + ATS；EU DLT Pilot 授权 | official-doc：[Securitize Q1 2026 Results](https://www.prnewswire.com/news-releases/securitize-reports-first-quarter-2026-results-302777848.html)；[NYSE partnership, 2026-03](https://ir.theice.com/press/news-details/2026/New-York-Stock-Exchange-and-Securitize-Agree-to-Memorandum-of-Understanding-to-Support-Tokenized-Securities/default.aspx) |
| **ConsenSys UniversalToken** | Archived（2025-03-25） | 最完整的开源 ERC-1400 实现。仓库已归档为只读。关联产品 Codefi Assets 的当前状态未知 | github-confirmed：[GitHub repo archived](https://github.com/ConsenSys/UniversalToken) |
...
| **src-4** (security_analysis, >=2) | **covered** | [ConsenSys Diligence Codefi ERC1400 Assessment, 2020-06](https://consensys.net/diligence/audits/2020/06/codefi-erc1400-assessment/)、[QuillAudits ERC-1400 analysis](https://www.quillaudits.com/research/rwa-development/relevant-standards/erc-1400-token-standard)、[Zealynx ERC-1400 security analysis](https://www.zealynx.io/blogs/erc1400-compliance) |
...
| **Securitize 当前标准栈的具体技术细节** | low | Securitize 使用自有 DS Protocol，具体技术实现非公开。仅基于官方文档和 SEC 注册信息确认其已脱离 ERC-1400。标注 "official-doc"。 |
...
| **ConsenSys UniversalToken 的实际部署数量** | low | 仓库已归档，无公开的部署统计。Codefi Assets 平台的当前状态和使用 UniversalToken 的项目数量未知。 |
