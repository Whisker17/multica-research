...
artifact_paths:
  ...
  draft: "compliance-token-standards/research-sections/erc1400-series-analysis/drafts/round-2.md"
...
adversarial_caveats_applied:
  ...
  - caveat: "ERC-20 fallback framing"
    resolution: "Section 3 将 ConsenSys Codefi 定性为 default partition 路由模式的实现实例和审计证据（非'正确实现范例'）。ConsenSys Diligence 2020 审计发现 critical 级别漏洞：用户可通过 setDefaultPartitions 自选 partition 绕过 whitelist 限制。修复（PR #13, 2019-07-05）移除了用户自选能力。明确区分审计前/后行为，结论：ERC-20 fallback 是已验证的攻击面。"
  - caveat: "Source classification"
    resolution: "所有历史/采用 claims 标注证据类型：official-doc / github-confirmed / third-party-analysis / inferred。'40% of 31 requirements' 明确标注为 third-party-analysis (Oraclizer Research, 2026-03)。Securitize 协议栈描述（DS Protocol + ERC-20 + 多链策略）标注为 inferred（非 official-doc），AUM/NYSE/SEC 注册信息保持 official-doc。"

revision_feedback:
  - round: 2
    source: "Orchestrator Decision (comment 8329df2a)"
    severity: major
    changes_required:
      - "Section 3 rewrite: ConsenSys/Codefi framing — 从'正确实现'改为审计证据和 default partition 路由实例"
      - "Securitize evidence classification: DS Protocol 协议栈描述保持 inferred/third-party-analysis"
---
...
**审计建议的三种缓解方案**：
1. 确保 whitelist 中的接收方仅为实现了 partition 限制的合约（通过 ERC-777 receiving hook 检查源 partition 并拒绝不当转账）
2. 将 ERC-20 转出分离为独立的标准 ERC-20 token（ERC-1400 转出时执行 partition 限制，获得 ERC-20 token 后自由转账）
3. **不允许持有者自行设置默认 partition**，由 token 合约指定一个统一的、无限制的 partition 用于所有 ERC-20 转账

**修复状态**：ConsenSys 团队选择了方案 1 + 方案 3 的组合，在 [PR #13](https://github.com/Consensys/UniversalToken/pull/13)（2019-07-05 合并）中修复：移除了持有者自选默认 partition 的能力，改为 token 级统一默认 partition 配置；同时要求发送方和接收方均在 whitelist 中（github-confirmed）。修复后的 `_transferByDefaultPartitions` 使用 token 合约配置的统一 partition，持有者无法更改。
