...
artifact_paths:
  ...
  draft: "compliance-token-standards/research-sections/erc3643-trex-analysis/drafts/round-1.md"
...
- `forcedTransfer(from, to, amount)` — Agent 专用，绕过合规检查
...
**全地址冻结**：
- `setAddressFrozen(address, bool)` — 冻结/解冻整个地址。被冻结的地址不可执行任何 transfer（作为 sender），但仍可接收 token（如果 receiver 通过了 identity 验证）
...
`forcedTransfer(from, to, amount)` — Agent 可执行强制转账，无需双方同意。典型用途：
- 法院命令执行
- 监管机构要求的资产转移
- 反洗钱资产追回
- 公司行为（如股票分割、合并、强制回购）

Forced transfer **绕过标准 transfer 路径中的合规检查**——这是设计意图，因为行政操作本身由授权 Agent 执行，无需再次验证合规性。
...
`pause()` / `unpause()` — 全局暂停/恢复所有 token 操作（transfer、mint、burn）。用于紧急情况（如发现安全漏洞、重大市场事件）。暂停期间 Agent 仍可执行 `forcedTransfer` 和 `recovery`。
