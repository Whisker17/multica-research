...
1. **技术栈现状**：Mantle 当前运行 op-geth + reth 双执行客户端架构，已完成 Everest 硬分叉并规划了 Skadi（Prague 对齐）→ Limb（Osaka 对齐）→ Arsia（重大架构升级）的渐进路径。经代码验证，Mantle **当前不存在任何自定义 precompile**，仅包含标准 EVM 预编译合约与 BLS12-381 曲线操作。这意味着引入协议层合规能力需要全新的 precompile 开发与硬分叉部署流程。
...
2. **三条路线评估**：
   ...
   - **选项 B（Precompile 原生标准路线）**：以 Base B20（pinned commit `base/base@8e87672`）为主要 EVM L2 precompile 类比。B20 展示了 Rust trait 组合、4-slot PolicyRegistry、7-role RBAC 的协议层合规设计范式，但成熟度仅 10/30 且尚未进入生产。TIP-20 提供了协议层合规方案可以生产部署的经验验证，但其支付链定位与 Mantle RWA 场景存在显著差异。**适合中长期差异化战略，但需 12+ 月投入**。
   - **选项 C（混合路线）**：PolicyRegistry precompile + ERC-3643/ERC-20 adapter，兼具协议层 Gas 效率与应用层标准兼容。架构复杂度高，但提供渐进式过渡路径。**推荐作为中期探索方向**。
...
#### 1.2 reth Sequencer（Arsia/Hoodi）技术特性
...
- **MantleEverest**（已激活）：基线升级
- **MantleSkadi**：对齐 Ethereum Prague 升级
- **MantleLimb**：对齐 Ethereum Osaka 升级
- **MantleArsia**：重大架构升级（Gas 基础设施重构，引入 OperatorFeeVault）

主网激活时间戳（源自 `op-geth/params/mantle.go`）：Everest = 1742367600，Skadi = 1756278000，Limb = 1768374000，Arsia = 1776841200。
...
- 需要通过硬分叉流程部署（最早可能在 Arsia 之后的下一次硬分叉窗口）
...
**Arsia Gas 重构影响**：Arsia 硬分叉引入 GasPriceOracle 升级与 OperatorFeeVault（源自 `op-node/rollup/derive/arsia_upgrade_transactions.go`），Gas 定价机制的演进可能影响合规 token 交易的成本结构，需在实施阶段持续监测。
...
**Step 4 — 硬分叉部署**：
- 将新 precompile 纳入下一次硬分叉（最早 Arsia 之后）
...
| 硬分叉协调 | **高** | 治理流程、节点运营商升级协调、社区共识 |
| 生态工具链 | **高** | SDK、dashboard、explorer 集成均需从零开发 |
| **合计开发周期** | **12-18 月** | 从设计到主网部署（参考 TIP-20 经验：设计到生产 12+ 月） |
...
3. **深度集成 Mantle 特有功能**：合规逻辑可与 sequencer 排序、Gas 定价（Arsia OperatorFeeVault）、跨链桥等原生功能深度集成
...
5. **硬分叉依赖**：部署受限于 Mantle 硬分叉节奏（Arsia 之后的下一个窗口）
...
| 硬分叉协调 | **中** | 仅一个 precompile，影响范围比选项 B 小 |
| 测试验证 | **中** | precompile + adapter 的集成测试 |
| 生态工具 | **中** | adapter SDK，但可复用 ERC-3643 工具链 |
| **合计开发周期** | **6-9 月** | precompile 开发 3-4 月 + 集成测试 2-3 月 + 硬分叉部署 1-2 月 |
...
**Mantle 硬分叉时间线**（源自本地代码仓库验证）：

| 硬分叉 | 对齐 | 主网时间戳 | 状态 |
|---|---|---|---|
| Everest | — | 1742367600（2025 Q1） | 已激活 |
| Skadi | Prague | 1756278000（2025 Q3） | 即将激活 |
| Limb | Osaka | 1768374000（2026 Q1） | 规划中 |
| Arsia | 重大架构 | 1776841200（2026 Q2-Q3） | 规划中 |

**Precompile 引入的最早可行窗口**：Arsia 之后的下一次硬分叉（预计 2026 H2 — 2027 H1），因为：
- Skadi/Limb 主要是 Ethereum 对齐升级，引入自定义 precompile 不合适
- Arsia 是重大架构升级（Gas 重构），已有明确范围，追加合规 precompile 风险高
- 下一次硬分叉窗口是引入合规 precompile 的最合理时机

**对路线选择的影响**：
- 选项 A（ERC-3643）：不受硬分叉限制，可立即启动
- 选项 B/C（precompile）：最早 2026 H2 — 2027 H1 才能部署，存在 12-18 月的空窗期
- **策略推论**：短期必须以选项 A 覆盖合规需求，选项 B/C 作为中长期投资
...
| 身份层复用效率 | 中 | 高 | precompile 身份查询更高效 |
| DeFi composability 影响 | 低影响 | 需评估 | precompile 路线可能影响 DeFi 协议集成 |
| 实现优先级 | 可早期实现 | 需等待 precompile | 选项 A 可在 3 月内实现 mETH 合规 wrapper |

**关键判断**：mETH/cmETH 合规包装不是路线选择的决定性因素——两条路线均可实现。但**时间因素有利于选项 A**，因为早期的 mETH 合规包装可为 Mantle 机构化叙事提供实质支撑。
...
| Arsia 硬分叉实施 | 高 | Arsia 实施期间不宜追加合规 precompile |

**资源约束推论**：选项 A 的资源需求最低（智能合约团队即可执行），不影响核心基础设施开发。选项 B/C 需要执行客户端核心工程师参与，资源竞争显著。
...
| 决策因素 | 权重 | 选项 A 评分 | 选项 B 评分 | 选项 C 评分 |
|---|---|---|---|---|
| 发行方需求匹配度 | 25% | 9/10 | 4/10 | 7/10 |
| 上市时间（Time-to-Market） | 20% | 10/10 | 2/10 | 5/10 |
| 协议层差异化价值 | 15% | 2/10 | 9/10 | 6/10 |
| 标准化与生态接受度 | 15% | 10/10 | 2/10 | 6/10 |
| 资源效率 | 15% | 9/10 | 3/10 | 5/10 |
| mETH/cmETH 协同 | 10% | 7/10 | 8/10 | 7/10 |
| **加权总分** | **100%** | **8.15** | **4.15** | **5.95** |

**决策矩阵结论**：选项 A 在当前条件下综合评分最高（8.15/10），主要优势为发行方需求匹配、上市时间与标准化。选项 C 作为中期过渡（5.95/10）优于选项 B 的纯长期投资（4.15/10）。但此评估基于当前 Mantle 的优先级权重——若战略层面提升"协议层差异化"的权重，选项 B/C 的相对优势将增大。
...
3. **6-12 月决策（选项 B）**：基于 ERC-3643 使用数据、B20 Beryl 后生态反应、Mantle 资源状况做出原生标准最终决策
...
| Gas 基准测试 | M2 | ERC-3643 vs ERC-20 Gas 对比数据（含 L1 data gas） | 工程团队 |
...
| Hardfork 窗口评估 | M4 | precompile 部署的最早可行硬分叉窗口确认 | 治理团队 |
| mETH/cmETH 合规包装验证 | M5-M6 | 基于 ERC-3643 的 mETH 合规包装生产就绪评估 | DeFi 团队 |
| B20 Beryl 发布后竞争分析更新 | M4-M6 | 竞争态势更新报告（B20 实际能力 vs 预期） | 研究团队 |

**关键里程碑**：
- M4 end：PolicyRegistry precompile 设计完成，hardfork 窗口确认
- M5 end：混合架构 PoC 初步数据
- M6 end：混合路线 go/no-go 决策，ERC-3643 MVP 生产部署

**Go/No-go 决策门控**：
- 混合路线 PoC 展示显著 Gas 收益且架构复杂度可控 → 纳入下一次硬分叉
- 混合路线 Gas 收益有限或架构过于复杂 → 维持纯 ERC-3643 方案，选项 B 延后评估
...
| 主网部署路线图 | M10-M12 | 硬分叉包含合规 precompile 的主网部署计划 | 全团队 |

**最终路线情景**：
- **情景 1 — 纯 ERC-3643**：发行方需求满足、Gas 可控、差异化需求不强 → 维持选项 A
- **情景 2 — 混合路线**：PolicyRegistry precompile 验证成功、发行方需要更高 Gas 效率 → 选项 A + C
- **情景 3 — 完整原生标准**：战略决定全面差异化、资源充足、B20 竞争压力明确 → 选项 A + B（远期）
...
| 风险 | 影响 | 概率 | 缓释措施 |
|---|---|---|---|
| Hardfork 延迟 | 选项 B/C 部署时间推迟 | 中 | 选项 A 先行覆盖，precompile 作为增量 |
...
```
  ...
  (立即启动)       - Gas 数据可接受      正面             - 资源确认
                   - MVP 范围清晰     - Hardfork 窗口    - Hardfork 窗口
                                        可行               确认
...
gantt
    ...
    Mantle Arsia Hardfork             :crit, ext2, 2026-06, 2M
```
...
| Mantle 本地代码仓库 | 1, 4, 6 | 一手代码验证 | 硬分叉定义（Everest→Skadi→Limb→Arsia）、无自定义 precompile、EIP-1559 Gas、Arsia Gas 重构 |
...
| GAP-2 | mETH/cmETH 合规包装的具体技术方案未展开 | item-2 LST 协同分析深度 | 需单独 PoC 验证，超出本策略建议范围 |
| GAP-3 | Mantle 硬分叉后的时间窗口（Arsia 之后）未确认 | item-6 Hardfork 约束精确度 | 待 Mantle 治理团队确认下一次硬分叉规划 |
...
| GAP-6 | 发行方需求未经实际调研验证 | item-6 决策矩阵权重的合理性 | 短期行动 #4：发行方需求访谈前置 |
