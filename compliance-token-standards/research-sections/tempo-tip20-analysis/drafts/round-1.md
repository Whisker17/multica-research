...
artifact_paths:
  ...
  draft: "compliance-token-standards/research-sections/tempo-tip20-analysis/drafts/round-1.md"
...
4. End-of-block system transactions（StablecoinDEX 撮合、Fee AMM 结算）
...
**FeeManager Precompile**：地址 `0xfeec000000000000000000000000000000000000`。单例合约，实现所有 Fee AMM 池的功能 [docs-stated]。
...
graph TB
    ...
    subgraph "Block Execution Order"
        ...
        E3 --> E4["4. End-of-block System TX<br/>(DEX matching + Fee AMM settlement)"]
    end
    PL --> FEE
    FEE --> POOL
    POOL --> LP
...
sequenceDiagram
    ...
    User->>TX: Submit tx (fee token = USDC)
    TX->>FeeAMM: transferFeePreTx(USDC, fee_amount)
    Note over FeeAMM: Deduct USDC from user
    FeeAMM->>Pool: Swap USDC → USDT @ 0.9970
    Note over Pool: LP earns 0.3%
    Pool->>FeeAMM: Return USDT
    FeeAMM->>Validator: Accumulate USDT fees
    Note over TX: Execute user transaction
    TX->>FeeAMM: transferFeePostTx (refund if overpaid)
    Note over FeeAMM: Refund excess USDC to user<br/>(even if token paused)
    Validator->>FeeAMM: distributeFees() to claim
```
