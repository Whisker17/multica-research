...
# Research Outline: Confidential Compliance Token 路线横向对比
...
- **Route-capable candidates**（可独立承载 CCT 路线）：
  - Zama/ERC-7984 + OZ Confidential Contracts backend
  - ERC-3643 + confidential overlay（应用层合规 + confidential accounting backend 的组合路线）
  - B20-like native design（Mantle 原生 precompile/protocol 路线）
  - Inco Lightning / confidential token route
  - VOSA-RWA
  - Nightfall / EY enterprise

- **Component/supplement candidates**（不能独立承载路线，但可补强）：
  - Fhenix/CoFHE（backend-replaceable FHE）
  - Railgun/Privacy Pools（compliance-disclosure 补强）
  - Paladin/Privacy Groups（enterprise workflow 补强）

- **Benchmark/reference-only**：
  - Aztec（privacy-native L2 上限参照）
  - Starknet STRK20（非 EVM native token benchmark）
  - EIP-8182（protocol shielded-pool benchmark）
...
|----|------|-------------|-----------|
| src-1 | prior_final | WHI-266 requirements framework final: rubric definitions, capability model, evidence reuse map, Mantle lightweight constraints, Inco/Optalysys classification boundaries | 1 |
| src-2 | prior_final | WHI-267 Zama deep dive final: architecture, standards tension, lifecycle model, risk register, rubric scores, verdict | 1 |
| src-3 | prior_final | WHI-268 PSE constraints final: blocker taxonomy, account vs note model comparison, anti-pattern checklist, product/UX rubric supplement, disclosure matrix | 1 |
| src-4 | prior_final | WHI-269 compliance token extension final: compliance capability model, B20 skeleton, ERC-3643 baseline, phase boundary table, backend maturity gate, disclosure vectors | 1 |
| src-5 | prior_final | WHI-270 candidate survey final: candidate profiles, screening verdicts, rubric traceability matrix, Inco code PoC, gap register, bucket-decision rule | 1 |
| src-6 | prior_privacy_final | evm-privacy-research accepted finals for ERC-7984, confidential coprocessor, VOSA, shielded pool, Aztec, EEA benchmark, privacy EIPs; each claim needs path and commit SHA | 5 |
| src-7 | prior_compliance_final | compliance-token-standards accepted finals for ERC-3643, B20, Mantle strategy; reused only where WHI-269 has not already synthesized | 2 |
| src-8 | external_standard | ERC-7984, ERC-3643, EIP-8182 official specs for interface/standard boundary claims | 2 |
| src-9 | external_vendor | Zama docs, Inco docs/blogs, Fhenix docs, Optalysys pages — only for performance/roadmap/deployment claims not already covered by prior finals; all vendor claims must be labeled | as needed |
| src-10 | issue_record | Multica issue records for WHI-266 through WHI-270 dependency verification; WHI-271 dispatch and outline approval | 1 |
| src-11 | soft_input | WHI-262 (Mantle RWA 市场调研, status=done/path absent) and WHI-263 (Mantle CCT 竞品对标, status=in_progress) — M2/M3 soft inputs; NOT consumed; M1-only fallback applies per WHI-266 item-4.4 non-blocking rule | 0 |

## Patch Log

| Round | Action | Target | Reason | Source |
|-------|--------|--------|--------|--------|
| 2 | add | item-7: 主推-selection synthesis rule | (major) 主推-selection method was undefined; added dimension-priority ordering, pairwise-elimination verification, and anti-vendor-preference check to make verdict traceable to matrix | Review finding #1 |
| 2 | add | frontmatter: soft_inputs section | (major) WHI-262/263 soft-input handling was absent; added status/source tracking and M1-only fallback statement per acceptance criterion | Review finding #2 |
| 2 | add | item-3 rule 2: Inco engineering evidence | (minor) Inco confidential ERC20 framework not tied as Inco-route engineering evidence; added explicit call-out with commit SHA `bb39e4f7` | Review finding #3 |
