...
ERC-3643 is the strongest short-term compliance base because it is application-layer Solidity, EVM-portable, and already defines identity and issuer controls. Its six-part architecture is: Token, ONCHAINID, Identity Registry, Identity Registry Storage, Claim Topics Registry, Trusted Issuers Registry, and Modular Compliance. It gives Mantle a deployable phase 1 compliance baseline without waiting for a hardfork or custom precompile.
...
The outline review caveat is correct: phase 1 cannot simply say “encrypted balance is must-have” without naming the backend that makes it feasible. This draft uses the following production-readiness gate:
...
This draft separates prior research reuse from current local-code verification.

| Claim class | Status | Source anchor | Draft conclusion |
|---|---|---|---|
| B20 architecture skeleton | Reused and current-state spot-checked | `base-b20-analysis/final.md` @ `f42915e`; local `base/base` @ `01e732c`; local `base/docs` @ `9aace7f` | Reusable as capability skeleton. Current code/docs still expose Factory, PolicyRegistry, ActivationRegistry, Asset, Stablecoin. |
| B20 formal docs availability | New current-state update | `base/docs/docs/base-chain/specs/upgrades/beryl/b20.mdx` @ `9aace7f` | Prior “no formal docs checked” caveat should be updated in final: local docs now include a B20 spec page. |
| B20 confidential/private feature | New current-state spot check | `rg confidential/encrypted/FHE/ERC7984/ONCHAINID` over current B20 precompile modules | No current B20 private extension found in targeted scan; needs deeper review before final production claim. |
| B20Security/redeem | Code verification required | prior B20 final; current quick search signals in tests/storage but no separate precompile dir in module listing | Keep as local/evolutionary signal, not phase 1 dependency. |
| Mantle native precompile route | Reused and current-state spot-checked | `mantle-compliance-token-strategy/final.md` @ `f42915e`; local `mantle/revm` @ `bcf1a6a`; `op-geth` @ `3c1c571`; `reth` @ `a881fee` | Current targeted search did not find B20/compliance/confidential-token native precompile. `revm` shows OP/EVM cryptographic precompile plumbing and fork labels, not a CCT precompile path. |
...
| Reuse class | Meaning in this draft |
|---|---|
...
| code_verification_required | Draft does not have enough code evidence to promote claim as fact. |
