...
| Phase 1b | 3-6 months | Pilot readiness assessment, not production launch by default. | Security review scope; operator/KMS runbook; p50/p95/p99/cost measurements; wallet/indexer UX acceptance; incident drill; compliance memo; numeric SLA/cost thresholds for pilot decision. | Continue only if backend governance, disclosure evidence, latency/cost thresholds, UX and security scope are acceptable. If thresholds are not set or not met, remain PoC-only. | `app_integration` + `sidecar_operator_dependency` |
...
The PoC should record metrics before setting production SLA. Numeric thresholds should be chosen after baseline measurement; the go/no-go gate is whether the measured path is usable for the intended pilot workflow and whether failures are observable and recoverable. This is a sequencing rule, not a waiver: **before any Phase 1b pilot decision, the team must set concrete numeric p50/p95/p99, cost, indexing-lag and recovery-time thresholds and compare the measured PoC data against them**. If thresholds are absent, the only valid decision is PoC-only / defer pilot.
...
| 3-6 months | Phase 1b: pilot readiness | Determine whether PoC can become a limited pilot. | p50/p95/p99/cost metrics, KMS/operator runbook, incident drill, compliance memo, security review scope, numeric SLA/cost thresholds. | Project lead + operator/security/compliance. | Pilot only if governance, latency/disclosure/security/UX gates pass and numeric thresholds are set and met. Otherwise narrow or stop. |
...
| C-20 | Phase 1b | Produce p50/p95/p99 and cost measurements for all PoC operations, then set concrete numeric SLA/cost thresholds before pilot decision. | Engineering + ops | Metrics report plus threshold table in decision memo. | not_started | No measured data, only vendor claims, or no numeric threshold before pilot decision. |
...
5. **Performance evidence and thresholds must be generated**. Vendor docs and Optalysys material are useful inputs, but p50/p95/p99, encrypted operation cost, burst and failure recovery must be measured on the actual PoC path. Concrete numeric SLA/cost thresholds must then be set before any pilot decision; otherwise the result remains PoC-only.
...
| final | 2026-06-24 | Final promotion | Promoted approved draft `299931f6ba4bcdd53aafebad7616e2b93849e3c8` to final after Review Verdict approve (`fdeb3b3b-5e7f-4f82-9059-701de8dbbbdd`). Addressed minor caveats by making numeric SLA/cost thresholds mandatory before any pilot decision and reconciling the outline frontmatter to `status: approved`. |
