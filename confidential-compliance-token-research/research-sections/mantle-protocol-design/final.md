...
|---|---|---|---|---|---|
| `confidentialBalanceOf(address account)` | returns encrypted handle or viewer-targeted encrypted payload | ERC-7984-aligned | phase_1_must_have | Plaintext authorization check may revert; balance value is never compared in public control flow and unauthorized viewer gets no decrypt path | ERC-7984 defines confidential balance as pointer / handle; OZ returns `euint64` |
| `confidentialTransfer(address to, bytes32 encryptedAmount, bytes proof)` | sender transfers encrypted amount | ERC-7984-aligned | phase_1_must_have | Plaintext checks (receiver identity, blocklist, registry lookup, proof format, backend availability) may revert; encrypted balance / amount-limit predicates must use `FHE.select(ok, amount, 0)`, zero-transfer, encrypted failure flag, or explicit selective decrypt. Reverting on encrypted predicates leaks the comparison result | Amount is encrypted; receiver address remains visible |
| `confidentialTransferFrom(address from, address to, bytes32 encryptedAmount, bytes proof)` | operator / custody transfer | ERC-7984-aligned + Mantle-specific policy | phase_1_must_have | Plaintext checks (operator right, executor scope, sender / receiver eligibility) may revert; encrypted insufficient-balance, operator-limit or amount-policy predicates must use select / zero-transfer or selective decrypt, not predicate-dependent revert | ERC-7984 operator is time-bounded but may move any amount while active; requires UX warning |
| `mint(address to, bytes32 encryptedAmount, bytes proof)` | issuer mints encrypted amount | ERC-3643-aligned issuer + ERC-7984 value | phase_1_must_have | Plaintext checks (issuer role, receiver eligibility, mint policy ID) may revert; encrypted cap / threshold / supply predicates must use select / zero-mint, encrypted denial state, or selective decrypt | Receiver KYC / mint-receiver policy is plaintext |
| `burn(address from, bytes32 encryptedAmount, bytes proof)` | issuer or holder burns encrypted amount | ERC-3643-aligned + ERC-7984 value | phase_1_must_have | Plaintext checks (burn role, account authorization, redeem request existence) may revert; encrypted balance / burn-limit predicates must use select / zero-burn or selective decrypt | May feed redeem / unshield |
...
| `unshield(address from, bytes32 encryptedAmount, address recipient)` | unwrap / redeem to public asset, cash leg or bridge | OZ Wrapper-aligned + Mantle-specific | phase_1_optional, required if redeem exists | Plaintext checks (redeem authorization, recipient, settlement rail) may revert; encrypted available-balance predicates should be resolved by decrypt-to-settlement actor or select / zero-finalize before releasing clear funds | Clear amount and destination become settlement evidence |
| `freeze(address account, bytes32 encryptedAmount, FreezeMode mode)` | full or partial confidential freeze | ERC-3643-aligned + OZ Freezable / RWA | phase_1_must_define | Plaintext checks (freezer role, legal trigger, account target) may revert; encrypted available-balance / partial-freeze amount predicates must use select or full-freeze fallback rather than leaking predicate through revert | Partial freeze needs encrypted available balance logic |
...
| `updatePolicy(bytes32 policyId, PolicyConfig config)` | bind or upgrade scoped policy | B20-inspired + ERC-3643-aligned | phase_1_must_have | Plaintext checks (policy admin, timelock, registry presence, version) may revert; encrypted-rule unsupported means the rule class is disabled / unavailable, not evaluated by leaky encrypted-condition revert | B20-inspired means policy vocabulary only, not B20 confidentiality |

Failure-semantics rule: **revert / fail-closed is safe for plaintext facts that are already public or intentionally visible**: identity registry presence, blocklist, operator-right, issuer-agent authorization, role, registry lookup and malformed proof. **Revert is unsafe for encrypted amount / balance predicates** such as insufficient balance, amount limit, holder cap, threshold or confidential frozen balance. Those checks must use FHE-native `select` / zero-transfer, encrypted failure state, explicit selective decrypt to an authorized actor, or exclude the rule from phase 1.
...
| Mint / wrap / shield | issuer or holder, wrapper | receiver, underlying deposit amount if shield, mint event metadata | encrypted minted amount and new balance | mint role, receiver policy | shield amount visible if public ERC-20 enters wrapper | plaintext receiver / role / proof checks may fail closed; encrypted cap predicates use select / zero-mint or selective decrypt |
| Confidential transfer | sender / operator, receiver, token, policy, backend | from, to, operator, policy IDs, event existence | encrypted amount, balances, frozen balances | identity / blocklist plus FHE-native or decrypt amount policy | none unless policy requests disclosure | plaintext checks may fail closed; encrypted amount / balance checks use zero-transfer/select or selective decrypt |
...
| Redeem / unshield | holder, issuer, custodian, bridge / redeem agent | recipient, settlement rail, legal redemption record | burn / unwrap encrypted amount until finalization | holder eligibility and issuer liquidity | clear amount and destination disclosed to settlement leg | plaintext settlement checks may fail closed; encrypted availability resolves through decrypt-to-settlement or select / zero-finalize |
...
| encrypted compare / select | required for amount policy; optional for identity-only PoC | selective decrypt or fail closed at rule-configuration time; do not revert on encrypted predicate result |
...
| amount_policy_gap | compliance/protocol | blocking | ERC-3643 `canTransfer(from,to,amount)` assumes plaintext amount; CCT amount is encrypted. Reverting on encrypted amount / balance predicates such as insufficient balance, threshold or amount-limit failure leaks the comparison result and reintroduces an amount side channel | Implement FHE-native policy with `FHE.select(ok, amount, 0)` / zero-transfer or encrypted denial state; use selective decrypt only when explicitly authorized; otherwise mark amount rules unsupported and fail closed at rule-configuration time, not by predicate-dependent encrypted revert | protocol + compliance engineering |
...
2. At least one amount-dependent policy is implemented and tested with encrypted values using the two-path failure rule: plaintext checks may revert; encrypted amount / balance predicates must use FHE-native select / zero-transfer, encrypted denial state, or explicit selective decrypt. Predicate-dependent encrypted reverts are blocked unless deliberately accepted as disclosure.
...
2. **Amount-dependent compliance is the hardest technical spike**. Plain ERC-3643 modules are not compatible with encrypted amounts as-is. The project must choose FHE-native policy, selective decrypt, or rule-configuration fail-closed exclusion. A predicate-dependent revert on encrypted amount / balance is not an acceptable default because it leaks the comparison result.
...
| final | final_promotion | Promoted approved draft `fb0fa996b2fc4a3ad4762b16b733d895be3fc243` to `final.md` after Review Verdict approve (`919e9d20-7ba4-4aea-9f58-f9844ba7bb15`). Applied required final refinement: plaintext checks may revert / fail closed, while encrypted amount / balance predicates must use select / zero-transfer, encrypted denial state or explicit selective decrypt because reverting leaks the predicate. |
