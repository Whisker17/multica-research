...
| 能力（Capability） | 必要性 | Token core / 外部依赖 | 最低验收口径 | 主要复用证据 |
|---|---|---|---|---|
...
| business_state_privacy | optional/route-specific | Coprocessor / TEE / privacy group | 若目标超过 token ledger，需要隐藏合约逻辑和业务状态；否则标为超出范围（out of scope）。 | privacy framework; EEA benchmark; coprocessor final |
...
| 来源（Source） | Commit pin | 复用类别（Reuse class） | 可直接复用的结论 | 边界 |
|---|---|---|---|---|
...
可追溯性说明：confidential-coprocessor 这一行合理地指向 `0041e3a...`，因为这正是该文件自身的 last-modifying commit。其他此前共享该快照 pin 的复用文件已上移到上表中各自的文件级 pin。

#### 4.2 工程 PoC 参考（Engineering PoC Reference）

| 来源（Source） | Pin / 访问方式 | 分类（Classification） | 用途（Use） | 边界（Boundary） |
|---|---|---|---|---|
| `Inco-fhevm/confidential-erc20-framework` | GitHub HEAD `bb39e4f788742121f2fc93de33af58758360545b`，已于 2026-06-24 核实 | engineering_poc | wrapper、confidential ERC20、CompliantConfidentialERC20、Identity、ExampleTransferRules 的工程结构参考。 | README 明确“not audited”且仅 proof of concept；不得作为生产安全或标准成熟度证据。 |
...
| 来源（Source） | 访问方式（Access） | 分类（Classification） | 用途（Use） | 边界（Boundary） |
|---|---|---|---|---|
| Optalysys `Tokenised RWAs are a trillion-dollar opportunity - but only if privacy scales` | 官方 WP 搜索结果 id 3165，URL `https://optalysys.com/resource/tokenised-rwas-are-a-trillion-dollar-opportunity-but-only-if-privacy-scales/`，访问于 2026-06-24 | performance_reference | RWA privacy scaling narrative、FHE 性能瓶颈与 institutional adoption 背景。 | 厂商文章；不能当独立 benchmark。 |
| Optalysys `Fully Homomorphic Encryption hits the data movement wall - why photonics is emerging as the way forward` | 官方 WP 搜索结果 id 3368，访问于 2026-06-24 | performance_reference | FHE data movement wall、硬件/photonic acceleration 生产化背景。 | 厂商文章；只影响 performance/SLA gap。 |
| Optalysys `Silicon photonics: our approach to acceleration` | 官方 WP 搜索结果 id 2868，访问于 2026-06-24 | performance_reference | Photonic acceleration 路线参考。 | 不提供 CCT token model。 |
| Optalysys `Confidential RWA Tokenisation` | 官方 WP 搜索结果 id 2750，访问于 2026-06-24 | performance_reference | Confidential RWA tokenisation 用例包装和生产化叙事。 | 不是标准、不是 Mantle 集成方案、不是第三方审计。 |
...
| 条目（Item） | 2026-06-24 观察到的状态 | 软输入处理方式 |
|---|---|---|
| `WHI-262` `[M2-对比] EVM 隐私方案横向对比分析` | Multica issue 列表显示状态为 `done`，但 `evm-privacy-research/research-sections/cross-comparison/final.md` 在本 branch 快照中不存在。 | 本 draft 中不作为 hard evidence 引用。只有当本项目所用 branch 中出现直接路径与 commit pin 后，它才能升级为 hard reuse。 |
| `WHI-263` `[M2-策略] Mantle 轻量级机构隐私方案策略建议` | `in_progress`；本 branch 中无 final 制品。 | 仅作软输入；非阻塞。在升级为 final 并打上 commit pin 之前，未来的 route recommendation 不得覆盖本 CCT 需求框架。 |
| `WHI-264` `[M3-报告] EVM 隐私方案最终调研报告整合` | `todo`；无 final 制品。 | 仅作软输入；非阻塞。它是下游综合产物，而非上游证据。 |
| WHI-254..261 之外的、未完成的 evm-privacy-research M2/M3 材料 | 具体未来 ID 可能变化。 | 按类别视为 `soft_input`：cross-comparison drafts、strategy drafts、final-report drafts、route recommendations、评论、issue plans，以及尚未 promote 的材料。 |

**非阻塞规则（Non-blocking rule）**：soft inputs 可以指导 framing 并识别未来问题，但不能用于裁定某条 CCT 需求、为某个候选打分，或主张 Mantle route preference。只有 commit-pinned final artifacts、primary standards/docs、audited code，或经明确分类的 PoC/performance 参考，才能支撑 hard claims。

### item-5: 候选分层与参考分类（Candidate Layering and Reference Classification）

#### 5.1 CCT 分层栈（CCT Layer Stack）

| 层（Layer） | 角色（Role） | 示例参考（Example references） | CCT 解读（CCT interpretation） |
|---|---|---|---|
| 资产/合规基座 | Identity、transfer policy、issuer controls、法律生命周期 | ERC-3643、B20 PolicyRegistry、TIP-20、ERC-1400 legacy | 合规底座；其本身不隐藏账务数据。 |
| Confidential token 接口 | Encrypted amount/balance API、operator/callback、wrapper | ERC-7984、ERC-7945 | 接口锚点；仍需合规与隐私后端。 |
| 隐私后端 | FHE/ZK/TEE/MPC/GC 计算与披露 | Zama fhEVM、Inco Lightning、Fhenix CoFHE、COTI Coprocessor | 提供 encrypted state 与 decrypt/re-encrypt 流程；信任/性能各异。 |
| 合规/披露模块 | Observer、policy hooks、RWA agent、identity check、audit logs | OZ ERC7984ObserverAccess/Rwa/Hooked/IdentityCheck；Inco transfer rules | 把 privacy token 转化为具备合规能力的 CCT。 |
| 桥/赎回/资产 adapter | Wrap/unwrap、L1/L2 bridge、redemption、reserve accounting | ERC7984ERC20Wrapper、Inco wrapper、B20 mint/burn/redeem 类比 | RWA 生产与 DeFi/custody 集成所必需。 |
| DeFi/custody 集成 | Wallet/indexer/risk engine/DEX/lending 支持 | 后续 section | 不作默认假设；encrypted amount 会打破许多 ERC-20 假设。 |

#### 5.2 Inco 分类（Inco Classification）

Inco 在本项目中以两种不同角色出现：

1. **Inco Network / Lightning 作为隐私后端候选**：依据 `confidential-coprocessor/final.md`，Lightning 当前以 TEE 优先、Base mainnet 现有支持，Atlas FHE 属 roadmap。这是用于 backend route 评估的 candidate/reference 证据。
2. **`Inco-fhevm/confidential-erc20-framework` 作为工程 PoC 参考**：依据 GitHub repo `bb39e4f...`，它是 unaudited PoC 代码，不得抬高为生产标准。

对本 CCT 框架而言，该 GitHub framework 仅被归类为 **engineering_poc**。它可以启发：

- 如何切分 wrapper 与 confidential token，
- 如何把转账经由外部 transfer-rules 合约路由，
...
在当前证据中，Optalysys 不是 token standard、privacy protocol、compliance token framework，也不是 Mantle integration path。它属于 **FHE 性能 / 生产化参考**。

可将 Optalysys 用于：

- FHE throughput/latency 瓶颈 framing，
- photonic acceleration 与硬件 roadmap 问题，
- 作为生产就绪叙事的 RWA privacy scaling，
- 后续 `fhe-performance-and-production-sla` section 应深入研究的 SLA 问题。
...
从 compliance-token 研究继承的 Mantle 约束：

- Mantle 基于 OP Stack，采用 op-geth + reth 双执行客户端。
- 当前证据表明 Mantle 无自定义 precompile；新增 precompile 需要执行客户端工作和硬分叉部署。
- ERC-3643 风格的应用合约无需协议变更即可部署。
- B20 风格的 precompile 在 gas/确定性上有较强优势，但属于长周期路线。

对 CCT 而言，由此得到一个实用的决策树：

| 路径（Path） | 描述（Description） | 轻量性结果（Lightweight result） | 工程改动面（Engineering delta） | 何时研究（When to study） |
|---|---|---|---|---|
| A：应用层合规 + confidential wrapper | ERC-3643/issuer policy 合约 + ERC-7984/OZ 风格的 confidential token 或 wrapper。 | 若隐私后端为 bolt-on，则很可能是轻量的。 | 合约、SDK、observer/audit 服务、indexer/wallet 改动。 | 第一条 PoC 路径。 |
| B：带外部 coprocessor 的 confidential token | 类 ERC-7984 token 使用 Zama/Inco/Fhenix/COTI 风格后端。 | 若无新链/桥/全节点/硬分叉，则为轻量。 | 后端服务依赖、KMS/TEE/coprocessor 信任、延迟、ACL logging。 | 主隐私后端研究。 |
| C：B20 风格 private feature / PolicyRegistry precompile | 带隐私特性的协议级 token 或 policy 执行。 | 对即时的 Mantle 而言非轻量；因硬分叉属中量/重量。 | op-geth/reth 改动、fraud-proof、治理、审计。 | Mantle 长期差异化，非 M1 依赖。 |
| D：独立隐私链 / validium / private L2 | 使用 Aztec/Prividium/Linea/Silent Data/COTI-L2 风格环境。 | 对 Mantle private feature 而言不满足轻量。 | 新链/桥/流动性、operator stack。 | 仅作参考，除非 Mantle 策略变更。 |
| E：Shielded pool / VOSA | 带匿名集或 exposed-graph 模型的应用层隐私。 | 可能轻量，但 compliance/issuer controls 各异。 | Proof circuits、pool accounting、source-of-funds proofs、wallet UX。 | 作为 ERC-7984 CCT 的替代方案进行对比。 |

**任意 CCT 候选的工程改动面 checklist**：

| 面（Surface） | 问题（Questions） |
|---|---|
| Token 合约 | 它暴露 ERC-20 兼容、ERC-7984 兼容，还是全新接口？在 wallet/indexer 中哪些会失效？ |
| 身份与策略 | KYC 身份是 onchain、offchain、claim-based、encrypted，还是基于 hash/attestation？ |
| ACL/披露 | 谁授予访问、谁撤销、历史访问如何处理，以及哪些日志可证明它？ |
| 桥/赎回 | confidential amount 在 redemption 时如何变为公开或发行方可见？若 decrypt backend 不可用怎么办？ |
| DeFi | DEX/lending/risk engines 能比较 encrypted balances 吗？这些 adapters 是否可信？ |
| 运维 | 谁运行 KMS/TEE/coprocessor/notary；SLA 是什么；停机期间会发生什么？ |
| 法律/合规 | 该模型是否满足 issuer books、auditor evidence、regulator request、Travel Rule、AML 与 GDPR caveats？ |
...
| 路径（Path） | 状态（Status） | 备注（Note） |
|---|---|---|
| `confidential-compliance-token-research/outlines/requirements-framework.md` | 存在 | 已批准的 outline 制品。 |
| `confidential-compliance-token-research/research-sections/requirements-framework/drafts/` | 存在 | 本 draft 在此持久化 `round-1.md`。 |
| `confidential-compliance-token-research/research-sections/requirements-framework/final.md` | planned | 仅在 draft 获批 promote 后创建。 |
| `confidential-compliance-token-research/report/assets/` | 存在 | final report 资产的占位。 |
| `confidential-compliance-token-research/research-sections/_index.md` | 仅作提案 | Research Agent 边界：不写入；由 Orchestrator/TW 整合。 |
...
**索引整合说明（Index integration note）**：在 Orchestrator 创建子 issue、随后 Technical Writer / Orchestrator 更新项目索引之前，状态保持 `planned`。本 draft 仅是提案。

### item-8: 缺口登记与所需新研究（Gap Register and New Research Required）

| 缺口（Gap） | 为何重要（Why it matters） | 当前处理（Treatment now） | 后续 section（Follow-up section） |
|---|---|---|---|
| 具体的未来 M2/M3 issue 集合可能变化 | 下游 route/comparison 工作可能被误当作 hard evidence。 | 已实例化 soft-input 规则；WHI-263/WHI-264 明确非阻塞；WHI-262 在本 branch 中不作为 hard evidence 使用。 | 所有下游 section |
| `evm-privacy-research/issue-plan.md` 缺失 | Dispatch 引用了它，但本地无该文件。 | 改用 direct commit-pinned final artifacts 与 Multica issue 记录。 | 如有需要，Orchestrator 可在后续补充 issue-plan |
| ERC-3643 + ERC-7984 组合尚未设计 | CCT 很可能同时需要合规身份与 confidential accounting。 | 需要新研究。 | `erc3643-erc7984-composition` |
| Base B20 private feature 类比并非 confidential token 设计 | B20 提供 precompile/policy 架构，而非隐私后端。 | 仅作架构类比使用。 | `base-b20-private-feature-analogy` |
...
| Inco PoC 未审计 | 读者可能过度看重工程代码。 | 仅作 Engineering PoC。 | `inco-confidential-erc20-poc-fit` |
| Optalysys 证据是厂商性能叙事 | 生产 SLA 不能依赖营销主张。 | 仅作性能参考。 | `fhe-performance-and-production-sla` |
| 桥/赎回与 force-exit 语义不清 | RWA token 需要法律层面的赎回与失败恢复。 | 必须单列为独立 section。 | `bridge-redeem-and-defi-composability` |
| 加密金额下的 DeFi 可组合性未解决 | ERC-20 集成假设余额/金额公开。 | 标记为 should-have 但尚未解决。 | `bridge-redeem-and-defi-composability` |
| Mantle 最新硬分叉/precompile 状态可能变化 | 协议路线可行性具有时效性。 | M1 使用 pinned 的既有研究；若 Mantle repo 变更，在 final/report 中刷新。 | `mantle-route-comparison-and-recommendation` |
...
### diag-3: 角色与可见性流（Actor and Visibility Flow）
...
### diag-4: 拟议 9-section 依赖图（Proposed 9-Section Dependency Graph）
...
| 来源需求（Source requirement） | 覆盖（Coverage） | 备注（Notes） |
|---|---|---|
| src-1 既有合规研究 | 已覆盖 | Compliance final report、Base B20 final、Mantle strategy final 均为 direct commit-pinned。 |
| src-2 既有隐私研究 | 已覆盖 | Privacy framework、ERC-7984、coprocessor、EEA benchmark、VOSA/EIP/shielded/Aztec finals 为 direct 或 bounded commit-pinned。 |
| src-3 Inco PoC 源 | 已覆盖 | GitHub HEAD 已核实为 `bb39e4f...`；仅分类为 engineering_poc。 |
| src-4 Optalysys 外部源 | 已覆盖 | 官方 Optalysys WP 搜索结果已于 2026-06-24 核实；仅分类为 performance_reference。 |
| src-5 项目骨架 / 索引 | 已覆盖 | 已记录骨架状态；`_index.md` 提案为嵌入，未写入。 |
| src-6 Multica issue 记录 | 已覆盖 | 抽样 WHI-254 与 WHI-261 作为 done M1 记录；用项目 issue 列表实例化 WHI-263/WHI-264 soft input。 |

### 证据置信度汇总（Evidence Confidence Summary）

| 置信度（Confidence） | 证据类型（Evidence type） | 示例（Examples） |
|---|---|---|
| 高 | Commit-pinned final 研究制品与标记为 done 的 issue 记录 | Compliance final report；privacy framework；ERC-7984 final；B20 final |
| 中 | 仅用于 bounded analogy 的 commit-pinned final 制品 | VOSA、shielded pool、Aztec、EEA benchmark |
| 中偏低 | 带明确 unaudited 警告的工程代码 PoC | Inco confidential ERC20 framework |
| 对 hard claims 低 / 对提出问题有用 | 厂商性能与生产化材料 | Optalysys 页面；FHE 性能主张 |
| 非阻塞 | in_progress/todo 的下游 M2/M3 材料 | WHI-263、WHI-264、未来 M2/M3 drafts |

## 缺口分析（Gap Analysis）

本 draft 是一个需求框架，而非最终 route recommendation。风险最高的未决问题是：

1. **组合缺口（Composition gap）**：尚无现有来源证明存在一个干净的 ERC-3643 + ERC-7984 生产组合，能同时具备 identity、confidential accounting、issuer controls 与 revocable selective disclosure。这需要新的 primary research。
2. **披露撤销缺口（Disclosure revocation gap）**：FHE ACL 模型在结构上似乎难以撤销。任何 CCT 评分都必须把 future authorization revocation 与 historical handle access revocation 分开。
3. **审计日志缺口（Audit log gap）**：许多方案暴露 events，但鲜有方案证明 disclosure grants、re-encryption、regulator requests 与 offchain audit evidence 是防篡改且可查询的。
4. **性能/SLA 缺口（Performance/SLA gap）**：在若干来源中，FHE throughput 与 latency 仍为厂商主张。Optalysys 有助于 framing 生产问题，但并未将其闭合。
5. **Mantle 集成缺口（Mantle integration gap）**：coprocessor 的“no hardfork”主张仍需核查 Mantle 上的 host contracts、finality/callback timing、sequencer behavior、data availability、KMS availability 与运维归属。
6. **桥/赎回缺口（Bridge/redeem gap）**：RWA 生产需要现金/基金份额赎回与 reserve accounting，而不仅是 confidential transfer。redemption 时的披露必须被设计。
7. **DeFi 缺口（DeFi gap）**：encrypted balances 打破了普通 indexer、DEX、lending、liquidation 与 oracle 的假设。adapters 可能重新引入可信第三方或公开泄露。
8. **法律/产品缺口（Legal/product gap）**：GDPR、MiCA、Travel Rule、AML/CFT、broker-dealer books、fund accounting 与 regulated custody 都需要在本 M1 技术框架之外做特定司法辖区的解读。

## 修订日志（Revision Log）

| 轮次（Round） | 日期（Date） | 变更（Change） |
|---:|---|---|
| 1 | 2026-06-24 | 基于已批准 outline 的首版 deep draft。新增 CCT 定义、能力模型、统一 rubric、证据复用地图、soft-input 实例化、Inco/Optalysys 分类、Mantle 轻量约束、索引提案、图示、来源覆盖与缺口登记。落实 outline review carry-forward 项：显式 WHI-263/WHI-264 soft input、per-file commit pins，以及以直接引用替代依赖 stale index。 |
