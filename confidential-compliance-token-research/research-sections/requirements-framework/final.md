...
|---|---|---|---|---|
| Optalysys `Tokenised RWAs are a trillion-dollar opportunity - but only if privacy scales` | URL `https://optalysys.com/resource/tokenised-rwas-are-a-trillion-dollar-opportunity-but-only-if-privacy-scales/`, accessed 2026-06-24 | performance_reference | RWA privacy scaling narrative、FHE 性能瓶颈与 institutional adoption 背景。 | Vendor article；不能当独立 benchmark。 |
| Optalysys `Fully Homomorphic Encryption hits the data movement wall - why photonics is emerging as the way forward` | URL `https://optalysys.com/resource/fully-homomorphic-encryption-hits-the-data-movement-wall-why-photonics-is-emerging-as-the-way-forward/`, accessed 2026-06-24 | performance_reference | FHE data movement wall、硬件/photonic acceleration 生产化背景。 | Vendor article；只影响 performance/SLA gap。 |
| Optalysys `Silicon photonics: our approach to acceleration` | URL `https://optalysys.com/resource/silicon-photonics-our-approach-to-acceleration/`, accessed 2026-06-24 | performance_reference | Photonic acceleration 路线参考。 | 不提供 CCT token model。 |
| Optalysys `Confidential RWA Tokenisation` | URL `https://optalysys.com/confidential-rwa-tokenisation-blockchain-use-case/`, accessed 2026-06-24 | performance_reference | Confidential RWA tokenisation 用例包装和生产化叙事。 | 不是标准、不是 Mantle 集成方案、不是第三方审计。 |
...
| final | 2026-06-24 | Promoted approved round-1 draft to `final.md` after draft-review approval. Applied required minor fix: item-4.3 Optalysys external citations now use stable URLs for data-movement-wall, silicon-photonics, and Confidential RWA Tokenisation entries. |
