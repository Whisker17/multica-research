...
The approved outline recorded WHI-262 as done/path absent and WHI-263 as in progress. This draft notes the current checkout now contains both finals, but preserves the outline's non-blocking rule: hard claims and route verdicts below are grounded in WHI-266..270.
...
- Zama/OZ anchor from WHI-267: privacy 4, compliance 3, disclosure 3, lightweight 3, engineering 3, maturity 3, mantle fit 4. This draft adds low_lock_in 2 and performance_predictability 2 because Mantle support, Gateway/KMS/coprocessor operations and FHE latency remain gating.
...
**Step 2: Dimension-priority ordering.** Among route-capable candidates, compare dimensions in this order:
...
**Step 3: Pairwise-elimination check.**
...
| Inco vs VOSA | compliance 3 < 4, but VOSA maturity 1 and production gate caveat make VOSA a PoC fallback, not production backup | No contradiction; different backup role |
...
**Step 4: Anti-vendor-preference check.**
...
#### 6.2 Performance and production constraints
...
#### 6.3 Optalysys reference treatment
...
#### 6.4 Productionization checklist
...
See §6.4 Mermaid production-constraint graph.
...
1. **Outline file status mismatch**: the persisted outline frontmatter remains `status: candidate`; Orchestrator dispatch supplies the approval evidence. This draft records both and does not edit the outline.
