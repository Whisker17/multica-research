...
promotion_metadata:
  promoted_from: "evm-privacy-research/research-sections/confidential-coprocessor/drafts/round-1.md"
  approved_draft_round: 1
  approved_draft_commit: "15ac3c03e55e2f93e23ec6cc8ba6fca77ea59aca"
  approval_evidence: "Review Verdict: approve, comment a2ceab65-c51f-4107-8b67-a5cee54295d7, round 1, severity minor (no critical/major)"
  promoted_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  promoted_at: "2026-06-23"
  promotion_changes:
    - "[required] §7.3 Zama Mantle path — added cross-chain availability caveat (Zama live on Ethereum mainnet+Sepolia; multi-chain EVM expansion H1-2026 roadmap; Mantle integration requires waiting for expansion or self-hosting full Coprocessor+Gateway+KMS stack)"
    - "[optional] §1 component framing — added inline note that acceptance criterion lists 四组件; Solidity Library added as a fifth component for completeness"
    - "[optional] §1.5/§5/§6 — added OZ v0.5 second audit finding (permissionless discloseEncryptedAmount: transient operator access escalating into permanent, publicly-emittable cleartext)"
...
> **组件计数说明**：验收标准列「四组件」（Host Contract / Coprocessor / Gateway / 门限 KMS）。本 section 将 **fhEVM Solidity Library** 作为第五组件单列以求完整——它是开发者接触面，独立于链上 Host Contracts，分列更便于集成评估。下文「五组件」与验收标准的「四组件」一致，仅多出开发者库一项，非偏差。
...
> **⚠️ OZ v0.5 审计 critical 发现（两项，同主题）** [审计 openzeppelin.com/news/...v0.5-diff-audit]：
>
> 1. **Hook 模块持久 ACL 不可撤销**：已安装的 hook 模块可在任意 handle 上授予持久 FHE ACL 给任意第三方账户，且这些授权在模块 uninstall 后仍然存续，因为 FHE library 不支持撤销。OZ 文档明确警示「模块安装是对 token 加密状态的不可逆机密性决策」，应在 admin 层严格门控。
> 2. **`discloseEncryptedAmount` operator 模式越权披露**：同一轮审计还发现一个无权限（permissionless）的 `discloseEncryptedAmount` 路径——临时（transient）operator 访问可被升级为**永久的、可公开 emit 的明文披露**。即一个本应短暂的授权被固化成对外公开的解密结果。
>
> 两项发现共同强化了 items 5/6 的核心主题：**FHE-ACL 的「易授权、难撤销」非对称性是本家族的结构性弱点**。对 Mantle 评估的直接意义：**FHE ACL 的不可撤销性 + operator 披露越权面，使 GDPR right-to-be-forgotten 与最小披露原则的合规成为结构性障碍**（见 item-5、item-6、item-7）。
...
- **撤销性 + 披露越权（合规结构性障碍）**：OZ v0.5 审计两项发现——FHE ACL 授权不可撤销，且 `discloseEncryptedAmount` 临时 operator 访问可固化为永久公开明文披露（见 §1.5）。两者共同构成 GDPR right-to-be-forgotten 与最小披露原则的结构性挑战，且适用于整个 FHE-ACL 家族（非 Zama 独有）。
...
| 部署状态（2026-06-23） | **Ethereum 主网（2025-12）+ Sepolia**；多链 EVM 扩展 H1-2026 roadmap（Mantle 需等扩展或自托管全栈） [一手/二手] | **Base 主网（2026-06-15）+ Base Sepolia**；仅 Base，Mantle 需 Inco 扩展 [一手] | **测试网为主**（ETH Sepolia/Arb Sepolia/Base）；主网状态博客与文档有张力，保守判「早期/限定」 [一手矛盾已限定] |
...
1. **Zama Protocol 当前仅托管于 Ethereum 主网 + Sepolia**——多链 EVM 扩展在 H1-2026 roadmap（关键可用性依赖，与 §7.4 Inco「仅支持 Base」对称）。「部署 fhEVM host contracts 到 Mantle」需**二选一**：等待 Zama 官方 EVM 扩展支持 Mantle，或自托管全套 Coprocessor + Gateway + KMS（9/13 门限 + Nitro Enclave）协议栈——后者是显著更重的工程投入（运维一条 KMS L1 链 + Coprocessor 节点网络），其量级远超「轻量级 bolt-on」的应用层集成范畴 [一手/推论]。

前提：Mantle 是 OP Stack（geth fork）Optimistic Rollup L2，gas token MNT。在上述可用性前提满足后，Coprocessor 模式（最轻量）[推论，基于一手集成文档]：

2. 部署 fhEVM host contracts（ACL + Executor）到 Mantle。
3. 配置 Gateway 连接（Zama 官方或 self-hosted）。
4. 开发者 import fhEVM Solidity library + OZ Confidential Contracts 写 confidential 合约（RWA 场景可用 ERC7984Rwa）。
5. 无需 Mantle 协议层修改。
6. 考量：Zama 商业许可谈判；EVM 扩展时间线 vs 自托管成本权衡；KMS 信任模型（9/13 + Nitro）对 Mantle 治理叙事的影响；FHE 性能（~20 TPS CPU）对 UX 的影响；FHE ACL 不可撤销 + `discloseEncryptedAmount` 越权面对 GDPR 合规的结构性约束。
...
- OpenZeppelin Confidential Contracts v0.5 Diff Audit（openzeppelin.com/news）— **critical #1: hook 模块持久 FHE ACL 不可撤销**；**critical #2: permissionless `discloseEncryptedAmount` 临时 operator 访问越权为永久公开明文披露**；v0.3.0 Release Audit。
...
5. **撤销性 + 披露越权（三方共性）**：FHE ACL 不可撤销 + `discloseEncryptedAmount` operator 越权披露已由 OZ v0.5 审计就 Zama 栈坐实（两项 critical）；Inco/Fhenix 的撤销机制均为 `unverified`。GDPR right-to-be-forgotten 与最小披露原则的合规影响为本家族结构性问题。
...
- **Final promotion（2026-06-23）**：基于 draft-review approve（commit 15ac3c0，approval ref a2ceab65，severity minor，无 critical/major）。promote round-1.md → final.md，并 incorporate Orchestrator dispatch 指定的修改：
  1. **[required] §7.3 Zama Mantle 路径跨链可用性 caveat**：新增首行 caveat——Zama 当前仅托管于 Ethereum 主网 + Sepolia，多链 EVM 扩展 H1-2026 roadmap；「部署 fhEVM host contracts 到 Mantle」需等待 Zama EVM 扩展或自托管全套 Coprocessor + Gateway + KMS（9/13 + Nitro）栈（显著更重），与 §7.4 Inco 可用性框架对称。§7.2 部署状态行同步限定。
  2. **[optional] §1 组件计数说明**：新增内联说明——验收标准列「四组件」，本 section 将 Solidity Library 作为第五组件单列以求完整，非偏差。
  3. **[optional] OZ v0.5 第二项审计发现**：§1.5 补充 permissionless `discloseEncryptedAmount`（临时 operator 访问越权为永久公开明文披露），并在 item-5/6/7 强化 FHE-ACL 披露/撤销主题。
