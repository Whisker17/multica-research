...
final_metadata:
  ...
  approved_draft_path: "evm-privacy-research/research-sections/erc7984-confidential-token/drafts/round-1.md"
  approved_draft_round: 1
  approved_draft_commit: "b39eaae0957c1e870421643ab2ceb8b6dbf86f58"
  approval_evidence: "Review Verdict comment id=66267d77-2e86-4307-85d0-27d886fe47c4 on WHI-255; severity=minor; recommendation=approve"
  order: 1
  ...
  minor_caveats_carried:
    - "M1: added explicit transfer return/failure semantics row to ERC-7984 vs ERC-7945 table."
    - "M2: strengthened Hooked ACL persistence from unverified to confirmed-persistent after module uninstall."
    - "M3: made v0.5.0 Hooked header vs latest v0.4 audit caveat explicit for production recommendations."
...
选择性披露方面，ERC-7984 本体只有 `AmountDisclosed(bytes32,uint256)` 事件，披露触发和解密流程由实现决定；真正的合规能力来自 OpenZeppelin 扩展族：`ObserverAccess`、`Freezable`、`Restricted`、`Rwa`、`Hooked`、`ERC20Wrapper`、`Omnibus`、`Votes` 等。这里必须非常谨慎：`ERC7984ObserverAccess` 明确给 observer **permanent ACL access**，它能访问账户余额与转账金额 handle；`setObserver(..., address(0))` 只是把后续 observer 字段清空，并不构成历史 handle 访问权可撤销的证明。`ERC7984Hooked` 更强：代码注释明确把 hook module 视为 trusted contract，且模块授予自身或第三方的 ACL allowance 在 uninstall 后仍可持续。因此 WHI-254 披露向量中的 revocability 应拆开标注：ObserverAccess 的历史撤销性为 **constrained / historical revocation unverified**；Hooked 模块授予的 ACL allowance 在卸载后 **confirmed-persistent**。二者都不能写成普通可撤销披露。[source_confidence: OZ 代码直接引用]
...
| Revocability | module list 可卸载；module 已授予 ACL allowance 在卸载后 confirmed-persistent |
...
| Return/failure          | returns bytes32 actual amount| returns bool success         |
| semantics               | actual amount may be zero    | caller must handle false     |
...
另一个集成层差异是 failure 表达：ERC-7984 transfer/callback 族返回 `bytes32` actual amount；在 OZ/FHE 语义下，hook 或安全数学路径可以把实际转账金额变成 encrypted zero，调用方需要理解“成功调用但实际金额为 0”的情况。ERC-7945 的 `confidentialTransfer` / `confidentialTransferFrom` 返回 `bool success`，EIP 明确要求调用方处理 false。[source_confidence: EIP-7984/EIP-7945 原文 + OZ `ERC7984Hooked.sol`]
...
| 审计 | OZ repo 有 `audits/2025-05-v0.1.pdf`, `2025-07-v0.2.pdf`, `2025-11-v0.3.pdf`, `2026-01-v0.3.1.pdf`, `2026-03-v0.4.pdf`；但 docs 同时声明 moving repo code as-is、无正式保证、无 Immunefi 覆盖；且 `ERC7984Hooked.sol` header 为 v0.5.0，新于最新 v0.4 audit | 未找到公开官方审计报告 |
...
| OpenZeppelin release audits | OZ repo 中存在多份 audit PDF；README 说 npm install latest audited release；`ERC7984Hooked.sol` header v0.5.0 新于最新 `2026-03-v0.4.pdf` | 可引用，但不得说 current HEAD 完全 audited；生产建议应 pin 到 tagged audited release |
...
| Gap | Impact | Treatment in this final | Follow-up |
|---|---|---|---|
| Outline file frontmatter still says `status: candidate` even though Orchestrator dispatch says `outline-approved` | Strict skill validation would normally block candidate outlines | Final proceeds based on Orchestrator approval state and commit `2e55c62...`; frontmatter caveat recorded | Orchestrator may choose to patch outline status during integration if desired |
...
| ACL revocation semantics are constrained | Core compliance and privacy risk | ObserverAccess historical handle revocation remains constrained/historical-unverified; Hooked module-granted ACL persistence after uninstall is confirmed by source comment | Review Zama ACL docs/source and test whether non-Hooked `allow` grants are revocable |
| Current OZ repo audit status ambiguous | Risk of saying current HEAD fully audited | Separate release audit PDFs from docs caveat that moving repo code is as-is and not formally audited; note `ERC7984Hooked.sol` header v0.5.0 is newer than latest `2026-03-v0.4.pdf` audit | Tie any production recommendation to a tagged audited npm release, not master HEAD |
...
| final | 2026-06-23 | Promoted approved round-1 draft to final. Carried minor review caveats M1-M3: added transfer return/failure semantics row, strengthened Hooked ACL persistence to confirmed-persistent, and made the v0.5.0 Hooked vs v0.4 audit caveat explicit. |
