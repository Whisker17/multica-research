...
promotion_metadata:
  promoted_from: "evm-privacy-research/research-sections/privacy-eips-survey/drafts/round-1.md"
  approved_draft_round: 1
  approved_draft_commit: "bffa9275956ec729ba0d0a04a87c24e37ba37b29"
  approval_evidence: "Review Verdict: approve (severity minor, no critical/major), comment 55d34395-8789-480c-b4dd-e4875a71a812 — https://multica.ai/issues/3f53d683-eefb-4af0-ae59-8ee77f02c537#55d34395-8789-480c-b4dd-e4875a71a812"
  promoted_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  promoted_at: "2026-06-23"
  order: 2
  caveats_addressed_at_promotion:
    - "Caveat 1: EIP-8182 'Hegota 硬分叉（计划 H2 2026）' downgraded to explicit [unverified] tag in §4.2/§4.6 (no source triplet available)"
    - "Caveat 3: EIP-8182 proof system refined to note split-proof design (fork-managed Groth16 BN254 pool proof + user-registered auth-verifier proof) per draft-review verdict"
  caveats_carried_to_tw:
    - "Discourse search.json result counts are point-in-time snapshots (50-result pagination cap disclosed in §1.2 / Gap Analysis) — reproducibility is at query-method level, not exact-count level"
...
| 证明系统 | Groth16 on BN254（**split-proof 设计**：fork 管理的 Groth16 BN254 pool proof + 用户注册的 auth-verifier proof）`[refined per draft-review verdict 55d34395, 2026-06-23]` |
| 部署方式 | 系统合约（system contract），硬分叉激活 |
| 管理密钥 | 无 admin key，无 proxy，不可升级 |
| 目标升级 | Hegota 硬分叉 `[unverified: H2 2026 时间线未见于 EIP-8182 源文本，无 URL+访问日期+commit SHA 三元组佐证；仅作社区方向性预期]` |
| 支持资产 | ETH + 任意 ERC-20 |
| 防双花 | Nullifier 机制 |
...
| final | 2026-06-23 | 由 round-1 draft（commit `bffa9275`）promote。Adversarial 审查结论 approve（severity minor，无 critical/major），evidence 评论 `55d34395`。Promotion 阶段处理 2 项 minor caveat：(1) **caveat 1** — §4.2「Hegota 硬分叉（计划 H2 2026）」无源三元组佐证，降级为显式 `[unverified]` 标注（§4.6 / Gap Analysis 原已 hedge）；(2) **caveat 3** — §4.2 证明系统补充 split-proof 设计说明（fork 管理的 Groth16 BN254 pool proof + 用户注册 auth-verifier proof）。caveat 2（Discourse search.json 结果数为时点快照）无需改动，已在 §1.2 / Gap Analysis 披露，转交 TW。正文实质内容相对 approved draft 未变更。 |

> 本 section 已通过 Adversarial 审查（approve, minor）并 promote 为 final。promotion 仅作文件提升 + 上述 minor caveat 整合，未引入新研究。
