...
artifact_paths:
  ...
  draft: "evm-privacy-research/research-sections/privacy-landscape-framework/drafts/round-2.md"
...
4. **COTI 具备值级+部分执行级隐私能力**：EEA COTI 方案 profile 明确描述其 GC 技术加密 inputs、balances、on-chain state 和 smart contract logic，覆盖范围超出纯值级隐私。COTI 是唯一达到 General Availability 的方案（≥3 客户跨 ≥2 机构类别、≥12 月运营、≥2 次审计），而 Silent Data 为 Early Production，其余 5 方案处于 Pilot 阶段。
5. **Mantle 关键约束**：作为 institutional blockchain 追求隐私（私密转账/余额/账本），偏好「不改变基础链架构」的 bolt-on 方案。本框架的 rubric 和轻量级判定标准将在后续竞品分析中直接应用。
...
| 方案 | EEA 判定 | EEA 报告证据 |
|------|---------|-------------|
| **COTI** | **General Availability** | Privex（COTI 原生 DEX, $25bn aggregate volume）；StaTwig + UNICEF + 孟加拉国政府（国家级供应链, 10M 私有计算）；ECB Digital Euro Pioneer Partner。~14 个月 COTI 主网运营（自 2025-03）。审计：Sayfer Mar 2026, Sayfer Nov 2025, Hacken Aug 2025, Sayfer Apr 2025, Hacken Mar 2025 [EEA §09 `rm-status prod`] |
| **Silent Data** | **Early Production** | Archax 代币化基金（Aberdeen, BlackRock, Fidelity, State Street）2026-02 上线；CRYOPDP/DHL SAP 奖 2025-12；Bank of England Digital Pound Lab Phase 1 [EEA §09 `rm-status early`] |
...
**COTI 隐私层级说明（Round 2 修订）**：Round 1 将 COTI 归为纯值级隐私，但 EEA COTI 方案 profile 明确声称 GC 技术加密 "inputs, balances, on-chain state, and smart contract logic"，并描述 COTI 为 "programmable privacy layer"，支持 "Standard Solidity (Fully EVM compatible) with parameters specifying confidential data elements" [EEA §10 COTI Solution Profile]。这意味着 COTI 的 GC 框架不仅保护 token 值，还支持对开发者标注的合约逻辑和状态进行加密计算。因此 COTI 应归为值级 + 执行级（部分），但与 Silent Data TEE 的全自动执行隐私不同——COTI 要求开发者在 Solidity 合约中显式标注哪些参数是机密的（"parameters specifying confidential data elements"）[EEA §10; source_confidence: EEA 报告直接引用]。

### 2.5 EEA 7 方案技术家族标签

| 方案 | 主技术家族 | 辅助/混合 | 隐私层级 |
|------|-----------|---------|---------|
| **COTI** | GC | 探索 ZKP (Nightfall 集成)、MPC | 值级 + 执行级（部分） |
...
- **业务逻辑/合约状态（Round 2 修订）**：
  - **COTI**: Round 1 评为 ○（不保护），Round 2 修订为 ◐（部分保护）。EEA COTI 方案 profile 明确声称 "sensitive data including inputs, balances, on-chain state, and smart contract logic, remains encrypted end-to-end across storage, computation and transmission" [EEA §10 COTI Solution Profile]。COTI 的 GC 框架支持 "Standard Solidity (Fully EVM compatible) with parameters specifying confidential data elements"，即开发者在 Solidity 合约中标注机密参数，GC 引擎对这些参数进行加密计算。评为 ◐ 而非 ● 的原因：COTI 要求开发者显式标注机密数据元素，而非像 Silent Data TEE 那样自动保护全部执行逻辑；其加密计算范围取决于开发者标注的粒度，非全自动全覆盖 [source_confidence: EEA 报告直接引用 + 研究者分析]。
...
|------|-----------|---------|---------|-------|-------------|---------|
| COTI | key-holder | viewing-key-share | amount+identity | per-tx, per-account | unverified-revocable* | existence, timing |
...
|---------|---------|---------|---------|--------|
| **GDPR right to be forgotten** | 维度 e 包含 `revocable`（已验证机制） | （无方案完全满足） | COTI (unverified-revocable — 声称支持但机制未验证) | Nightfall (one-time)、Paladin/Linea/Prividium/CDK/Silent Data (permanent) |
...
| **金融审计** | 维度 e 包含 `auditable-log`（已验证实现） | Prividium, Silent Data | Paladin (partial-auditable-log — 审计设计意图明确但实现细节未验证) | COTI (无明确审计日志), Nightfall (无明确审计日志) |

**Round 2 合规映射修订说明**：
- **COTI GDPR**: Round 1 将 COTI 列为满足 GDPR（基于 `revocable*`）；Round 2 降级为「部分满足」，因为 COTI 的 revocability 声明缺少公开技术文档确认具体撤销机制（key rotation? access revocation? data deletion?）。在 GDPR 合规评估中，未经验证的声称不能等同于满足。
...
**Mantle 启示**：作为 institutional blockchain，Mantle 需要方案同时满足 MiCA/Travel Rule（identity 披露给监管方）和金融审计（auditable-log）要求。当前 7 方案中，Prividium 在合规向量上最完整（支持 regulator + compliance-gate + audit-request + auditable-log），但其部署形态（独立 L2）不符合 Mantle 轻量级偏好。COTI 和 Paladin 作为轻量级候选，合规能力存在待验证缺口：COTI 的 revocability 和 Paladin 的 auditable-log 实现均需进一步技术验证。
...
- **COTI multichain 扩展**：计划覆盖 70+ 链，GC 计算在链下执行，链上部署隐私合约接口。集成方无需运维额外基础设施 [COTI 2026 Roadmap; EEA §10 COTI: "Deploy on COTI network or bridge existing assets"; source_confidence: EEA 报告直接引用 + 官方文档补充]。
...
**COTI 判定说明**：COTI 作为 multichain 扩展/协处理器模式，不需要部署新链、新桥或全节点运维，也不需要硬分叉。GC 路线指标（1-2 轮交互、低带宽）满足轻量级。Round 2 修订：Round 1 注明 COTI「不覆盖业务逻辑/合约状态隐私 (R4)」，Round 2 基于 EEA 报告证据修正——COTI GC 框架支持对标注的合约逻辑和状态进行加密计算（部分覆盖 R4），但仍以值级隐私为核心优势 [EEA §10 COTI Solution Profile]。
...
**Mantle 启示**：在 7 方案中，仅 COTI 明确通过轻量级判定，Paladin 处于边界。COTI 的优势在于同时通过轻量级判定且覆盖值级+部分执行级隐私（基于 EEA 证据），但其合规向量（unverified-revocable, 无 auditable-log）需补充验证。Mantle 如追求轻量级 bolt-on，候选空间极窄。如果扩展到 EEA 7 方案之外，Zama fhEVM coprocessor 也是 bolt-on 架构的候选（链下 FHE 运算 + 链上 encrypted handle），但 FHE 性能开销（当前 ~20 TPS on CPU）是关键瓶颈。
...
| 方案 | 分类 | 依据 |
|------|------|------|
| COTI | A + B（部分） | 值级：encrypted values + GC；执行级（部分）：GC encrypted computation on developer-annotated smart contract logic and on-chain state [EEA §10] |
...
见 item-2 §2.2 — 6 类技术家族 × 7 项隐私原语维度矩阵，含保护能力等级（强/中/弱）和 EEA 7 方案技术家族标签。Round 2 新增 GC 列在业务逻辑/合约状态维度的评分说明（通用 GC 评「中」，但 COTI 方案级评分在轴 2 中独立调整）。
...
**说明**：EEA 报告全文 URL (https://entethalliance.github.io/wg-privacy/privacy-report.html) 在 Round 2 修订过程中成功直接获取 HTML 内容。§09 Readiness Matrix 各方案状态、判定标准和证据文本均直接从报告 HTML 结构中提取验证（CSS class `rm-status prod` / `rm-status early` / `rm-status pilot` + 对应 `rm-evidence` 和 `rm-disclosure` 文本）。§10 Solution Profiles（特别是 COTI profile）的 Privacy Approach 和 Integration Requirements 文本亦直接提取用于 Round 2 修订。Round 1 中标注的 G1 缺口（EEA 报告无法直接获取）在 Round 2 中已部分消解。
...
- ✓ 五轴 rubric 结构和 EEA 7 方案评估完整（Round 2 修订 COTI 维度）
- ✓ 选择性披露 6 维向量模型定义 + 7 方案向量填充完整（Round 2 修订不确定性标签）
- ✓ 轻量级判定标准（一票否决 + 路线指标 + 决策规则）完整（Round 2 新增局限性说明）
- ✓ 隐私账本二义判定口径定义 + 7 方案分类完整（Round 2 修订 COTI 分类）
