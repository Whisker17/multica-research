...
在 10 维度能力矩阵中，go-ethereum 在 CI/Testing、PR Governance、Security（Rulesets）维度达到"成熟"级别，在 Documentation/Infra 维度为"基础"级别，但在 Release Pipeline、AI Code Review、Upstream Auto-Sync、Interactive Agent、Benchmark 维度为"缺失"状态——这与其项目性质一致（go-ethereum 是上游源头而非 fork，且发布流程可能由外部系统管理）。
...
| Secrets 名称 | `GET /repos/ethereum/go-ethereum/actions/secrets` | HTTP 403 — 需要 collaborator 权限 |
| Branch Protection (legacy API) | `GET /repos/ethereum/go-ethereum/branches/master/protection` | HTTP 404 — 已迁移至 Rulesets |
...
| Secrets 名称不可访问 | **Minor** | 记录为"不可访问 (HTTP 403)"；从 ci-notes.md 中识别 PPA_SIGNING_KEY 和 PPA_SSH_KEY |
