...
artifact_paths:
  ...
  draft: github-actions-optimization/research-sections/base-github-actions/drafts/round-1.md
...
4. **安全加固**：全仓库采用 `step-security/harden-runner`（审计模式）+ 全 SHA 钉扎 Action 引用，达到 SLSA Level 2+ 水准。
...
所有工作流均声明顶层 `permissions` 块，不依赖默认的 `GITHUB_TOKEN` 权限。特别值得注意：
...
| 7 | Security & Supply Chain | **成熟** | 全仓库 harden-runner + 全 SHA 钉扎 + GPG 签名 + build attestation + 最小权限 | 所有工作流 | contracts/docs harden-runner 版本滞后 |
...
**实现步骤**: 在每个工作流的第一个 step 添加 harden-runner。
...
图例: BLOCK ████ = egress block   AUDIT ▓▓▓▓ = egress audit   无 ░░░░ = 未配置
     ✓ = 已配置   ✓* = 仅 contracts/docs（v2.12.1 旧版）
```
