...
- **Codecov 配置**：
  - `disable_search: true`（两仓库均设置；手动指定文件路径而非自动搜索）
...
| final | promotion | 从 approved round-2 draft 提升为 final。应用 minor caveat：`disable_search: true` 归属更正为两仓库均设置（非仅 stateless-validator）。来源：adversarial review comment 6ee677a0。 |
