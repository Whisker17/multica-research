...
draft_metadata:
  ...
  access_method_external: "curl HTML extraction + GitHub .mdx source"

promotion_metadata:
  promoted_from: "hoodi-launch-notice/research-sections/reference-notice-analysis/drafts/round-1.md"
  approved_draft_round: 1
  approved_draft_commit: "e2748ab1e31e3f67942f3131dc84dea7d306bc1d"
  approval_evidence: "comment 4673dccb-75f7-4bea-a80d-2bb3bd87b435 (Research Review Agent approve verdict)"
  promoted_at: "2026-05-27T05:50:00Z"
  promoted_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  corrections_applied:
    - "Optimism source: replaced WebSearch-only limitation with .mdx source citations"
    - "Base Azul: reframed activation dates as versioned/tentative examples"
---
...
|------|------|-----|---------|------|---------|---------|
| Optimism | Upgrade 14: MT-Cannon & Isthmus L1 | docs.optimism.io/notices/upgrade-14 | 官方 notice | L1 合约升级 + Operator Fee 引入 | 与 Arsia 的 Operator Fee 特性直接对应；覆盖多链升级协调 | ✅ 官方 `.mdx` 源文件可访问（[GitHub](https://github.com/ethereum-optimism/docs/blob/main/pages/notices/upgrade-14.mdx)） |
| Optimism | Holocene Breaking Changes | docs.optimism.io/notices/holocene-changes | 官方 notice | OP Stack hardfork | OP Stack 上游 hardfork notice 的标准格式；覆盖 activation dates、chain/node operator instructions | ✅ 官方 `.mdx` 源文件可访问（[GitHub](https://github.com/ethereum-optimism/docs/blob/main/pages/notices/holocene-changes.mdx)） |
...
| **激活时间** | ❌ 无 timestamp | ✅ 精确 UTC timestamp + 日期（Sepolia: Apr 9; Mainnet: Apr 25） | ⚠️ 激活日期跨页面存在版本差异：Blog 公告目标 May 13, 2026 mainnet；Spec/Guide 标注 May 28, 2026 mainnet（tentative）。应视为可变/版本化示例，非最终确定事实 |
...
| src-3 | official_docs | Optimism 通告 | Upgrade 14 ([`.mdx` source](https://github.com/ethereum-optimism/docs/blob/main/pages/notices/upgrade-14.mdx)) + Holocene ([`.mdx` source](https://github.com/ethereum-optimism/docs/blob/main/pages/notices/holocene-changes.mdx)) | ✅ 2 篇对照（官方 `.mdx` 源文件完整获取） |
...
| Optimism docs | GitHub `.mdx` source files | ✅ 官方 `.mdx` 源文件可通过 GitHub raw 链接完整获取（[Upgrade 14](https://github.com/ethereum-optimism/docs/blob/main/pages/notices/upgrade-14.mdx)、[Holocene](https://github.com/ethereum-optimism/docs/blob/main/pages/notices/holocene-changes.mdx)）。内容支持激活时间、chain/operator 章节、节点版本、支持路径等核心声明。 |
...
| E9 | Base Azul Blog | "tl;dr Base Azul, our first independent network upgrade, is targeting mainnet activation on May 13, 2026." ⚠️ Blog 目标日期为 May 13；Node Operator Guide/Spec 标注 May 28, 2026 mainnet activation（tentative）。跨页面日期不一致，应视为可变/版本化示例。 | item-6 tl;dr 风格参考 + 激活日期版本化示例 |
...
| E12 | Optimism Upgrade 14 (`.mdx` source) | "OP Sepolia, Ink Sepolia, and Minato Sepolia upgrades are targeting **April 9th, 2025**. OP Mainnet, Soneium Mainnet, Ink Mainnet are expected to execute Upgrade 14 on **April 25th, 2025**." | item-6 激活时间表对比 |
| E13 | Optimism Holocene (`.mdx` source) | "Chain operators should upgrade their nodes ahead of the activation times to a release that contains the Holocene changes and has the activation times for their chains baked in, or set the activation times manually via overrides." | item-6 操作指引对比 |
...
| item-6 Optimism 对照 | **High** | 官方 `.mdx` 源文件完整获取，支持激活时间、chain/operator 章节、节点版本和支持路径等核心声明 |
...
| 1→final | promote_to_final | corrections: Optimism .mdx source citations + Base Azul activation date versioning caveat | Orchestrator dispatch final-promotion (comment b0ed74ae); Research Review Agent approval (comment 4673dccb) with severity: minor |
