...
artifact_paths:
  ...
  draft: mantle-base-codebase-evaluation/research-sections/architecture-advantage-summary/drafts/round-1.md
...
Base Azul 升级（mainnet code-set target 2026-05-28）标志着 Base 首次在代码层脱离 OP Stack，形成独立的 Base Stack。本综述从七个维度系统分析 Base 在此次升级中的架构设计变更及其对 Mantle 的参考价值。

核心发现：

1. **执行层统一**：Base 以 base-reth-node（paradigmxyz/reth v1.11.4 fork）+ base-consensus（Kona-inspired）单进程架构取代 OP Stack 的 op-geth + op-node 双进程模型（snapshot: op-geth v1.101702.2-rc.3 / optimism monorepo d905be1e03），消除进程间 JSON-RPC 通信开销。

2. **TEE+ZK dual-proof 安全架构**：AggregateVerifier 合约（1041 行 Solidity，PROOF_THRESHOLD=1 常量）实现 TEE + ZK 双证明聚合，三种结算路径将最终性从 7 天缩短至 ≤1 天（min(createdAt+7d, secondProofAt+1d)）。
...
5. **时间压力**：op-geth EOL 定于 2026-05-31，Mantle 基于 mantlenetworkio/op-geth@v1.4.2（commit 9c428cf）面临迁移紧迫性。

本综述的架构先进性排名将 TEE+ZK dual-proof 列为对 Mantle 最高价值改进（安全性 + 最终性双重提升），其次是单客户端架构（性能 + 可维护性），Flashblocks 排第三（用户体验提升）。
...
2. **性能**：单客户端架构（base-reth-node）消除 EL-CL 进程间通信瓶颈，Flashblocks 提供 200ms 预确认
...
| 影响项 | 详述 | 时间节点 |
|--------|------|----------|
| op-geth EOL | OP Stack 官方宣布 op-geth 进入 end-of-life（snapshot: optimism monorepo d905be1e03 中 fork timeline 最高仅到 JovianTime，无 AzulTime）| 2026-05-31 |
| Base Stack 分叉 | Base 的代码 fork 意味着 Base Stack 与 OP Stack 在执行层、证明系统层形成永久分叉；共享部分收敛于 Superchain 治理层 | Azul mainnet code-set 2026-05-28 |
...
| 维度 | 评估 |
|------|------|
| 相关性 | Mantle 基于 mantlenetworkio/op-geth@v1.4.2 + mantle-v2@v1.5.4 op-node（snapshot: commit 9c428cf / fccacf5b），面临 op-geth EOL 2026-05-31 同等压力 |
| 采纳路径 | 两条可选路径：(A) 跟随 OP Stack 迁移 op-reth（与 Optimism 主线保持一致）；(B) 直接采纳 Base codebase（获取 TEE+ZK dual-proof + 单客户端 + Flashblocks 全套改进，但承担 fork 维护成本） |
...
Base 的执行层采用单进程架构（evidence: base-strategy-azul-overview final.md + flashblocks-network-changes final.md）：

- **base-reth-node**：基于 paradigmxyz/reth v1.11.4 的 fork，承担完整的执行层（EL）功能
- **base-consensus**：Kona-inspired 的 derivation 逻辑，嵌入 base-reth-node 进程内，取代独立的 op-node CL 进程
- **单二进制部署**：EL + CL 在同一进程内运行，通过共享内存直接通信
...
| 共识/Derivation | base-consensus (Rust, Kona-inspired, 嵌入 EL 进程) | op-node (Go, 独立 CL 进程) |
| 进程间通信 | 无（共享内存） | JSON-RPC over IPC/HTTP |
| 代码语言 | Rust 统一 | Go (op-geth) + Go (op-node) |
...
1. **消除进程间通信开销**：OP Stack 每个 L2 block 需要多次 JSON-RPC 往返（fcU + newPayload + getPayload），每次涉及序列化/反序列化、网络栈、上下文切换；Base 单进程架构将这些转化为函数调用
2. **共享内存池**：交易池、状态缓存、derivation 状态在同一进程内共享，避免重复的数据传输和缓存不一致
3. **统一状态管理**：消除 EL 与 CL 之间的状态同步延迟，特别是在 unsafe head 更新路径上

#### maintainability

| 维度 | Base Stack（单代码库） | OP Stack（双代码库） |
|------|----------------------|---------------------|
| 语言统一性 | Rust 单语言 | Go 双仓库（op-geth + op-node） |
| 构建系统 | Cargo workspace 统一构建 | 两个独立的 Go module |
| 版本同步 | 单版本号，原子升级 | 需要协调 op-geth 与 op-node 版本兼容性 |
| 调试体验 | 单进程 profiling，统一日志 | 跨进程追踪，分散日志 |
...
Mantle 当前使用 eth/68，升级到 eth/69 属于网络层变更，影响节点间 P2P 通信。

#### Mantle 获益评估

| 维度 | 评估 |
|------|------|
| 当前状态 | mantlenetworkio/op-geth@v1.4.2 (commit 9c428cf) + mantle-v2 op-node（snapshot: commit fccacf5b） |
| 迁移工程量 | 从双组件 Go 架构迁移到单客户端 Rust 架构是重大工程变更——涉及语言栈切换（Go → Rust）、构建系统重构、运维工具链更新 |
| 性能收益 | 消除 EL-CL JSON-RPC 瓶颈的延迟改善预估：每 block 减少 2-5ms RPC 往返（视 IPC vs HTTP 配置而定），在 200ms flashblock 场景下影响显著 |
...
| 路径 | 条件 | 最终性 | 公式 |
|------|------|--------|------|
| Path A: TEE-only | 仅 TEE 证明通过 | 7 天（SLOW window） | createdAt + 7d |
| Path B: ZK-only | 仅 ZK 证明通过 | 7 天（SLOW window） | createdAt + 7d |
| Path C: TEE+ZK | 双证明均通过 | **≤1 天**（FAST window） | **min(createdAt + 7d, secondProofAt + 1d)** |

Path C 的 min() 公式意味着：如果双证明在游戏创建后不久（<6d）就齐全，最终性将在第二个证明提交后仅 1 天内达成——这对桥接资金解锁和 DeFi 组合性至关重要。
...
DelayedWETH 合约实现 1 天 delay 的 bond escrow：

- dispute 成功后 → resolveGame() → claimUnlock()（启动 1 天 timer）→ 等待 timer 到期 → withdraw()
- 1 天 delay 防止恶意 resolve + 即时 withdraw 的攻击向量
...
- **演进方向**：多 ZK backend 并存（SP1 / RISC Zero）、更强 TEE（Intel TDX、AMD SEV-SNP）、Proposer 内嵌 ZK 激活 1-day fast finality
...
| 最终性改善 | 从 7 天标准 challenge period 缩短到 ≤1 天（Path C）：对桥接资金解锁（7d → 1d = 资本效率提升 7x）和 DeFi 组合性意义重大 |
...
| 5 | EIP-7642 eth/69 | ✅ | ✅ already_live (Limb) | — |
...
| 13 | Engine API V5 + eth/69 wire | ✅ | ❌ not_live (Engine API V3, eth/68) | 协议升级 |

#### already_live

6/13 特性已通过 Mantle Limb 硬分叉（2026-01-14）上线：EIP-7823、7883、7939、7951 + EIP-7642 eth/69 + EIP-7910 eth_config。这说明 Mantle 在 Osaka EVM 对齐方面已走在前列。
...
5. Engine API V5 + eth/69 wire — 中优先级（协议升级，配合单客户端迁移）
...
4. **Engine API V3 → V5**：协议升级，需要 sequencer/op-node 侧同步更新
...
| 时间压力 | op-geth EOL 2026-05-31 + Base Azul mainnet code-set 2026-05-28 形成双重时间压力 |
...
|------|---------|--------|------|----------|--------|----------|----------|----------|
| **1** | **TEE+ZK dual-proof** | 5 | 4 | 2 | 4 | 4 | **3.8** | 安全性质变（单证 → 双证）+ 最终性 7d→≤1d；工程成本高但价值最大 |
| **2** | **单客户端 (reth)** | 3 | 5 | 2 | 5 | 4 | **3.8** | 性能最大提升 + op-geth EOL 紧迫性最高；但 Go→Rust 迁移风险大 |
| **3** | **Flashblocks (200ms)** | 2 | 5 | 3 | 3 | 5 | **3.6** | UX 质的提升；工程成本中等（可复用 OP Stack 上游 option b）；生态对齐最高 |
| **4** | **Osaka EVM (5 EIPs + companion)** | 2 | 2 | 5 | 3 | 5 | **3.4** | 4/5+2 已完成（Limb），仅 EIP-7825 待评估；工程成本极低 |
| **5** | **Engine API V5 / eth/69** | 1 | 3 | 4 | 3 | 4 | **3.0** | 配合单客户端迁移的协议层升级；独立价值有限 |
...
**渐进式采纳路径**：
1. **Phase 0（已完成）**：Osaka EVM 对齐 — Limb 已完成 4/5 EIP + companion changes
...
4. **Phase 3（长期，6-12 月）**：单客户端迁移 — 从 op-geth + op-node 迁移到 reth-based 单进程架构（可选：Base reth fork 或自建 reth fork）
...
推荐渐进式采纳：降低每次变更的风险面，允许逐步验证，且在 op-geth EOL 之前优先解决最紧迫的问题。
...
| op-geth EOL 时间压力 | 高 | 2026-05-31 EOL 距今极近 | 优先评估 OP Stack 官方 op-reth 迁移路径（非 Base 独立路径） |
...
| 时间节点 | 行动项 | 依赖 |
|----------|--------|------|
| 2026-05-31 | op-geth EOL — 必须在此之前确定 EL 迁移方案 | — |
| 2026 Q3 | Flashblocks 集成（Phase 1）上线 testnet | rollup-boost + op-rbuilder 部署 |
| 2026 Q3-Q4 | AggregateVerifier + TEE Prover 部署到 testnet | AWS Nitro 基础设施 + SP1 cluster |
| 2027 Q1 | TEE+ZK dual-proof 主网上线 | 安全审计 + Stage 1→2 评估 |
| 2027 Q2 | 单客户端迁移评估完成 | reth 上游稳定性评估 |

#### Mantle 获益评估

| 维度 | 评估 |
|------|------|
| Top 3 最高价值改进 | (1) TEE+ZK dual-proof — 安全性质变 + 最终性提升；(2) 单客户端 — 性能提升 + 可维护性；(3) Flashblocks — UX 质变 |
| Quick wins | Osaka EVM（已近完成）；Flashblocks（可直接复用 OP Stack 上游 option b） |
| 战略决策 | 是否采纳 Base codebase 取决于：(a) Mantle 特有功能在 Rust/reth 中的重实现成本 vs 收益；(b) 对 Base 上游 fork 维护的长期承诺意愿；(c) 是否能在 op-geth EOL 前完成至少 Phase 1 |
...
|---|-----------|---------------|-----------------|--------|
| 1 | `base-azul-upgrade/research-sections/base-strategy-azul-overview/final.md` | Items 1, 2 | Fork 三层维度、Base Stack 组件表、Azul 三大目标、Base Stack vs OP Stack 对照 | full |
| 2 | `base-azul-upgrade/research-sections/mantle-impact-assessment/final.md` | Items 1, 5, 6 | 13x7 特性矩阵、6/13 已 live、4 BREAK-CHANGE、op-geth EOL 时间线、code anchors | full |
...
| 5 | `base-azul-upgrade/research-sections/multiproof-architecture/final.md` | Item 4 | AggregateVerifier 1041 行、PROOF_THRESHOLD=1、三种结算路径、DelayedWETH、OptimismPortal2、Stage 2 路径 | full |
