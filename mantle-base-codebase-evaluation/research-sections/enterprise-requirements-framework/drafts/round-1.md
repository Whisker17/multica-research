...
artifact_paths:
  ...
  draft: "mantle-base-codebase-evaluation/research-sections/enterprise-requirements-framework/drafts/round-1.md"
...
| 层次 | 具体需求 | 模块/接口 |
|---|---|---|
| 执行层 | 发行方策略注册表（铸造/销毁/冻结权限）| PolicyRegistry(0x42...0031) + PolicyExecutor(0x42...0021)（WHI-354 §2.3） |
| 共识 | 类型化终局性预言机 | FinalityOracle 接口（WHI-386 §2.2, line 139） |
| 合规 | 风险分级交易限制 + 储备证明接口 | ComplianceRegistry(0x42...0020)（WHI-354 §5.1） |
| 互操作 | 合规 DEX/路由器 + 桥消息合规决策哈希 | 企业桥接（WHI-350 §1.2.8） |

**路径分流**（WHI-389 §6, WHI-390 §6）：
- 单发行方→L3 Zone（专用策略空间）
- 多发行方 + 公共流动性→L2（共享结算层）
- CBDC/主权→L1（最高合规深度）
...
| 层次 | 具体需求 | 模块/接口 |
|---|---|---|
| 执行层 | 资产冻结/解冻控制 | PolicyExecutor(0x42...0021) |
| 隐私 | 每租户 Sequencer + DA + KMS 分区 + 查看密钥管理 | L3 Zone 隔离 + SelectiveDisclosure(0x42...0034)（WHI-389 §1, line 42） |
| 合规 | 带策略门控的提款队列 + NAV/储备证明预言机 | ComplianceRegistry + 自定义预言机 |
| 业务 | 投资者入驻工作流（KYB + SSO + API 密钥）+ 托管集成适配器 + 角色范围私有浏览器 | RPC Auth Gateway(N1) + Admin Dashboard(N15)（WHI-354 §2.3） |
