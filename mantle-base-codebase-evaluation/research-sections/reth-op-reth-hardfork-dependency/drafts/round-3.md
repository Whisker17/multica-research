...
- **EVM 对齐**：Azul 在执行层采纳了 Ethereum Osaka 的 5 项 EIP——EIP-7825（交易 gas 上限 2^24）、EIP-7823（MODEXP 输入限制）、EIP-7883（MODEXP 计价提升）、EIP-7939（CLZ opcode）、EIP-7951（secp256r1 gas 翻倍）。通过 `BaseUpgrade::Azul => SpecId::OSAKA` 映射（来源：`base-azul-upgrade/research-sections/osaka-evm-changes/final.md`）。
