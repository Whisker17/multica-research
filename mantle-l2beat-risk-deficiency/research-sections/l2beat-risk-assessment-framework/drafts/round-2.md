...
artifact_paths:
  ...
  draft: mantle-l2beat-risk-deficiency/research-sections/l2beat-risk-assessment-framework/drafts/round-2.md
...
prior_round_artifact: mantle-l2beat-risk-deficiency/research-sections/l2beat-risk-assessment-framework/drafts/round-1.md
prior_round_commit: 2904c4b191a7f47bcf738483f0ee1f3dc714ac5a
...
draft_metadata:
  ...
  authored_at: "2026-05-20T06:30:00Z"
  evidence_fetched_at: "2026-05-20T06:15:00Z"
...
# L2Beat Risk Chart Proposer Failure 与 Exit Window 评估标准解析（Round 2 Draft）

## 0. Round 2 Revision Notes

本 round 针对 [Review Verdict (round-1)](https://github.com/Whisker17/multica-research/blob/main) 的三项 finding 做定点修订：

| Finding | 等级 | 影响章节 | 处理方式 |
|---|---|---|---|
| **F1** src-5（L2Beat 项目页/docs 快照 min 3）与 src-6（Forum/governance 帖 min 2）未抓取 | MAJOR | §4 Source Coverage、§6 Gap Analysis、新增 §A 附录 | 抓取并锚定 3 条 src-5 + 4 条 src-6；Mantle 项目页验证"whitelisted proposers ... withdrawals are frozen"为 L2Beat 当前**公开展示的官方文案**；L2Beat-team 对 "OpSuccinct → PROPOSER_CANNOT_WITHDRAW" 设计意图的**专门**书面解释经多渠道检索后**not found after search**；§2.2.5 与 §6.3 收紧措辞。 |
| **F2** Arbitrum One Exit Window 误标 (warning/good) | MAJOR | §2.3.3、§2.5.2、§2.5.3、diag-5 | 改写为 primary/emergency = `None`（bad，红），regular 子字段承载较长 window；引用 `riskView.ts:741-757` `EXIT_WINDOW_PERMISSIONLESS_BOLD` `withRegularExitWindow(EXIT_WINDOW(0, selfSeq), EXIT_WINDOW(l2TL+l1TL, selfSeq))` 实证；下游对照结论同步矫正——Arbitrum One **不**是"Exit Window 显著优于 OP Stack 默认"的反例。 |
| **F3** orbitStack 覆盖与 Arbitrum Proposer Failure "good" 内部不一致 | MINOR | §2.2.4、§2.5.2、diag-5、§4 src-2 | 直接补 `orbitStack.ts` 精确路径：`L967-971` validatorWhitelistDisabled、`L1041-1069` proposerFailure 三分支（whitelistDisabled→SELF_PROPOSE_ROOTS / totalDelay>1y→CANNOT_WITHDRAW / 否则 SELF_PROPOSE_WHITELIST_DROPPED(totalDelay)），以及 `L1048-1058` validatorAfkBlocks 计算；Arbitrum One Proposer Failure "good" 判定从 §2.5.2 矩阵中**保留**，并附 commit + 行号锚定。 |
...
4. **官方公开的"为什么 OpSuccinct → CANNOT_WITHDRAW"专项书面解释 — Not found after search（本 round 新增）**：经在以下渠道的多轮检索均未发现 L2Beat 团队对该具体映射的**专门**书面理由（即 dedicated post / PR description / docs 段落）：
   ...
   - [Forum: Methodology & Framework 全部 6 条主题](https://forum.l2beat.com/c/methodology-and-framework/5) (浏览于 2026-05-20)：The Risk Rosette Framework / The Stages Framework / Data Availability Risk Framework / Stage 1 update: 7d→5d / **New Stage 1 requirements for L2 proving systems** / Rollup maturity ranking proposal。其中 [#413 New Stage 1 requirements for L2 proving systems](https://forum.l2beat.com/t/new-stage-1-requirements-for-l2-proving-systems/413) (2026-02-16) 提出了 ZK proving system 的四项 Stage 1 要求（no red trusted setups / prover source published / verifiers reproducible / ZK and optimistic programs reproducible）——**但这是 Stage 框架的要求，不是 Risk Chart sentiment 的判定理由**（详 §A.2.2 src-6 quote）。该帖**未**专门解释"OpSuccinct 在 Risk Chart 中为何被判 CANNOT_WITHDRAW"。
...
- **Q-2.2（partial resolution，本 round 收紧）**：源码层面证实 OpSuccinct → CANNOT_WITHDRAW 是模板 hardcoded 映射；§2.2.5 已核验。**未解决残余**：L2Beat 团队对该映射的**专门**官方书面解释经多渠道检索后 not found after search（[Mantle 项目页](https://l2beat.com/scaling/projects/mantle) / [Glossary](https://l2beat.com/glossary) / [Stages page](https://l2beat.com/stages) / [Methodology & Framework Forum 全部 6 条主题](https://forum.l2beat.com/c/methodology-and-framework/5) / GitHub PR / commit message 均未提供 dedicated 解释）。本 draft 在 §2.2.5 已将此项明确归类为"not found"，正文措辞从"我们解读"收紧为"最可能的间接成因"。Gap 标记 §6.3。
...
| src-6 | governance_proposals | L2Beat Forum 一手帖子（评估方法、Proposer/Exit Window 评估口径变更）（min 2） | **本 round 抓取 4 条**：(1) [Forum: The Risk Rosette Framework](https://forum.l2beat.com/t/the-risk-rosette-framework/292) (2024-06-06)；(2) [Forum: New Stage 1 requirements for L2 proving systems](https://forum.l2beat.com/t/new-stage-1-requirements-for-l2-proving-systems/413) (2026-02-16)；(3) [Forum: The Stages Framework](https://forum.l2beat.com/t/the-stages-framework/291) (2024-06-06)；(4) [Forum: Methodology & Framework category index](https://forum.l2beat.com/c/methodology-and-framework/5)。详 §A.2 附录 src-6 quotes | ✅（min 2 达成；§A.2 附录） |
...
**总结（本 round 更新）**：8 项源码要求全部满足 min count，含本 round 抓取的 3 条 src-5 与 4 条 src-6（详 §A 附录）。orbitStack.ts 深挖完整（Finding 3 已修复）。所有事实链均锚定至 commit `c4d95930574ebdab2a986e673553b1824111990a`。
...
### 5.2 src-6（L2Beat Forum 帖）— round-2 已抓取

- **状态**：CLOSED at round-2。已抓取 4 条（Risk Rosette Framework / New Stage 1 requirements for L2 proving systems / The Stages Framework / Methodology & Framework category index），见 §A.2 附录。
- **保留 caveat**：本类目当前可见主题仅 6 条；其中 [Stage 1 update: 7d→5d](https://forum.l2beat.com/t/stage-1-update-minimum-challenge-period-reduction-from-7d-to-5d/425) 与 [Rollup maturity ranking proposal](https://forum.l2beat.com/t/rollup-maturity-ranking-proposal-call-for-final-feedback/119) 属 Stage 框架，与 Risk Chart sentiment 判定无直接关系，本 draft 不引用。
...
- **gap**：源码层已证 hardcoded（opStack.ts L1438-1440），生产页面文案 cross-check 完成（[Mantle 项目页](https://l2beat.com/scaling/projects/mantle) 显示 PROPOSER_CANNOT_WITHDRAW.description 镜像）。但 L2Beat 团队对该具体映射的**专门**官方书面解释（dedicated post / PR description / docs 段落）在以下渠道**均未发现**：
  ...
  - [Methodology & Framework Forum 全部 6 条主题](https://forum.l2beat.com/c/methodology-and-framework/5)（包括 [#413 Stage 1 ZK Proving requirements](https://forum.l2beat.com/t/new-stage-1-requirements-for-l2-proving-systems/413)，是 Stage 框架要求而非 Risk Chart sentiment 理由）
...
- ✅ **src-5 抓取 3 条**（Mantle 项目页 / Glossary / Stages）（Finding 1 修复）。
- ✅ **src-6 抓取 4 条**（Risk Rosette Framework / Stage 1 ZK Proving / The Stages Framework / Methodology category index）（Finding 1 修复）。
- ✅ **Arbitrum One Exit Window primary 矫正为 bad / None**（Finding 2 修复）；regular 子字段为 warning。
- ✅ **orbitStack.ts L940-1071 完整深挖**，Arbitrum Proposer Failure "good" 判定的源码路径明确（Finding 3 修复）。
- ✅ OpSuccinct → CANNOT_WITHDRAW 官方专门理由"not found after search"明确标注。

---

## A. Appendix: src-5 & src-6 Verbatim Evidence（本 round 新增）

> 本附录满足 outline `src-5` (min 3) 与 `src-6` (min 2) 的强制要求，对应 Finding 1。所有 quote 均从 fetched 2026-05-20 的当前快照中提取；如 L2Beat 网站不提供版本化页面，下游需要时可配合 Web Archive 重新存档。
...
#### A.2.4 [Forum: Methodology & Framework category index](https://forum.l2beat.com/c/methodology-and-framework/5) (browsed 2026-05-20)

Complete topic listing as of browse date (this is the universe of L2Beat methodology forum posts):

1. [The Data Availability Risk Framework](https://forum.l2beat.com/t/the-data-availability-risk-framework/318) — Last Activity 2024-11-06 — out of scope (DA dimension only)
2. [The Risk Rosette Framework](https://forum.l2beat.com/t/the-risk-rosette-framework/292) — Last Activity 2024-06-08 — **primary src-6 evidence (A.2.1)**
3. [The Stages Framework](https://forum.l2beat.com/t/the-stages-framework/291) — Last Activity 2024-06-06 — context only (A.2.3)
4. [Stage 1 update: minimum challenge period reduction from 7d to 5d](https://forum.l2beat.com/t/stage-1-update-minimum-challenge-period-reduction-from-7d-to-5d/425) — Last Activity 2026-04-30 — Stage scope, not Risk Chart sentiment
5. [New Stage 1 requirements for L2 proving systems](https://forum.l2beat.com/t/new-stage-1-requirements-for-l2-proving-systems/413) — Last Activity 2026-02-16 — **primary src-6 evidence for OpSuccinct gap (A.2.2)**
6. [Rollup maturity ranking proposal - call for final feedback](https://forum.l2beat.com/t/rollup-maturity-ranking-proposal-call-for-final-feedback/119) — Last Activity 2025-04-07 — Stage scope

**Conclusion**: Forum coverage is complete for this category. None of the 6 topics provides a dedicated rationale for the OpSuccinct → PROPOSER_CANNOT_WITHDRAW mapping (per §2.2.5 / §5.3 "not found after search").
