...
artifact_paths:
  ...
  draft: mantle-l2beat-risk-deficiency/research-sections/recommendation-proposal/drafts/round-2.md
...
draft_metadata:
  ...
  authored_at: "2026-05-21T14:43:47Z"
  ...
  revision_feedback:
    review_comment: a7c77659-f77c-455b-b981-78a83ef94823
    orchestrator_revision_request: e725571b-9838-4ab0-8750-754099afee5a
    selected_fix: "Option (a): Phase 1 / minimum-pass fallback must be no longer than the effective exit window minus an explicit safety margin; with 10d timelock and 12h exitDelay, fallbackTimeout must be <=7d, leaving about 2.5d margin."
  approved_draft_artifact: mantle-l2beat-risk-deficiency/research-sections/recommendation-proposal/drafts/round-2.md
  approved_draft_commit: 8501f829590a861bd641ba86711029f6190a1519
  approval_evidence_comment: 7e352368-183b-44e3-ac99-98c79c05d7a2
  promoted_to_final_at: "2026-05-21T15:00:35Z"
  final_promotion_caveats:
    - "L2Beat must still confirm the exact Risk View mapping for the recommended package."
    - "Unrestricted emergency bypasses must be closed before claiming pass status."
    - "Mantle must prove storage-layout, audit, and deployed-parameter evidence before claiming live remediation."
---

# Proposer Failure 与 Exit Window 综合推荐改进方案（Final）

## 1. Executive Summary

Mantle 要让 L2Beat Risk Analysis 中的 **Proposer Failure** 与 **Exit Window** 两个缺陷项通过复核，最小可信路线不是"提交说明让 L2Beat 改页面"，而是同时交付两类可验证的部署属性：

1. **Proposer Failure**：上线一个真实的用户自助 / permissionless fallback。推荐优先级最高的方案是 **OP Succinct validity oracle fallbackTimeout 或等价 delayed self-propose**：保留受控 proposer 快路径，但当最后一次有效 output 超过固定 timeout 后，任意用户可提交有效 SP1 proof 推进 output root。**Phase 1 / minimum-pass fallback timeout 必须不长于有效 Exit Window 扣除明确安全余量后的值**；在本 final 推荐的 10d timelock、12h exitDelay 下，有效窗口为 9.5d，因此 minimum-pass fallback 应设为 **<=7d**，保留约 2.5d 退出余量。**14d fallback 不能与 10d timelock 一起声称 minimum pass**；它只适合搭配更长 timelock（例如 31d target，或至少让有效窗口明显大于 14d 并保留余量）。仅扩大白名单、仅增加多签/治理替换 proposer、仅提交 L2Beat override 都不足以构成强通过方案。
2. **Exit Window**：把 core rollup upgrade path 从当前 0-delay Safe/ProxyAdmin 模式迁移到 **TimelockController 或等价 delayed executor**，并覆盖 ProxyAdmin 与直接 owner 权限。若 Mantle 的 exitDelay 仍为 `OPSuccinctL2OutputOracle.finalizationPeriodSeconds = 43200s`，Risk Chart 非红最低门槛为 **upgradeDelay >= 7.5d**，绿色目标为 **upgradeDelay >= 30.5d**。推荐目标不是刚好 7.5d，而是 **30.5d regular timelock + narrow emergency path**；若需要分阶段，第一阶段至少 **10d timelock + <=7d fallback** 以同时满足 Exit Window arithmetic 和 proposer-outage self-help timing。
3. 两条线可并行工程实施，但 **L2Beat 复核应按一个 internally consistent minimum pass package 提交**：Proposer fallback、core timelock、owner-role timelock、emergency bypass 限定、监控/公告/evidence package 一起交付，且 fallback timeout 必须小于有效退出窗口并留出安全余量。只修一个维度会留下明显审查缺口：只修 Proposer Failure 仍有 instant upgrade；只修 timelock 仍可能无法让用户在 proposer outage 下真实退出。

本 final 继承上游三个 final sections 的结论，并在 2026-05-21 对 L2Beat `main` 做 fresh source verification：`ff492aa2567be30d83a07b666f5cc9079bbd46a5` 仍显示 Mantle 通过 OP Stack template 评估，Mantle 项目配置未设置 `nonTemplateRiskView` 覆盖，`OpSuccinct` / `OpSuccinctFDP` 仍被映射到 `PROPOSER_CANNOT_WITHDRAW`，Exit Window 仍走 `EXIT_WINDOW(0, finalizationPeriod)`。同轮 Ethereum RPC read 仍显示 Mantle core `ProxyAdmin` 与 `OPSuccinctL2OutputOracle.owner` 为 MantleSecurityMultisig、oracle finalization period 为 43200s、zero-address permissionless proposer 未启用，且 deployed v2.0.1 oracle 没有 callable `fallbackTimeout()` getter。

Final promotion caveats are explicit: L2Beat must still confirm the exact Risk View mapping for this package; unrestricted emergency bypasses must be closed before Mantle claims pass status; and Mantle must provide storage-layout, audit, and deployed-parameter evidence before claiming live remediation.
...
#### 2.2 Fresh verification used in this final
...
2. **Parameter**: choose **7d fallback** for Phase 1 / minimum-pass when paired with a 10d regular timelock and 12h exitDelay. This keeps fallback activation within the 9.5d effective exit window and leaves roughly 2.5d for users to submit/complete exit actions before a queued unwanted upgrade can execute. A **14d fallback** may still be considered for a later conservative-prover posture, but only in a longer-timelock package where the effective exit window is comfortably greater than 14d. Either value should be documented as "fallback after last accepted output", not "governance may replace proposer".
...
| What timeout should be used? | Phase 1 minimum-pass: <=7d when paired with a 10d timelock / 9.5d effective window. A 14d upstream-default timeout belongs only in a longer-timelock package where the effective exit window is >14d with explicit margin, not in the Phase 1 minimum-pass package. |
...
3. **Set regular delay above the arithmetic minimum**. If `exitDelay=43200s`, exact minimum is 648000s = 7.5d. Recommended production phase:
   - **Phase 1 minimum**: 10d regular timelock, producing a 9.5d effective window, paired only with proposer fallback `<=7d` so users retain about 2.5d after fallback activation. A 14d fallback does not fit this Phase 1 minimum.
   - **Phase 2 target**: 31d regular timelock, producing a 30.5d effective window and a green-class arithmetic target. This target can accommodate a 14d fallback with much more exit margin if Mantle prefers OP Succinct's upstream default.
...
| 10d timelock for ProxyAdmin + critical owner roles | Non-red warning arithmetic: 9.5d effective window. | Must remove unrestricted emergency path; proposer fallback must be <=7d for minimum-pass consistency. | Strong minimum-pass candidate only with <=7d fallback; 14d fallback is not compatible with this minimum. | Medium-high, 6-10 weeks. | **Recommended Phase 1.** |
| 31d timelock for ProxyAdmin + critical owner roles | `31d - 0.5d = 30.5d`, green arithmetic target. | Same bypass closure required; can support 7d or 14d fallback with clear margin. | Strong target; may map to good if no primary instant route remains. | Medium-high, 8-12 weeks plus ops readiness. | **Recommended Phase 2 / target state.** |
...
| Fallback timeout must be shorter than effective exit window with explicit margin | Users must be able to wait for fallback activation and still exit before an unwanted queued upgrade executes. With 10d timelock and 12h exitDelay, 7d fallback leaves about 2.5d margin; 14d fallback requires a longer timelock. | Cross-dimension timing invariant from Proposer Failure + Exit Window interaction. |
...
| Proposer fallback <=7d + 10d core/owner timelock + no unrestricted emergency bypass | Self-help after timeout, with about 2.5d remaining exit margin after fallback activation. | Warning-class effective window. | Minimum credible L2Beat pass package. |
| Proposer fallback 14d + 10d core/owner timelock | Self-help activates after the effective exit window has already expired. | Warning-class arithmetic alone, but cross-dimension timing fails. | **Not a minimum-pass package**; excluded by the Phase 1 timing constraint. |
| Proposer fallback 7d or 14d + 31d core/owner timelock + constrained emergency path | Self-help after timeout, with substantial remaining exit margin. | Green arithmetic target if emergency path accepted as bounded. | Best-practice target package. |
...
2. Configure fallback timeout and publish exact parameter. **Gating constraint: with a 10d timelock and 12h exitDelay, Phase 1 / minimum-pass `fallbackTimeout` must be `<=7d`; this leaves about 2.5d margin inside the 9.5d effective exit window.**
...
- A 14d fallback paired with a 10d timelock / 9.5d effective window is **not** minimum-pass; users could be unable to wait for fallback activation and still exit before a queued unwanted upgrade executes. Under this revision's selected constraint, 14d fallback belongs only to a longer-timelock package whose effective exit window is >14d with explicit margin.
...
2. 31d regular timelock on ProxyAdmin and critical owner roles. This target has enough effective exit window to support a 14d fallback while retaining substantial margin.
...
|---|---|---:|---|---|
| Proposer fallback spec | Define fallback condition, timeout, proof route, prover tooling, emergency interactions, and the timing invariant `fallbackTimeout <= effectiveExitWindow - margin`. | 1-2 weeks | Mantle protocol/security, OP Succinct maintainers. | Design review. |
...
| Proposer outage | Trusted operator/governance recovery. | Users can trigger fallback after timeout with proof; Phase 1 minimum uses <=7d timeout so activation fits inside the 9.5d effective exit window. | Last-output-age alerts, public fallback guide, proof infra readiness, and countdown alerts that show remaining upgrade-delay margin. |
...
| P1 | Proposer fallback design and implementation | High | Medium-high | Without user self-help, Proposer Failure remains red and Exit Window remains practically weak; Phase 1 timeout must be <=7d unless timelock is raised. |
| P1 | Core ProxyAdmin + critical owner timelock migration | High | Medium-high | Without delayed upgrade path, Exit Window remains `None`; 10d timelock is minimum-pass only when paired with <=7d fallback. |
...
|---|---|---|---|---|---|
| A1 | Send L2Beat pre-review note asking whether a deployed OP Succinct `fallbackTimeout` plus source-available prover would be treated as self-propose or needs project override. | L2Beat liaison / protocol lead | This final. | Written response or issue/PR discussion captured. | Link to L2Beat forum/GitHub discussion. |
| A2 | Produce Mantle Proposer Fallback Specification. | Protocol engineering | A1 optional but recommended. | Spec defines timeout, proof route, mode interactions, roles, failure cases, and the invariant `fallbackTimeout <= effectiveExitWindow - explicit margin`; Phase 1 default is <=7d for a 10d timelock / 9.5d effective window. | Markdown spec with threat model. |
...
| A5 | Deploy oracle upgrade and configure fallback timeout. | Protocol ops / Safe signers | A4. | On-chain reads show new implementation, fallback timeout, unchanged critical state, successful output progression; if Phase 1 timelock is 10d, timeout read must be <=7d. | Deployment txs, verified source, post-deploy read log. |
| A6 | Deploy core rollup timelock with initial `minDelay >= 864000s`. | Governance / protocol ops | Role inventory. | Timelock source verified; roles assigned; delay cannot be reduced except through timelock self-call; if fallback timeout is 14d, this action must instead use a longer minDelay that leaves effective window >14d with margin. | Contract address, role read log, `getMinDelay()` output. |
...
| A11 | Prepare L2Beat evidence package and PR. | L2Beat liaison | A5-A10. | PR includes source permalinks, deployment txs, role reads, timing formulas, caveats, and a table proving `fallbackTimeout < effectiveExitWindow` with explicit margin. | GitHub PR / issue link. |
...
- Calculation table: `effectiveExitWindow = upgradeDelay - exitDelay`; current, Phase 1, Phase 2 values; and `fallbackTimeout <= effectiveExitWindow - margin`. For the Phase 1 recommendation, this table must show `10d - 0.5d = 9.5d`, `fallbackTimeout <= 7d`, and margin `>=2.5d`.
...
flowchart TD
  ...
  B -->|"Yes, after bounded timeout"| D{"Timeout fits inside effective exit window with margin?"}
  D -->|"No"| R3["Not minimum-pass: raise timelock or accept risk"]
  D -->|"Yes"| V{"Fallback is deployed and source-verified?"}
  V -->|"No, docs only"| R2["Not sufficient: evidence package only"]
  V -->|"Yes"| E{"Valid proof required and public prover usable?"}
...
gantt
  ...
  Configure proposer fallback <=7d; margin >=2.5d :a0, after a1, 7d
  Timelock + emergency spec                         :a2, after a1, 14d
  Deploy timelock and transfer ProxyAdmin/owners    :a3, after a2, 21d
  Set delay >=10d; effective window >=9.5d; fallback <=7d :milestone, m1, after a3, 1d
...
quadrantChart
  ...
  "Proposer fallbackTimeout spec <=7d for Phase 1": [0.45, 0.90]
...
| Source requirement | Coverage in this final | Notes / gaps |
|---|---|---|
...
| l2beat_project_pages (4) | Partially covered through prior final sections and current source/config. | This final does not rely on fresh page snapshots; evidence package should attach dated page snapshots for Mantle and comparators. |
...
| l2beat_review_process (2) | Partially covered by L2Beat config/source pattern and project-page evidence. | Need L2Beat pre-review / PR link once initiated; package must explicitly document fallback-timeout vs effective-exit-window margin. |

## 5. Gap Analysis

1. **L2Beat page snapshots are not persisted in this final.** The recommendation relies on source/config and chain reads; the evidence package should attach dated page snapshots for Mantle, Morph, OP Mainnet, Arbitrum, and any positive Exit Window comparator used in the L2Beat PR.
2. **L2Beat reviewer intent remains an open question.** The technical recommendation is designed to map to existing self-propose semantics, but only L2Beat can confirm whether OP Succinct `fallbackTimeout` should become a template-level change or Mantle-specific `nonTemplateRiskView`. This does not change the Phase 1 timing constraint: 10d timelock / 9.5d effective window requires `fallbackTimeout <=7d` for the minimum-pass package.
3. **Emergency path design requires Mantle policy input.** This final recommends constraining instant powers, but cannot select the final allowed selector/target list without Mantle's incident-response requirements.
...
| 2 | 2026-05-21 | Focused revision for major review finding. Chose option (a): Phase 1 / minimum-pass fallback must fit inside the effective exit window with explicit margin. Updated executive summary, Proposer Failure and Exit Window option matrices, minimum package, action items, re-review checklist, and diagrams so 10d timelock / 9.5d effective window requires <=7d fallback; 10d + 14d is no longer presented as minimum-pass. |
