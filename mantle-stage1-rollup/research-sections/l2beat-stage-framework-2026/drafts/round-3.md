...
# L2Beat Stage 框架最新标准解析（2026 版）—— Round 3 深度稿

> **持久化路径**: `mantle-stage1-rollup/research-sections/l2beat-stage-framework-2026/drafts/round-3.md`
>
...
- **要求**：L1 verifier 合约部署的 vkey（verification key）或 fault-proof game 合约配置必须可由**公开 circuit + 公开 setup artifacts** 或**公开 fault-proof 规范**独立重新生成，且与链上配置字节一致；
...
- **代表实例**：
  ...
  - **Facet v1 (OR — Round-3 关键证据)**：项目页快照显示 ~90 天降级倒计时至 Stage 0，原因之一是 verifier 合约重建标准未满足——这是 (5c) 对 OR 同等适用的关键事实（Facet 是 OR 项目，触发 (5c) 不达标）；
  - **OP Stack fault-proof game 合约**：通用情况下 onchain dispute game / fault-proof verifier 合约的 vkey 或 game 配置须可由公开 monorepo 独立重建。
...
| (5c) verifiers reproducible | ✅ 适用（含 fault-proof game 合约的 vkey / 配置可重建）；Facet v1 为命名 Fail | ✅ 适用 |
...
- **(5c) 适用（applicable: all）** —— 核验 Mantle 主网 verifier 合约 vkey 与 fault-proof 合约配置：
  - 是否可由公开 SP1 program ELF + SRS 独立重建 verifier vkey？需对比链上 verifier 合约 vkey 字节 vs 公开 SP1 build artifact；
  - 若 optimistic toggle 启用，fault-proof game 合约 (`absolutePrestate` 等) 的配置也须可由 mantle-v2 monorepo 独立重建；
  - 下游 issue 必须给出"链上 vkey hash / absolutePrestate → SP1 program build hash / Cannon prestate hash → 公开源码"的完整 traceability。
...
| **(5c) Verifiers reproducible (vkey/fault-proof game config)** (Round-3：从 ZK 特有移至通用) | all | grace-period-active | Stage 1 |
...
- 代入 item-5 四子项（**Round-3 按 applicability 全适用**）：
  ...
  - (5c) 适用（all）：Mantle 主网 verifier 合约 vkey 的独立重建 + fault-proof game 合约配置可重建（如启用 toggle）；
...
| 链上 vkey 是否可由公开 SP1 ELF 重建？fault-proof game 合约配置（若 toggle 启用）是否可重建？(5c applicable: all) | `proposer-decentralization-zk-compliance` |
