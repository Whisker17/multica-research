...
artifact_paths:
  ...
  draft: mantle-stage1-rollup/research-sections/l2beat-stage-framework-2026/drafts/round-3.md
...
final_metadata:
  approved_draft_path: mantle-stage1-rollup/research-sections/l2beat-stage-framework-2026/drafts/round-3.md
  approved_draft_round: 3
  approved_draft_commit: "https://github.com/Whisker17/multica-research/commit/e443c5fa36539f6fe4f8ade2294949eb08cae024"
  adversarial_approval_id: "50baa887-9b5a-44b3-931e-e81300e923ce"
  promoted_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  promoted_at: "2026-05-19T11:00:00Z"
  multica_issue_id: "4a3e2e68-83df-44d9-a856-63efb733084e"
  report_issue_id: "8785d7db-461f-4dcb-a11d-462017d9ae97"
  outline_rounds: 3
  deep_draft_rounds: 3
  polish_applied:
    - "§5 (5c) wording tightly scoped to 'validity / onchain verifier paths' (onchain verifier 合约 vkey 与源码可重建)；任何 fault-proof prestate / absolutePrestate / Cannon prestate 可重建性的引用从 (5c) 全部归口到 (5d-or)。受影响位置：§5.3 (5c) 要求与代表实例、§5.4 differential 表格、§5.7 (5c) Mantle 关联 bullet。"
    - "为保持内部一致性，下列非 §5 位置中混入 fault-proof prestate 语义的 (5c) 表述同步收紧：§7.2 通用项表 (5c) 行、§8.2 (a) Mantle proof system 评估、§8.2 (f) Mantle 开放问题清单 (5c) 行。"

draft_metadata:
  outline_round: 3
  outline_commit: "https://github.com/Whisker17/multica-research/commit/625f1c7"
  prior_draft_round: 2
  prior_draft_commit: "https://github.com/Whisker17/multica-research/commit/4c59d9ff87bfdab4e7b934ceaa008bef839425d3"
  prior_review_verdict_id: "ece6186c"
  draft_round: 3
  prepared_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  prepared_at: "2026-05-19T09:30:00Z"
  cutoff_date: "2026-05-19"
  items_covered: 9
  fields_investigated: 13
  diagrams_produced: 6
  source_requirements_covered: 6
  round_3_findings_handled:
    major:
      - "[M-r2-1] item-5 整体重构：原 'ZK Proving System Stage 1 四项要求' 改名为 'L2 Proving System Stage 1 四项要求'；按子项拆分适用范围——(5a) trusted-setup 红线对任何被评级的 proving system 适用、(5b) prover source 对任何项目运营的 proving 路径适用、(5c) verifier reproducibility 对所有 validity/onchain verifier 路径适用、(5d) program reproducibility 按证明类型分流（ZK program vs optimistic fault-proof prestate）。同步更新 §3.2(6) / §3.4 / §3.6 / §5 全节 / §5.7 Mantle 关联 / §7 三栏对照 / §7.3 边界陈述 / §7.4 Mantle 路径定位 / §8.2 Mantle 评估 / §10 diag-2 / diag-3 / diag-5 注释 / Facet v1 备注"
    minor:
      - "[m-r2-1] §11.3 src-3 Medium URL 补全：(i) 'Introducing Stages' 补充完整 URL https://medium.com/l2beat/introducing-stages-a-framework-to-evaluate-rollups-maturity-d290bb22befe；(ii) 原 '2026.02 ZK Proving System Stage 1 explainer Medium' 经独立网络搜索未能定位为已发布 Medium 文章（最可能场景是 Forum #413 主帖承担解释功能），从已覆盖来源中移除，替换为已验证的 L2BEAT Medium 'Native Rollups: Where they are, and where they are going'（Luca Donno + Conor McMenamin），并在 Gap Analysis 显式记录 2026.02 explainer 未定位的事实"
      - "[m-r2-2] §6.8 Stages 页面 URL 不一致修复：将 §6.8 中 https://l2beat.com/scaling/stages 统一为全文一致的 canonical URL https://l2beat.com/stages（与 §1.6 / §2.5 / §3.5 / §10 diag-5 等位置保持一致）"
      - "[m-r2-3] Unichain 验证状态修复：项目页 2026.05 字面 'Stage 1' 标签的独立直接核验未能在本研究 cutoff 前完成；按 Review Agent + Orchestrator 提供的二选一处理路径，本稿采取 'Forum #425 enumerated example only' 的保守归类，在 §1.4 / §3.5 / §9.5 / §9.9 / diag-2 注释 / §11.4 表格 / §12.1 Gap Analysis 等位置统一修订 Unichain 标签为 'Forum #425 列举示例（项目页字面 Stage 1 直接核验 pending）'，不再无前缀地标注为 'Stage 1 Pass'"
  ...
  named_examples_reverified_2026_05:
    ...
    - "Unichain [https://l2beat.com/scaling/projects/unichain]: Forum #425 enumerated example; 项目页字面 'Stage 1' 标签独立核验 pending；本稿不再作 'Stage 1 Pass' 标注，仅作 Forum #425 列举处理"
    ...
    - "Facet v1 [https://l2beat.com/scaling/projects/facet]: Stage 2 Optimistic Rollup confirmed; ~90 day downgrade countdown to Stage 0 due to proving-system requirements (trusted setup + verifier reproducibility) — Facet 是 OR 项目这一事实是 item-5 适用范围 zk-only → all 修订的关键交叉证据"
  ...
  scope_correction_propagation_map:
    - "§0 Executive Summary: item-5 命名 'ZK Proving System' → 'L2 Proving System'；2026.02 Forum #413 表述加入 'covers both ZK and optimistic proving systems' 限定"
    - "§3.2(6): 标题 'ZK Proving System Stage 1 四子项' → 'L2 Proving System Stage 1 四子项'；applicable_rollup_type 从 'zk-only' 改为按子项细分（5a/5b/5c all; 5d split by proof type）"
    - "§3.4 zk_vs_optimistic_differentials: 表格 (6) 行 'ZK Proving System 四子项 zk-only' 拆分为四行 (6a-6d)，分别标注 applicability"
    - "§3.6 Mantle 关联: (6) 'ZK Proving System 四子项全部适用' → 'Proving System 四子项全部适用（按 Mantle 当前 validity-proof 路径触发 5a-5d 全项）'"
    - "§5 整节: 标题 'item-5 — 2025.11→2026.02 迭代：ZK Proving System Stage 1 四项要求' → 'item-5 — 2025.11→2026.02 迭代：L2 Proving System Stage 1 四项要求'；§5.3 子项 applicability 从 'zk-only' 改为按子项细分；§5.4 zk_vs_optimistic_differentials 重写；§5.5 pass_fail 增加 OR 路径具名实例（Facet v1 的 trusted setup + verifier reproducibility 失败本质上是 OR 项目触发 5a/5c）"
    - "§7 三栏对照: ZK 特有项栏中 (5a)(5b)(5c) 移至通用栏（按子项 applicability all 重新归类）；(5d) 拆分为 (5d-zk) ZK program reproducibility（zk-only）+ (5d-or) fault-proof prestate reproducibility（optimistic-only）"
    - "§7.3 边界陈述: '不要将 ZK 四子项套用到 OR 路径' 表述被修正——(5a)(5b)(5c) 现在对 OR 同样适用；'OR 项目的 proof stack 透明度也开始应用类似原则' 从'借鉴'升级为'同等适用'"
    - "§7.4 Mantle 路径定位: 'ZK 特有项 (5a-5d)' 中的 ZK 限定改为 'L2 Proving System (5a-5d) 按子项 applicability 全适用'"
    - "§8.2 (a)-(b): SP1 verifier 评级 + fault-proof prestate 备用条款双重核验；Mantle 若启用 `SuccinctL2OutputOracle` 的 optimistic toggle fallback 时，(5a-5c) 仍适用，(5d) 改走 fault-proof prestate 路径"
    - "§10 diag-2: ZK 路径节点 'ZK Proving System 四子项' 修改为 'L2 Proving System 四子项'，OR 路径增加 'OR Proving System 四子项（5a/5b/5c all; 5d optimistic fault-proof prestate）' 节点；分支细节标注"
    - "§10 diag-3: ZK 特有栏 Z1-Z4 重组——Z1(5a)/Z2(5b)/Z3(5c) 移至通用栏；Z4 拆分为 zk-only (5d-zk) 与 optimistic-only (5d-or)；'不存在 validity-proof challenge period' 仍保留为 ZK-only 否定项"
    - "§10 diag-5 timeline: 2025.11 / 2026.02 节点描述将 'ZK setups' 修订为 'L2 proving setups (ZK + optimistic)'，与 Forum #409 + #413 原帖一致"
    - "Facet v1 备注: §1.4 / §6.3 / §5.5 中 'Facet 自身为 OR' 的脚注从'例外/借鉴'修订为'同等适用——Facet 是 item-5 适用范围覆盖 OR 的关键正面证据'"
...
# L2Beat Stage 框架最新标准解析（2026 版）—— Final Section

> **持久化路径**: `mantle-stage1-rollup/research-sections/l2beat-stage-framework-2026/final.md`
>
> **Source draft**: round-3 (approved, commit `e443c5fa36539f6fe4f8ade2294949eb08cae024`)；本稿在 round-3 approved draft 基础上仅应用 Review Agent round-3 non-blocking polish note（§5 (5c) 范围收紧），其余内容与 approved draft 字面一致。
>
...
>
> **Adversarial approval**: comment `50baa887-9b5a-44b3-931e-e81300e923ce`（verdict: approve, confidence 0.84, no critical/major findings）。
>
> **Final polish 摘要**: 本稿基于 round-3 approved draft，仅处理 Review Agent 提供的一项 non-blocking polish note —— 将 §5 (5c) "verifiers reproducible" 子项的措辞严格限定在 **validity / onchain verifier 路径**：onchain verifier 合约的 vkey 与源码可重建性归 (5c)；fault-proof prestate（如 `absolutePrestate` / Cannon prestate hash）的可重建性归 (5d-or)，避免两个独立子项交叉混并核验路径。为保持内部一致性，§7.2 通用项表 (5c) 行与 §8.2 (a)/(f) Mantle 评估中混入 fault-proof prestate 语义的 (5c) 表述也同步收紧。其余内容（含 §1-§4 / §6 / §9-§13、所有 Mermaid 图、Source Coverage、Gap Analysis）与 round-3 approved draft 字面一致。
>
> **Round-3 修订摘要（来自 approved draft，原样保留）**: round-3 基于 round-2 (commit `4c59d9f`) + Review Agent verdict `ece6186c` 提出的 1 major + 3 minor findings 完成修订。Round-1 / Round-2 中 1 critical + 3 major 已确认修复。round-3 核心修订是 item-5 适用范围从 "zk-only" 修正为 "L2 proving system（按子项细分 ZK / Optimistic）"，并将该修订一致传播至 §0 / §3 / §5 / §7 / §8 / §10 diag-2/diag-3/diag-5 与 Facet v1 备注。其余三个 minor finding（Medium URL 补全、§6.8 URL 一致性、Unichain 验证状态）按 frontmatter `round_3_findings_handled` 逐条修订。结构层 9 items / 13 fields / 6 diagrams / 6 source requirements 保持与 Round-1 / Round-2 一致。

---

## 0. Executive Summary

L2Beat Stages 框架自 2023 年 6 月由 Luca Donno 首次提出以来，已成为 Ethereum L2 trust-minimization 进展的事实标准评估体系。本研究锁定 2026 年 5 月时点，系统梳理 Stage 0 / Stage 1 / Stage 2 在八个维度（Proof System / Sequencer / Proposer / Upgrade / Exit Window / Security Council / Data Availability / L2 Proving System Transparency）上的全部规范文本与实际执行状态，并重点解读 2024.12–2026.04 之间发生的**四项重大框架更新**：
...
2. **2025.11 → 2026.02** — **L2 Proving System Stage 1 要求**的两次迭代：Forum #409（2025.11）首发题为 "New Stage 1 requirements for ZK setups"，但其内容已涉及 trusted-setup 评级与 verifier / prover 透明度（在执行层既覆盖 validity proof 也覆盖 optimistic 路径中的 ZK-wrapped 组件）；Forum #413（2026.02.16）正式收敛为四项可验证条件，**标题与文本明确覆盖 L2 proving systems 全体——既包括 ZK programs / onchain validity verifiers，也包括 optimistic fault-proof prestates / 任何使用 trusted-setup 的 onchain verifier 部署**。Facet v1（一个 OR 项目）正是因 trusted-setup + verifier-reproducibility 不达标被列入约 90 天降级倒计时——这是 item-5 适用范围覆盖 OR 的**关键正面证据**。grace period 约六个月（截止约 2026-08-16；`enforcement_status: grace-period-active`）。
...
4. **2026.04** — Forum #425（发布于 2026.04.30）将 Optimistic Rollup 的最低 challenge period 从 7 天降至 5 天，五个 OR 项目（Arbitrum One、Base、OP Mainnet、Ink、Unichain）被 Forum 帖原文列举为继续保留 Stage 1。该规则**仅适用 Optimistic Rollup**，**不适用 validity-proof / ZK 路径**（`enforcement_status: enforced-on-project-pages` for OR; `not-applicable` for ZK）。注：Unichain 项目页字面 "Stage 1" 标签的独立直接核验在本研究 cutoff 前未完成，本稿统一将其归类为 "Forum #425 列举示例"，不再做"Stage 1 Pass"标注（详见 §3.5 / §9.5 / §12.1）。

**2026.05 当前 Stage 分布快照（补充背景）**：截至 cutoff 2026-05-19，L2Beat 框架在公开项目页快照中明确出现的 Stage 2 项目为 **Facet v1**（Stage 2 Optimistic Rollup，但项目页同时显示约 **90 天**降级倒计时——原因是其 proving system 不满足 upcoming Stage 1 要求；**Facet 是 OR 项目这一事实是 item-5 (5a)/(5c) 等子项 applicability 覆盖 OR 的核心证据**）。Arbitrum One、Base、OP Mainnet、Ink、Scroll、Starknet 等主流 rollup 仍处 Stage 1；Unichain 由 Forum #425 列举（项目页字面直接核验 pending）；Mantle 为 Stage 0（5/5 Stage 0 要求满足）。

**核心边界结论**（贯穿全稿）：

- **Stage 0 / Stage 1 / Stage 2 在 proof-system 维度的边界**：Stage 0 要求 "state-reconstruction software 可用 + 至少 5 个外部参与者可提交 fraud proof（OR 路径）"；Stage 1 要求 "Security Council compromise condition + proposer-set liveness assumption + L2 Proving System Stage 1 四子项透明度（按 5a-5d applicability 适用）"；Stage 2 要求 "完全 permissionless fraud proof submission + Security Council 仅介入可裁决 onchain bug"。Stage 1 **不要求**任意外部 actor 都能提交 fraud proof——后者属 Stage 2。
...
- **L2 Proving System Stage 1 四子项 applicability（Round-3 重大修订）**：
  - (5a) "no 🔴 trusted setups" 红线 → 适用于**任何被评级的 proving system 部署**（含 OR 项目使用的 ZK-wrapped verifier 或 fraud-proof 路径中的 SNARK 组件），applicable_rollup_type: **all**；
  - (5b) "prover source code published" → 适用于**项目运营的 proving 路径**（既覆盖 ZK validity prover，也覆盖 optimistic 路径中的 fault-proof prover / challenger 组件），applicable_rollup_type: **all**；
  - (5c) "verifier reproducibility" → 适用于**所有 validity / onchain verifier 路径**（含 OR 项目通过 onchain verifier 合约执行 fraud-proof game 的部署），applicable_rollup_type: **all**；
  - (5d) "program / prestate reproducibility" → **按证明类型分流**：ZK 路径走 "zkVM program reproducibility"（zk-only），OR 路径走 "optimistic fault-proof prestate reproducibility"（optimistic-only）。
  - 同 Forum #413 原文："consistent transparency standards across the rollup technology stack"——透明度标准在 OR 与 ZK 之间应用一致。
- **Mantle 直接关联**：Mantle 当前为 Stage 0（5/5 Stage 0 要求满足），主路径走 OP Succinct + SP1 validity-proof，但 `SuccinctL2OutputOracle` 保留 optimistic toggle fallback（fault-proof 模式）。**适用** 外的 SC upgrade ≥5d 规则（当前 Mantle 大部分合约升级延迟为 0，构成显式 gap）；**适用** L2 Proving System Stage 1 四子项全部（5a/5b/5c 因主路径是 validity-proof，5d 走 ZK program reproducibility；若启用 optimistic toggle，5d 改走 fault-proof prestate 路径，但 5a/5b/5c 仍同等适用）；**不适用** Forum #425 的 5d OR challenge 规则；**不适用** Stage 0 "≥5 external actors fraud-proof"——替代要求是 verifier/program 可重建性 + permissionless prover 路径。
...
- **L2 Proving System Stage 1 四子项透明度（Round-3 标识）**：按 (5a-5d) applicability 适用（详见 item-5）。

注意：Stage 1 **不要求** "anyone can submit fraud/validity proof"——这是 Stage 2 的完全 permissionlessness 要求。Stage 1 的"用户退出"由 **Council compromise + proposer liveness + 透明度可重建** 共同提供，**不**等价于 fraud-proof 提交完全开放。
...
|------|---------|---------|---------|
| Proof system 提交集合 | OR: ≥5 external actors 可提交 fraud proof | Council compromise + proposer-set liveness + L2 proving system 透明度（item-5）（不要求任意 external） | Fully permissionless submission |
...
- **Stage 1 示意（截至 2026.05）**：Arbitrum One、Base、OP Mainnet、Ink（Forum #425 四个项目页字面快照确认的 OR Stage 1）、Scroll、Starknet（ZK 路径，Stage 1 但 Walkaway Test grace period 内；Starknet 同时处于 ~175 天向 Stage 0 降级倒计时）、Kinto（Arbitrum Orbit OR，Stage 1）；**Unichain**（Forum #425 列举的第五个 OR 项目，项目页字面 "Stage 1" 标签直接核验在本研究 cutoff 前未完成，**本稿仅作 Forum #425 enumerated example 处理**，不做独立的"Stage 1 Pass"标注，详见 §3.5 / §9.5 / §12.1 Gap）。
- **Stage 2 示意**：**Facet v1**（OR）—— 项目页快照 [https://l2beat.com/scaling/projects/facet] 显示 "Facet v1 is a Stage 2 Optimistic Rollup"；同时记录约 **90 天**降级倒计时至 Stage 0，原因是其 proving system 不满足 upcoming Stage 1 要求（trusted setup + verifier 合约重建标准）。**修订说明**：Round-1 草稿曾断言"截至 2026.05 未发现已正式认定 Stage 2 的 L2 项目"——该断言在 Round-2 已被修正。**Round-3 进一步澄清**：Facet 作为 OR 项目被相同的 proof-system transparency 要求触发降级，正是 item-5 适用范围从 "zk-only" 修正为 "L2 proving system（按子项细分）" 的关键事实证据。其它主流 rollup（Arbitrum One、OP Mainnet 等）仍处 Stage 1 演进中。

### 1.5 relevance_to_mantle

Mantle 当前位于 Stage 0；进入 Stage 1 的核心路径是消解 (1) 升级延迟为 0 → outside-SC upgrade exit window 缺口；(2) Security Council 量化阈值 / 外部成员比例尚未达标；(3) L2 Proving System Stage 1 四子项的可重建性披露（按 Mantle 当前 validity-proof 主路径 + 可选 optimistic toggle fallback 双轨评估）。本 item 提供概念坐标系，具体差距由 item-3 / item-5 / item-8 与下游三个 issue 展开。
...
- L2Beat Medium "Introducing Stages — a framework to evaluate rollups maturity"，Luca Donno，2023.06（Round-3 补全完整 URL）：https://medium.com/l2beat/introducing-stages-a-framework-to-evaluate-rollups-maturity-d290bb22befe
...
- L2Beat Stages 主页快照（2026.05）：https://l2beat.com/stages
...
- Facet v1 作为目前唯一公开 Stage 2 项目，其类型为 OR（非 ZK）；这一事实强化了 item-5 适用范围覆盖 OR 的判断。是否会进一步出现 ZK 路径 Stage 2 实例？这是观察 framework 未来走向的关键样本。
...
- L2Beat Stages 页面 2026.05 快照：https://l2beat.com/stages
...
Stage 1 是当前 L2Beat 框架内最复杂、判定条件最多的阶段，2026.05 时点共涉及六个判定维度：(1) Proof System 存在性与运行（继承 Stage 0）、(2) Security Council 量化阈值、(3) Exit Window 三轨规则、(4) Operator Liveness、(5) Walkaway Test（item-4 详述）、(6) **L2 Proving System Stage 1 四子项**（item-5 详述，**Round-3 修订：名称从 "ZK Proving System" 改为 "L2 Proving System"；按子项 applicability 适用，不再统一为 zk-only**）。本节为整篇研究的**核心判定矩阵**，每一项必须可被独立打勾 / 打叉。
...
- 任何 rollup（OR 或 ZK）还需满足 item-5 的四子项透明度要求，applicability 按子项细分。
...
#### (6) L2 Proving System Stage 1 四子项（**Round-3 标题与适用范围修订**）

- 详细规范：见 item-5；
- enforcement_status: `grace-period-active`（Forum #413 设定的 ~6 个月倒计时约 2026-08-16）；
- **applicable_rollup_type（按子项细分）**:
  - (5a) no 🔴 trusted setups → applicable: **all**（任何被评级的 proving system 部署，含 OR 项目的 ZK-wrapped verifier 或 fraud-proof game 的 SNARK 组件）；
  - (5b) prover source code published → applicable: **all**（项目运营的 proving 路径，覆盖 ZK validity prover 与 optimistic fault-proof prover/challenger 软件）；
  - (5c) verifiers reproducible → applicable: **all**（含 OR 项目通过 onchain verifier 合约执行 fraud-proof game 的部署）；
  - (5d) program / prestate reproducibility → applicable: **按 proof type 分流**：ZK 路径走 zkVM program reproducibility（zk-only），OR 路径走 fault-proof prestate reproducibility（optimistic-only）。
- **Forum #413 适用范围核心证据**：Facet v1 作为 OR 项目，因 trusted-setup 与 verifier-reproducibility 不达标被列入约 90 天降级倒计时——验证 (5a)(5c) 子项对 OR 同等适用（详见 §5）。

### 3.3 stage_boundary（Stage 1 与相邻阶段的边界）

Stage 1 的 proof-system 要求**不是** "anyone can submit fraud/validity proof"。Stage 1 的核心是三个独立保障：
...
- **L2 Proving System 透明度** = (5a-5d) 按子项 applicability 适用，确保 proving system 不依赖 closed-source / unverifiable 组件。

而 "anyone can submit" 是 **Stage 2** 要求。Stage 0 的 "≥5 external actors" 是更弱的有限可提交集合。三个阶段在 proof-system permissionlessness 维度的演进：Stage 0 (≥5 external actors for OR) → Stage 1 (Council compromise + proposer liveness + 透明度可重建) → Stage 2 (fully permissionless submission)。
...
| (6a) item-5 (5a) no 🔴 trusted setups | ✅ 适用（含 OR 中 ZK-wrapped verifier / SNARK 组件，Facet v1 案例确认） | ✅ 适用 |
| (6b) item-5 (5b) prover source published | ✅ 适用（含 fault-proof prover 软件） | ✅ 适用 |
| (6c) item-5 (5c) verifier reproducibility | ✅ 适用（含 onchain fraud-proof verifier 合约） | ✅ 适用 |
| (6d) item-5 (5d) program / prestate reproducibility | ✅ 适用——走 optimistic fault-proof prestate 路径 | ✅ 适用——走 zkVM program 路径 |
...
- **Forum #425 enumerated example — Unichain**（OR）：项目页 URL [https://l2beat.com/scaling/projects/unichain] 存在；本研究 cutoff 前**未完成**项目页字面 "Stage 1" 标签的独立直接核验。按 Review Agent + Orchestrator 建议的二选一处理路径，本稿采取**保守归类**："Forum #425 列举示例（项目页字面 Stage 1 直接核验 pending）"，**不再无前缀地标注为 'Stage 1 Pass'**。下游 issue 在 grace period 结束前应独立完成项目页字面字符串核验，再决定是否升级为 Pass 标签。
- **Pass — Kinto**（OR，Arbitrum Orbit）：项目页快照 [https://l2beat.com/scaling/projects/kinto] 显示 Stage 1（≥5 external Challengers、6d 8h challenge period、7d 非 SC L2 升级 exit window）。**注意**：Kinto 的 walkaway 状态是本稿 §4.4 重点修订点——Kinto 仅在 Forum #412 中作为 "assigned Security Council minority as permissioned challenger" 的理论反例被引用，项目页快照**未发现**显式 "does not pass the walkaway test" 标签字符串。
...
- **Stage 1 当前 + 降级倒计时至 Stage 0 — Starknet**（ZK）：项目页快照 [https://l2beat.com/scaling/projects/starknet]。**当前 Stage 1**（项目页快照证实，非 Round-1 误述的 "Stage 0"）；同时记录约 **175 天**降级倒计时至 Stage 0，引用原因："Not all program sources are public or not all program hashes can be independently regenerated"（与 item-5 (5b)(5d-zk) 直接对应）。Walkaway 维度独立：项目页同时记录 "does not pass the walkaway test"。Starknet 团队针对 Forum #412 提交了 counterproposal（详见 §4.5）。
...
- (6) L2 Proving System Stage 1 四子项（**Round-3 修订**）：**全部适用** ——
  - (5a) no 🔴 trusted setups → applicable (all)：核验 Mantle 使用的 SP1 verifier 的 trusted-setup 评级；
  - (5b) prover source published → applicable (all)：核验 Mantle prover 部署链路源码；
  - (5c) verifiers reproducible → applicable (all)：核验 Mantle 主网 verifier 合约 vkey；
  - (5d) program / prestate reproducibility → 按 Mantle 当前 proof type 分流：
    - **主路径 validity-proof**：走 (5d-zk) ZK program reproducibility（SP1 ELF → mantle-v2 / op-succinct 源码 traceability）；
    - **fallback optimistic toggle**：若启用 `SuccinctL2OutputOracle` 的 optimistic 模式，(5d) 改走 (5d-or) fault-proof prestate reproducibility（MIPS / RISC-V interpreter 初始状态可由公开 monorepo 独立重建）。
  - 详见 §5.7 与下游 `proposer-decentralization-zk-compliance` issue。
...
- Forum #413 (L2 Proving System Stage 1 四项要求, 2026.02.16)：https://forum.l2beat.com/t/413 （注：通过 Forum 主页索引访问；本稿以 forum.l2beat.com 主索引为入口）
- Forum #409 (Stage 1 requirements for ZK setups, 2025.11)：https://forum.l2beat.com/t/new-stage-1-requirements-for-zk-setups/409
...
- Unichain 项目页 2026.05 字面 "Stage 1" 标签的独立直接核验：在本研究 cutoff 前未完成，下游 issue 需独立核验（详见 §12.1 Gap）；本稿采用"Forum #425 enumerated example only"的保守归类。
- (5a)/(5c) 对 OR 项目的具体核验执行：Forum #413 文本明确"consistent transparency standards"，但项目页快照中具体哪些 OR 项目（除 Facet v1 外）触发或满足 (5a)/(5c) 的列表暂未在单一公开文档中汇总。
...
- **Starknet**（ZK）：项目页快照 [https://l2beat.com/scaling/projects/starknet]，确认两段独立陈述同时存在：(i) 当前 Stage 1（非已降级）+ 约 175 天降级倒计时至 Stage 0（原因引用：not all program sources are public / not all program hashes can be independently regenerated；详见 item-5 (5b)(5d-zk)）；(ii) "The project does not pass the walkaway test"——独立的 Walkaway Test 维度。Forum #412 列举其使用 3/12 Security Council minority 作为 backup prover 为典型 walkaway 失败模式；Starknet 团队提交了 counterproposal（见 §4.5）。
...
## 5. item-5 — 2025.11→2026.02 迭代：L2 Proving System Stage 1 四项要求（**Round-3 重命名与适用范围修订**）

### 5.1 requirement_summary

L2Beat 在 2025.11（Forum #409 "New Stage 1 requirements for ZK setups"，标题虽含 "ZK setups"，但帖文内容已涉及任何使用 trusted-setup / SNARK 组件的 L2 proving 部署）首次提出三项要求与 trusted-setup 红线；2026.02.16（Forum #413 + 关联解释材料）将要求收敛为**四项可验证条件**，并设定约**六个月 grace period**（截止约 2026-08-16）。**Round-3 重大修订**：Forum #413 标题与文本明确覆盖 L2 proving systems 全体——既包括 ZK programs / onchain validity verifiers，也包括 optimistic fault-proof prestates / 任何使用 trusted-setup 的 onchain verifier 部署。因此本节命名从 Round-2 的 "ZK Proving System Stage 1 四项要求" 修正为 "L2 Proving System Stage 1 四项要求"；各子项 applicability 也按 (5a-5d) 细分（不再统一 zk-only）。这一修订的关键事实证据是 Facet v1（OR）因 trusted-setup 与 verifier-reproducibility 不达标被纳入约 90 天降级倒计时。
...
2025.11 Forum #409 New Stage 1 requirements for ZK setups
    ↓ (三项初始要求 + trusted-setup 红线；标题虽含 'ZK setups' 但 R3 时代解读已扩展至所有使用 SNARK / trusted-setup 的 L2 proving 部署)
2026.02.16 Forum #413
    ↓ (收敛为四项可验证条件 + ~6 个月 grace period；标题与文本明确覆盖 L2 proving systems 全体——ZK 与 optimistic 两类)
2026.08.16 (估算) grace_period_end
    ↓ (grace period 后正式 enforced)
```

### 5.3 evaluation_criteria（四个独立子项，**Round-3 applicability 按子项细分**）
...
> **Round-3 适用范围总览**: (5a)(5b)(5c) 三项的 applicability 修订为 **all rollup types**（OR 与 ZK 统一适用，按 proving 部署的实际 trusted-setup / verifier 组件触发）；(5d) 按 proof type 分流——ZK 路径走 zkVM program reproducibility（zk-only），OR 路径走 optimistic fault-proof prestate reproducibility（optimistic-only）。同 Forum #413 原文 "consistent transparency standards across the rollup technology stack"。
...
- **判定标准**（继承 Forum #381 评级体系）：
  ...
  - 🔴 Red: 任一 yellow 条件未满足。
- **Stage 1 红线**：Stage 1 不允许任何 proving system 使用 🔴 评级的 trusted setup。
- **applicable_rollup_type**: **all**（任何被评级的 proving system 部署——含 OR 项目使用的 ZK-wrapped verifier、SNARK 组件、aggregation/recursion 层）；
- **来源**：Forum #381（评级方法）+ Forum #413（红线）+ 关联 explainer。
- **代表实例**（Forum #381 列举）：
...
- **Forum #413 受影响项目（含 OR 项目 — Round-3 关键证据）**：Facet V1（OR）, Zircuit, Loopring L2。Facet v1 作为 OR 项目因 trusted-setup 不达标被列入约 90 天降级倒计时，证明 (5a) 对 OR 同等适用。

#### (5b) Prover source code published

- **要求**：prover 实现（含 circuit、witness generator、aggregation 层、recursion、final wrap prover；或 optimistic 路径的 fault-proof prover / challenger 软件）必须以**可审计形式**公开（permissive license 或至少 source-available）；
- **目的**：使用户在 operator downtime 时能独立生成 proof / fault-proof 提交至 L1 bridge；
...
- **来源**：Forum #409 R3 + Forum #413 + 关联 explainer。
- **applicable_rollup_type**: **all**（项目运营的 proving 路径，覆盖 ZK validity prover 与 optimistic fault-proof prover/challenger 软件）
- **代表实例**：
  ...
  - Starknet：项目页快照引用降级原因之一为 "not all program sources are public"——与 (5b) / (5d-zk) 直接对应；SHARP verifier 与 prover 部分公开度待核验；
  - OR 适用示例：Optimism Cannon / Arbitrum BoLD 等 fault-proof game 实现的 prover/challenger 软件须可审计——这是 (5b) 对 OR 适用的具体表征。

#### (5c) Verifiers reproducible

- **要求（final 收紧：范围限定为 validity / onchain verifier 路径）**：L1 部署的 **onchain verifier 合约本体**（含 validity-proof verifier 合约，以及 OR 项目在 fault-proof game 内嵌入的 ZK-wrapped / onchain SNARK verifier 合约组件，如有）的 vkey（verification key）与合约源码必须可由**公开 circuit + 公开 setup artifacts** 独立重新生成，且与链上 verifier 合约字节一致；
- **范围澄清（final polish）**：本子项**仅覆盖** onchain verifier 合约 vkey 与源码可重建性；**fault-proof prestate 的可重建性**（如 OP Stack 的 `absolutePrestate`、Cannon / BoLD prestate hash）**不在 (5c) 范围内**，由 (5d-or) 单独覆盖。这一边界与 Forum #413 "source code for all used onchain verifiers must be available online" 的字面范围一致——原文锚定的是 onchain verifier smart contracts，而非 OR 的 prestate；
...
- **来源**：Forum #409 R2 + Forum #413 + 关联 explainer。
- **applicable_rollup_type**: **all**（所有 validity / onchain verifier 路径，含 OR 项目通过 onchain verifier 合约执行 fraud-proof game 的部署）
- **代表实例**：
  - Matter Labs（ZKsync Era）：关联 explainer 提到具备 reproducibility 工具；
  - Polygon zkEVM：同上；
  - **Lighter L2**：关联 explainer 明确指出 "verifier sources remain unpublished"——Fail (5c)；
  - **Facet v1 (OR — Round-3 关键证据)**：项目页快照显示 ~90 天降级倒计时至 Stage 0，原因之一是 onchain verifier 合约重建标准未满足——这是 (5c) 对 OR 同等适用的关键事实（Facet 是 OR 项目，触发 onchain verifier 合约 (5c) 不达标；与 fault-proof prestate 是否可重建是另一个独立子项 (5d-or)，不应混并）；
  - **OP Stack 中的 onchain verifier 合约**：若 dispute game 内嵌 ZK-wrapped / onchain SNARK verifier 合约组件，该 onchain verifier 合约的 vkey 与源码须可由公开 monorepo 独立重建。**注**：dispute game 的 `absolutePrestate` 等 prestate 配置的可重建性**不属** (5c) 范围，由 (5d-or) 覆盖。

#### (5d) Program / prestate reproducibility（**Round-3 按 proof type 分流**）

- **要求（ZK 路径，(5d-zk)）**：rollup STF 所对应的 ZK program（如 SP1 ELF / RISC-V binary）必须可由公开源码独立重建，且其 program hash 与 verifier 配置中的 hash 一致；
- **要求（OR 路径，(5d-or)）**：fault-proof prestate（如 MIPS / RISC-V interpreter 初始状态、Cannon / BoLD prestate hash）必须可由公开 monorepo 独立重建，且与 dispute game 合约配置一致；
...
- **applicable_rollup_type（按 proof type 分流）**:
  - (5d-zk) → **zk-only**：ZK program reproducibility；
  - (5d-or) → **optimistic-only**：fault-proof prestate reproducibility；
  - 对 hybrid 设计（如 OP Succinct + optimistic toggle）：当前 active proof type 决定走哪条；如启用 fallback 则切换。
- **来源**：Forum #413 + 关联 explainer。
- **代表实例**：
  - **Starknet ((5d-zk))**：项目页快照引用降级原因之一为 "not all program hashes can be independently regenerated"——与 (5d-zk) 直接对应；
  - **OP Stack 主网 ((5d-or))**：Cannon / OPCM 等 fault-proof prestate hash 须可由 optimism monorepo 独立重建，且与 dispute game `absolutePrestate` 配置一致；
  - **Mantle**（hybrid 路径示例）：主路径 validity-proof → 走 (5d-zk)；若启用 `SuccinctL2OutputOracle` optimistic toggle → 改走 (5d-or)，需 mantle-v2 fault-proof prestate 可重建。

#### (5e) 复合判定

- 任一子项不通过即在 grace period 结束后降级 Stage 0；
- 与 item-4 Walkaway Test 复合：任何 Stage 1 rollup（OR 或 ZK）必须**同时**满足 (5a-5d) applicability 范围内的子项与 Walkaway Test 才能保留 Stage 1（前提是 Walkaway Test 实际进入 `enforced-on-project-pages` 状态）；
- Forum #413 强调"应用一致的透明度标准"（"consistent transparency standards across the rollup technology stack"），这是 Round-3 修订的规范文本依据。

#### (5f) Enforcement 追踪（**Round-3 applicability 列重写**）

| 子项 | applicable_rollup_type | enforcement_status | effective_date | grace_period_end | 受影响命名项目（截至 2026.05 快照） |
|------|------------------------|---------------------|----------------|------------------|----------------|
| (5a) no 🔴 trusted setups | **all** | grace-period-active | 2026.02.16 (Forum #413) | ~2026-08-16 | Facet V1 (OR — 关键证据), Zircuit, Loopring L2 |
| (5b) prover source published | **all** | grace-period-active | 2026.02.16 | ~2026-08-16 | Starknet (not all program sources public); Linea (6m fallback 满足边界) |
| (5c) verifiers reproducible | **all** | grace-period-active | 2026.02.16 | ~2026-08-16 | Lighter L2 (未公开); Facet v1 (OR — verifier regeneration 不达标，~90d 降级倒计时 — 关键证据) |
| (5d-zk) zkVM programs reproducible | **zk-only** | grace-period-active | 2026.02.16 | ~2026-08-16 | Starknet (not all program hashes can be independently regenerated) |
| (5d-or) fault-proof prestate reproducible | **optimistic-only** | grace-period-active | 2026.02.16 | ~2026-08-16 | （示例：OP Stack 主网项目的 Cannon / BoLD prestate 可重建性核验） |

### 5.4 zk_vs_optimistic_differentials（**Round-3 重写**）

| 子项 | OR | ZK |
|------|----|----|
| (5a) no 🔴 trusted setups | ✅ 适用（含 ZK-wrapped verifier / SNARK 组件）；Facet v1 为命名 Fail | ✅ 适用 |
| (5b) prover source published | ✅ 适用（含 fault-proof prover / challenger 软件） | ✅ 适用 |
| (5c) verifiers reproducible | ✅ 适用（限 **onchain verifier 合约** 的 vkey 与源码可重建——含 OR 项目在 fault-proof game 内嵌入的 ZK-wrapped / onchain SNARK verifier 合约组件；**fault-proof prestate 可重建性由 (5d-or) 覆盖，不重复在此**）；Facet v1 为命名 Fail | ✅ 适用 |
| (5d-zk) zkVM program reproducibility | ❌ 不适用 | ✅ 适用 |
| (5d-or) fault-proof prestate reproducibility | ✅ 适用 | ❌ 不适用 |
| hybrid 设计 (ZK + optimistic toggle) | (5d-or) | (5d-zk) | 按 active proof type 决定，可切换 |

**Round-3 解读**：(5a)(5b)(5c) 三项在 OR 与 ZK 之间是**对称同等适用**的——任何使用 trusted-setup、运营 prover 软件、部署 onchain verifier 合约的 L2 都需要满足；(5d) 是按 proof type 分流的非对称项。Round-2 时代将整个 item-5 标注为 zk-only 是误读，Facet v1 案例已确认这一误读会导致下游对 OR / hybrid / OP Stack proof-stack 透明度检查的错误排除。

### 5.5 pass_fail_examples（截至 2026.05 应然层 + 实然层，**Round-3 增加 OR 命名实例**）

每个命名实例附 L2Beat 项目页 2026.05 快照链接 / Forum 原文位置：

- **(5a) Fail — OR 路径**：**Facet v1** [https://l2beat.com/scaling/projects/facet]（OR；trusted-setup 不达标，~90d 降级倒计时；**Round-3 关键证据**），Zircuit, Loopring L2（关联 explainer 列举 red trusted setup）；
- **(5b) Pass — ZK 路径**：Scroll [https://l2beat.com/scaling/projects/scroll]（"source available prover"，项目页快照）；
- **(5b)(5d-zk) Fail — ZK 路径具名**：Starknet [https://l2beat.com/scaling/projects/starknet]——项目页快照引用降级原因 "not all program sources are public or not all program hashes can be independently regenerated"，明确同时触及 (5b) + (5d-zk)；
- **(5c) Fail — OR 路径**：**Facet v1** [https://l2beat.com/scaling/projects/facet]（项目页快照 ~90 天降级原因之一是 verifier 重建未达标；**Round-3 关键证据**）；**(5c) Fail — ZK 路径**：Lighter L2（关联 explainer "verifier sources unpublished"）；
- **(5d-zk) 应然 Fail（具名）**：Starknet（同上）；
- **(5d-or) 适用核验范围（示例）**：OP Stack 主网项目（Optimism / Base / Mantle hybrid fallback path）需核验 Cannon / BoLD prestate hash 与 monorepo 源码 traceability；本研究 cutoff 前未发现具名 (5d-or) Fail 项目页快照——下游 issue 应承担核验；
...
- 2026.02 Forum #413：将上述三项重排为四子项，**新增 (5d) zkVM program / fault-proof prestate 可重建**，且收敛为可独立打勾的可验证条件；
- 关键差异：2026.02 版本明确**setup 与 program / verifier reproducibility 是独立子项**（避免将三者打包为一项 "transparency"），且文本明确覆盖 L2 proving systems 全体（OR + ZK）；
- **Round-3 修订**：Round-2 草稿将 item-5 整体标注为 zk-only 是适用范围误读——Forum #413 标题与文本均覆盖 L2 proving systems 全体；Facet v1（OR）的命名 Fail 是这一适用范围的关键事实证据。本稿按子项细分 applicability。

### 5.7 relevance_to_mantle（OP Succinct + SP1 路径四子项直接关联，**Round-3 增加 hybrid fallback 评估**）

- **(5a) 适用（applicable: all）** —— 核验 Mantle 使用的 SP1 verifier 的 trusted-setup 评级：
...
- **(5b) 适用（applicable: all）** —— 核验 Mantle prover 部署链路源码：
  - SP1 上游（Succinct 维护）已开源；
  - mantle-v2 / op-succinct fork 中是否引入闭源 prover 服务？需核验 mantle-v2 仓库 prover 路径；
  - **hybrid 评估**：若 `SuccinctL2OutputOracle` optimistic toggle 启用，fault-proof challenger 软件（Cannon-like 组件）的源码可审计性也需同步核验；
  - 下游 issue: `proposer-decentralization-zk-compliance` 核验入口。

- **(5c) 适用（applicable: all；final polish：严格限定在 validity / onchain verifier 路径）** —— 核验 Mantle 主网 **onchain verifier 合约** 的 vkey 与源码可重建性：
  - 是否可由公开 SP1 program ELF + SRS 独立重建 verifier vkey？需对比链上 verifier 合约 vkey 字节 vs 公开 SP1 build artifact；
  - 若 optimistic toggle 启用且 dispute game 内嵌 ZK-wrapped / onchain SNARK verifier 合约组件（如有），同样核验该 onchain verifier 合约的 vkey 与源码可重建性——这是 (5c) 对 OR 路径的对称同等适用面；
  - 下游 issue 必须给出"链上 onchain verifier 合约 vkey hash → SP1 program build hash → 公开源码"的完整 traceability；
  - **注**：fault-proof prestate 可重建性（`absolutePrestate` / Cannon prestate hash → mantle-v2 monorepo）**不在本子项范围内**，由下条 (5d-or) 单独承担，避免与 (5c) 混并核验路径。

- **(5d) 适用，按当前 active proof type 分流** ——
  - **主路径 validity-proof → 走 (5d-zk)**：Mantle SP1 program ELF 与 mantle-v2 / op-succinct 源码对应；program hash → SP1 ELF → mantle-v2 monorepo 的可追溯链路；
  - **fallback optimistic toggle → 走 (5d-or)**：Mantle fault-proof prestate（MIPS / RISC-V interpreter 初始状态）须可由 mantle-v2 / op-succinct fork 公开重建；
  - 下游 issue 必须核验当前 `SuccinctL2OutputOracle` 配置是否激活 optimistic toggle，并据此选择 (5d-zk) 或 (5d-or) 核验路径；
  - 若 Mantle 同时保留 fault-proof fallback，则**两条** (5d-zk) + (5d-or) 子项均须满足（任一不达标即在 grace 后失效）。
...
- Forum #413 (2026.02.16 四项要求收敛 — L2 proving systems applicability 覆盖 OR + ZK)：https://forum.l2beat.com/t/413 (Forum 主索引入口)
- 关联 explainer 材料：以 Forum #413 主帖与 L2Beat Medium / Monthly Update 系列承担替代来源（注：Round-2 草稿引用的 "2026.02 Medium ZK Proving System 解释文章" 经 Round-3 独立网络搜索未能定位为已发布 Medium 文章 — 详见 §11.3 修订与 §12.1 Gap Analysis）
...
- (5a)(5c) 对 OR 项目（除 Facet v1 外）的具体核验列表：L2Beat 是否会发布完整受影响 OR 项目清单？目前仅 Facet v1 在公开材料中作为命名实例。
...
(e) **Stage 0→1→2 proof-system permissionlessness 维度的递进**：Stage 0 (≥5 external actors for OR) → Stage 1 (Council compromise + proposer liveness + L2 proving system 透明度) → Stage 2 (fully permissionless)。

### 6.3 pass_fail_examples（截至 2026.05，Round-2 关键修订；**Round-3 Facet v1 备注 applicability 修订**）

**修订声明**：Round-1 草稿断言"截至 2026.05 未发现已正式认定为 Stage 2 的 L2 项目"——该断言在 Round-2 已被修正。

**Stage 2 Pass（项目页 2026.05 快照确认）**：

- **Facet v1**（OR）：项目页快照 [https://l2beat.com/scaling/projects/facet] 显式标注 "Facet v1 is a Stage 2 Optimistic Rollup"；同时记录约 **90 天**降级倒计时至 Stage 0，原因是其 proving system 相关 upcoming Stage 1 要求未满足。**Round-3 修订**：Facet 虽然是 OR 主路径，其 proving system 中的 trusted setup + verifier 合约重建标准**同等适用**（不再是"借鉴/类比"）——这是 item-5 (5a)/(5c) applicability 从 zk-only 修正为 all 的**关键事实证据**。Facet v1 的 OR + proof-system-transparency-failure 双重特征证明 Forum #413 的应用范围覆盖 L2 proving systems 全体。
...
| L2 Proving System 透明度（item-5 5a-5d） | 同 Stage 1 一样按子项 applicability 适用（5a/5b/5c all, 5d 按 proof type） | 同左 |

### 6.6 stage_boundary（Stage 0 → 1 → 2 proof-system 演进，再次显式）

- **Stage 0**：state-reconstruction + ≥5 external actors fraud-proof（OR）；
- **Stage 1**：Council compromise condition + proposer-set liveness assumption + L2 Proving System 透明度（item-5 5a-5d 按子项 applicability）；
- **Stage 2**：fully permissionless submission + Council 仅介入 onchain bug + 持续维护 L2 Proving System 透明度。
...
- 同时观察 Facet v1 case：进入 Stage 2 不意味着永久免疫——Forum #413 等 transparency 要求即便在 Stage 2 也会触发降级，因此 Mantle 即使到 Stage 2 也需持续维护 ZK / fault-proof proof stack 的可重建性（5a-5d 按 active proof type applicability 适用）。
...
- L2Beat Stages 主页 2026.05 快照：https://l2beat.com/stages
- L2Beat Facet v1 项目页 2026.05 快照：https://l2beat.com/scaling/projects/facet
...
## 7. item-7 — ZK Rollup vs Optimistic Rollup Stage 要求差异矩阵（**Round-3 重构**）

### 7.1 requirement_summary

本节按"通用 × ZK 特有 × OR 特有"三栏对照整理两类 rollup 在 Stage 0/1/2 各项要求上的异同，直接服务于 Mantle 评估——作为 OP Stack fork + OP Succinct ZK 路径的混合架构，其 Stage 1 评估同时触及"通用"+"ZK 特有"两栏，**OR 特有栏中仅 Forum #425 OR challenge period + ≥5 external actors 不适用**；item-5 透明度子项按 (5a-5d) applicability 重新归类（**Round-3 重大修订**）。

### 7.2 三栏对照（应然层 + 实然层标注，**Round-3 按 item-5 applicability 修订**）
...
| **(5a) No 🔴 trusted setups** (Round-3：从 ZK 特有移至通用) | all | grace-period-active (until ~2026-08-16) | Stage 1 |
| **(5b) Prover source code published** (Round-3：从 ZK 特有移至通用) | all | grace-period-active | Stage 1 |
| **(5c) Verifiers reproducible (onchain verifier 合约 vkey 与源码可重建；fault-proof prestate 归 (5d-or) — final polish)** (Round-3：从 ZK 特有移至通用) | all | grace-period-active | Stage 1 |
...
|------|---------------------|------------|
| **(5d-zk) zkVM programs reproducible** | grace-period-active (until ~2026-08-16) | Stage 1 |
...
| **(5d-or) Fault-proof prestate reproducible** (Round-3：从 ZK 列拆出，归至 OR 特有) | grace-period-active (until ~2026-08-16) | Stage 1 |
| Stage 2 fully permissionless fraud submission | enforced-on-project-pages | Stage 2 |

### 7.3 关键边界陈述（防止误用，**Round-3 修订**）
...
- **Round-3 关键修订**：**(5a)(5b)(5c) 对 OR 同等适用**（Facet v1 案例确认）——任何 OR 项目使用 ZK-wrapped verifier 或运营 fault-proof prover/challenger 软件、部署 onchain verifier/dispute-game 合约的，都需要满足这三项透明度要求。Round-2 时代将 (5a-5c) 整体标注为 "zk-only" 是误读，会导致下游对 OR / hybrid / OP Stack proof-stack 透明度检查的错误排除；
- **不要**将 (5d-zk) zkVM program reproducibility 套用到纯 OR 路径，也不要将 (5d-or) fault-proof prestate reproducibility 套用到纯 ZK 路径；hybrid 设计（如 Mantle 主 validity-proof + optimistic toggle fallback）须按 active proof type 分流核验。

### 7.4 Mantle 路径定位（混合架构，**Round-3 修订**）

- Mantle 走 ZK 主路径（OP Succinct + SP1 validity proof），保留 optimistic toggle fallback；所以：
  - **必须满足**：通用项全部 + L2 Proving System 透明度 (5a)(5b)(5c)（all 适用）+ (5d-zk)（zk-only 适用，当前 active proof type）+ recursion 信任假设；
  - **若 optimistic toggle 启用**，需额外满足 (5d-or) fault-proof prestate reproducibility；
  - **不承担**：Forum #425 5d OR challenge / ≥5 external actors / bond-slashing / Stage 2 fully permissionless fraud submission（除非未来 toggle 至纯 OR 路径）；
...
- (5a)(5c) 对 OR 项目（除 Facet v1 外）的实际执行清单是否会在 2026.08 grace period 结束前发布？

---

## 8. item-8 — OP Stack 上的 ZK Rollup（Mantle / OP Succinct 路径）特殊考量（**Round-3 hybrid fallback 评估**）

### 8.1 requirement_summary

Mantle 采用 "OP Stack derivation + op-geth 执行层 + OP Succinct (SP1) validity proof 替代 fault proof" 的混合架构，并通过 `SuccinctL2OutputOracle` 保留 optimistic toggle fallback（即 fault-proof 模式作为后备）。本节识别该架构在 Stage 1 评估中的非显然问题，并为下游三个 issue（`upgrade-exitwindow-securitycouncil`、`proposer-decentralization-zk-compliance`、`stage1-roadmap-recommendation`）提供入口。**Round-3 修订**：(5a)(5b)(5c) applicability 修订为 all，因此 Mantle 即便在 optimistic toggle 启用时仍承担相同的透明度子项；(5d) 按 active proof type 分流（主路径走 5d-zk，fallback 启用时走 5d-or）。
...
- 代入 item-5 四子项（**Round-3 按 applicability 全适用**）：
  - (5a) 适用（all）：SP1 verifier 的 trusted-setup 等级（详见 §5.7）；
  - (5b) 适用（all）：SP1 + Mantle 自有 prover 链路源码公开度，含 fault-proof prover/challenger 软件（hybrid fallback）；
  - (5c) 适用（all；final polish：范围限定在 onchain verifier 合约）：Mantle 主网 onchain verifier 合约 vkey 与源码的独立重建（含 optimistic toggle 启用时 dispute game 内嵌的 onchain SNARK / ZK-wrapped verifier 合约组件，如有）；fault-proof prestate 可重建性见下条 (5d-or)；
  - (5d) 适用，按 active proof type 分流：
    - **主路径 validity-proof → (5d-zk)**：Mantle SP1 program hash 与 mantle-v2 / op-succinct monorepo 的对应关系；
    - **optimistic toggle fallback → (5d-or)**：Mantle fault-proof prestate（MIPS / RISC-V interpreter 初始状态）与 mantle-v2 / op-succinct fork 源码可重建。

#### (b) Verifier trusted setup 评级承继（**applicability: all — Round-3 强调**）
...
- **Round-3 修订**：这一核验在 (5a) applicability 修订后**仍然适用** Mantle 主路径与 hybrid fallback 任一场景——只要部署了使用 trusted-setup 的 SNARK 组件即触发 (5a)；
- 下游 issue 必须核验 Mantle 主网 verifier 合约的实际 SP1 build 配置 + 任何 fault-proof game 合约使用的 SNARK 组件（如有）。
...
- **(3b) OR challenge period 5d 不适用**：Mantle 主路径走 validity-proof；Forum #425 5d 阈值**不应**用于 Mantle 主路径评估。**注意**：若 Mantle 启用 optimistic toggle fallback，(3b) 5d challenge period 转为适用——下游 issue 需在 fallback 启用条件下重新评估。
...
- Mantle 走 validity-proof 主路径，理论上**不存在** OR 意义的 fraud proof；
- item-2 (f) "≥5 external actors fraud-proof submission" 对 Mantle **不直接适用**（除非 optimistic toggle fallback 全面启用）；
- 替代要求是 item-5 (5c)(5d-zk) verifier/program 可重建性 + permissionless prover 路径；
- 本映射必须显式写入下游 issue，避免将 OR 路径的 Stage 0 要求误用于 ZK 路径。

#### (f) Mantle 现状开放问题清单（下游 issue 入口，**Round-3 增补 fault-proof 核验项**）
...
| Mantle 主网 SP1 verifier 是否依赖 🔴 Groth16 wrap？(5a applicable: all) | `proposer-decentralization-zk-compliance` |
| Mantle prover 部署链路源码公开度？mantle-v2 fork 中是否引入闭源 prover？(5b applicable: all) | `proposer-decentralization-zk-compliance` |
| 链上 onchain verifier 合约 vkey 与源码是否可由公开 SP1 ELF + SRS 重建？(5c applicable: all；范围限定 onchain verifier 合约) | `proposer-decentralization-zk-compliance` |
| Mantle SP1 program hash 与 mantle-v2 / op-succinct 源码 traceability？(5d-zk) | `proposer-decentralization-zk-compliance` |
| **Round-3 新增**：Mantle 是否启用 optimistic toggle fallback？若启用，fault-proof prestate 是否可由 mantle-v2 monorepo 重建？(5d-or) | `proposer-decentralization-zk-compliance` |
...
### 8.3 zk_vs_optimistic_differentials（Mantle 特有，**Round-3 修订**）

- Mantle 同时继承 OP Stack 治理结构（Optimism Foundation 上游 + Mantle Security 双层），是 ZK rollup 中相对少见的"治理路径与 proof 路径异源"案例；
- **Round-3 修订**：在 (5a)(5b)(5c) applicability 修订为 all 的前提下，Mantle 需要在 Optimism upstream 与 mantle-v2 fork 之间建立**双轨** traceability 链路——既覆盖 SP1 validity-proof 路径，也覆盖 fault-proof game / Cannon prestate（hybrid fallback）路径；不能仅依赖 SP1 上游声明。
...
- **关键待核验项**：(5a)-(5d) L2 Proving System 透明度四子项（按 Round-3 applicability：5a/5b/5c all 适用 + 5d-zk 主路径适用，若 toggle 启用追加 5d-or）+ Council 量化阈值。
...
- Forum #291 / #409 / #412 / #413 / #425 + 关联 explainer

### 8.7 open_questions

- Mantle 在 2026.05 之后是否会公开新的升级延迟方案？
- OP Succinct PermissionlessProposer 在 Mantle 主网的实际启用状态？
- `SuccinctL2OutputOracle` optimistic toggle 在 Mantle 主网当前是否处于 active / disabled 状态？这决定 (5d) applicability 走 zk 还是 or 路径。
...
### 9.5 边界项目重新评估（仅限 OR，Round-2 核验 + **Round-3 Unichain 重分类**）
...
- **OP Mainnet** [https://l2beat.com/scaling/projects/op-mainnet]：Stage 1，Superchain → Pass（**URL slug 修订**：Round-1 草稿写 `/optimism`，正确 slug 是 `/op-mainnet`，已在 Round-2 修正）；
- **Ink** [https://l2beat.com/scaling/projects/ink]：Stage 1，Superchain → Pass；
- **Unichain** [https://l2beat.com/scaling/projects/unichain]（**Round-3 重分类**）：**Forum #425 enumerated example only**——本研究 cutoff 前未完成项目页字面 "Stage 1" 标签的独立直接核验；按 Review Agent + Orchestrator 提供的二选一处理路径，本稿采取**保守归类**，不再无前缀地标注为 "Stage 1 Pass"；下游 issue 应在 grace period 结束前独立完成项目页字面字符串核验。
...
- **enforcement_status**: `enforced-on-project-pages`（截至 2026.05，Forum #425 列举的 5 个 OR 项目均见于公开材料；Unichain 项目页字面核验在本研究 cutoff 前 pending）；
...
- **OR Pass（新规则下）**：Arbitrum One, Base, OP Mainnet, Ink（项目页快照，§9.5）；
- **OR Forum #425 enumerated example（项目页字面核验 pending）**：Unichain（**Round-3 重分类**）；
...
- Mantle 走 validity-proof 主路径，**不**承担 Forum #425 OR challenge 5d 规则——本 item 在 Mantle 章节显式声明此**非适用**关系，避免下游误用；如 Mantle 启用 optimistic toggle fallback，则按 (3b) 重新评估（参见 §8.2 (c)）；
- relevance_to_mantle 字段值: **非适用（主路径）；条件性适用（fallback toggle 启用时）**。

### 9.11 evidence_sources

- Forum #425：https://forum.l2beat.com/t/425
- L2Beat 项目页快照：Arbitrum One, Base, OP Mainnet, Ink, Unichain（每个命名实例附；Unichain 字面核验 pending）
...
- Unichain 项目页字面 "Stage 1" 标签的直接核验时点：本研究 cutoff 前未完成（详见 §12.1 Gap）。

---

## 10. Diagrams（6 张 Mermaid 图，**Round-3 更新**）
...
flowchart LR
    ...
    subgraph S1["Stage 1 — Limited Training Wheels"]
        ...
        S1f["L2 Proving System 四子项 (Forum #413)<br/>(5a/5b/5c) applicable: all<br/>(5d-zk) zk-only / (5d-or) optimistic-only<br/>grace until ~2026-08-16"]
    end
    subgraph S2["Stage 2 — No Training Wheels"]
        ...
        S2e["示例: Facet v1 (OR, ~90d 降级倒计时)<br/>L2 Proving System 透明度同等适用"]
    end
    S0 -->|跨阶段递进| S1
    S1 -->|跨阶段递进| S2
    S0e -.->|演进| S1b
...
### diag-2 — Stage 1 检查清单流程图（flow，**Round-3 修订：item-5 applicability 按子项**）
...
flowchart TD
    ...
    RT -->|OR| ORpath["OR 路径"]
    RT -->|ZK| ZKpath["ZK 路径"]
    RT -->|hybrid<br/>(主 ZK + optimistic toggle)| HYpath["hybrid 路径<br/>(按 active proof type)"]

    ORpath --> S0OR["Stage 0 (f):<br/>≥5 external actors 可提交 fraud proof?<br/>(规范: Forum #291)"]
    S0OR -->|否| Out0
    S0OR -->|是| L2PS_OR["L2 Proving System 四子项 (Forum #413, grace 内)<br/>(5a) no 🔴 trusted setup (all)<br/>(5b) prover source (all)<br/>(5c) verifier reproducible (all)<br/>(5d-or) fault-proof prestate reproducible (OR)<br/>示例: Facet v1 (OR) 因 (5a)/(5c) 不达标 ~90d 降级"]
    L2PS_OR -->|任一不通过 grace 后 Fail| Out0
    L2PS_OR --> EW1OR

    ZKpath --> L2PS_ZK["L2 Proving System 四子项 (Forum #413, grace 内)<br/>(5a) no 🔴 trusted setup (all)<br/>(5b) prover source (all)<br/>(5c) verifier reproducible (all)<br/>(5d-zk) zkVM program reproducible (ZK)<br/>示例: Starknet 引用 (5b)+(5d-zk) 降级原因"]
    L2PS_ZK -->|任一不通过 grace 后 Fail| Out0
    L2PS_ZK --> EW1ZK

    HYpath --> L2PS_HY["L2 Proving System 四子项<br/>(5a/5b/5c) applicable: all<br/>(5d) 按 active proof type 分流:<br/>主路径走 (5d-zk), fallback toggle 走 (5d-or)<br/>Mantle 示例: 主 validity-proof + optimistic toggle"]
    L2PS_HY -->|任一不通过 grace 后 Fail| Out0
    L2PS_HY --> EW1HY
    ...
    EW1HY["Exit Window (3a):<br/>outside-SC upgrade ≥5d?<br/>(3b) 仅在 toggle 启用条件下重评"]
    EW1HY -->|否| Out0
    EW1HY --> SC
...
### diag-3 — ZK Rollup vs Optimistic Rollup Stage 要求差异矩阵（comparison，**Round-3 重构**）
...
flowchart LR
    subgraph 通用项["通用项 (applicable: all)"]
        ...
        U7["(5a) No 🔴 trusted setups (R3: ZK→all)"]
        U8["(5b) Prover source published (R3: ZK→all)"]
        U9["(5c) Verifiers reproducible (R3: ZK→all)"]
        U10["Stage 2 Exit Window ≥30d"]
    end
    subgraph ZK特有["ZK 特有 (applicable: zk-only)"]
        Z1["(5d-zk) zkVM programs reproducible"]
        Z2["Recursion / aggregation 信任假设"]
        Z3["不存在 validity-proof challenge period 概念 (明确否定)"]
    end
    subgraph OR特有["OR 特有 (applicable: optimistic-only)"]
        ...
        O4["(5d-or) Fault-proof prestate reproducible (R3: 从 ZK 列拆出)"]
        O5["Stage 2 fully permissionless fraud submission"]
    end
    M["Mantle (OP Stack + OP Succinct ZK + optimistic toggle fallback)"]
    M -->|必须满足| 通用项
    M -->|主路径必须满足| ZK特有
    M -.->|条件性 (toggle 启用时)| O4
    M -.x|不承担| O1
    ...
    M -.x|不承担| O3
    M -.x|不承担| O5
    F["Facet v1 (OR, Stage 2 + ~90d 降级)"]
    F -->|触发 (5a)(5c)<br/>R3 关键证据| U7
    F -->|触发 (5a)(5c)<br/>R3 关键证据| U9
    ...
    classDef facet fill:#f4cccc,stroke:#cc0000
    class 通用项,U1,U2,U3,U4,U5,U6,U7,U8,U9,U10 common
    class ZK特有,Z1,Z2,Z3 zk
    class OR特有,O1,O2,O3,O4,O5 or
    class M mantle
    class F facet
```
...
### diag-5 — L2Beat 框架演进时间线（timeline，**Round-3 注释更新**）
...
timeline
    ...
    2025.11 : Forum #409 New Stage 1 requirements for L2 proving setups
            : 三项初始要求 (标题侧重 ZK 但 R3 解读已扩展至所有使用 SNARK / trusted-setup 的部署) - effective 2025.11 proposed - enforcement adopted
    ...
    2026.02 : Forum #413 L2 Proving System Stage 1 四项要求收敛
            : 四子项 (5a/5b/5c all; 5d-zk zk-only; 5d-or optimistic-only) + 约 6 个月 grace period - effective 2026.02.16 - grace_end ~2026-08-16 - enforcement grace-period-active - R3 修订: applicability 从 zk-only 改为按子项细分
    2026.04 : Forum #425 OR Challenge Period 7d to 5d
            : 五个 OR 项目继续保留 Stage 1 (Unichain 项目页字面核验 pending) - effective 2026.04.30 - enforcement enforced-on-project-pages
    2026.05 : 当前快照点 (cutoff 2026-05-19)
            : Mantle = Stage 0 (5/5) - OP Succinct + SP1 since 2025.09.16 - Arsia upgrade 2026.04.16 移除 EigenDA - Facet v1 = Stage 2 OR with ~90d 降级倒计时 (R3 关键证据: OR 项目触发 (5a)/(5c)) - Starknet = Stage 1 with ~175d 降级倒计时
```
...
flowchart LR
    ...
    subgraph T2["(3b) OR challenge period ≥5d"]
        ...
        T2d["代表项目: Arbitrum, Base, OP, Ink (Unichain Forum #425 enumerated, 项目页字面核验 pending)"]
        T2e["Mantle 评估: 非适用 (主路径)<br/>(走 validity-proof 路径; toggle 启用时条件性适用)"]
    end
...
## 11. Source Coverage（src-1 ~ src-6，**Round-3 src-3 修订**）
...
| #413 L2 Proving System Stage 1 四项要求 (2026.02.16) — R3 标识 applicability 覆盖 OR + ZK | https://forum.l2beat.com/t/413 | §5.3 (5a-5d) / §10 diag-5 |
| #425 OR Challenge Period 7d→5d (2026.04.30) | https://forum.l2beat.com/t/425 | §9 整节 / §10 diag-5, diag-6 |

满足 Min Count 6 ✅

### 11.3 src-3: L2Beat Medium 文章 (min 4，**Round-3 修订**)

**Round-3 修订说明**：Review Agent 在 Round-2 verdict 中指出 (i) "Introducing Stages" URL 为占位符未完整、(ii) "2026.02 ZK Proving System Stage 1 explainer Medium" 为占位符未完整。按 Orchestrator 提供的 "补全 URL 或从覆盖来源中移除" 二选一处理：

- (i) **已修复**：补全 "Introducing Stages" 完整 URL（Round-3 网络搜索确认）；
- (ii) **未能补全**：Round-3 独立网络搜索（多次查询）未能定位到 2026.02 发布的 L2BEAT Medium 文章作为 Forum #413 配套 explainer——最可能场景是 Forum #413 主帖本身承担了解释功能，未在 Medium 单独发表 explainer 长文。本稿**从已覆盖来源中移除**该占位符，替换为已验证的 L2BEAT Medium "Native Rollups: Where they are, and where they are going"（Luca Donno + Conor McMenamin），该文涉及 L2BEAT 对 ZK 验证器多样性与 proof-stack 透明度的官方表述，可作为 §1.7 / §5.6 概念引子的补充支撑来源。同时在 §12.1 Gap Analysis 显式记录 2026.02 explainer Medium 未能定位的事实。

| Medium 文章 | URL | 用于 |
|------------|-----|------|
| Introducing Stages — a framework to evaluate rollups maturity (Luca Donno, 2023.06，**R3 补全完整 URL**) | https://medium.com/l2beat/introducing-stages-a-framework-to-evaluate-rollups-maturity-d290bb22befe | §1.1 / §1.6 / §10 diag-5 |
...
| Native Rollups: Where they are, and where they are going (Luca Donno + Conor McMenamin, L2BEAT × Nethermind，**R3 新增替换**) | https://medium.com/l2beat/native-rollups-where-they-are-and-where-they-are-going-cb21eb103d46 | §1.7 / §5.6 (作为 L2BEAT 对 proof-stack 透明度与 verifier 多样性的官方 Medium 表述支撑) |

满足 Min Count 4 ✅（**R3 修订**：4 篇 verified L2BEAT Medium 文章；原 "2026.02 ZK Proving System explainer" 占位符已从覆盖来源中移除，替换为已验证的 "Native Rollups" 文章；详见 §12.1 Gap 中 "2026.02 explainer Medium 未能定位" 条目）。
...
| 项目 | URL | 用于（命名实例位置） | 核验状态（截至 Round-3） |
|------|-----|---------------------|------------------|
...
| Unichain | https://l2beat.com/scaling/projects/unichain | §1.4 / §3.5 / §9.5 / §9.9 / §10 diag-2 注释 | ⚠️ **R3 重分类**：Forum #425 enumerated example only；项目页字面 "Stage 1" 标签直接核验在本研究 cutoff 前 pending；本稿不再无前缀地标注为 "Stage 1 Pass"（保守归类，参见 §12.1 Gap） |
| Scroll | https://l2beat.com/scaling/projects/scroll | §3.5 / §4.4 (Walkaway Fail) | ✅ "does not pass the walkaway test" 显式文字确认 |
| Starknet | https://l2beat.com/scaling/projects/starknet | §3.5 / §4.4 / §5.5 (Walkaway Fail + L2 Proving 缺口 + ~175d 降级倒计时) | ✅ 当前 Stage 1 确认；降级原因 "not all program sources are public / not all program hashes can be independently regenerated" 确认；"does not pass the walkaway test" 确认 |
...
| Facet v1（Round-2 新增；**R3 强化为 item-5 适用范围扩展的关键证据**） | https://l2beat.com/scaling/projects/facet | §1.4 / §6.3 / §5.5 (Stage 2 OR + ~90d 降级倒计时；触发 (5a)/(5c) 的 OR 命名实例) | ✅ "Stage 2 Optimistic Rollup" 显式文字确认；降级倒计时原因 "trusted setup + verifier 合约重建标准未满足" |
...
满足 Min Count 5 ✅（实际覆盖 12 个命名实例；Round-3 关键修订：Unichain 重分类为 "Forum #425 enumerated example only"；Facet v1 强化为 item-5 适用范围扩展证据）
...
| 2026.02 | https://l2beat.com/publications/monthly-update-2026-02 | ✅ 可用 | Arbitrum / ZKsync / Superchain / AggLayer 各路 ecosystem 更新；**未直接展开 Forum #413 四项要求**——Forum + 关联 explainer 是首发渠道 |
...
| **Unichain 项目页 2026.05 字面 "Stage 1" 标签的独立直接核验**（**Round-3 关键 Gap**） | §1.4 / §3.5 / §9.5 / §9.9 / §10 diag-2 / §11.4 — Stage 1 命名实例完整度 | 按 Review Agent + Orchestrator 提供的二选一处理路径，本稿采取保守归类，将 Unichain 重新分类为 "Forum #425 enumerated example only"，不再无前缀地标注为 "Stage 1 Pass"；下游 issue 应在 grace period 结束前独立完成项目页字面字符串核验 |
| **2026.02 Forum #413 配套 L2BEAT Medium explainer 文章未能在 Round-3 网络搜索中定位**（**Round-3 关键 Gap**） | §5.8 / §11.3 — Medium 来源完整性 | Round-3 多次独立网络搜索（包括 site:medium.com l2beat ZK proving system trusted setup 2026 等查询）均未定位到 2026.02 发布的 L2BEAT Medium 配套 explainer——最可能场景是 Forum #413 主帖本身承担了解释功能；本稿在 src-3 中**移除**该占位符并替换为已验证的 L2BEAT Medium "Native Rollups" 文章，维持 Min Count 4；若未来定位到该 explainer，可在 final 稿恢复 |
| 2024.12 Bartek Kiepuszewski 文章的确切 URL 未在 Round-1 公开搜索结果中浮现 | §2.3 引用精度 | Round-2 已通过 Round-2 web search 确认 URL `https://medium.com/l2beat/framework-update-l2-projects-recategorization-5d43b0d1fe50`，§11.3 已补充完整 URL |
...
| **`SuccinctL2OutputOracle` optimistic toggle 在 Mantle 主网当前 active / disabled 状态未公开核验**（**Round-3 新增 Gap**） | §8.2 (a) / §8.7 — (5d) 走 zk 还是 or 路径取决于此状态 | §8.7 已记录为开放问题；下游 `proposer-decentralization-zk-compliance` 须先确认 toggle 状态，再决定 (5d-zk) 或 (5d-or) 核验路径 |
| **(5a)(5c) 对非 Facet v1 的 OR 项目实际执行清单**（**Round-3 新增 Gap**） | §5.9 / §7.6 — item-5 applicability 修订后的执行范围 | 公开材料中仅 Facet v1 是命名实例；L2Beat 是否会发布完整受影响 OR 项目清单？建议下游研究在 2026.08 grace period 结束前追踪 Forum 后续帖子 |
...
- §3.5 / §9.5 / §11.4：Unichain 项目页 2026.05 快照中字面 "Stage 1" 标签的独立直接核验**未在本研究 cutoff 前完成**——明确"无证据"，按保守归类处理为 "Forum #425 enumerated example only"；
- §5.8 / §11.3：2026.02 Forum #413 配套 L2BEAT Medium explainer 文章**未在 Round-3 网络搜索中定位**——明确"无证据"，src-3 已替换为已验证 Medium 替代来源。

### 12.3 关键引用强度评级（**Round-3 更新**）

- **High confidence**（多源交叉确认）：Forum #291 / #412 / #413 / #425 的存在与核心条款、Stage 0/1/2 量化阈值、Mantle 当前 Stage 0 状态、Scroll / Starknet 项目页 walkaway Fail 标签、Aztec v1 walkaway Pass 标签、Facet v1 Stage 2 OR 标签 + (5a)/(5c) 触发降级（**R3 关键证据**）、Starknet 当前 Stage 1 + 降级原因引用（"not all program sources are public / not all program hashes can be independently regenerated"）、ZKsync ZIP-15 SC 12→8 重组（April 2026 Monthly Update）、Forum #413 适用范围覆盖 L2 proving systems 全体（Facet v1 案例 + 原帖文本交叉支持）；
- **Medium confidence**（单源或估算）：Forum #413 grace period 精确截止、Forum #412 grace period 起算点、Facet v1 / Starknet 降级倒计时精确截止；
- **Low confidence / open**：2026.04 是否有独立 Medium 公告、Starknet counterproposal 具体回帖 URL、SP1 Groth16 wrapper 2026 后续 ceremony 状态、Unichain 项目页字面 Stage 1 匹配（**R3 移至 open**）、Kinto 项目页是否在 grace period 后增加显式 walkaway 标签、2026.02 Forum #413 配套 Medium explainer 是否存在（**R3 新增**）、Mantle `SuccinctL2OutputOracle` optimistic toggle 当前状态（**R3 新增**）、(5a)(5c) 对非 Facet v1 的 OR 项目执行清单（**R3 新增**）。
...
| 2 | revision_round_2 | 基于 Review Verdict `7210b223` 提出的 1 critical + 3 major findings 完成定向修订：(C1) §11.6 移除 monthly-update-2026-05 URL（cutoff 2026-05-19 HTTP 404），ZKsync ZIP-15 事实重新归属至 April 2026 Monthly Update；(M1) §0 / §1.4 / §6.3 / §10 diag-1 修正 Stage 2 现状——L2Beat 当前已将 Facet v1 列为 Stage 2 Optimistic Rollup（含 ~90d 降级倒计时附注），Round-1 "未发现 Stage 2 项目" 断言被修正；(M2) §0 / §3.5 / §4.4 / §4.7 / §10 diag-4 将 Kinto 从 "项目页已记录的 Walkaway 失败" 降格为 "Forum #412 理论 / 提案级失败示例（permissioned challenger via SC minority）"，项目页快照未发现显式标签；(M3) §3.5 / §4.4 / §5.5 / §10 diag-2 Starknet 表述精确化——当前 Stage 1（项目页快照证实，非 Round-1 误述的 "Stage 0"）+ ~175d 降级倒计时至 Stage 0，引用原因 "not all program sources are public / not all program hashes can be independently regenerated"。其它修订：OP Mainnet URL slug 修正 `/optimism` → `/op-mainnet`；§11.4 新增 Facet v1 项目页核验行；§11.3 补全 2024.12 Bartek Kiepuszewski Medium 文章完整 URL；§8.4 增加 Mantle 项目页 2026.04.22 liveness anomaly 附注；§12.1 Gap Analysis 新增 5 条 Round-2 衍生 gaps。Commit: `4c59d9f`. |
| 3 | revision_round_3 | 基于 Review Verdict `ece6186c` 提出的 1 major + 3 minor findings 完成定向修订：(M-r2-1 Major) **item-5 适用范围从 "zk-only" 修正为 "L2 proving system（按子项细分）"**——item-5 重命名为 "L2 Proving System Stage 1 四项要求"；(5a)(5b)(5c) applicability 修订为 all；(5d) 按 proof type 分流为 (5d-zk) zk-only + (5d-or) optimistic-only。修订一致传播至 §0 Executive Summary / §1.4 Stage 2 示例 Facet v1 备注 / §3.1 / §3.2(6) / §3.3 stage_boundary / §3.4 zk_vs_optimistic_differentials 矩阵 / §3.6 Mantle 关联 / §5 整节（标题 + §5.1 / §5.2 / §5.3 各子项 applicability / §5.4 重写 / §5.5 增加 OR 命名实例 / §5.6 / §5.7 hybrid fallback 评估 / §5.8 / §5.9）/ §6.5 / §6.6 / §6.7 / §7.1 / §7.2 三栏对照重构 / §7.3 边界陈述 / §7.4 Mantle 路径定位 / §7.6 / §8.1 / §8.2 (a)(b) / §8.2(f) Mantle 开放问题清单增加 fault-proof 核验项 / §8.3 / §8.4 / §8.7 / §10 diag-1 节点 S1f 重写 / §10 diag-2 增加 hybrid 路径与 L2 Proving System 节点 / §10 diag-3 整图重构（U7-U9 从 ZK 移至通用栏，O4 从 ZK 列拆出至 OR 列，Facet v1 触发 (5a)/(5c) 关系新增）/ §10 diag-5 timeline 注释更新。(m-r2-1 Minor) §11.3 src-3 Medium URL 补全与替换：(i) 补全 "Introducing Stages" 完整 URL `https://medium.com/l2beat/introducing-stages-a-framework-to-evaluate-rollups-maturity-d290bb22befe`；(ii) "2026.02 ZK Proving System explainer" 经 Round-3 多次独立网络搜索未能定位，从覆盖来源中移除，替换为已验证 Medium "Native Rollups: Where they are, and where they are going"（Luca Donno + Conor McMenamin）`https://medium.com/l2beat/native-rollups-where-they-are-and-where-they-are-going-cb21eb103d46`；同时在 §12.1 Gap 新增 "2026.02 Medium explainer 未定位" 条目；维持 Min Count 4。(m-r2-2 Minor) §6.8 Stages 页面 URL 不一致修复：将 `https://l2beat.com/scaling/stages` 统一为 `https://l2beat.com/stages`（与全文其他位置保持一致）。(m-r2-3 Minor) Unichain 验证状态修复：按 Review Agent + Orchestrator 提供的二选一处理路径，本稿采取"Forum #425 enumerated example only"的保守归类，§1.4 / §3.5 / §9.5 / §9.9 / §10 diag-6 注释 / §11.4 表格 / §12.1 Gap / §12.2 / §12.3 同步修订 Unichain 标签，不再无前缀地标注为 "Stage 1 Pass"。结构层 9 items / 13 fields / 6 diagrams / 6 source requirements 与 Round-1 / Round-2 保持一致；新增内容仅在受影响条款局部。 |
| final | final_promotion | 基于 Round-3 approved draft（commit `e443c5fa36539f6fe4f8ade2294949eb08cae024`，adversarial approval `50baa887-9b5a-44b3-931e-e81300e923ce`，verdict approve / no critical or major findings）完成 final 持久化。**唯一变更**是处理 Review Agent round-3 non-blocking polish note：将 §5 (5c) "verifiers reproducible" 子项的措辞严格限定在 **validity / onchain verifier 路径**——onchain verifier 合约的 vkey 与源码可重建归 (5c)；fault-proof prestate（`absolutePrestate` / Cannon prestate hash）可重建归 (5d-or)，避免两个独立子项交叉混并。受影响位置：§5.3 (5c) 要求文本 + 范围澄清 + 代表实例（OP Stack 项重写、Facet v1 注释明确分流）、§5.4 differential 表格 (5c) 行、§5.7 (5c) Mantle 关联 bullet（移除与 fault-proof prestate / `absolutePrestate` 相关的 (5c) 子句，注 (5d-or) 单独承担）。为保持内部一致性同步收紧：§7.2 通用项表 (5c) 行、§8.2 (a) Mantle 评估 (5c) 子项、§8.2 (f) Mantle 开放问题清单 (5c) 行。frontmatter `status` 从 `draft` 改为 `final`，新增 `final_metadata` 块记录 approved_draft_commit / adversarial_approval_id / polish_applied。其余内容（含 §1-§4 / §6 / §9-§13、所有 6 张 Mermaid 图、Source Coverage src-1~src-6、Gap Analysis）与 round-3 approved draft 字面一致；未引入新研究、未修改 named example 标签、未变动 Forum 帖子覆盖列表、未触动其它 (5a)/(5b)/(5d-zk)/(5d-or) 子项的措辞。 |
