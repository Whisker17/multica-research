...
artifact_paths:
  ...
  draft: mantle-stage1-rollup/research-sections/mantle-architecture-2026/drafts/round-1.md
...
draft_metadata:
  ...
  source_requirement_coverage:
    src-1: covered
    src-2: covered
    src-3: covered (exhaustive critical-contract coverage)
    src-4: covered
...
- **Proof system**：`OPSuccinctL2OutputOracle 0x31d543…f481`（impl `0x4059…6f50`）→
  ...
  Mantle 当前部署的 implementation **不暴露 `fallbackTimeout()`**，没有
  "permissionless self-propose" 路径；`optimisticMode() = false`，
  `finalizationPeriodSeconds() = 43200`（12h）。`aggregationVkey =
  0x0006…81cf`、`rangeVkeyCommitment = 0x1d1e…6004`。
- **Governance（三条独立路径）**：
  - **(a) Core rollup**：`ProxyAdmin 0xca35…7794` 与 `OPSuccinctL2OutputOracle.owner` 均为
    **MantleSecurityMultisig `0x4e59…D40f`（6 of 15 Gnosis Safe，链上读取值；
    L2Beat 项目页表述为 6/14，差异已在 §3 标注待复核）**，**无 TimelockController**，
...
- **Sequencer / Proposer**：sequencer 为单一 EOA `0x2f40…d749`；proposer 为单一 EOA
  `0x6667…D77D`（迁移至 `approvedProposers` mapping，仍只有 1 个 active proposer）；
  `seq_window_size = 3600 L1 blocks`（12h），`max_sequencer_drift = 600 s`（10 分钟）。
- **L2Beat 风险**：当前 2 条 CRITICAL（instant upgrade、centralized sequencer）+ 6 条 HIGH
  ...
  unable to route"** 两条 SP1VerifierGateway 专属风险）+ 1 条 MEDIUM。Stage 0 已满足，
  Stage 1 仍差 1 项，Stage 2 仍差 2 项。
...
1. 核心 rollup 升级路径 **无 exit window**（≥7d 要求未满足，目前 0d）；
2. Security Council 阈值 **6/15**（or 6/14 per L2Beat）**未达 ≥8 成员 + >75% 阈值** 的 SC 资格
   门槛（成员身份未公开，无法核 ≥50% 独立性）；
3. ZK verifier transparency：SP1VerifierGateway 由 Succinct 2/3 Safe 控制，**无 timelock**；
   route freeze/add 立即生效；Mantle Oracle `verifier` 字段更新由 Mantle 6/15 Safe 立即执行，
   未走 timelock；
4. 单 proposer EOA + 无 permissionless self-propose 路径 →
   `Funds can be frozen if permissioned proposer fails` 风险路径在 Stage 1 提款保证下不达标。
...
`0xCafEf00d…6878` 拥有，**无 timelock**；`addRoute` 与 `freezeRoute` 立即生效且不可逆。Mantle 当前
部署的 Oracle implementation 不暴露 `fallbackTimeout()`，因此 Mantle 当前未启用 permissionless
self-propose 时间窗 — 这与 L2Beat 标注的 "permissioned proposer 失效则提款冻结" 风险一致。

#### architecture_components (A) SuccinctL2OutputOracle 提案与验证层

| Component | Storage | 关键状态机 | 来源 |
|-----------|---------|-----------|------|
| `OPSuccinctL2OutputOracle` proxy | `0x31d543e7BE1dA6eFDc2206Ef7822879045B9f481` | proxy slot EIP-1967 → impl `0x4059509ffb703b048d1e9ce3118f90e759076f50` | 链上读取 |
| `version` | `v3.0.0-rc.1` | semver | `op-succinct/contracts/src/validity/OPSuccinctL2OutputOracle.sol` L181–L182 |
| `verifier` | 单一 ISP1Verifier 地址；由 owner 通过 implementation 升级路径修改（合约本身不暴露 `updateVerifier`，需通过 implementation 升级或在 implementation 中扩展） | `OPSuccinctL2OutputOracle.sol` L89 |
| `approvedProposers` (mapping) | `address => bool`；由 owner 通过 `addProposer/removeProposer` 管理 | `OPSuccinctL2OutputOracle.sol` L99, L591–L601 |
| `challenger` | EOA / multisig；可调用 `deleteL2Outputs` 在 finalizationPeriod 内删除未 finalize 的 output | `OPSuccinctL2OutputOracle.sol` L68, L268–L290 |
| `optimisticMode` (bool) | 由 owner `enableOptimisticMode/disableOptimisticMode` toggle；启用后 `proposeL2Output` 绕过 verifyProof | `OPSuccinctL2OutputOracle.sol` L105, L605–L618 |
| `fallbackTimeout` (uint256) | 当 `block.timestamp - lastProposalTimestamp() > fallbackTimeout` 且 proposer 未提交，则任何人可代为提交 — **Mantle 当前 implementation 不暴露此字段，无 fallback 路径**（链上 `cast call` revert）| `OPSuccinctL2OutputOracle.sol` L110, L324–L329 |
| `opSuccinctConfigs[GENESIS_CONFIG_NAME]` | 存储 `aggregationVkey / rangeVkeyCommitment / rollupConfigHash` | `OPSuccinctL2OutputOracle.sol` L113, L253–L257 |
| `submissionInterval` | min L2 blocks between proposals；owner 可调整 | `OPSuccinctL2OutputOracle.sol` L60, L543–L547 |
| `finalizationPeriodSeconds` | 提款 finalize 等待时间；Mantle 当前 = 43200 (12h) | `OPSuccinctL2OutputOracle.sol` L77 |

`proposeL2Output(_configName, _outputRoot, _l2BlockNumber, _l1BlockNumber, _proof, _proverAddress)`
状态机（OPSuccinctL2OutputOracle.sol L315–L372）：

1. `whenNotOptimistic` modifier (L322, L201–L204) — 若 optimisticMode = true，整个函数 revert；
   走 L385 的 `proposeL2Output(_outputRoot, _l2BlockNumber, _l1BlockHash, _l1BlockNumber)`
   payable 重载（whenOptimistic，跳过验证）。
2. proposer 校验（L324–L329）：`approvedProposers[tx.origin]` 或 `approvedProposers[address(0)]`
   或 `block.timestamp - lastProposalTimestamp() > fallbackTimeout`。
3. block number / timestamp 检查（L331–L340）。
4. 加载 `OpSuccinctConfig` 并校验非零（L343–L344）。
5. checkpoint blockHash 命中（L346–L349）。
6. 构造 `AggregationOutputs publicValues` 并调用
   `ISP1Verifier(verifier).verifyProof(config.aggregationVkey, abi.encode(publicValues), _proof)`
   （L361）。
7. push `l2Outputs`（L365–L371）。

> 注：`whenNotOptimistic` 路径用 `tx.origin` 作为 proposer 鉴权，
> `whenOptimistic` 路径用 `msg.sender`。源码注释（L302–L314）解释为：optimistic
> dispute-game 路径中 proposeL2Output 由 OPSuccinctDisputeGame 经 DisputeGameFactory
> 以 CWIA pattern clone 而成，msg.sender 是 clone 合约；此设计在 trust 模型下与 msg.sender
> 等价，但属合约层耦合，应在审计中复核。
...
| 场景 | 后果 | Mantle 恢复路径 |
|------|------|----------------|
| Mantle 当前使用的 PLONK v6.0.x route（selector `0xbb1a6f29`）被 Succinct freeze | `verifyProof` revert `RouteIsFrozen` → `proposeL2Output` 全部失败 → 提款 finalize 全面冻结 | Mantle 6/15 Safe 紧急升级 Oracle implementation 或 update `verifier` 字段到新 verifier 地址；或开启 `optimisticMode`（绕过 verifyProof） — **均需 0 delay，但触发实例需 Mantle 自身 Safe 行动** |
...
| 项 | 状态 | 来源 |
|----|------|------|
| Verifier 源码可读 | ✅ 已 verify | sp1-contracts/contracts/src/v6.1.0/SP1Verifier*.sol（commit d3629729c3216eb51bd4859d027a8eb729399fa4）|
| `VERIFIER_HASH` & `VK_ROOT` | 编译时常量 | `SP1VerifierGroth16.sol` L33/L38；`SP1VerifierPlonk.sol` L33/L38 |
| Trusted setup ceremony | Groth16/PLONK 使用 Aztec/Hermez perpetual powers-of-tau（行业标准）；Succinct 进行 circuit-specific phase-2 | succinctlabs/sp1 主仓 + Succinct blog；⚠️ **phase-2 transcript 没有像 Hermez 那样集中公开 — partial transparency** |
| Verifier 部署可复现 | ✅ CREATE2 + 固定 `CREATE2_SALT` | `v6.1.0/SP1VerifierGroth16.s.sol` L17 |
| `aggregationVkey` / `rangeVkeyCommitment` | 由 SP1 SDK `prover.setup(ELF)` 计算；只要给定 ELF + SP1 toolchain version，任何人可复现 | `op-succinct/scripts/utils/src/config_common.rs` L86–L93 |
| SP1 程序源码 | ✅ 公开 | https://github.com/succinctlabs/op-succinct/tree/main/programs |
| Mantle SP1 版本 | `feature/sp1-v6.0.2`（落后 Succinct 主仓 v6.1.0/v6.1.1 一个 minor） | mantle-xyz/op-succinct commit `664a1bd` |
...
| `owner()` | `0x4e59e778a0fb77fBb305637435C62FaeD9aED40f` (Mantle Security Multisig, 6/15 Safe) | 链上 |
...
| `submissionInterval()` | 🔍 需补；典型值为 10–1800 L2 blocks（按 SDK 默认 10）| follow-up |
...
| `rollupConfigHash()` (deprecated) | `0x6681c11eccf96068a081bbb888fd64ce72aa83bd1ccda5bbb53b4c43368cf87f` | 链上 |
| `approvedProposers(0x6667…D77D)` | true (Mantle proposer EOA) | 推断（与 L2Beat 项目页 sole-proposer 一致）|
| `fallbackTimeout()` | **revert（字段不在当前 implementation）** | 链上 cast call |
| `version` | `v3.0.0-rc.1`（按 mantle-xyz/op-succinct `OPSuccinctL2OutputOracle.sol` L181–L182；🔍 implementation `0x4059…6f50` bytecode 与该 source 是否完全对应需 Etherscan verified source 核对） | mantle-xyz/op-succinct `feature/sp1-v6.0.2` |
| `verifier` 字段是否有 `updateVerifier` setter | ❌ 该版本 source 无独立 setter；变更需 ProxyAdmin upgrade | OPSuccinctL2OutputOracle.sol L89, L596+ |

#### trust_assumptions

| Trusted Party | 单点能力 | 最坏后果 |
|---------------|----------|----------|
| Mantle Security Multisig 6/15 (`0x4e59…D40f`) | upgrade Oracle implementation；toggle optimisticMode；add/remove proposer；transfer ownership | 立即升级到恶意 implementation → funds stolen；启用 optimistic + 撤掉 challenger → 接受无效 output → funds stolen |
...
| Mantle proposer EOA (`0x6667…D77D`) | 唯一可调用 `proposeL2Output`；可选择任一活跃 route | 选用旧/未审计路由 → 若该路由后被发现漏洞且未及时 freeze，可提交假 output → funds stolen；停机 → funds frozen（无 fallback timeout，无 permissionless self-propose）|
| Mantle Engineering Multisig 3/7 (challenger) | 在 finalizationPeriod (12h) 内调用 `deleteL2Outputs` | 拒绝 challenge / 私钥失效 → optimistic mode 下假 output 可 finalize（当前 optimisticMode=false，此风险待 toggle 后激活）|

#### l2beat_risk_mapping

| L2Beat 风险措辞 | 严重度 | 根因映射 |
|----------------|--------|---------|
...
| "Funds can be stolen if the proposer routes proof verification through a malicious or faulty verifier by specifying an unsafe route id" | HIGH | item-2 (B) `bytes4(proofBytes[:4])` proposer-selected route + Gateway 多版本共存 + Succinct 未及时 freeze 旧漏洞路由 |
| "Funds can be frozen if … the permissioned proposer fails to publish state roots to the L1" | HIGH | 单 proposer EOA + 当前 implementation 无 `fallbackTimeout` + 无 permissionless self-propose 路径 |
| "Funds can be frozen if … the SP1VerifierGateway is unable to route proof verification to a valid verifier" | HIGH | Gateway 全部路由 frozen 或 owner 私钥丢失 → `proposeL2Output` 全部 revert |
...
| Verifier source published (Tier b: ZK transparency) | b | ✅ | sp1-contracts/v6.1.0 全部已 verify |
| Program (ELF) reproducible (Tier b: ZK transparency) | b | ✅ | succinctlabs/op-succinct/programs 公开 |
| Trusted setup transcript transparency (Tier b: ZK transparency) | b | ⚠️ partial | phase-2 transcript 未集中公开 |
| Verifier 升级 ≥7d exit window (Tier b: outside-SC upgrade) | b | ❌ 0d | Mantle Oracle `verifier` 字段更新走 ProxyAdmin 升级，由 6/15 Safe 立即执行，无 TimelockController |
| Permissioned proposer 失效恢复 (Tier b: liveness assumption) | b | ❌ | 无 fallback timeout；无 permissionless self-propose；唯一恢复路径是 Mantle 6/15 Safe 紧急介入 |
...
- **2025-09-16 后**：proposer 从 `proposer` 单一字段迁移到 `approvedProposers` mapping
  （OPSuccinctL2OutputOracle.sol L70–L73、L99）。当前仍只有 1 个 active proposer。
- **OPSuccinctL2OutputOracle v3.0.0-rc.1 新增**：`opSuccinctConfigs` mapping（命名配置，
  支持多 vk 切换 — Mantle 当前只用 `GENESIS_CONFIG_NAME`）；`fallbackTimeout`
  （Mantle 当前 implementation **未启用** — 上线时点 implementation 早于该字段引入）。

#### evidence_sources

| Source | Type | Retrieval / commit |
|--------|------|--------------------|
| `op-succinct/contracts/src/validity/OPSuccinctL2OutputOracle.sol` L1–L619 | primary, code (src-4) | mantle-xyz/op-succinct `664a1bd` |
...
- `op-succinct/contracts/src/validity/OPSuccinctL2OutputOracle.sol` L201–L204 (`whenNotOptimistic`),
  L315–L372 (`proposeL2Output` ZK 路径), L385–L434 (`proposeL2Output` optimistic 路径),
  L605–L618 (`enableOptimisticMode/disableOptimisticMode`), L543–L547 (`updateSubmissionInterval`),
  L591–L601 (`addProposer/removeProposer`)。
...
MantleSecurityMultisig（6/14 per L2Beat / 6/15 per on-chain `getOwners` — 数字差异待复核）
**立即升级** OptimismPortal、SystemConfig、OPSuccinctL2OutputOracle、L1StandardBridge、
L1CrossDomainMessenger、ProxyAdmin、Mantle Oracle `verifier` 字段等，违反 Stage 1
"non-SC upgrade ≥7d exit window" 要求。**(b)** 经一日 TimelockController 解耦但同一 Security
Council 持有 PROPOSER 角色，因此**形式上有 1d delay，但实质决策权未与 (a) 解耦**。**(c)** mETH 由
独立 Mantle LSP 仓库托管，L2Beat 项目页未覆盖，本轮调研仅记录 "Source unavailable"。

#### (a) 核心 rollup 合约升级路径 — Stage 1 评估主路径

| 合约 | Proxy | Implementation (EIP-1967) | Admin / Owner (proxy) | 直接控制者 | Timelock? |
|------|-------|---------------------------|-----------------------|------------|-----------|
| OptimismPortal | `0xc54cb22944F2bE476E02dECfCD7e3E7d3e15A8Fb` | 🔍 需链上 EIP-1967 slot 读取（follow-up） | ProxyAdmin `0xca35…7794` | MantleSecurityMultisig 6/14–15 | ❌ |
...
**数字调和** — Security Council 大小：
- L2Beat 公开页（2026-05-19）: **6 of 14**
- 一名 research agent 链上 `getOwners()` 报告: **6 of 15**

两个数字相差 1。可能原因：(i) L2Beat 数据更新延迟（最近某 owner 增删未被反映）；
(ii) 链上读取时点更晚。**Final 阶段须以 Etherscan / 链上 `cast call 0x4e59…D40f
"getOwners()(address[])"` 的当前快照为准并记录块高 + 时间戳**。

**关键缺失项 — 没有 TimelockController**：

- MantleSecurityMultisig 是 ProxyAdmin 的 owner，同时是 Oracle 的 owner。任何 upgrade
  操作（包含 verifier 字段、optimisticMode、proposer 集合、challenger 等敏感字段更新）
  **立即生效，0 delay**。
- Guardian 路径：MantleEngineeringMultisig 3/7（OptimismPortal.pause 等紧急操作）
  与升级路径解耦，但仅能 pause withdrawals 与 blacklist dispute games，不能升级合约。
- 跨链 admin：Mantle op-stack 当前 fork 中 L1 → L2 admin 消息未在 L2Beat 列表中突出，
  follow-up：检查 L2Toolbox / AddressManager 是否存在反向 admin 路径。

**Security Council 资格门槛**：
- 成员数 6/14 或 6/15 ≪ Stage 1 ZK setups 框架建议的 ≥8 + >75% 阈值。
- 当前阈值 6/14 ≈ 42.86%；6/15 = 40%；均 ≪ 75%。
- 成员公开身份与独立性（≥50% 外部）：**L2Beat 未公开 14/15 个 EOA owner 的真实身份与组织归属
  → Source unavailable**。
...
|---------------|---------|----------|
| MantleSecurityMultisig 6/14–15 | 立即升级 (a) 任意合约 | 任何 funds-stolen 路径（恶意 implementation、移除 challenger、启用 optimistic、改变 verifier 至无 vk 校验的 mock）|
...
| 维度 | Tier | (a) | (b) | (c) |
|------|------|-----|-----|-----|
| outside-SC upgrade ≥7d exit window | b | ❌ 0d | ❌ 1d (<7d) | ❓ |
| Security Council ≥8 members | b | ❌ 6/14–15 | 同 | 同 |
| Security Council >75% threshold | b | ❌ 6/14 = 42.86% (or 6/15 = 40%) | 同 | 同 |
| Security Council walkaway test | b | ❌ 无 — Council 失活时无 fallback admin / 用户提款仍依赖 proposer | 同 | 同 |
| ZK verifier transparency — verifier upgrade timelock | b | ❌ Mantle Oracle `verifier` 字段更新无 delay | n/a | n/a |
...
| Challenger | MantleEngineeringMultisig 3/7 `0x2F44…daC9` | 1 multisig | `challenger` 字段更新 → ProxyAdmin upgrade (instant) |
...
| 2 | **Proposer failure** | L2 RPC 正常但 OPSuccinctL2OutputOracle 无新 output | Mantle 6/15 Safe addProposer 新地址 或 implementation 升级启用 fallbackTimeout | MantleSecurityMultisig | 取决于 multisig 协调（小时 ~ 天）| **是**（提款 finality 完全停滞）|
| 3 | **Prover failure** (Mantle Succinct Server / Succinct Prover Network) | Oracle 无 new proof；proposer 持有有效 batch 但无 proof | (a) Succinct Prover Network 恢复；(b) 切换到本地 prover；(c) MantleSecurityMultisig `enableOptimisticMode` 跳过 verifyProof | Succinct + Mantle | 小时 ~ 天 | **是** |
| 4 | **SP1VerifierGateway / route failure**（route freeze / route registry 异常）| `proposeL2Output` 始终 revert `RouteIsFrozen / RouteNotFound` | Mantle 6/15 Safe (a) upgrade Oracle 切到非 Gateway 的 SP1Verifier 实例；(b) `enableOptimisticMode` | MantleSecurityMultisig | multisig 协调 | **是** |
| 5 | **Optimistic-mode fallback liveness** | 启用 optimisticMode 后，challenger 失效，假 output 12h 后 finalize | challenger 替换或 disableOptimisticMode | MantleSecurityMultisig（challenger 字段更新 + optimistic toggle）| instant | **funds stolen 风险**而非 frozen — 12h finalize 后用户可被掠夺 |
| 6 | **Pause / Guardian recovery** | OptimismPortal.pause → 提款全部冻结 | MantleEngineeringMultisig 3/7 调用 `unpause` | MantleEngineeringMultisig | 协调速度 | pause 期间所有提款冻结（功能性 frozen）|
| 7 | **Permissionless self-propose / 自动恢复路径** | — | **不存在**：当前 Mantle Oracle implementation 无 `fallbackTimeout`（链上 revert） | — | — | n/a — 不可用 |
...
| MantleSecurityMultisig | 替换 proposer / sequencer | 失活则 (1) 与 (2) 故障难以恢复 |
...
|---|----------|----------------|---------------|----------|----------|--------------|------------|
| 1 | Funds can be stolen if | a contract receives a malicious code upgrade. There is no delay on code upgrades | **CRITICAL** | item-3 (a) MantleSecurityMultisig 6/14–15 持有 ProxyAdmin owner + Oracle owner，0 delay | Security Council 多签被攻陷 / 失活 | funds stolen | Tier b — 阻断 Stage 1 (exit window 0d) |
| 2 | Funds can be stolen if | in non-optimistic mode, the validity proof cryptography is broken or implemented incorrectly | HIGH (not explicitly labeled) | item-2 SP1 zkVM / Groth16-PLONK 电路漏洞；item-3 Mantle 不能在不通过 Succinct 升级路径下替换 verifier | SP1 主电路漏洞，未被 Succinct 及时披露/修复 | funds stolen | Tier b — ZK verifier transparency 阻断 |
| 3 | Funds can be stolen if | optimistic mode is enabled and no challenger checks the published state | HIGH (not explicitly labeled) | item-2 `enableOptimisticMode`；item-4 #5 challenger 失效 | Security Council toggle optimistic + challenger 私钥失效 | funds stolen | Tier b — instant toggle |
| 4 | Funds can be stolen if | the proposer routes proof verification through a malicious or faulty verifier by specifying an unsafe route id | HIGH (not explicitly labeled) | item-2 (B) Gateway 多版本路由共存；proposer 用 `bytes4(proofBytes[:4])` 自选路由；Succinct 未及时 freeze 漏洞旧 route | 旧版 verifier 后期被发现漏洞，未及时 freeze；恶意 prover 故意走该路径 | funds stolen | Tier b — verifier route 透明度阻断 |
...
| 6 | Funds can be frozen if | the permissioned proposer fails to publish state roots to the L1 | HIGH (not explicitly labeled) | item-4 #2 单 proposer + 无 fallbackTimeout | proposer EOA 失效 | funds frozen | Tier b — Stage 1 liveness assumption 阻断 |
| 7 | Funds can be frozen if | in non-optimistic mode, the SP1VerifierGateway is unable to route proof verification to a valid verifier | HIGH (not explicitly labeled) | item-2 (B) Gateway 全部路由 frozen / owner 私钥失效 + Mantle Oracle 不能自动切换 verifier | Succinct 2/3 Safe 失效 + Mantle 不及时紧急升级 | funds frozen | Tier b — verifier 升级 timelock 阻断 |
...
- ✅ 风险 6: 链上读 `fallbackTimeout()` revert — proposer 失效**无自动 fallback** — 措辞 +
  风险等级与链上现状一致。
...
| Req | current_state | gap | severity | tier | blocking_for_stage1 | owner |
|-----|---------------|-----|----------|------|--------------------|-------|
| **Outside-SC upgrade ≥7d exit window** | ❌ 0d；core rollup contracts 由 MantleSecurityMultisig 立即升级；Oracle `verifier` 字段同样 0d | 必须引入 TimelockController（min 7d）覆盖 ProxyAdmin 与 Oracle.owner 路径；保留 Guardian (3/7) 仅作 pause/emergency 而非升级 | **critical** | b | **yes** | Mantle Foundation + Security Council |
| **Security Council ≥8 members** | ❌ 6/14 (L2Beat) or 6/15 (链上) | 扩 Council 至 ≥8 含 ≥50% 公开身份外部成员 | **critical** | b | **yes** | Mantle Foundation |
| **Security Council >75% threshold** | ❌ 6/14 = 42.86% (or 40%) | 阈值上调至 >75% (e.g. 8/10) | **critical** | b | **yes** | Security Council |
...
| **ZK verifier transparency — verifier upgrade timelock** | ❌ Mantle Oracle `verifier` 字段更新 0d；Gateway 由 Succinct 2/3 Safe 即时 freeze/add route | (a) Mantle 自身引入 ≥7d timelock；(b) 与 Succinct 协商 Gateway 加 timelock 或 freeze-only 路径 | **critical** | b | **yes** | Mantle + Succinct |
| **ZK verifier transparency — vk 公开、program hash 可复现** | ✅ vk hard-coded constant；ELF 公开；CREATE2 复现 | none | none | b | no | n/a |
...
| **Permissioned proposer liveness assumption（1-of-N 或等价活性保障）** | ❌ N=1 + 无 fallback timeout；唯一恢复路径走 Multisig 紧急介入 | (a) 启用 `fallbackTimeout`（implementation upgrade）+ 短延迟后任何人可代为 propose；(b) 扩 proposer set ≥2 并将 proposer 切换自动化 | **major** | b | **yes** | Mantle Labs |
...
1. **Core rollup upgrade ≥7d exit window**（critical / no-delay 是 L2Beat 唯一明确标出的 Stage 1 阻断）
2. **Security Council ≥8 members + >75% 阈值 + walkaway test**（critical）
3. **ZK verifier transparency — verifier upgrade timelock**（critical）
4. **Permissioned proposer liveness assumption**（major）
5. **Trusted setup transcript transparency**（major）
...
flowchart LR
  ...
  subgraph L1["Ethereum L1 (chain id 1)"]
    ...
    SCMS["MantleSecurityMultisig<br/>0x4e59…D40f<br/>Gnosis Safe 6 of 14–15"]:::gov
  ...
  %% Proof flow
  OpNode -- "blocks" --> MSS
  MSS -- "Groth16/PLONK aggregation proof" --> Proposer
  Proposer -- "proposeL2Output(_proof, _proverAddress)" --> Oracle
  Oracle -- "verifier().verifyProof(...)" --> Gateway
...
sequenceDiagram
  ...
  Note over MSS,Gateway: ZK proof pipeline
  OPN->>MSS: blocks + L1 batch data
  MSS->>MSS: Kona derive + SP1 range proofs
  MSS->>MSS: Aggregate → Groth16/PLONK
  MSS->>Prop: aggregation proof bytes
  Prop->>Oracle: proposeL2Output(configName, outputRoot,<br/>l2BlockNumber, l1BlockNumber, proof, proverAddress)
  Note right of Oracle: whenNotOptimistic +<br/>approvedProposers[tx.origin] check
...
flowchart TB
  ...
  subgraph A["(a) 核心 rollup — Stage 1 主路径"]
    ...
    A_SCMS[MantleSecurityMultisig 6/14–15<br/>0x4e59…D40f]:::council
  ...
  subgraph B["(b) L1MantleToken / MNT — 1d timelock"]
    ...
    B_SCMS[MantleSecurityMultisig 6/14–15<br/>(PROPOSER_ROLE)]:::council
    B_MNT --> B_MNTPA
    B_MNTPA --> B_TLC
...
flowchart LR
  ...
  subgraph B_TIER["Tier (b) — Stage 1 真实门槛 (blocking)"]
    ...
    B2[Security Council ≥8 members<br/>当前 6/14–15]:::blocker
...
flowchart LR
  ...
  %% Contracts / actors
  SCMS([MantleSecurityMultisig<br/>0x4e59…D40f<br/>6/14–15]):::actor
...
| src-3 (Etherscan verified source / 链上读取) | exhaustive critical-contract | ✅ Covered (with `Source unavailable` for mETH) | OptimismPortal、SystemConfig、OPSuccinctL2OutputOracle (proxy+impl)、SP1VerifierGateway、3 个 SP1Verifier、ProxyAdmin (rollup)、MantleSecurityMultisig、MantleEngineeringMultisig、L1MantleToken、MantleTokenProxyAdmin、TimelockController、L1StandardBridge、L1CrossDomainMessenger 全部覆盖；mETH 标 Source unavailable |
...
| OPSuccinctL2OutputOracle | `0x31d5…f481` | `0x4059…6f50` | ProxyAdmin + `owner` slot (both → MantleSecurityMultisig) | verifier, optimisticMode, finalizationPeriodSeconds, aggregationVkey, rangeVkeyCommitment, approvedProposers, challenger | MantleSecurityMultisig (owner), MantleEngineeringMultisig (challenger), Proposer EOA | ⚠️ implementation verified? 🔍 follow-up | source @ mantle-xyz/op-succinct `feature/sp1-v6.0.2` |
...
| MantleSecurityMultisig | `0x4e59…D40f` | n/a (Gnosis Safe v1.3.0) | self-managed | getOwners, getThreshold | 14–15 EOAs (身份未公开) | ✅ verified (Safe std) | n/a |
...
1. **MantleSecurityMultisig 大小 6/14 vs 6/15**：L2Beat 项目页与 on-chain `getOwners()` 报告
   不一致；final 阶段须以最新 Etherscan / 链上读取的当前快照为准，记录块高 + 时间戳。
2. **TimelockController 角色持有者**：PROPOSER_ROLE / EXECUTOR_ROLE / TIMELOCK_ADMIN_ROLE
   ...
   `hasRole(roleHash, addr)` 链上验证或 Etherscan event scan。
3. **OPSuccinctL2OutputOracle implementation `0x4059…6f50` 与 source `OPSuccinctL2OutputOracle.sol`
   的对应关系**：源代码声明 `v3.0.0-rc.1`，但 implementation 是否完全等同需 Etherscan
   verified bytecode 核对（fallbackTimeout 不暴露暗示 implementation 早于该字段引入）。
4. **OptimismPortal / SystemConfig / L1StandardBridge / L1CrossDomainMessenger 各 proxy 的
   implementation 地址**：未链上核读 EIP-1967 slot。
...
7. **submissionInterval 当前值** 与 fallbackTimeout 默认值（理论上 2 weeks）。
...
- MantleSecurityMultisig 14/15 个 owner 的真实身份与组织归属。
