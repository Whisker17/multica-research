...
artifact_paths:
  ...
  draft: mantle-stage1-rollup/research-sections/mantle-architecture-2026/drafts/round-2.md
...
draft_metadata:
  ...
  outline_round: 2
  prior_round_artifact: "mantle-stage1-rollup/research-sections/mantle-architecture-2026/drafts/round-1.md @ 73b645db73acfc2857eb688a5fa34a4df2c8d787"
  adversarial_feedback_comment: "619a62c6-ed1f-4468-900f-9024284fd659"
  gate_decision_comment: "0ad5c60c-fe55-49c8-a70c-eea55d7bc88a"
...
- **Proof system**：`OPSuccinctL2OutputOracle 0x31d543…f481`（impl `0x4059…6f50`）→
  ...
  **deployed implementation `version() = "v2.0.1"`**（对应 `mantle-xyz/op-succinct` commit
  ...
  upgrade path"）；`optimisticMode() = false`，`submissionInterval() = 1800`（链上确认），
  ...
  不使用 named-config mapping）。Owner 可通过 `updateVerifier(address)`/`updateAggregationVkey(bytes32)`/
  `updateRangeVkeyCommitment(bytes32)`/`updateRollupConfigHash(bytes32)` **直接、0 delay** 改写关键字段，
  无需 ProxyAdmin upgrade。
- **Governance（三条独立路径）**：
...
3. ZK verifier transparency：SP1VerifierGateway 由 Succinct 2/3 Safe 控制，**无 timelock**；
   ...
   `updateVerifier(address)` 0 delay 更新（无需 ProxyAdmin upgrade）；
...
| `submissionInterval` | `uint256 public submissionInterval;` 直接 slot；**链上确认 = 1800** L2 blocks | `updateSubmissionInterval(uint256)` (`onlyOwner`) | L47; L513 |
| `challenger` | `address public challenger;` | 字段在 v2.0.1 中无独立 setter；变更需 ProxyAdmin upgrade | L55; L281–L304 (`deleteL2Outputs`) |
| `optimisticMode` | `bool public optimisticMode;` toggle | `enableOptimisticMode(uint256)` / `disableOptimisticMode(uint256)` | L89; L569–L584 |
| `verifier` (单一地址) | `address public verifier;` **直接 storage slot**（v2.0.1 无 `verifier` 命名 config） | **`updateVerifier(address)` (`onlyOwner`) — owner 可 0 delay 直接改写** | L74; L534 |
| `aggregationVkey` | `bytes32 public aggregationVkey;` **直接 slot** | `updateAggregationVkey(bytes32)` (`onlyOwner`) — owner 可 0 delay 改写 | L67; L520 |
| `rangeVkeyCommitment` | `bytes32 public rangeVkeyCommitment;` **直接 slot** | `updateRangeVkeyCommitment(bytes32)` (`onlyOwner`) — owner 可 0 delay 改写 | L71; L527 |
| `rollupConfigHash` | `bytes32 public rollupConfigHash;` **直接 slot** | `updateRollupConfigHash(bytes32)` (`onlyOwner`) — owner 可 0 delay 改写 | L77; L541 |
| `finalizationPeriodSeconds` | `uint256 public finalizationPeriodSeconds;` = 43200 (12h) | `updateFinalizationPeriodSeconds(uint256)` | L64; L587 |
| `historicBlockHashes` | `mapping(uint256 => bytes32) public historicBlockHashes;` | `checkpointBlockHash(uint256)` external | L86; L423 |
| `owner` | `address public owner;` = MantleSecurityMultisig `0x4e59…D40f` | `transferOwnership(address)` (`onlyOwner`，单步) | L80; L548 |
...
> updateRollupConfigHash` 也可由 owner 0 delay 改写，构成 Stage 1 exit-window 违反的关键面。
...
| `submissionInterval()` | **1800** L2 blocks | 链上 (round-2 adversarial finding confirmed) |
...
| `updateVerifier(address)` | **EXISTS — `onlyOwner`，0 delay** | OPSuccinctL2OutputOracle.sol L534 (commit `8cc7015`) |
| `updateAggregationVkey(bytes32)` | **EXISTS — `onlyOwner`，0 delay** | L520 |
| `updateRangeVkeyCommitment(bytes32)` | **EXISTS — `onlyOwner`，0 delay** | L527 |
| `updateRollupConfigHash(bytes32)` | **EXISTS — `onlyOwner`，0 delay** | L541 |
...
| Trusted Party | 单点能力 (v2.0.1 deployed) | 最坏后果 |
|---------------|------------------------------|----------|
| Mantle Security Multisig 6/14 (`0x4e59…D40f`) | upgrade Oracle implementation；**直接 `updateVerifier(address)`、`updateAggregationVkey/updateRangeVkeyCommitment/updateRollupConfigHash`（0 delay）**；toggle optimisticMode；add/remove proposer；transfer ownership | 立即将 `verifier` 改写为 mock/恶意验证器，绕过 ZK 校验 → funds stolen；启用 optimistic + 撤掉 challenger → 接受无效 output → funds stolen；升级 implementation 引入后门 → funds stolen |
...
- **v2.0.1 deployed** — `mantle-xyz/op-succinct` commit `8cc7015` `contracts/src/validity/
  ...
  - L513–L545 `updateSubmissionInterval` / `updateAggregationVkey` / `updateRangeVkeyCommitment` /
    `updateVerifier` / `updateRollupConfigHash`（**全部 `onlyOwner`，0 delay，直接改写 storage**）
  - L548 `transferOwnership` (single-step)
  - L555–L566 `addProposer/removeProposer`
  - L569–L584 `enableOptimisticMode/disableOptimisticMode`
  - L587 `updateFinalizationPeriodSeconds`
...
- MantleSecurityMultisig 是 ProxyAdmin 的 owner，同时是 Oracle 的 owner。任何 upgrade
  操作（含 ProxyAdmin upgrade）以及 **Oracle 直接 setter（`updateVerifier(address)`、
  `updateAggregationVkey`、`updateRangeVkeyCommitment`、`updateRollupConfigHash`、
  `updateFinalizationPeriod`、`updateSubmissionInterval`，全部 `onlyOwner` 且 0 delay
  — v2.0.1 已暴露）** 均立即生效，**0 delay**。
...
|---------------|---------|----------|
| MantleSecurityMultisig 6/14 | 立即（0 delay）升级 (a) 任意合约；**直接 `updateVerifier(address)`/`updateAggregationVkey`/`updateRangeVkeyCommitment`/`updateRollupConfigHash` onlyOwner setters（v2.0.1 已暴露，无需 ProxyAdmin upgrade）** | 任何 funds-stolen 路径（恶意 implementation、移除 challenger、启用 optimistic、`updateVerifier` 改写为 mock/无 vk 校验的 verifier）|
...
| Security Council ≥8 members | b | ❌ 6 of 14 | 同 | 同 |
| Security Council >75% threshold | b | ❌ 6/14 ≈ 42.86% ≪ 75% | 同 | 同 |
...
| 4 | **SP1VerifierGateway / route failure**（route freeze / route registry 异常）| `proposeL2Output` 始终 revert `RouteIsFrozen / RouteNotFound` | Mantle 6/14 Safe (a) **直接 `updateVerifier(address) onlyOwner`（v2.0.1 已暴露 0 delay setter，L534）**切到非 Gateway 的 SP1Verifier 直接实例；(b) `enableOptimisticMode`；(c) 走 ProxyAdmin upgrade 替换 implementation | MantleSecurityMultisig | multisig 协调 | **是** |
...
| 2 | Funds can be stolen if | in non-optimistic mode, the validity proof cryptography is broken or implemented incorrectly | **HIGH** | item-2 SP1 zkVM / Groth16-PLONK 电路漏洞；item-3 Mantle 6/14 Safe 可通过直接 `updateVerifier(address) onlyOwner` (v2.0.1 L534, 0 delay) 或 ProxyAdmin upgrade 替换 verifier，但 Council 失活时无紧急路径 | SP1 主电路漏洞，未被 Succinct 及时披露/修复 | funds stolen | Tier b — ZK verifier transparency 阻断 |
| 3 | Funds can be stolen if | optimistic mode is enabled and no challenger checks the published state | **HIGH** | item-2 `enableOptimisticMode`；item-4 #5 challenger 失效 | Security Council toggle optimistic + challenger 私钥失效 | funds stolen | Tier b — instant toggle |
...
| 7 | Funds can be frozen if | in non-optimistic mode, the SP1VerifierGateway is unable to route proof verification to a valid verifier | **HIGH** (round-2: 由 MEDIUM 升级为 HIGH — Mantle 当前 verifier 字段单值指向 SP1VerifierGateway `0x3B60…185e`，所有 PLONK route freeze 或 Gateway owner 失效时 Oracle 立即无法验证；Mantle 紧急可走 `updateVerifier` 0 delay setter，但 Council 失活则无路径) | item-2 (B) Gateway 全部路由 frozen / owner 私钥失效 + Mantle Oracle 不能自动切换 verifier | Succinct 2/3 Safe 失效 + Mantle 不及时紧急升级 | funds frozen | Tier b — verifier 升级 timelock 阻断 |
...
| Req | current_state | gap | severity | tier | blocking_for_stage1 | owner |
|-----|---------------|-----|----------|------|--------------------|-------|
| **Outside-SC upgrade ≥7d exit window** | ❌ 0d；core rollup contracts 由 MantleSecurityMultisig 6/14 立即升级；**Oracle `verifier` 字段同样 0d — v2.0.1 暴露 `updateVerifier(address) onlyOwner` 直接 setter（L534），无需 ProxyAdmin upgrade 即可立即改写**；其他 0-delay setters：`updateAggregationVkey`、`updateRangeVkeyCommitment`、`updateRollupConfigHash`、`updateFinalizationPeriod`、`updateSubmissionInterval`、`addProposer`/`removeProposer`、`enableOptimisticMode`/`disableOptimisticMode` | 必须引入 TimelockController（min 7d）覆盖 ProxyAdmin 与 Oracle.owner 路径（含上述所有直接 setter）；保留 Guardian (3/7) 仅作 pause/emergency 而非升级 | **critical** | b | **yes** | Mantle Foundation + Security Council |
...
| **ZK verifier transparency — verifier upgrade timelock** | ❌ Mantle Oracle `verifier` 字段更新 **0d（直接 `updateVerifier(address) onlyOwner` setter，非 ProxyAdmin upgrade 路径）**；Gateway 由 Succinct 2/3 Safe 即时 freeze/add route | (a) Mantle 自身引入 ≥7d timelock（覆盖 `updateVerifier` 等 onlyOwner setter）；(b) 与 Succinct 协商 Gateway 加 timelock 或 freeze-only 路径 | **critical** | b | **yes** | Mantle + Succinct |
...
1. **Core rollup upgrade ≥7d exit window**（critical / no-delay 是 L2Beat 唯一明确标出的 Stage 1 阻断）；含 Oracle 的多个 0-delay onlyOwner setter（v2.0.1 直接暴露）
2. **Security Council 6/14 → ≥8 members + ≥75% 阈值 + walkaway test**（critical）
3. **ZK verifier transparency — verifier upgrade timelock**（critical）
4. **Permissioned proposer liveness assumption**（major）— v2.0.1 无 `fallbackTimeout`
5. **ZK verifier transparency — Mantle ELF → programVKey 项目级复现 / L2Beat verification**（major，round-2 新增）
6. **Trusted setup transcript transparency**（major）
...
7. **submissionInterval / fallbackTimeout**：round-2 已确认 `submissionInterval() = 1800` 秒
   （30 min；与 OP Stack 默认一致），`fallbackTimeout` 不存在于 v2.0.1（链上 cast call revert
   且 v2.0.1 源码中无该字段）。无遗留 sub-gap。
