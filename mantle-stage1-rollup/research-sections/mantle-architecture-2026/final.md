...
final_metadata:
  multica_issue_id: "e7988e5f-a834-4ce6-99a8-9a53f52348a6"
  mode: "composable (report_issue_id present)"
  promoted_from_draft: "mantle-stage1-rollup/research-sections/mantle-architecture-2026/drafts/round-3.md"
  promoted_from_commit: "6a9e9c18c0dffcb6d22b3b18a72f247db5f878d3"
  adversarial_approval_comment_id: "bf57a276-17e2-4a1c-9980-47f2638855c9"
  orchestrator_gate_decision_comment_id: "389063fd-500a-43ab-a7a4-871f8a8b8722"
  orchestrator_final_dispatch_comment_id: "2e9c1bb9-ee0d-4716-8123-c5ad74d989fb"
  ...
  final_promotion_changes:
    optimistic_mode_terminology_polish: "Replaced '12h finalize' / '12h 后 finalize' shorthand with 'challenger-set finalization window' in optimistic-mode rows (Trusted Party trust-bias table, l2beat_risk_mapping, Optimistic-mode fallback liveness row). Non-structural — `finalizationPeriodSeconds=43200 (12h)` definitional references and seq_window_size / withdrawal / force-inclusion rows kept as-is."
...
| **Mantle Engineering Multisig 3/7** (`0x2F44…daC9`) — `challenger` slot | `onlyChallenger` 0-delay setters：`enableOptimisticMode(uint256)`、`disableOptimisticMode(uint256)`、`updateFinalizationPeriodSeconds(uint256)`；以及 `deleteL2Outputs(uint256)`（在 finalizationPeriod 内可剪枝坏 output）| (a) **单独**启用 optimisticMode 后任意 assertion 在 challenger-set finalization window 后 finalize — 但仍需 6/14 Safe 控制的 proposer 提交假 output 才能完成盗资（盗资路径需 3/7 + 6/14 协调）；(b) 把 `finalizationPeriodSeconds` 设极大值 → funds 临时 frozen（提款冻结）；(c) 设极小值 → 缩短挑战窗 → 配合启用 optimistic 后盗资路径变宽。**单 Safe 不足以独立盗资，但可独立 freeze 提款。** |
...
| "Funds can be stolen if optimistic mode is enabled and no challenger checks the published state" | **HIGH** | `enableOptimisticMode` + challenger 失活 → 无效 output 在 challenger-set finalization window 后 finalize |
...
| 5 | **Optimistic-mode fallback liveness** | 启用 optimisticMode 后，challenger 失效，假 output 在 challenger-set finalization window 后 finalize | (a) **`disableOptimisticMode(uint256)` `onlyChallenger`** (`L575+`) — MantleEngineeringMultisig 3/7 即可恢复；(b) `challenger` slot 更换需 ProxyAdmin upgrade（MantleSecurityMultisig）| MantleEngineeringMultisig (challenger toggle) + MantleSecurityMultisig (challenger slot 更换) | instant | **funds stolen 风险**而非 frozen — challenger-set finalization window 结束后用户可被掠夺。**两条治理面**：(i) 3/7 已是 challenger 主体，单 Safe 可 toggle on/off；(ii) 真正"无 challenger" 场景需 3/7 失活 + Mantle 6/14 ProxyAdmin upgrade 重设 challenger slot — round-3 纠正 round-2 把全部归在 6/14 的描述错误 |
