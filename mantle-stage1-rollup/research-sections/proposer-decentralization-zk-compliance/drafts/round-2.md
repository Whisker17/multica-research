...
draft_metadata:
  ...
  outline_round: 2
  draft_round: 2
  prior_draft_commit: a7e783cfbafc3b2c00687bf6a1829eab0bab7a77
  prior_draft_branch: agent/deep-research-agent/bfc3b242
  adversarial_review_comment: c7133872 (verdict needs-attention / revise, confidence 0.72)
  authored_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  authored_at: "2026-05-19T08:30:00Z"
  ...
  round2_revision_summary:
    - "Major-1 (reproducibility): Acknowledged `op-succinct@f2bb062:book/advanced/verify-binaries.md` exists with concrete reproduction commands; documented exact runtime blockers (sp1up / cargo prove not installed, SP1 Docker builder image not pre-pulled, no chain RPC); relabeled (5b)(5c)(5d) from 'partially satisfied' to 'not independently verified by this research' per reviewer Major-1."
    - "Major-2 (live L2Beat re-verification): Re-fetched live pages 2026-05-19 for Mantle / Starknet / Facet v1 / OP Mainnet / Scroll / Arbitrum One; corrected Facet v1 from Stage 1+90d to Stage 2+90d; added verbatim L2Beat quotes for Mantle freeze risk, Starknet downgrade reason, Facet v1 downgrade reason; refreshed Mantle TVS from `~$300M` to `$1.42B` (2026-05-19 retrieval)."
    - "Major-3 (citation tightening): Corrected repository path from `program/` to `programs/range/*` and `programs/aggregation` (per `op-succinct@f2bb062:Cargo.toml` workspace members); corrected Rust pin from `nightly-2026-02-15` to `nightly-2025-09-15` (per `rust-toolchain.toml`); corrected SP1 SDK pin from `v6.0.2` to `=6.1.0` (per `Cargo.toml` L164/L166); added L2Beat 2026-05-19 verbatim cite for SP1Verifier route addresses; rebuilt item-6 freeze-window matrix with explicit 30d/90d/180d/indefinite per (whitelist, prover, verifier-route) scenario."
  caveats_inherited:
    - C2 (Starknet gate-4 sourcing) — from stage1-case-studies; this draft re-verifies via 2026-05-19 live snapshot and now has independent quote of Starknet downgrade reason ('not all program sources are public or not all program hashes can be independently regenerated', WebSearch 2026-05-19); residual uncertainty noted in updated G-7.
---

# Mantle Proposer 去中心化与 ZK Verifier 合规分析 — Deep Draft Round 2

> **Round 2 revisions** address Adversarial Agent review comment `c7133872` (verdict: needs-attention / revise, severity major, confidence 0.72). Three major findings accepted by Orchestrator and resolved as documented per item below; full revision summary in `draft_metadata.round2_revision_summary`. Unflagged Round-1 content preserved.
...
#### Live L2Beat re-verification 表（2026-05-19 retrieval；不直接继承 WHI-41 无 caveat 字段）

| 项目 | Stage (live) | proposer 机制 | ZK trusted setup | (5a-5d) 子项状态 (live) | downgrade countdown / grace period | retrieval evidence |
|------|-------------|--------------|-----------------|------------------------|----------------------------------|--------------------|
| **Mantle** | **Stage 0** (5 reqs met, Stage 1: 1 issue) | single whitelisted EOA `0x6667…D77D` | SP1 PLONK (Aztec Ignition 🟢) | (5a) ✅ / (5b)(5c)(5d) 工具完整但未独立验证 | Forum #413 grace period (researcher-derived ~2026-08-16) | L2Beat Mantle 2026-05-19 WebFetch ✅ |
| **Arbitrum One** | **Stage 1** | permissionless via BOLD (proposer bond 必须；watchtower nodes permissionless) | n/a (OR) | (5a)(5b)(5c) all applicable, status not independently re-verified in this turn — Round 1 carry-forward via WHI-41 (with C2 caveat) + WebSearch 2026-05-19 indicates "only L2 at Stage 1 via BOLD" | n/a | WebSearch 2026-05-19 confirms BOLD live on Arbitrum One/Nova/Sepolia; L2Beat live page WebFetch 内容被截断 (G-5) |
| **OP Mainnet** | **Stage 1** (per WebSearch 2026-05-19) | permissionless via DisputeGameFactory (bond 0.08 ETH per claim, cumulative max 691.43 ETH at depth 73) | n/a (OR) | (5a)(5b)(5c) — status not independently re-verified | n/a | WebSearch 2026-05-19 ("OP Mainnet" L2Beat Stage 1 DisputeGameFactory permissionless fault proofs 2026); L2Beat live page WebFetch 截断 (G-5) |
| **Scroll** | **Stage 1** ZK Rollup (per WebSearch 2026-05-19, "Stage 1 networks include … Scroll") | sequencer + permissioned proposer; "If the Proposer fails, users can leverage the source available prover to submit proofs to the L1 bridge" (WebSearch verbatim) | SNARK with trusted setup; 评级 not independently re-verified by this research | (5a) at risk under Forum #409 (red trusted setup → potential demotion); (5b-5d) status not independently re-verified | Forum #409/#413 grace period applies | WebSearch 2026-05-19 ("Scroll L2Beat Stage 1 ZK proving system 2026 trusted setup downgrade"); L2Beat live page WebFetch 截断 (G-5) |
| **Starknet** | **Stage 0** (per WebSearch 2026-05-19, "Starknet has been moved to Stage 0 because it does not satisfy upcoming Stage 1 requirements") | whitelisted operator + StarknetSCMinorityMultisig | SHARP verifier (STARK + FRI onchain) | (5d) failing: "not all program sources are public or not all program hashes can be independently regenerated" (WebSearch verbatim) — **Round 2 修正 Round 1 ~175d 倒计时声称：当前 WebSearch 显示已 downgrade，非 countdown 中** | already downgraded | WebSearch 2026-05-19 ("Starknet L2Beat Stage 1 downgrade countdown 2026 program hashes") |
| **Facet v1** | **Stage 2** (with 90d downgrade countdown to Stage 0) | OR + ZK dual-track | SP1 Turbo (Plonk + Groth16) | (5a) failing: "proof system does not meet the minimum trusted setup requirements" (WebFetch verbatim) + (5c) failing: "Not all onchain verifiers' smart contracts can be independently regenerated from the verifier source code" (WebFetch verbatim) | **90d 3h 53min 27s** (WebFetch 2026-05-19 verbatim) | L2Beat Facet 2026-05-19 WebFetch ✅ |

**Round 2 修正 vs Round 1**：
- Round 1 标 Starknet "~175d downgrade countdown" — 现 WebSearch 2026-05-19 显示已 downgraded 至 Stage 0；
- Round 1 标 Facet v1 "Stage 1 + 90d downgrade" — 现 WebFetch 2026-05-19 显示 Stage 2 + 90d 倒计时到 Stage 0；
- Round 1 标 Mantle TVS `~$300M` — 现 L2Beat 2026-05-19 verbatim `$1.42B`；
- 三处修正均来自 Adversarial c7133872 Major-2 提醒。

#### 对 Mantle 的借鉴点

- **Arbitrum BOLD**：bond + trustless bonding pool 的 sybil resistance 设计；Mantle 方案 B 若实施可参考 bond 经济学（但 ZK rollup 对 bond 的需要弱于 OR，因 SP1Verifier 直接验证 proof 合法性，spam attack 的 economic cost 已由 proof generation 成本承担）；
- **OP Mainnet DisputeGameFactory**：permissionless fault proof 的 bond 设计（0.08 ETH 起步）作为方案 B 经济学参考；
- **Scroll**：source available prover + permissionless fallback 模式（"If the Proposer fails, users can leverage the source available prover"）—— 与 Mantle 方案 C 类似，建议跟踪 Scroll v2 的 (5a) trusted setup 路径；
- **Starknet**：已 downgrade 案例直接证明 Forum #413 (5d) "program sources / program hashes" 是 hard blocker；Mantle 必须在 grace period 前完成 (5d) attestation；
- **Facet v1**：(5a)(5c) 不达标可在 Stage 2 项目上触发 90d 倒计时 — 直接同类型证据，证明 SP1 wrap path 选择 + verifier reproducibility 是 evaluation 的核心。
...
6. **回滚策略**：v3.0.0-rc.1 上线后若 fallbackTimeout 触发 spam，6/14 可调用 setter 缩短 fallback (前提：v3 合约 ABI 已暴露 setter — 本研究未独立确认 setter 是否暴露 owner，G-10)；最坏情况回退到 v2.0.1 需准备 6/14 紧急升级。
7. **风险清单**：
...
#### evidence_sources (item-7)

- **Primary**：L2Beat Mantle 2026-05-19 WebFetch (verbatim quotes — item-1 evidence)
- **Primary**：L2Beat Facet 2026-05-19 WebFetch (Stage 2 + 90d 03:53:27 countdown + (5a)(5c) failure 原文)
- **Primary**：WebSearch 2026-05-19 ("Starknet L2Beat Stage 1 downgrade countdown 2026 program hashes")
- **Primary**：WebSearch 2026-05-19 ("Arbitrum One L2Beat Stage 1 BOLD bond permissionless validation 2026")
- **Primary**：WebSearch 2026-05-19 ("OP Mainnet L2Beat Stage 1 DisputeGameFactory permissionless fault proofs 2026")
- **Primary**：WebSearch 2026-05-19 ("Scroll L2Beat Stage 1 ZK proving system 2026 trusted setup downgrade")
- **Secondary**：Wave-0 `stage1-case-studies` final (commit 146ad79, with C1/C2 caveats — 本 item 已重新验证关键差异)
- **Primary**：`mantle-xyz/op-succinct@f2bb062:contracts/src/validity/OPSuccinctL2OutputOracle.sol` (v3.0.0-rc.1 fallbackTimeout 实现)
...
| src-2 (L2Beat + 对标项目页) | L2Beat ZK Catalog + 对标对象 live snapshot | 6 | 4 直接 WebFetch (Mantle ✅, Facet ✅) + 4 间接 WebSearch (Arbitrum, OP Mainnet, Scroll, Starknet — L2Beat 直接 WebFetch 截断 G-5) | ⚠️ partial — see G-5 |
...
| src-6 (industry) | 对标项目官方 docs + L2Beat 项目页 | 5 | Arbitrum BOLD docs (via WebSearch), Optimism fault proof blog (via WebSearch), Scroll L2Beat (via WebSearch), Starknet community forum (via WebSearch), Facet L2Beat (WebFetch) | ✅ |
...
| **G-5** | L2Beat live 页面 WebFetch 对 Starknet / OP Mainnet / Scroll / Arbitrum One 返回 "Content truncated"；本研究改用 WebSearch 2026-05-19 索引 + Wave-0 carry forward；retrieval timestamp 与 verbatim 引用见 item-7 evidence_sources。Adversarial Major-2 提醒已**部分采纳**：Mantle / Facet 已直接 WebFetch 确认，Starknet / Scroll / OP / Arbitrum 因 fetch 工具限制改用 WebSearch (含 timestamp) | major (per Adversarial Major-2) | Round 3 直接通过 https + browser inspector 重新 fetch 四页 full content；或 L2Beat 团队 RSS / API 接入 |
...
| **G-10** | OPSuccinctL2OutputOracle v3.0.0-rc.1 是否暴露 `fallbackTimeout` setter (例如 `updateFallbackTimeout(uint256) onlyOwner`) 本研究未独立 `grep` 确认 (仅确认 immutable / initializer / proposer-check 3 处 occurrences) | minor | Round 3 `grep -n "fallbackTimeout" -- contracts/src/` 完整审查 setter |

---

## Revision Log

| Section | Round 1 | Round 2 | Reason |
|--------|---------|---------|--------|
| Executive Summary | TVS `~$300M`；称未发现 OP Succinct reproducibility 工具；Starknet downgrade countdown ~175d；Facet v1 标 Stage 1 + 90d | TVS `$1.42B` (L2Beat 2026-05-19 verbatim)；明确引用 `op-succinct@f2bb062:book/advanced/verify-binaries.md` 完整公开；Starknet 已 downgrade 至 Stage 0；Facet v1 标 Stage 2 + 90d 倒计时；Forum #413 grace cliff (researcher-derived ~2026-08-16) 显式标 estimate | Adversarial c7133872 Major-1 + Major-2 |
| item-2 (SP1 architecture) | (5b)(5c)(5d) 标 "partially satisfied" 或 "evidence unavailable" | 改为 "not independently verified by this research"；引用 verify-binaries.md 53 行完整 reproduce 命令；修正 Rust pin `nightly-2026-02-15` → `nightly-2025-09-15`；SP1 SDK `v6.0.2` → `=6.1.0`；目录 `program/` → `programs/range`, `programs/aggregation` | Adversarial Major-1 + Major-3 |
| item-3 (Stage 1 mapping) | 主要 carry forward | 新增 (5a-5d) Stage 三栏 gap matrix；明确 Forum #413 grace cliff 推算 (G-8 caveat) | Adversarial Major-1 |
| item-4 (3-scheme comparison) | 方案 A 未明确 transitional 标注 | 方案 A 显式 "transitional, NOT Stage 1 sufficient"；新增 stage_classification + withdrawal_freeze_evidence 8 维评估；新增 OP Succinct upstream `FALLBACK_TIMEOUT_FP_SECS` evidence chain (AccessManager.sol + AccessManager.t.sol + deploy.md + environment.md) | Adversarial Major-2 + outline Major-2 |
| item-5 (force inclusion) | 主要 carry forward | 增加 12h verbatim L2Beat quote；维持方案 5.1 (6h) 推荐 | minor |
| item-6 (failure scenarios) | freeze 量化不充分 | 30d/90d/180d/indefinite × (whitelist/prover/route) 完整矩阵；TVS 基准从 `~$300M` 更新为 `$1.42B`；新增 v2.0.1 vs v3.0.0-rc.1 对比 | Adversarial Major-2 + Major-3 |
| item-7 (case studies) | Starknet ~175d countdown；Facet Stage 1 + 90d；OP / Scroll / Arbitrum carry-forward WHI-41 无 re-verify | Live 2026-05-19 re-verify：Starknet 已 downgrade；Facet Stage 2 + 90d 03:53:27；Mantle 直接 WebFetch；Arbitrum/OP/Scroll WebSearch 2026-05-19 + L2Beat WebFetch 截断标 G-5 | Adversarial Major-2 + outline minor |
| diag-2 | (5a-5d) 节点不完整 | 4 个 reproducibility 检查节点全标注当前判定（✅ / ⚠️ / ❌）；保守标 ⚠️ 而非 ❌ | Adversarial Major-1 + outline modify_diagram diag-2 |
| diag-4 | 方案 A 未明确 transitional | 方案 A 标 "transitional, NOT Stage 1 sufficient" 红色节点 | outline modify_diagram diag-4 |
| diag-5 | 状态节点缺 freeze-window 量化 | 每个 frozen 状态附 30d/90d/180d/indefinite 量化与 Stage 1 blocker 判定 | outline modify_diagram diag-5 |
| Source Coverage | 主要 carry forward | 新增 G-1/G-2/G-3/G-5/G-8/G-10 6 项 gap；区分 L2Beat WebFetch 成功 (Mantle/Facet) vs WebSearch fallback (4 项) | Adversarial Major-2 |
| Gap Analysis | G-1～G-4 简略 | 新增 G-5～G-10 6 项；G-1 重写为 "evidence available, not independently regenerated by this research" 而非 "tools not found" | Adversarial Major-1 + Major-2 + Major-3 |
