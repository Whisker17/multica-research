...
draft_metadata:
  ...
  base_draft: mantle-stage1-rollup/research-sections/stage1-case-studies/drafts/round-2.md
  base_draft_commit: 1779afc3
  source_count_estimate: 80+ URLs across L2Beat, official docs, governance forums, on-chain explorers, and audit reports
  revision_focus: "Adversarial Round 2 feedback: (1) MAJOR rewrite Item-7(C) exit-window matrix with three-window separation; (2) MAJOR update Item-8 upgrade exit window row to be consistent with Item-7(C); (3) MINOR Starknet gate (4) cite Starknet-specific L2Beat/ZK Catalog text directly (replace Sorare analogy); (4) MINOR Scroll proving-system PASS explicitly labeled as author synthesis (not published L2Beat verdict); (5) MINOR fill M1/M3/M5 gaps (Base DisputeGameFactoryProxy address, Scroll program hash hex values, OP 13 SC EOA table)"
---
