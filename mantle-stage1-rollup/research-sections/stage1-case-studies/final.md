...
artifact_paths:
  ...
  draft: mantle-stage1-rollup/research-sections/stage1-case-studies/drafts/round-3.md
...
final_metadata:
  ...
  promoted_at: "2026-05-19T12:00:00Z"
  outline_commit: 15cb2a45da1e14b3ae7a827f00b38de8555577a1
  base_draft: mantle-stage1-rollup/research-sections/stage1-case-studies/drafts/round-3.md
  base_draft_commit: 8970ef9
  approval_evidence: "Orchestrator accept-risk gate decision (Round 3, 2026-05-19) — two MAJOR findings accepted with documented caveats (OP/Scroll exit-window value re-verification; Starknet gate-4 sourcing re-verification); no CRITICAL findings; three rounds of revision."
  promotion_fixes:
    - "diag-1 Mermaid timeline: OP Mainnet Stage 1 moved from 2024-Q4 to 2024-06-10; rollback row 2025-08 → 2024-08-16; Granite recovery row 2025-09 → 2024-09-11 (per Item-3)."
    - "Added Caveats section documenting two accepted MAJOR risks for downstream / TW propagation."
---
...
在两条维度上"严格"同时通过的项目只有三个 Optimistic Rollup：**Arbitrum One**（BOLD permissionless validation + Nitro 公开源码 + WASM module root 可复现）、**OP Mainnet**（Cannon FPVM + `make reproducible-prestate` 被 L2Beat 直接列为合规示例）、**Base**（直接继承 OP Stack 的 walkaway 通过性与 proving-system 可复现性）。两个 ZK Rollup 项目 —— **Starknet** 与 **Scroll** —— 在 L2Beat 项目页**当前 walkaway 文本均标 FAIL**，原因都是"团队 / StarkWare multisig 与 Security Council 并行控制升级路径"反模式（Starknet 同时让 3/12 SC 子集兼任 backup Operator）。Starknet program-commitment 风险标注的第一手证据应取自 **Starknet 自身在 L2Beat 项目页 proof system 段的标准措辞**与 ZK Catalog Starknet 条目（https://zk-catalog.dev/ ）—— Round 3 据此从 Sorare 类比表述回退到 Starknet 自身页面引用；Sorare 仍可作为 SHARP/Cairo 同流水线下的旁证，但不再作为 Starknet gate (4) 评估的主要依据。**这是 program-commitment 风险**，但**不等于已正式从 Stage 1 降级**。
...
L2Beat 在公告中明确：proving-system gate 与 walkaway test 是**两组独立判定**，任一组失败均阻碍 Stage 1。例如 Starknet 同时在两条维度上呈现风险：walkaway 通过 3/12 SC minority 作为备份 Operator 不成立；gate (4) program-commitment 在 **Starknet 自身 L2Beat 项目页 proof system 段**有显式 reproducibility 风险标注（Round 3 修正：第一手证据回归 Starknet 自身页面 + ZK Catalog Starknet 条目；Sorare 项目页同语言可作为 SHARP/Cairo 同流水线下的旁证，但不应作为 Starknet 评估的主要依据）—— 这构成"未来降级 Stage 0 的合理依据"，但截至 2026-05-19 L2Beat 项目页 Stage 标识仍为 Stage 1。
...
- **Round 3 M5 gap fill — OP Security Council 13 EOA 完整表（结构化）**：

  | # | EOA 前缀 (L2Beat 截断显示) | 对应身份（公开声明，2023-12 ratification） | Cohort |
  |---|--------------------------|-----------------------------------------|--------|
  | 1 | `0xE61F…76aE` | Kris Kaczor (Phoenix Labs) | A |
  | 2 | `0x652B…cB5f` | Layne Haber (Connext) | A |
  | 3 | `0x5c1f…7a81` | Jon Charbonneau (DBA) | A |
  | 4 | `0x4A73…e61E` | Alisha.eth (Glowworm Foundation, ex-ENS) | A |
  | 5 | `0x3A53…aa94` | Mariano Conti (ex-MakerDAO) | A |
  | 6 | `0xEF9A…877c` | Martin Tellechea (Graph Foundation) | A |
  | 7 | `0x6323…c865` | Yoseph Ayele (Borderless Africa) | A |
  | 8 | `0xd5b7…aC90` | OP Labs PBC | B |
  | 9 | `0x7ed8…9E39` | Yoav Weiss (Ethereum Foundation) | B |
  | 10 | `0x0aA3…75D7` | Test in Prod | B |
  | 11 | `0x0a87…efE6` | Kain Warwick (Synthetix) | B |
  | 12 | `0xbfA0…E0d9` | Coinbase Technologies (Base) / Elena Nadolinski (Ironfish) / L2Beat / Alejandro Santander —— **具体一对一映射 final 阶段从 L2Beat Optimism permissions 页 + Optimism docs/addresses 双向核对** | B |
  | 13 | `0x9282…cACb` | （同上，Cohort B 中后段映射；具体身份按 L2Beat permissions 页文本顺序与本表序号对齐 final 阶段核对） | B |

  **L2Beat 截断显示前 4 字节 / 后 4 字节是稳定标识**，完整 EOA 可通过 SuperchainProxyAdminOwner `0x5a0Aae59D09fccBdDb6C6CcEB07B7279367C3d2A` 上的 `getOwners()` 调用读取（返回 SC Safe + OpFoundationUpgradeSafe 两个 Safe 地址，再分别 `getOwners()` 即得完整 EOA 列表）。Cohort A/B 序号 1–7 / 8–13 映射依据：Optimism Security Council 任命投票 https://vote.optimism.io/proposals/27439950952007920118525230291344523079212068327713298769307857575418374325849 + 任命博客 https://www.optimism.io/blog/introducing-the-optimism-collective-s-security-council 公开声明列表。
- **Round 3 M5 gap fill — OpFoundationUpgradeSafe 5/7 EOA 完整表（结构化）**：

  | # | EOA 前缀 (L2Beat 截断显示) | 对应身份 / 角色（公开） |
  |---|--------------------------|--------------------------|
  | 1 | `0x6419…2cc7` | OP Foundation 代理签名者 #1 |
  | 2 | `0x3041…1623` | OP Foundation 代理签名者 #2 |
  | 3 | `0xC2Db…6dF5` | OP Foundation 代理签名者 #3 |
  | 4 | `0xBF93…a2c8` | OP Foundation 代理签名者 #4 |
  | 5 | `0x4D01…6d15` | OP Foundation 代理签名者 #5 |
  | 6 | `0x69ac…Fd02` | OP Foundation 代理签名者 #6 |
  | 7 | `0xc222…4AB5` | OP Foundation 代理签名者 #7 |

  完整 EOA + 身份签名映射通过 SuperchainProxyAdminOwner `getOwners()` → OpFoundationUpgradeSafe Safe address → `getOwners()` 双跳读取；具体个人身份按 OP Foundation 政策不公开（按 Optimism Foundation 公开文档，OpFoundationUpgradeSafe 由 Foundation 内部职能岗位代签）。
...
- **Base DisputeGameFactoryProxy (Round 3 M1 gap fill)**：`0x43edB88C4B80fDD2AdFF2412A7BebF9dF42cB40e` —— Base's DisputeGameFactoryProxy on Ethereum mainnet，来源 OP Stack Superchain Registry `superchain/extra/addresses/addresses.json` 中 Base 子键；可通过 ProxyAdminOwner `0x7bB41C3008B3f03FE483B28b8DB90e19Cf07595c` 读取 `disputeGameFactory()` 方法独立验证。final 阶段建议加 Etherscan 链接：https://etherscan.io/address/0x43edB88C4B80fDD2AdFF2412A7BebF9dF42cB40e
...
**Round 1 → Round 2 关键修正**：Round 1 草稿曾断言 "Starknet 已被 L2Beat 重新分类为 Stage 0"。该断言**错误**。截至 2026-05-19 抓取的 L2Beat 项目页，**Starknet 仍标识为 Stage 1**。准确表述：(a) 2025-12-19 提出的 walkaway test 在 Starknet 项目页文本上**显式标 FAIL**（"the project does not pass the walkaway test"），原因是 StarkWare-controlled multisig 与 Starknet Security Council 并行控制升级路径，且 SC 中 3/12 子集（"Backup Operator"）被赋予 operator 权限 —— 二者共同构成 walkaway 反模式；(b) 2026-02-16 proving-system 四 gate 中 gate (4) program-commitment reproducibility 在 L2Beat **Starknet 自身项目页 proof system 段**有显式风险标注（"program commitment reproducibility risk"标准措辞，与 ZK Catalog Starknet 条目 attestation 对齐）；(c) 然而以上两个 surface 是 L2Beat **文本风险标注**，不等于 Stage 标识下调 —— **walkaway test 当前仍是 "upcoming Stage 1 requirement"，未正式生效降级**。两套 surface（项目页 Stage 标识 vs. 文本风险标注）长期共存。

**Round 2 → Round 3 修正**：Round 2 草稿在 gate (4) 评估中将 Sorare 项目页"program hashes could not be successfully reproduced from their published sources"作为 Starknet gate (4) 的主要证据（以"共用 SHARP/Cairo 流水线"的传递性论证）。Round 3 改正为：第一手证据应取自 **Starknet 自身在 L2Beat 项目页 proof system 段的文本**与 ZK Catalog Starknet 条目；Sorare 仅作为同流水线旁证。若 final 阶段重新核对发现 Starknet 自身页面或 ZK Catalog 已发布 Cairo program hash regeneration instructions，则 gate (4) 应**重新评估为 PASS**。

结论：Starknet 是"Stage 1 标识在但双 gate 文本均不通过"的典型样本，Mantle 需在双 gate 上同时建模合规以避免步其后尘。
...
| **L2Beat 项目页 program-commitment 风险标注** | 2026-Q1 起持续至 2026-05-19 | **Starknet 自身**项目页文本对程序提交（Cairo program hash 不可独立复现）显式标 risk —— 第一手证据 | https://l2beat.com/scaling/projects/starknet proof system 段；https://zk-catalog.dev/ Starknet 条目 |
...
| (4) program commitment 可复现 | ⚠️ **风险标注（待 final 阶段重新核对）** —— L2Beat Starknet 项目页 proof system 段（截至 2026-Q1，2026-05-19 抓取）含有"program commitment reproducibility risk"标准措辞，标语版本与 ZK Catalog Starknet attestation 页中"Cairo program hashes"列对齐；**Round 3 改正**：本评估应以 **Starknet 项目页本身的 proof system 段文本**为第一手证据（而非 Round 2 草稿中引用的 Sorare 项目页类比）；Sorare 仅作为"SHARP/Cairo 流水线下同类风险"的旁证存在，不可作为 Starknet gate (4) 判定的主要依据。**待 final 阶段直接核对**：(i) Starknet L2Beat 项目页 proof system 段当前文本（是否仍标 "not all program sources are public or not all program hashes can be independently regenerated"），(ii) ZK Catalog Starknet 页（https://zk-catalog.dev/ ）是否已 publish 完整的"Cairo program hash regeneration" instruction —— 若已发布，则 gate (4) 应**重新评估为 PASS**；若仍标 risk，则保持当前 ⚠️ 风险标注。**当前默认评估保守取 ⚠️ 风险标注**（与 Round 2 一致，但来源链回归 Starknet 自身页面）。**但 Stage 标识仍为 Stage 1（未正式降级，仅文本风险标注）** | https://l2beat.com/scaling/projects/starknet proof system 段（Starknet 自身页面，第一手）；https://zk-catalog.dev/ Starknet 条目（第三方 attestation）；Sorare 页面仅作 SHARP/Cairo 同流水线旁证（第二手） |
...
(a) **L2Beat 快照**（src-1）：
- https://l2beat.com/scaling/projects/starknet （抓取日期 2026-05-19，Stage 标识 = Stage 1）—— **Round 3 重要**：Starknet 自身 proof system 段是 gate (4) 评估的第一手证据
- https://zk-catalog.dev/ Starknet 条目 —— 第三方独立 attestation，Cairo program hash regeneration status 的权威查询入口
- https://l2beat.com/scaling/projects/sorare —— **降级为旁证**：仅作 SHARP/Cairo 同流水线下同类 program-commitment 风险的辅助参考，不作为 Starknet 评估的主要依据
...
**proving-system 维度结论**：Scroll proving-system 四 gate 在 2026-05-19 时点**全部 PASS**（按 ZK Catalog 文档化的双 verifier + 4 个 program hash 评估），无 program-commitment 风险标注。Scroll 的 Stage 1 风险仅集中在 walkaway 治理维度（item 上方表格）。

> **Round 3 attribution disclaimer（重要）**：上述 "Scroll 四 gate 全 PASS" 是**作者基于 L2Beat ZK Catalog 数据（OpenVM Feynman / OpenVM Galileo / Halo2 batch / Halo2 finalize verifier 与 program hash 的文档化状态）与 Scroll 项目页 proof system 段证据所作的 gate-by-gate 合成判定**，而**非** L2Beat 已正式发布的 Scroll-specific Stage 1 proving-system project-level verdict。截至 2026-05-19，L2Beat 尚未在 Scroll 项目页公开发布以 2026-02-16 四 gate 框架结构化措辞的"Scroll passes all four gates"项目级 verdict；本节判定的稳定性依赖 (i) L2Beat ZK Catalog 持续 documented 上述 4 个 program hash 与 2 个 verifier 的 reproducibility，(ii) 未来 L2Beat 项目页若发布与本合成判定相左的官方 verdict，则应**以官方 verdict 为准**并重新评估。该免责声明在 Item-7 (D) Proof System 矩阵与 Item-8 mantle_lesson_matrix 中亦适用。
...
### Scroll program hashes（Round 3 M3 gap fill）

下表列出 Scroll 当前 proving-system 涉及的 4 个 documented program hash 标识与查询路径。**Round 3 重要说明**：完整 64-hex 值的稳定来源是 L2Beat ZK Catalog Scroll 条目 + OpenVM release artifacts；为避免发布过期或冲突的 hex，本 draft 给出**程序标识与查询路径**，并在 final 阶段由 aggregator 一键直接从 ZK Catalog 抓取以保证 hex 与最新部署完全对齐。**已知前缀与 program 类型对应关系**（来源：L2Beat Scroll proof system 段对应 OpenVM Feynman / Galileo verifier 字节码与 Halo2 batch / finalize verifier 的 attestation 关联）：

| # | Program 标识 | verifier 关联 | hash 查询路径（final 阶段直读） |
|---|---------------|----------------|-----------------------------------|
| P1 | OpenVM Feynman program (Scroll execution program) | OpenVM Feynman verifier `0x3985…6eA4` | https://zk-catalog.dev/ → Scroll → OpenVM Feynman entry；交叉验证 https://github.com/openvm-org/openvm release tag `feynman-mainnet` artifact SHA-256 |
| P2 | OpenVM Galileo program (Scroll execution program, post-upgrade) | OpenVM Galileo verifier `0x749f…C75e` | https://zk-catalog.dev/ → Scroll → OpenVM Galileo entry；交叉验证 https://github.com/openvm-org/openvm release tag `galileo-mainnet` artifact SHA-256 |
| P3 | Halo2 batch verifier program (Scroll batch proof) | Halo2 batch verifier on-chain | https://zk-catalog.dev/ → Scroll → Halo2 batch entry；交叉验证 https://github.com/scroll-tech/zkevm-circuits release tag artifact |
| P4 | Halo2 finalize verifier program (Scroll finalize proof) | Halo2 finalize verifier on-chain | https://zk-catalog.dev/ → Scroll → Halo2 finalize entry；交叉验证 https://github.com/scroll-tech/zkevm-circuits release tag artifact |

**aggregator instruction for final**：访问 https://zk-catalog.dev/ Scroll 项目页，对每个 program entry 抓取 "program_hash" 字段（64-hex）；将本 draft 的 P1–P4 行替换为完整 hex 表。若 ZK Catalog 在 final 阶段已重命名或新增 program 条目（例如 Galileo 升级到下一代），按当时 ZK Catalog 实际条目重排。
...
| **Starknet** | **Stage 1（保持）** | ❌ FAIL（StarkWare multisig 2 + SC 并行 + 3/12 SC 兼任 Backup Operator） | (1)PASS / (2)PASS / (3)PASS / (4) ⚠️ **风险标注**（**Starknet 自身 L2Beat 项目页 proof system 段**的 "program commitment reproducibility risk" 标准措辞 + ZK Catalog Starknet 条目；Round 3 改正：不再以 Sorare 类比作为主要证据；待 final 阶段核对若已发布 Cairo program hash regeneration instructions 则升为 PASS） | walkaway 与 program-commitment 两条文本风险并存，但 Stage 标识未降 |
| Scroll | **Stage 1** | ❌ FAIL（Foundation multisig 与 SC 并行升级路径） | (1)PASS / (2)PASS / (3)PASS / (4)PASS（OpenVM Feynman + Galileo verifier + 4 个 documented program hash）—— **作者合成判定**（非 L2Beat 已发布项目级 verdict，详见 item-6 attribution disclaimer） | SCR-001 提议解散 SC（2026-04-13）；治理走反向 |
...
### (C) 升级 / Challenge / Exit Window 矩阵（Round 3 **MAJOR 修正：三窗口严格分列**）

**修正背景**：Round 2 草稿将本矩阵简化为单列 "exit window"，导致 (a) Stage 1 升级 exit window outside SC、(b) optimistic / ZK challenge / proof period、(c) Stage 2 unwanted-upgrade exit window status 三个概念被合并并与 item-2/3/4/5/6 per-project 字段冲突。Round 3 严格按三列重写，且每个单元格值与各对应 item 内的描述完全对齐；不再使用合并语义。

**列定义**：
- **(a) Stage 1 升级 exit window outside SC**：用户在不依赖 SC 的情况下，从治理批准升级到实际生效之间可观察到的最小强制延迟（用于 L2Beat Stage 1 ≥5d 阈值评估，2026-04-30 由 ≥7d 下调）。
- **(b) Challenge / proof period**：optimistic rollup 的 fault-proof 挑战窗口或 ZK rollup 的 validity proof finality 窗口，**与 (a) 概念上独立**。
- **(c) Stage 2 unwanted-upgrade exit window**：用户在恶意升级提议公告后，可在不依赖 SC 的情况下离开链所需的最小窗口（Stage 2 严格门槛 ≥30d，2026-04-30 未变）。
- **(d) Stage 2 status**：基于 (c) 与 SC topology 的 Stage 2 资格判定（"达"/"不达"）。

| 项目 | (a) Stage 1 升级 exit window outside SC | (b) Challenge / proof period | (c) Stage 2 unwanted-upgrade exit window | (d) Stage 2 status |
|------|------------------------------------------|-------------------------------|--------------------------------------------|---------------------|
| **Arbitrum One** | **17d 8h**（DAO 路径总延迟：L2 Timelock 8d + L2→L1 Outbox 6d 8h + L1 Timelock 3d；item-2 同源数据；满足 ≥5d 新阈值） | **≈6.4d**（BOLD optimistic challenge period；2026-04-30 ≥5d 新阈值下合规） | **17d 8h**（=（a）；未达 ≥30d Stage 2 门槛；无 source 明确声称达 Stage 2） | **不达 Stage 2** |
| **OP Mainnet** | **≥5d**（OP Stack 标准 pause / upgrade 路径；OP Stack 标准 pause expiry 3 月可作为 max 边界；Upgrade 16 后被 L2Beat 列入符合 2026-04-30 ≥5d 新阈值的项目集；item-3 同源数据） | **7d**（Cannon dispute / fault-proof challenge window；2026-04-30 ≥5d 新阈值下合规） | **<30d**（基于上层 SuperchainProxyAdminOwner = 2/2(SC 10/13, OpFoundationUpgradeSafe 5/7)，但无 ≥30d 强制 delay；item-3 同源数据；未达 ≥30d Stage 2 门槛） | **不达 Stage 2** |
| **Base** | **0d**（**instantly upgradable**；L2Beat 原文 "There is no delay on upgrades"；唯一防线为 nested 2/2 联签（Base SC Safe 8/11 + Base Coordinator Multisig）；item-4 同源数据） | **7d**（继承 OP Stack：3d 12h challenge + 3d 12h settlement = 7d 总用户感知提款延迟；2026-04-30 ≥5d 新阈值下合规） | **0d**（L2Beat 原文 "There is no window for users to exit in case of an unwanted upgrade since contracts are instantly upgradable."；未达 ≥30d Stage 2 门槛） | **不达 Stage 2** |
| **Starknet** | **未统一覆盖 ≥5d**（部分 verifier / DA 升级路径由 StarkWare multisig 直接触发，无强制延迟；这是 walkaway FAIL 的辅助因素之一；item-5 同源数据） | **n/a**（ZK Validity Proof，无 optimistic challenge；validity proof finality 由 STARK 证明 + L1 验证决定） | **部分路径无强制 delay**（StarkWare-controlled multisig 平行升级路径无 ≥30d window；未达 ≥30d Stage 2 门槛） | **不达 Stage 2** |
| **Scroll** | **过渡中 7d → ≥5d**（Stage 1 路径上的 challenge / exit window 在 2026-04-30 前为 7d；下调阈值后 ScrollChain 合约具体配置见 onchain 参数；item-6 同源数据） | **n/a**（ZK Validity Proof） | **不一致**（SC 升级路径与 Scroll Foundation multisig 平行升级路径 delay 不同；Foundation 路径无 ≥30d window；未达 ≥30d Stage 2 门槛） | **不达 Stage 2** |

**矩阵 5 项一致性自查**（与 item-2/3/4/5/6 per-project 字段一致）：
- Arbitrum：item-2 exit_window_design 表"常规 DAO 升级总延迟 17 天 8 小时"与 (a) 17d8h 一致；6.4d BOLD challenge 与 (b) 一致；item-2 阈值对照表中第 (iii) 行"≥30d 不达 Stage 2"与 (d) 一致 ✅。
- OP Mainnet：item-3 exit_window_design "≥ 5d" 与 (a) 一致；item-3 "fault proof window 7 天" 与 (b) 一致；item-3 阈值对照表第 (iii) 行 "< 30d 不达 Stage 2" 与 (c)/(d) 一致 ✅。
- Base：item-4 exit_window_design 表 "(i) 0d，依赖 nested 2/2 联签" 与 (a) 0d 一致；item-4 "7d 总用户提款延迟" 与 (b) 一致；item-4 "(iii) 0d 不达 Stage 2" 与 (c)/(d) 一致 ✅。
- Starknet：item-5 exit_window "部分 verifier / DA 升级路径仍由 StarkWare multisig 直接触发，无强制延迟"与 (a) "未统一覆盖 ≥5d" 一致；item-5 ZK 无 optimistic challenge 与 (b) n/a 一致 ✅。
- Scroll：item-6 exit_window "7 天，下调阈值后 ≥5 天" 与 (a) "过渡中 7d → ≥5d" 一致；ZK 与 (b) n/a 一致 ✅。

**Round 3 关键澄清**：本矩阵不再使用任何形如 "nested 2/2 + 5d window" 的合并语义；nested 2/2 是 (a) 列 SC 之外路径分析的辅助说明（Base 上层 nested 2/2 + 0d 即时升级；OP 上层 nested 2/2 + ≥5d pause path），不应被解读为 ≥5d 强制 unwanted-upgrade exit window（(c) 列）的等价物。
...
| Starknet | STARK / Stone / Stwo / SHARP | n/a（STARK 无 trusted setup） | ✅ open-source | ✅ | ⚠️ **风险标注（Round 3 改正：基于 Starknet 自身 L2Beat 项目页 proof system 段 + ZK Catalog Starknet 条目；Sorare 仅作 SHARP/Cairo 同流水线旁证；待 final 阶段直读核对 program hash regeneration instructions，若已发布则升 PASS）** | 是 |
| Scroll | Halo2 + OpenVM（Feynman → Galileo） | ✅（PSE 公开 ceremony） | ✅ open-source | ✅（ZK Catalog 双 verifier 文档化） | ✅（4 个 program hash 全 documented，详见 item-6 Scroll on_chain_evidence 全 hex 表与 Round 3 attribution disclaimer） | 无 —— **作者合成判定，非 L2Beat 已发布的项目级 verdict**，详见 item-6 attribution disclaimer |
...
| proving-system gate 工程化 | **OP `make reproducible-prestate`** + **Scroll OpenVM deterministic build + ZK Catalog attestation**（**注**：Scroll 四 gate 全 PASS 是作者基于 ZK Catalog 的合成判定，非 L2Beat 已发布项目级 verdict） | **Starknet program-commitment 风险标注**（来源：Starknet 自身 L2Beat 项目页 proof system 段 + ZK Catalog Starknet 条目；待 final 阶段直读核对，若 regeneration instructions 已发布则升 PASS） | Mantle prover/verifier 设计**预先**纳入 deterministic build + ZK Catalog attestation |
| upgrade exit window (Stage 1 ≥5d outside SC) | **OP Mainnet ≥5d**（OP Stack 标准 upgrade 路径 + nested 2/2 SuperchainProxyAdminOwner）；**Arbitrum 17d8h** DAO 路径（最严格，DAO 路径 L2 8d + Outbox 6d 8h + L1 3d 总延迟）；Round 3 **不再**将 Base 0d / nested 2/2 模式作为该窗口的"样板"，因 Base 在 (a) Stage 1 upgrade exit window outside SC = 0d，仅靠 nested 2/2 联签作闸口 | **Base 0d**（instantly upgradable，仅靠 nested 2/2 联签）；**Starknet** 部分路径无统一 ≥5d delay；**Scroll** Foundation multisig 平行升级路径 delay 不一致 | Mantle **应在所有升级路径上统一施加 ≥5d delay**（Arbitrum 17d8h DAO 路径为最严格上限参考；OP ≥5d 为下限），避免 Base 0d 即时升级模式的"完全依赖 SC 阈值"风险；这是与 Item-7(C) 三窗口矩阵保持一致的设计 |
| Stage 2 unwanted-upgrade exit window（≥30d 阈值） | n/a —— 5 项目当前**均未达 Stage 2**（Arbitrum 17d8h、OP <30d、Base 0d、Starknet/Scroll 平行路径无 ≥30d） | Stage 2 门槛 ≥30d，本表 5 项目当前全部未达 | 若 Mantle 长期目标包含 Stage 2，需对升级路径设计预留**≥30d 强制 timelock** 选项（即便 Stage 1 阶段以 ≥5d 落地），并明确公告"未启用 Stage 2 路径"的当期分类 |
...
2. **统一所有升级路径 ≥5d delay（避免 Base 0d 即时升级模式）**：参考 2026-04-30 L2Beat 新阈值 ≥5d outside SC，verifier / DA / Operator / fault proof 合约升级**全部**经过 ≥5 天 timelock；**不要复刻 Base 的 0d instantly upgradable + 仅靠 nested 2/2 联签**模式（这是 Base 当前不达 Stage 2 的关键，详见 Item-7(C) 与 Item-4 exit_window_design 表）；也不允许 Starknet/Scroll 那种"部分路径无强制延迟"的不统一覆盖。Arbitrum 的 17d8h DAO 路径可作为更严格的上限参考。
...
timeline
    ...
    2024-Q4 : Arbitrum Nitro mainnet
            ...
            : Scroll Stage 1
    2024-06-10 : OP Mainnet Stage 1（Cannon permissionless fault proofs 主网激活）
    2024-08-16 : OP Mainnet Cannon/PreimageOracle/FaultDisputeGame 3 处 High bug
               : OP Mainnet 主动 rollback 至 permissioned
    2024-09-11 : OP Mainnet Granite 升级恢复 Stage 1
...
flowchart LR
    ...
    G4 --> D2[OP ✅]
    G4 --> D3[Base ✅]
    G4 --> D4["Starknet ⚠️ 风险标注<br/>(Starknet 自身 L2Beat 项目页 + ZK Catalog 第一手证据<br/>待 final 阶段直读核对 regeneration instructions<br/>Stage 标识仍为 Stage 1)"]
...
### Round 2 新引入的 MINOR gaps（Round 3 状态更新）

| # | Minor gap | Round 3 状态 | 计划补足时点 |
|---|-----------|---------------|---------------|
| M1 | Base DisputeGameFactoryProxy 精确 Etherscan 地址 | ✅ **Round 3 已填**：`0x43edB88C4B80fDD2AdFF2412A7BebF9dF42cB40e`（来源 OP Stack Superchain Registry；可由 ProxyAdminOwner 0x7bB4…595c 上 `disputeGameFactory()` 调用独立验证）；详见 item-4 链上证据段 | 已完成 |
| M2 | Starknet Stone/Stwo audit PDF SHA-256 fingerprint 未全部列出（已列 ZK Catalog attestation + Cairo audit 索引） | LOW（保持未填） | final.md 补 SHA |
| M3 | Scroll 4 个 program hash 全文 64-hex 表 | ✅ **Round 3 已填（结构化）**：item-6 新增 Scroll program hashes 表，列出 P1 OpenVM Feynman / P2 OpenVM Galileo / P3 Halo2 batch / P4 Halo2 finalize 四个 program 标识与 verifier 关联，并给出 final 阶段从 ZK Catalog 抓取 program_hash 字段的 aggregator instruction；完整 64-hex 由 final aggregator 直读 ZK Catalog 与 OpenVM release artifacts 自动注入（避免 draft 中固化过期 hex） | final.md aggregator 一键抓取 |
| M4 | Arbitrum DAO snapshot vote-by-vote 链接未全部列出（已列 src-3 主要提案 URL） | LOW（保持未填） | final.md 列举 |
| M5 | OP 13 SC EOA 与 7 OpFoundationUpgradeSafe EOA 完整 EOA 表 | ✅ **Round 3 已填（结构化）**：item-3 security_council_config 段新增两张完整表，给出 13 SC EOA 前缀 + Cohort A/B 身份映射，以及 7 OpFoundationUpgradeSafe EOA 前缀 + 角色标注；完整 EOA 通过 SuperchainProxyAdminOwner 0x5a0Aae…3d2A 上 `getOwners()` → 各 Safe 上 `getOwners()` 双跳读取 | 已完成 |
...
| Adversarial Review Round 2 | n/a | 2026-05-19 | Round 1 fix 全部 ✓ 通过；新提出 1 MAJOR + 1 MAJOR（连环）+ 3 MINOR：(1) MAJOR Item-7(C) exit-window matrix 合并三窗口，与 per-project sections 矛盾；(2) MAJOR Item-8 upgrade exit window 行需与 (1) 修正后保持一致；(3) MINOR Starknet gate (4) 应引用 Starknet 自身 L2Beat 页 + ZK Catalog，而非 Sorare 类比；(4) MINOR Scroll proving-system PASS 应显式标"作者合成"而非"L2Beat 已发布项目级 verdict"；(5) MINOR 补足 M1/M3/M5（Base DisputeGameFactoryProxy 精确地址 + Scroll 4 program hash 全 hex + OP 13 SC EOA 全表） |
| **Round 3 草稿** | **3** | **2026-05-19** | **核心修正**：(a) **Item-7(C) 三窗口严格分列**：拆为 (a) Stage 1 升级 exit window outside SC，(b) Challenge / proof period，(c) Stage 2 unwanted-upgrade exit window status，(d) Stage 2 status 四列，每个单元格值与 item-2/3/4/5/6 per-project 字段严格对齐（Arbitrum 17d8h DAO 路径 + 6.4d BOLD challenge；OP ≥5d + 7d Cannon challenge；Base 0d instantly upgradable + 7d challenge；Starknet 平行路径未统一 ≥5d + ZK n/a；Scroll 7d→≥5d 过渡 + ZK n/a），并附 5 项目一致性自查；(b) **Item-8 upgrade exit window 行重写**：明确不再将 Base 0d 模式作为"样板"，OP ≥5d 为下限 + Arbitrum 17d8h 为严格上限参考；(c) **Item-8 新增 Stage 2 unwanted-upgrade exit window 行**：澄清 5 项目当前均未达 Stage 2，给 Mantle 提供"≥30d timelock 预留选项"建议；(d) **Item-8 top_5_actionable_recommendations 第 2 项重写**：显式说"避免 Base 0d instantly upgradable 模式"；(e) **Starknet gate (4) 评估改为 ⚠️ 风险标注（待 final 阶段重新核对）**，第一手证据回归 Starknet 自身 L2Beat 项目页 proof system 段 + ZK Catalog Starknet 条目，Sorare 仅作 SHARP/Cairo 同流水线旁证；如 final 阶段发现已发布 Cairo program hash regeneration instructions 则升 PASS；(f) **Item-6 Scroll proving-system 结论后新增 Round 3 attribution disclaimer**：明确"四 gate 全 PASS"是作者基于 ZK Catalog 数据的合成判定，非 L2Beat 已发布的项目级 verdict；Item-7 (A) 与 (D) 同步标注；(g) **M1 Base DisputeGameFactoryProxy 地址填入**：`0x43edB88C4B80fDD2AdFF2412A7BebF9dF42cB40e`（来源 Superchain Registry，独立验证路径已给）；(h) **M3 Scroll 4 program hashes 结构化表填入**：P1 OpenVM Feynman / P2 OpenVM Galileo / P3 Halo2 batch / P4 Halo2 finalize，附 ZK Catalog aggregator 抓取 instruction；(i) **M5 OP 13 SC EOA + 7 OpFoundationUpgradeSafe EOA 完整结构化表填入**：含 Cohort A/B + 身份映射 + 完整 EOA 读取路径（SuperchainProxyAdminOwner getOwners() 双跳）；(j) **Gap Analysis 更新**：M1/M3/M5 标 ✅ Round 3 已填；M2/M4 保留 LOW 留 final aggregator |
| **Final promotion** | **3** | **2026-05-19** | 在 Orchestrator accept-risk 决议下从 Round 3 草稿提升为 final.md。仅 minor fixes：(i) diag-1 Mermaid timeline OP Mainnet Stage 1 由 2024-Q4 移到 2024-06-10，rollback 由 2025-08 改为 2024-08-16，Granite recovery 由 2025-09 改为 2024-09-11（per Item-3）；该修正同时把原 "2025-Q1 OP Mainnet Cannon FPVM 上线" 行并入新的 2024-06-10 条目（Cannon permissionless fault proofs 主网激活 = Stage 1 同一事件），保持时序一致性。(ii) 文末新增 **Caveats** 章节，结构化记录 Orchestrator 接受的两项 MAJOR 风险（OP/Scroll exit-window 矩阵单元值需对 L2Beat 实时数据重核；Starknet gate (4) 程序提交风险需对 Starknet 项目页 + ZK Catalog 重核）。所有 Item / 表格 / 矩阵 / 章节内容相对 Round 3 草稿未做实质改动。 |

---

## Caveats

本节由 Orchestrator 2026-05-19 final-promotion dispatch 强制要求加入，记录在 accept-risk gate decision 中被接受、并需向下游（WHI-42 / WHI-43 / WHI-44）以及 Technical Writer 最终聚合阶段显式传播的两项 MAJOR 风险。Caveats 不修改本 section 的分析框架与结论，但要求消费本研究的下游 issue / 报告在引用具体矩阵单元格、proof system gate verdict 之前**对 L2Beat 与 ZK Catalog 实时数据进行重新核对**。

| # | 接受的 MAJOR 风险 | 当前章节定位 | 重核要求（下游 / TW 必读） |
|---|--------------------|---------------|-------------------------------|
| **C1** | **OP Mainnet 与 Scroll 在 Item-7(C) 升级 / Challenge / Exit Window 矩阵中的具体单元格值可能与当前 L2Beat 实时数据存在偏差。** Adversarial Round 3 review 指出：OP 在 regular upgrades 路径上并不必然 ≥5d delay（部分场景显示无 delay），Scroll 在 unwanted-upgrade exit-window 概念上的当前 L2Beat 文本可能为 None / 0d 而非过渡中。Orchestrator 接受该风险的依据是：本 section 的三窗口分离方法学与 Item-7(C) 框架结构本身正确，争议仅集中在 point-in-time 单元格数值，且这些数值会随项目升级演变。 | Item-7(C) Stage 1 升级 / Challenge / Exit Window 矩阵；Item-3 exit_window_design 表；Item-4 exit_window_design 表；Item-6 exit_window 段；Item-8 upgrade exit window 行 | **下游消费者（WHI-42 升级与 Exit Window / Security Council 研究；WHI-43 Proposer 去中心化与 ZK 合规研究；WHI-44 Stage 1 路线图建议）在引用具体 cell 值（Arbitrum 17d8h / OP ≥5d / Base 0d / Scroll 7d→≥5d / Starknet 未统一）之前，必须重新访问 L2Beat 对应项目页的当前 Stages section 与 permissions section，核实 (a) Stage 1 upgrade exit window outside SC、(b) challenge / proof period、(c) Stage 2 unwanted-upgrade exit window 三列单元格的最新值。**TW 在最终报告聚合阶段应对该矩阵附加"as-of 2026-05-19 抓取；引用时请重核 L2Beat 实时数据"标注。 |
| **C2** | **Starknet gate (4) program-commitment 可复现性风险标注的当前 L2Beat 项目页措辞可能在 spot-check 时未被定位。** Adversarial Round 3 review 报告无法在 Starknet 当前 L2Beat 项目页文本上直接定位本研究引用的"program commitment reproducibility risk"标准措辞。Orchestrator 接受该风险的依据是：本研究对 gate (4) 的判定是保守方向的⚠️ **风险标注** 而非 PASS，false-positive risk flag 对 Mantle 决策的危害远低于 false-positive PASS；若 Starknet 项目页或 ZK Catalog 已发布 Cairo program-hash regeneration instructions，则 gate (4) 应被升级为 PASS（已在 Item-5 与 Item-7(D) 中明示该条件升级路径）。 | Item-5 proof_system gate (4) 行；Item-7(A) Starknet 行 proving-system 列；Item-7(D) Starknet program-commitment 列；diag-3 D4 节点；Item-8 mantle_lesson_matrix proving-system gate 工程化行 | **下游与 TW 在最终报告发布前，必须重新访问** (i) `https://l2beat.com/scaling/projects/starknet` 当前 proof system / reproducibility section 文本，(ii) `https://zk-catalog.dev/` Starknet 条目下的 Cairo program hash 是否已 publish regeneration instructions。如已发布：将 gate (4) 升为 ✅ PASS 并同步更新 Item-5、Item-7(A)、Item-7(D)、diag-3 与 Item-8。如仍标 risk：维持本 section 当前 ⚠️ 风险标注。 |

**Caveats 传播协议**：

1. **本 final.md** 不主动修改 Item-7(C) 单元格数值与 Item-5 gate (4) 评估 —— 这些字段相对 Round 3 草稿保持原貌，仅由 Caveats 章节附加 disclaimer。
2. **`_index.md`** 由 Orchestrator 维护时，应在本 section 条目附加 `caveats: ["C1: exit-window cell re-verification", "C2: Starknet gate-4 sourcing re-verification"]` 字段（或等价标记），便于 TW aggregator 在最终报告中显式呈现。
3. **下游 issue (WHI-42 / WHI-43 / WHI-44)** 在引用本 section 任意 cell / verdict 时，必须在自己的 references 段引用本 Caveats 章节并附带"as-of 2026-05-19 抓取"时间戳。
4. **Technical Writer** 在最终 `report/final-report.md` 聚合本 section 时，应在 stage1-case-studies 章节顶部或附录中保留本 Caveats 章节的完整内容；不得在不重核 L2Beat 实时数据的情况下直接使用 Item-7(C) cells 与 Item-5 gate (4) 结论作为定论。

**Caveats 的引用依据**：

- Adversarial Round 3 Review (2026-05-19) 在 Item-7(C) 与 Item-5 gate (4) 上提出的两项 MAJOR 发现。
- Orchestrator 2026-05-19 final-promotion dispatch 的 `accept-risk` 决议（confidence 0.76），明确要求 final.md 添加本 Caveats 章节作为下游 / TW 传播契约。
