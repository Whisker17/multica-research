...
artifact_paths:
  ...
  draft: mantle-stage1-rollup/research-sections/stage1-roadmap-recommendation/drafts/round-1.md
...
# Mantle Stage 1 路线图综合建议（Round 1 Draft）

> **本草案是综合 / 收尾课题（order=6）的 Round 1 deep draft，依赖上游 5 个 final sections 的所有结论。**
...
|-------|------|---------|--------|----------------------|-----------|--------|
| **Batch A** | Proposer liveness 升级 | Gap-04 | T0 | T0+11w / T0+15w | v3.0.0-rc.1 部署 + initializer 调用 + fallbackTimeout=14d | 与 Batch B/C 完全并行（合约层不重叠） |
| **Batch B** | SC + 治理 + 新合约 + 路径隔离 | Gap-02, Gap-06, Gap-07, Gap-01 | T0 | T0+12w / T0+22w | SC 9/12 上线；三新合约 audit complete + deploy；ProxyAdmin.owner → TimelockRegular；MNT 路径角色迁离 6/14 | 内部串行（SC 与 audit 并行；audit 完成 → 部署 → owner 迁移） |
| **Batch C** | ZK Proving 5a-5d reproducibility | Gap-05 | T0 | T0+8w / T0+13w | Mantle/Succinct 联合 attestation；L2Beat re-verification ；项目页文本更新 | 与 A/B 完全并行（无合约依赖） |
...
gantt
  ...
  L2Beat 项目页文本更新     :crit, p3-1, after c2, 28d
...
- 关键路径 = (Batch C → L2Beat re-verify) 与 (Batch B → Walkaway sim) 中的较长路径。
...
| AI-15 | OptimismPortal v3 升级（v3.0.0-rc.1 + fallbackTimeout=1209600，14d）| Gap-04 | 阶段 1 中期 | AI-03 audit done | PE | Upgrade tx hash + cast `version()` + `fallbackTimeout()` 输出 | yes |
...
**End of Round 1 Draft** —— 等待 Orchestrator 安排 Adversarial Review 调度。
