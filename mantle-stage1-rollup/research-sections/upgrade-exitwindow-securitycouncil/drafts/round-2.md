...
artifact_paths:
  ...
  draft: mantle-stage1-rollup/research-sections/upgrade-exitwindow-securitycouncil/drafts/round-2.md
...
draft_metadata:
  ...
  drafted_at: "2026-05-19T15:20:00Z"
  outline_commit: fe3fee2
  prior_draft_commit: e0cabef
  prior_draft_path: mantle-stage1-rollup/research-sections/upgrade-exitwindow-securitycouncil/drafts/round-1.md
  upstream_finals:
  ...
  revision_target_findings:
    - "Major-1: Emergency selector whitelist enforceability (§Item-4 / §Item-6 / diag-2)"
    - "Major-2: Walkaway Test active-pause / Guardian failure mode (§Item-5 / §Item-8 / diag-4)"
    - "Minor-3: §Item-3 SC quorum external-signer math correction"
    - "Minor-4: Source hygiene — Forum #425 attribution vs upstream Stage framework anchor"
    - "Minor-5: G-2 closure with on-chain hasRole confirmation"
...
**Round 2 关键变更**（vs Round 1）：

1. **紧急选择器白名单从"文档约束"升级为"合约层强制"**：引入 `GuardedExecutor` 路由合约，是 target 合约（`OPSuccinctL2OutputOracle`、`OptimismPortal` pause/unpause、其他 setter）的唯一 `owner`；`TimelockEmergency` 只能通过 `GuardedExecutor` 调用且**仅**白名单 selector 被通过，`TimelockRegular` 通过 `GuardedExecutor` 可调用任意 setter。这关闭了 Round 1 的高危漏洞 —— 一个 `minDelay = 0` 的 `TimelockController` 作为 `target.owner` 可以 0 延迟调用 **任何** `onlyOwner` 函数（含 `transferOwnership` / `addProposer` / `updateSubmissionInterval`），把"紧急权限白名单"完全绕过。
2. **Walkaway 暴露面新增"已 pause 失能"场景**：Round 1 将 Guardian 角色从 `MantleEngineeringMultisig` 迁出，但**没有**处理"SC 在 pause 状态下永久消失"。Round 2 引入 **auto-expiring pause 不变量**：`pause` 状态最长 `MAX_PAUSE_DURATION = 30d`，到期后自动失效；任何人可在 `pausedAt + MAX_PAUSE_DURATION` 之后调用 `permissionlessUnpause()`；强制提款合约（`OptimismPortal.proveWithdrawalTransaction` / `finalizeWithdrawalTransaction`）**长期**不在可 pause 表面，仅 `depositTransaction` 与 `proveWithdrawal` 的 pause 可被 Guardian 触发 —— `finalize` 路径永远不被阻塞。链上验证：Mantle 当前 `OptimismPortal` impl `0xe139…31a7` 版本 `1.7.0`，`pausedAt()` / `pauseDuration()` 均 revert，**不具备** auto-expiring pause，是确凿的 Stage 1 gap。
3. **§Item-3 SC quorum 数学修正**：9/12 阈值 + 7/12 外部组合，最坏情况下任何成功 quorum 含 **≥4 外部签名**（不是 Round 1 写的 "≥7"）。仍满足 "≥2 外部签名" 硬约束。
4. **来源归属清理**：明确 Forum #425 是 **OR challenge period 7d→5d** 的来源；Mantle 适用的 **outside-SC upgrade exit window ≥5d** 直接锚定 `l2beat-stage-framework-2026` final §item-3 的上游解读，**不**依赖 Forum #425 推断。
5. **G-2 / OQ-2 闭合**：链上 `hasRole` 直读确认 `MantleSecurityMultisig 0x4e59…D40f` 在 L1MantleToken `TimelockController 0x65331ff6…447F` 上同时持有 `PROPOSER_ROLE` / `EXECUTOR_ROLE` / `CANCELLER_ROLE`（cast 调用证据见 §Item-1 evidence）。三路径治理隔离 gap 规模**已确定**：MNT 路径与 core rollup 路径共享同一 6/14 Safe，必须拆分。
...
2. **可证明可执行的双轨制升级合约层（推荐架构 B':GuardedExecutor 路由）**：
   ...
   - **GuardedExecutor（新增）**：自定义合约，是 `OPSuccinctL2OutputOracle.owner` / `OptimismPortal.GUARDIAN`（pause/unpause caller）/ 其他直接 setter 的唯一 `owner`；接受来自 `TimelockEmergency` 与 `TimelockRegular` 的 `executeCall(target, data, value)`；若 `msg.sender == TimelockEmergency`，强制 `require(FunctionSelectorWhitelist[bytes4(data[:4])] == true)`；若 `msg.sender == TimelockRegular`，放行任意 selector；任何其他 caller revert。`FunctionSelectorWhitelist` 由 `TimelockRegular` 拥有，更新走 ≥5d delay；
   - **ProxyAdmin.owner = TimelockRegular**：implementation 替换（`upgradeAndCall`）只能通过 ≥5d 路径；TimelockEmergency 通过 GuardedExecutor 调用的是**直接 setter on proxy**（已部署 implementation 暴露的 onlyOwner 函数，如 `updateVerifier`），**不**触发 implementation 替换；
...
**函数选择器白名单初始集合（Round 2 精确版）**：

| 函数 | Selector | 合约 | 紧急用途 |
|------|----------|------|----------|
| `pause()` | `0x8456cb59` | OptimismPortal / L1StandardBridge / L1CrossDomainMessenger / OPSuccinctL2OutputOracle | hack 进行中冻结资金；**bounded by auto-expiring pause (§Item-5 5.f W-1)** |
| `unpause()` | `0x3f4ba83a` | 同上 | 紧急完结后恢复 |
| `updateVerifier(address)` | `bytes4(keccak256("updateVerifier(address)"))` | OPSuccinctL2OutputOracle | SP1Verifier route freeze 恢复 |
| `updateAggregationVkey(bytes32)` | `bytes4(keccak256("updateAggregationVkey(bytes32)"))` | 同上 | vkey 错误回退 |
| `updateRangeVkeyCommitment(bytes32)` | `bytes4(keccak256("updateRangeVkeyCommitment(bytes32)"))` | 同上 | 同上 |
| `updateRollupConfigHash(bytes32)` | `bytes4(keccak256("updateRollupConfigHash(bytes32)"))` | 同上 | 同上 |
| `permissionlessUnpause()` | TBD（Stage 1 新增，§Item-5 5.f） | OptimismPortal / GuardedExecutor 包装 | 用户自助解除已过期 pause |

**显式排除（绝不在 emergency whitelist）**：

| 函数 | Selector | 为何排除 |
|------|----------|---------|
| `transferOwnership(address)` | `0xf2fde38b` | 单步转让 owner = 单点放弃；必须走 ≥5d + DAO 投票 |
| `addProposer(address)` / `removeProposer(address)` | (TBD) | 改变 prover 集合 → 影响 Walkaway 路径 |
| `updateSubmissionInterval(uint256)` | (TBD) | 影响 L2 → L1 state root 节奏 |
| `upgradeAndCall(proxy, impl, data)` | `0x9623609d` | implementation 替换走 TimelockRegular |

**白名单更新机制**：必须通过 `TimelockRegular.schedule` + ≥5d delay + execute；GuardedExecutor 的 `FunctionSelectorWhitelist` owner = `TimelockRegular`，**禁止** SC / TimelockEmergency 修改白名单。

**每次紧急升级的事后公示要求**：

- 链上 event：`EmergencyUpgradeExecuted(bytes4 selector, address target, address caller, bytes calldata, uint256 timestamp)`，由 GuardedExecutor 在 `executeCall` 路径中 emit；
- 24h 内 Mantle 官方博客 + L2Beat 监控通报；
- 30d 内独立审计 + 后验报告。

**(3.f) 与现有 Mantle multisig 的迁移**

迁移步骤（与 §Item-4 阶段 1 协同 + Round 2 修订）：

1. 部署新 SC multisig（Gnosis Safe 9/12 with 7 外部 owners）；
2. 部署 TimelockEmergency + TimelockRegular（按 §Item-6 架构 B'）；
3. **部署 GuardedExecutor（Round 2 新增）**；
4. 部署 `FunctionSelectorWhitelist`，owner = TimelockRegular；
5. MantleSecurityMultisig 6/14 调用 `OPSuccinctL2OutputOracle.transferOwnership(GuardedExecutor)`（**风险点**：单步 transfer，无 acceptOwnership；建议在 v2.0.1 升级到 v3 或自定义两步 transfer 模式后再迁移）；
6. ProxyAdmin owner 通过 `Ownable.transferOwnership(TimelockRegular)` 迁移；
7. MantleEngineeringMultisig 3/7 的 challenger 角色：评估是否合并到 SC（推荐合并，Engineering Multisig 解散；或保留为单独的 Guardian Safe 但成员严格外部，避免 Scroll Team 2/4 反模式）；
8. **Guardian 角色迁移**：`OptimismPortal.GUARDIAN` 通过 implementation 内置 setter 或 ProxyAdmin upgrade 切换到 `GuardedExecutor`（详见 §Item-5 (5.f) (W-1) Guardian 重新路由方案）；
9. L1MantleToken TimelockController PROPOSER / EXECUTOR / CANCELLER 三个 role（Round 2 链上确认均由 6/14 Safe 持有）切换到独立 MNT DAO multisig（治理隔离，见 §Item-7）。
...
| 函数选择器白名单滥用 | TimelockRegular 路径上 SC 提议扩展白名单 | 白名单更新需 DAO 投票 + ≥5d Timelock；GuardedExecutor 在合约层强制 require(whitelist[selector]) |
...
**阶段 1 链上参数变更 transaction 序列**（Round 2 重写，原子化分组）：
...
tx-2-3: Deploy FunctionSelectorWhitelist
  // custom contract; stores mapping(bytes4 => bool); owned by TimelockRegular
  FunctionSelectorWhitelist.constructor(owner = TimelockRegular)

tx-2-4: Deploy GuardedExecutor (Round 2 核心新增)
  ...
  //       require(whitelist.isWhitelisted(sel), "GuardedExecutor: selector not in emergency whitelist");
  //       emit EmergencyUpgradeExecuted(sel, target, msg.sender, data, block.timestamp);
  //     } else if (msg.sender == TimelockRegular) {
  //       // any selector allowed
  //       emit RegularUpgradeExecuted(sel, target, msg.sender, data, block.timestamp);
  //     } else {
  ...
  GuardedExecutor.constructor(
    ...
    address selectorWhitelist = FunctionSelectorWhitelist
  )

tx-2-5: TimelockRegular 初始化 FunctionSelectorWhitelist
  // 通过 TimelockRegular.schedule + (immediate 0d on first config, 视设计) execute
  // 或 deployer-only 一次性 init
  FunctionSelectorWhitelist.addSelectors([
    ...
    bytes4(keccak256("permissionlessUnpause()"))  // Round 2 新增
  ])

tx-2-6: ProxyAdmin owner 迁移
  // 由 MantleSecurityMultisig 6/14 执行
  ProxyAdmin(0xca35…7794).transferOwnership(TimelockRegular)

tx-2-7: OPSuccinctL2OutputOracle owner → GuardedExecutor (Round 2 关键变更)
  // 由 MantleSecurityMultisig 6/14 执行；与 Round 1 (tx-1-5) 区别：target → GuardedExecutor，不直接给 TimelockEmergency
...
tx-2-8: Guardian 路由切换（Round 2 新增 — §Item-5 5.f Walkaway 不变量 W-1 前置）
  // 由 OptimismPortal proxy 当前 admin (ProxyAdmin → MantleSecurityMultisig) 通过 implementation 升级或
  // 现有 setter（如可用）切换：
  //   选项 A：implementation 升级到含 `setGuardian(address)` 的版本，将 Guardian 设置为 PauseTimelock 包装合约
  //   选项 B：保留 v1.7.0 实现；通过 implementation 替换为带 pausedAt + permissionlessUnpause 的 v1.7.x-mantle fork
  //   推荐选项 B（最小侵入，且能在阶段 1 内完成）
  // 注：选项 B 要求与 OP Stack 上游协调；若不可行，则 fallback 到 PauseTimelock 外层包装：
  //   PauseTimelock 合约持有 OptimismPortal 的 Guardian 角色，对 pause(...) 转发并记录 pausedAt；
  //   permissionlessUnpause() 任何人可在 pausedAt + 30d 后调用，转发 unpause() 到 OptimismPortal
  OptimismPortal(0xc54c…A8Fb).setGuardian(PauseTimelock)
  // 或：通过 ProxyAdmin 升级 implementation 到 v1.7.x-mantle fork

tx-2-9: L1MantleToken TimelockController minDelay 扩至 ≥5d
...
tx-2-10: L1MantleToken TimelockController 三角色迁移到 MNT DAO multisig
...
tx-2-11: SC multisig 在双 Timelock + GuardedExecutor 上的角色赋予
...
tx-2-12: 撤销旧 multisig 权限 + 链上验证
  // 在 tx-2-6 / tx-2-7 / tx-2-8 / tx-2-10 之后，
  // 各 owner / role 关系应满足：
  cast call OPSuccinctL2OutputOracle owner() == GuardedExecutor
  cast call ProxyAdmin owner() == TimelockRegular
  cast call OptimismPortal GUARDIAN() == PauseTimelock (or 等价含 auto-expiry 的合约)
  ...
  cast call FunctionSelectorWhitelist.owner() == TimelockRegular
```

**tx 序号变更说明（vs Round 1）**：
- Round 1 共 9 条 tx（tx-1-1 ~ tx-1-9）；Round 2 共 12 条（tx-2-1 ~ tx-2-12）；
- 新增：tx-2-4 (GuardedExecutor)；tx-2-8 (Guardian 路由切换)；tx-2-10 (MNT TimelockController 三角色迁移)；
- 修改：tx-2-7 (OPSuccinctL2OutputOracle owner 改为 GuardedExecutor，不再直接给 TimelockEmergency)；
- 重新编号：所有 tx-1-* → tx-2-*。
...
3. **演练 scenario A — SC 未 pause 即消失**：
   ...
   - (g) 用户 A 拿回 10 ETH；
4. **演练 scenario B (Round 2 新增) — SC pause 后消失**：
   - (a) SC 收到攻击信号，9/12 签名调用 GuardedExecutor.executeCall(OptimismPortal, pause(), 0)；GuardedExecutor 强制 require(whitelist[0x8456cb59])，通过；OptimismPortal.paused() = true；PauseTimelock 记录 pausedAt = block.timestamp；
   - (b) SC 全员私钥销毁；
...
| 实现选项 1 (推荐) | 部署 PauseTimelock 包装合约：持有 Guardian 角色；pause(...) 由 SC 通过 GuardedExecutor 经过白名单转发；unpause(...) 与 permissionlessUnpause() 任何人/SC 可调用；存储 pausedAt |
...
PauseTimelock 合约（W-1）+ GuardedExecutor 合约（§Item-6）+ FunctionSelectorWhitelist 合约 = 阶段 1 三个新部署 contract，建议在阶段 1 部署前**走全量独立审计**。
...
**结论**：**架构 B'（双 TimelockController + GuardedExecutor 路由）**。

**(6.b) GuardedExecutor 合约设计（Round 2 核心新增）**
...
contract GuardedExecutor {
    ...
    FunctionSelectorWhitelist public immutable whitelist; // owned by timelockRegular
    ...
    constructor(address te, address tr, address fsw) {
        timelockEmergency = te;
        timelockRegular = tr;
        whitelist = FunctionSelectorWhitelist(fsw);
    }
    ...
    function executeCall(
        ...
        if (msg.sender == timelockEmergency) {
            require(
                whitelist.isWhitelisted(sel),
                "GuardedExecutor: selector not in emergency whitelist"
            );
            emit EmergencyUpgradeExecuted(sel, target, msg.sender, data, block.timestamp);
        } else if (msg.sender == timelockRegular) {
            // regular path: any selector allowed
            emit RegularUpgradeExecuted(sel, target, msg.sender, data, block.timestamp);
        } else {
            revert("GuardedExecutor: unauthorized caller");
        }
...
**FunctionSelectorWhitelist 合约**：

```solidity
contract FunctionSelectorWhitelist is Ownable {
    mapping(bytes4 => bool) public isWhitelisted;
    ...
    function addSelector(bytes4 selector) external onlyOwner {
        isWhitelisted[selector] = true;
        emit SelectorAdded(selector);
    }
    function removeSelector(bytes4 selector) external onlyOwner {
        isWhitelisted[selector] = false;
        emit SelectorRemoved(selector);
    }
    function addSelectors(bytes4[] calldata selectors) external onlyOwner {
        for (uint256 i; i < selectors.length; ++i) {
            isWhitelisted[selectors[i]] = true;
            emit SelectorAdded(selectors[i]);
        }
    }
}
```
...
DAO governance → 投票通过
  → TimelockRegular.schedule(
      target = FunctionSelectorWhitelist,
      data = abi.encodeWithSelector(addSelector.selector, NEW_SELECTOR),
      delay >= 5d
    )
  → 等待 5d
  → TimelockRegular.execute(...)
  → FunctionSelectorWhitelist.addSelector(NEW_SELECTOR)
```

**TimelockEmergency 紧急调用路径（白名单 enforced）**：
...
SC (9/12 multisig, ≥75% + ≥4 外部签名 — Round 2 修正)
  ...
  → GuardedExecutor.executeCall(OPSuccinctL2OutputOracle, ...)
    require(whitelist.isWhitelisted(0xupdateVerifierSelector)) → pass
    emit EmergencyUpgradeExecuted(0x..., OPSuccinctL2OutputOracle, TimelockEmergency, data, ts)
...
- 唯一改 whitelist 的入口是 `FunctionSelectorWhitelist.addSelector/removeSelector` → 这两个函数 onlyOwner，owner = TimelockRegular → SC 必须走 ≥5d 路径才能扩展白名单；
- 因此 emergency 路径下，SC 即使想"提议把 `transferOwnership(address)` selector 加入 whitelist"，也得走 TimelockRegular ≥5d；在那 5 天里，用户可以退出。

**这就把"白名单是文档约束"升级为"白名单是合约层强制"**。

**(6.c) ProxyAdmin.owner 持有者设计（Round 2 修正版）**
...
| `OptimismPortal.GUARDIAN()` | **PauseTimelock**（详见 §Item-5 5.f）—— PauseTimelock 是唯一可 pause/unpause 的 caller，PauseTimelock 自身的 pause/unpause 入口由 GuardedExecutor 路由（SC 紧急 pause 走 GuardedExecutor，permissionlessUnpause 任何人调用） |
| `MantleEngineeringMultisig 3/7` (现 challenger / Guardian) | 阶段 1 后**解散**或**重新定位**为外部 Guardian Safe（成员严格外部）|

**关键性质（与架构 B'_naive 的区别）**：

| 性质 | 架构 B'_naive (Round 1) | 架构 B' (Round 2) |
|------|---------------------------|----------------------|
| target.owner | TimelockEmergency 直接持有 | **GuardedExecutor** 持有 |
| 白名单强制 | 仅文档约束 | **合约层 require** |
| 紧急路径 selector 范围 | 任意 onlyOwner | **仅白名单** |
...
**审计要求**：GuardedExecutor + FunctionSelectorWhitelist + PauseTimelock 三个新合约必须在阶段 1 部署前完成**独立审计**（建议两家独立审计机构 + Mantle 内部审计）。
...
- DAO 投票也作为 `FunctionSelectorWhitelist` 修改的唯一入口（必须走 TimelockRegular ≥5d）。
...
| **R-8** | 函数选择器白名单更新被滥用 | TimelockRegular 路径上 SC 提议扩展白名单 | 白名单更新需 DAO 投票 + ≥5d Timelock + 用户退出窗口；GuardedExecutor 合约层强制 require | `FunctionSelectorWhitelist.SelectorAdded/Removed` event |
...
### diag-2 — 推荐 Stage 1 双轨制升级架构图（架构 B'：双 TimelockController + GuardedExecutor 路由，Round 2 重写）
...
flowchart TB
    ...
    subgraph routing["Routing / Enforcement Layer (Round 2 新增)"]
        FSW[FunctionSelectorWhitelist<br/>owner = TimelockRegular<br/>update via ≥5d delay only]
        GE[GuardedExecutor<br/>require msg.sender ∈ TE TR<br/>if TE: require whitelist sel<br/>emit EmergencyUpgradeExecuted<br/>else if TR: any selector<br/>emit RegularUpgradeExecuted]
        PT[PauseTimelock<br/>= OptimismPortal.GUARDIAN<br/>MAX_PAUSE_DURATION = 30d<br/>permissionlessUnpause after expiry]

        FSW -.->|"whitelist read"| GE
        TE -->|"executeCall, whitelist enforced"| GE
        TR -->|"executeCall, any selector"| GE
        GE -.->|"pause path goes via"| PT
    end
    ...
    subgraph contracts["Target Contracts"]
        ...
        PORT2[OptimismPortal<br/>GUARDIAN = PauseTimelock<br/>pause via GE+TE whitelist<br/>unpause via GE+TE or<br/>permissionlessUnpause anyone after 30d]
    end

    GE -->|"whitelisted setters:<br/>pause/unpause/updateVerifier/<br/>updateAggregationVkey/<br/>updateRangeVkeyCommitment/<br/>updateRollupConfigHash"| OPO2
    GE -->|"any function via TR ≥5d:<br/>transferOwnership / addProposer<br/>/ updateSubmissionInterval"| OPO2
    TR -->|"upgradeAndCall any impl"| PA2
    PA2 -->|"admin"| PROXY
    PT -->|"pause / unpause"| PORT2
...
timeline
    ...
    2026-06 : Phase 1 start
            ...
            : Deploy GuardedExecutor + FunctionSelectorWhitelist
...
### diag-4 — Walkaway Test 场景下的用户强制退出路径流程图（Round 2 新增 Scenario B 分支）
...
flowchart TD
    ...
    Q4 -->|是| FAIL3["partial FAIL: 需 GuardedExecutor<br/>+ TE 白名单 updateVerifier 切换<br/>SC 消失情况下不可恢复<br/>除非 v3 允许 permissionless verifier"]
    ...
    Q0 -->|"Scenario B Round 2: 已 pause"| QB1{"当前 OptimismPortal<br/>是否有 auto-expiring pause?"}
    ...
    StepB1 --> StepB2["Step B-2: 任何人调用<br/>PauseTimelock.permissionlessUnpause<br/>→ OptimismPortal.unpause"]
...
flowchart LR
    ...
    subgraph mantle["Mantle Phase 1 Recommendation Round 2"]
        M1[SC: 9-12 members<br/>quorum 9/12 = 75%<br/>≥50% external 7/12 external<br/>≥4 external sigs in any quorum<br/>TimelockEmergency 0d + TimelockRegular ≥5d<br/>GuardedExecutor + FunctionSelectorWhitelist<br/>PauseTimelock 30d auto-expiry<br/>walkaway PASS target Scenario A + B]
    end
...
下表汇总 outline 的 7 个 source requirement 桶在 Round 2 draft 中的覆盖（Round 2 新增链上直读证据补强 src-1 / src-2）。
...
下表列出 Round 2 draft 中识别的 Gap，分为已闭合（CLOSED in Round 2）、已覆盖（Covered）与待 final 阶段补足（Open）三类。
...
| **G-14 (Round 2 新增)** | GuardedExecutor / FunctionSelectorWhitelist / PauseTimelock 三新合约独立审计 | Open（阶段 1 部署前 hard prerequisite）| §Item-6 / §Item-5 部署可执行性 | 阶段 1 部署前 |
