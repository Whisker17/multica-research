...
final_metadata:
  finalized_by: "agent:research-agent (Deep Research Agent, id=13a888db-49bb-4a19-9906-827729e156d9)"
  finalized_at: "2026-05-19T18:00:00Z"
  promoted_from_draft: mantle-stage1-rollup/research-sections/upgrade-exitwindow-securitycouncil/drafts/round-3.md
  promoted_from_commit: "2268922"
  adversarial_approval_comment: "eebdcdc9-37f1-4970-a8fc-6fc2142e2991"
  adversarial_verdict: "approve (confidence 0.84, no critical/major findings)"
  non_blocking_polish_applied:
    - "§Item-4 (4.a) phase table 阶段 1 deliverable (vi): Guardian 切换目标由 GuardedExecutor 改正为 PauseTimelock"
    - "§Item-4 (4.b) tx 顺序说明: 显式 canonical executable sequence = TargetSelectorWhitelist → GuardedExecutor → PauseTimelock → init whitelist (→ ProxyAdmin owner → OPSuccinctL2OutputOracle owner → Guardian → 后续 tx)，tx 标签保留为 tx-2-3/tx-2-4/tx-2-5/tx-2-6/..."
    - "§Item-8 (8.d) 失败/回滚路径 R-10 行: 紧急 pause 调用拼写改为 canonical PauseTimelock.pause(bytes32) 路由 (SC → TimelockEmergency → GuardedExecutor.executeCall(PauseTimelock, pause(bytes32), 0) → PauseTimelock.pause(bytes32) → OptimismPortal.pause())"
  upstream_finals:
    l2beat_stage_framework_2026: "commit d1834f9 — mantle-stage1-rollup/research-sections/l2beat-stage-framework-2026/final.md"
    mantle_architecture_2026: "commit 6025374 — mantle-stage1-rollup/research-sections/mantle-architecture-2026/final.md"
    stage1_case_studies: "commit 146ad79 — mantle-stage1-rollup/research-sections/stage1-case-studies/final.md"
---
...
| **阶段 1**（过渡态，2026.06–2026.08） | +1–2 月 | 部署 TimelockEmergency (minDelay=0) + TimelockRegular (minDelay≥5d) + **GuardedExecutor + auto-expiring pause wrapper** | (i) SC multisig 上线；(ii) 双 Timelock 部署；(iii) GuardedExecutor + TargetSelectorWhitelist 上线（(target, selector) pair）；(iv) 现有 ProxyAdmin owner 迁移到 TimelockRegular；(v) OPSuccinctL2OutputOracle owner 与 challenger 迁移到 `GuardedExecutor`（owner）与单独 Guardian Safe 或 SC 兼任（challenger）；(vi) **auto-expiring pause 改造**：`OptimismPortal.GUARDIAN` 从 MantleEngineeringMultisig 3/7 切换到 **`PauseTimelock`**（不是 GuardedExecutor —— Round 3 规范化调用链：`SC → TimelockEmergency → GuardedExecutor → PauseTimelock.pause(bytes32) → OptimismPortal.pause()`，PauseTimelock 作为 msg.sender 满足 `onlyGuardian` 并记录 `pausedAt`），pause 逻辑包装在 auto-expiry 模块（PauseTimelock，详见 §Item-5 5.f） |
...
tx-2-4: Deploy PauseTimelock (Round 3 规范化 — 作为 OptimismPortal.GUARDIAN)
  ...
  // 部署顺序约束：PauseTimelock 构造需要 GuardedExecutor 地址（immutable guardedExecutor）→ GuardedExecutor 必须先部署
  // GuardedExecutor 构造仅依赖 TimelockEmergency / TimelockRegular / TargetSelectorWhitelist —— 不依赖 PauseTimelock
  // ⚠️ Canonical executable sequence (Round 3, 不是 tx 标签顺序)：
  //     tx-2-3 (TargetSelectorWhitelist) → tx-2-5 (GuardedExecutor) → tx-2-4 (PauseTimelock) → tx-2-6 (init whitelist)
  //     → tx-2-7 (ProxyAdmin owner) → tx-2-8 (OPSuccinctL2OutputOracle owner) → tx-2-9 (Guardian → PauseTimelock)
  //     → tx-2-10 ~ tx-2-13
  // tx 标签 (tx-2-4 / tx-2-5) 保留为 Round 2/3 一脉相承的命名；部署脚本按上述依赖序列编排，不按 tx 标签数字顺序执行。
...
- ⚠️ **Canonical executable sequence (Round 3, 部署脚本依赖序列)**：`tx-2-3 (TargetSelectorWhitelist) → tx-2-5 (GuardedExecutor) → tx-2-4 (PauseTimelock) → tx-2-6 (init whitelist) → tx-2-7 (ProxyAdmin owner) → tx-2-8 (OPSuccinctL2OutputOracle owner) → tx-2-9 (Guardian → PauseTimelock) → tx-2-10 ~ tx-2-13`。tx 标签命名保留为 tx-2-4 (PauseTimelock) / tx-2-5 (GuardedExecutor)（沿用 Round 2/3 命名以便对账），但**实际执行顺序按依赖关系**：PauseTimelock 构造参数包含 `guardedExecutor` 地址（immutable），因此 GuardedExecutor 必须在 PauseTimelock 之前部署。本研究使用的 "executable sequence" 一律以此为准（§Item-3 (3.f) 迁移步骤 1-9、§Item-8 (8.a) 时间线、diag-3 部署节点排列均按此依赖序列）。
...
| 阶段 1 部署后发现 GuardedExecutor 严重 bug（R-10）| 回滚到阶段 0（恢复 MantleSecurityMultisig 6/14 直接控制）| 1. 紧急 pause (canonical chain, Round 3)：`SC 9/12 → TimelockEmergency → GuardedExecutor.executeCall(PauseTimelock, abi.encodeWithSelector(PauseTimelock.pause.selector, reason), 0) → PauseTimelock.pause(bytes32) → OptimismPortal.pause()` —— PauseTimelock 作为 msg.sender 满足 `OptimismPortal.onlyGuardian` 并记录 `pausedAt`（**绝不直接** `GuardedExecutor → OptimismPortal.pause()`，否则在 `onlyGuardian` 上 revert）；2. DAO 投票 + TimelockRegular ≥5d 后 OPSuccinctL2OutputOracle.transferOwnership(MantleSecurityMultisig)（回退）+ ProxyAdmin.transferOwnership(MantleSecurityMultisig)；3. 修复 GuardedExecutor / 重新部署；4. 重新执行阶段 1 | 紧急 pause: SC ≥75% + ≥4 外部；回滚 ownership：DAO + TimelockRegular ≥5d | 紧急 pause 走 TimelockEmergency；回滚 ownership 走 TimelockRegular |
...
| **Final Promotion** | **final** | **2026-05-19** | Promote from approved round-3 draft (commit `2268922`, adversarial approval comment `eebdcdc9-37f1-4970-a8fc-6fc2142e2991`, verdict approve, confidence 0.84, no critical/major findings). 3 non-blocking editorial polish items applied: (a) §Item-4 (4.a) phase table — 阶段 1 deliverable (vi) Guardian 切换目标由 stale `GuardedExecutor` 改正为 canonical `PauseTimelock`（与 §Item-5 5.f / §Item-6 6.c / §Item-3 (3.f) step 9 一致）；(b) §Item-4 (4.b) tx 顺序说明 — 显式 canonical executable sequence = `TargetSelectorWhitelist → GuardedExecutor → PauseTimelock → init whitelist → ProxyAdmin owner → OPSuccinctL2OutputOracle owner → Guardian → 后续 tx`，并在 tx-2-4 内联注释中明确 PauseTimelock 构造依赖 GuardedExecutor 地址 → GuardedExecutor 必须先部署；tx 标签数字保留为对账之用，不作为执行顺序；(c) §Item-8 (8.d) 失败/回滚 R-10 行 — 紧急 pause 调用拼写改为 canonical `PauseTimelock.pause(bytes32)` 路由（`SC → TimelockEmergency → GuardedExecutor.executeCall(PauseTimelock, pause(bytes32), 0) → PauseTimelock.pause(bytes32) → OptimismPortal.pause()`），消除"直接调用 OptimismPortal.pause"的 stale 表述。Frontmatter `status: draft → final`，新增 `final_metadata` block 记录 finalized_by / finalized_at / promoted_from_draft / promoted_from_commit / adversarial_approval_comment / adversarial_verdict / non_blocking_polish_applied / upstream_finals。无内容性变更：所有 round-3 已定稿章节（§Item-1 ~ §Item-8 主体、Source Coverage、6 个 diagram、Gap Analysis）保留原文。 |
