...
artifact_paths:
  ...
  draft: "mantle-github-actions/research-sections/mantle-baseline/drafts/round-1.md"
...
3. **上游同步自动化不足**：仅 reth 和 kona 有自动化上游同步机制，op-geth、op-succinct、mantle-v2 完全缺失。
...
├──────────────────────────┼────────┼────────┼─────────┼─────────────┼───────────┤
│ 1. Upstream Auto-Sync    │ 成熟   │ 基础   │ 缺失    │ 缺失        │ 缺失      │
...
| 仓库 | 评级 | 依据 |
|------|------|------|
| reth | **成熟** | 3 个相关工作流：`sync.yml`（每 6 小时同步测试）、`sync-era.yml`（ERA 模式同步）、`update-superchain.yml`（每周自动获取 OP superchain 配置并创建 PR）。此外 `dependencies.yml` 每周更新依赖。 |
...
| 仓库 | 独特优势 |
|------|----------|
| reth | 最全面的自动化覆盖（29 个工作流，7/10 维度成熟）；可复现构建验证；Mantle 定制发布流水线与上游发布流水线并存；CodSpeed 持续性能追踪 |
...
| 差距 | 影响范围 | 严重程度 |
|------|----------|----------|
| **上游同步自动化缺失** | op-geth, op-succinct, mantle-v2（3/5 仓库） | 高 — fork 仓库需要定期同步上游安全补丁和功能更新 |
...
| src-4（GitHub API） | ≥5 | 5（每个仓库均查询了 branch protection、environments、secrets、repo settings） | ✅ 通过 |
