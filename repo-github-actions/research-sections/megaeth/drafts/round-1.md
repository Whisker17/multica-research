...
artifact_paths:
  ...
  draft: "github-actions-optimization/research-sections/megaeth-github-actions/drafts/round-1.md"
...
- 所有 CI workflow 使用 `concurrency` 组 + `cancel-in-progress: true` 避免重复运行
...
- **超时**：45 分钟
- **并发**：与其他 workflow 相同的 cancel-in-progress 策略
- **Codecov 配置**：
...
| `CODECOV_TOKEN` | Codecov 覆盖率上传 | stateless-validator |

> 注：mega-evm 的 Codecov 未使用 token（可能依赖 GitHub App 集成或 public repo 无需 token）。
