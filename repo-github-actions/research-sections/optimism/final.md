...
round: 2
status: final
promoted_from: "github-action-optimization/research-sections/optimism-github-actions/drafts/round-2.md"
promoted_at: "2026-06-10T15:00:00Z"
approved_draft_commit: "fab96021b90ed4d8ba965baa21112fc2c9a3eb3b"
approval_evidence: "comment cfb30eb4-0be5-4359-a232-50c9779d5e59 (Review Verdict: approve, severity: none, round 2)"

artifact_paths:
  outline: "github-action-optimization/outlines/optimism-github-actions.md"
  draft: "github-action-optimization/research-sections/optimism-github-actions/drafts/round-2.md"
...
draft_metadata:
  ...
  revised_at: "2026-06-10T14:30:00Z"
  codebase_commit: "f6abdbd2bc4b40d3e52e39076dd3b4b4b8e1a80e"
  codebase_path: "/Users/whisker/Work/src/networks/optimism/optimism/"
  factory_commit: "25fa2ff591f9b4801d3dcadffd2b76ec2340cf8a"
  factory_repo: "ethereum-optimism/factory"
  ...
  round2_corrections_applied:
    - "item-1/item-3: factory build-flow logic corrected from direct source analysis — PR→affected only, non-PR non-tag→all images, tag push→single-entry matrix from tag name"
    - "item-3: factory section rewritten with direct citations from actions/plan/action.yml, .github/workflows/docker.yaml, README.md at commit 25fa2ff"
    - "src-3: upgraded from inference to direct analysis with file-level citations"
    - "gap-1: factory repo gap removed (direct analysis completed)"
---
...
- GHA 侧采用 **数据驱动的工厂模式** (factory pattern) 管理 23 个 Docker 镜像的构建，以 `.github/images.json` 为声明式清单，配合 `docker-bake.hcl` 和 `ethereum-optimism/factory` 可复用工作流（直接分析 commit `25fa2ff`），实现了从单一 workflow 文件管理全部镜像的高度抽象。Factory 的 `actions/plan` 根据事件类型采用三级策略：PR 仅构建受影响镜像、develop push/schedule 构建全部镜像、tag push 精确发布单个镜像。
...
注意：factory `actions/plan` 根据事件类型采用三种不同的构建策略（见 `actions/plan/action.yml` 第 7–14 行描述及对应代码段，commit `25fa2ff`）：

| 事件类型 | plan action 行为 | 构建范围 |
|---------|-----------------|---------|
| `pull_request` | diff PR base..head，匹配 `paths` 和 `shared_paths` | 仅受影响的镜像（若 `shared_paths` 命中则全量） |
| 非 PR 非 tag（`push` 到 `develop`、`schedule`、`workflow_dispatch`） | 调用 `build_full_matrix`，忽略 diff | **所有镜像**（完整矩阵） |
| tag push（格式 `<image_name>/v<semver>`） | 解析 tag 名称，查找对应 image | **单个镜像**，版本从 tag 注入 |

因此 `push` 到 `develop` 和 `schedule` 触发时**不执行变更检测**，始终构建全部 23 个镜像；路径过滤仅在 `pull_request` 事件时生效。

#### 工作流结构（4 个 Jobs）

**Job 1: `plan`** — 动态矩阵生成

- 使用 `ethereum-optimism/factory/actions/plan@25fa2ff...` 读取 `.github/images.json`，根据事件类型决定构建范围（源码: `actions/plan/action.yml`，commit `25fa2ff`）
- **PR 事件**: diff base..head，逐镜像匹配 `paths` + `shared_paths` / `shared_<type>_paths`，仅受影响的镜像进入矩阵
- **非 PR 非 tag 事件**: 调用 `build_full_matrix()`，所有镜像进入矩阵
- **Tag push 事件**: 解析 `<image_name>/v<semver>` tag，调用 `build_release_entry()`，单个镜像进入矩阵并注入 VERSION 到 env/build_args
- 输出: `matrix_json`（构建矩阵）、`has_builds`（是否有构建任务）、`is_release`（是否为 tag 发布）
...
#### ethereum-optimism/factory — 可复用工作流（直接源码分析）

> **Factory commit**: `25fa2ff591f9b4801d3dcadffd2b76ec2340cf8a`
> **分析来源**: 直接读取 `actions/plan/action.yml`、`.github/workflows/docker.yaml`、`README.md`

Factory 仓库提供构建编排的两个核心组件：

**1. `actions/plan/action.yml`** — 构建矩阵计划生成器

composite action，根据 GitHub 事件类型决定构建范围。核心逻辑（引自 action.yml 的 description 和 shell 脚本）：

> *"On pull_request events it diffs the PR against its base and emits a matrix of only the affected images. On any other push event to a non-tag ref it emits all images. On a tag push it emits a single-entry matrix by parsing the tag into image_name and version."*
> — `actions/plan/action.yml`, description 字段

三种执行路径的实现：

| 路径 | 代码入口 | 行为 |
|------|---------|------|
| **Tag push** | `if [[ "$REF_TYPE" == "tag" ]]` → `build_release_entry()` | 解析 `<image_name>/v<semver>` tag，查找 config 中对应 image，注入 VERSION 到 env/build_args，输出单条目矩阵。单镜像 repo 也接受裸 `v<semver>` tag |
| **Non-PR** | `if [[ "$EVENT_NAME" != "pull_request" ]]` → `build_full_matrix()` | 不执行 diff，直接枚举 config 中所有 images，全部进入构建矩阵 |
| **PR** | 剩余分支 → `git diff` + `matches_pattern()` | 计算 `BASE_SHA...HEAD_SHA` diff，逐文件匹配 `shared_paths`（命中则全量）→ `shared_<type>_paths`（命中则同类型全量）→ 各镜像 `paths`（命中则该镜像），仅受影响的镜像进入矩阵 |

输入/输出:
- 输入: `config`（默认 `.github/images.json`）
- 输出: `matrix_json`（构建矩阵数组）、`groups_matrix_json`（apko group 矩阵）、`has_builds`、`has_group_builds`、`is_release`

**2. `.github/workflows/docker.yaml`** — 可复用 Docker 构建工作流

`workflow_call` 触发的 reusable workflow，包含 4 个 jobs: `safety-checklist` → `build` → `merge` → `attest`。

支持四种构建模式（`inputs.mode`）：
- `build` — 标准 Dockerfile（`docker/build-push-action`）
- `bake` — docker buildx bake（`docker/bake-action`），Optimism 使用此模式
- `ko` — Go 静态二进制（`ko-build/setup-ko`）
- `apko` — 声明式 Wolfi/melange 镜像

关键特性：
- **多平台构建**: 默认 amd64 + arm64 并行构建，通过 `merge` job 创建 multi-arch manifest（`docker buildx imagetools create`）
- **Fork PR 安全隔离**: 检测 `pull_request` 事件中 `head_repo != base_repo`，fork PR 推送到 `ttl.sh/${SHA}` 临时仓库而非正式 registry（源码: `safety-checklist` job 的 `fork_ctx` step）
- **GCP OIDC 认证**: 通过 `google-github-actions/auth` + Workload Identity Federation，无长期密钥
- **SLSA 溯源**: `attest` job 使用 `actions/attest-build-provenance` 为最终 multi-arch manifest 生成溯源证明
- **构建缓存**: 使用 GitHub Actions cache (`type=gha`)，按 `image_name-platform` 分区，仅 protected branch 写入缓存

**3. `README.md`** — 使用文档

确认了 `images.json` 的契约：
- `shared_paths` — 任何匹配触发全部镜像重建
- `shared_<type>_paths` — 任何匹配触发同 `type` 的全部镜像重建
- per-image `paths` — PR diff 匹配时触发该镜像构建
- Tag 格式: `<image_name>/v<semver>`；单镜像 repo 可用裸 `v<semver>`
- 还提供 `integration-test.yaml` 可复用工作流用于服务级集成测试（Optimism 当前未在 build-images.yaml 中使用）

#### 架构优势

- **单一变更点**: 新增镜像只需在 `images.json` 添加条目 + `docker-bake.hcl` 添加 target，无需修改 workflow
- **三级智能构建策略**: PR 仅构建受影响镜像（最小化 CI 资源）、develop push/schedule 全量构建（保证镜像新鲜度）、tag push 精确发布单个镜像
- **安全性**: 所有第三方 action 通过 SHA 固定，attestation 自动生成，fork PR 隔离到临时仓库
...
- 三级智能构建策略（factory `actions/plan` 直接源码确认）：PR→仅受影响镜像，非 PR 非 tag→全量，tag push→单镜像发布
- 跨平台验证 (amd64 + arm64)
- 外部 fork PR 使用 ttl.sh 临时仓库（安全隔离）
- 每日 schedule 及 develop push 全量构建保证镜像新鲜度
...
- **采用路径**: 创建 `mantle-images.json` 声明所有镜像，编写单一 `build-images.yaml`，可直接复用或 fork `ethereum-optimism/factory`（已验证其三级构建策略: PR→增量, push/schedule→全量, tag→单镜像发布）
...
flowchart TD
    ...
    P1 --> P2
    P2 --> P2a
    P2a -->|pull_request| P2b
    P2a -->|push/schedule/dispatch| P2c
    P2a -->|tag push| P2d
    P2b & P2c & P2d --> P3
...
|---------|------|---------|---------|---------|---------|
| src-1 | code_analysis | 直接分析 workflow/action/CircleCI 配置文件 | 10 | 14+ | ✅ 满足 |
| src-2 | official_docs | GitHub Actions / CircleCI 文档 | 3 | 3 | ✅ 满足 |
| src-3 | code_analysis | `ethereum-optimism/factory` 仓库分析 | 1 | 3 | ✅ 满足 (直接分析 actions/plan/action.yml, .github/workflows/docker.yaml, README.md @ 25fa2ff) |
...
### ethereum-optimism/factory 直接分析 (src-3)

直接读取 `ethereum-optimism/factory` 仓库 commit `25fa2ff591f9b4801d3dcadffd2b76ec2340cf8a` 的三个核心文件：

15. `actions/plan/action.yml` — 完整逐行分析（composite action，~280 行 bash）。三路构建策略：PR→diff 匹配受影响镜像；非 PR 非 tag→全量矩阵；tag push→单镜像发布矩阵（解析 `<image_name>/v<semver>`）。支持 `shared_paths`（全局触发）、`shared_<type>_paths`（类型级触发）、per-image `paths`（镜像级触发）三级路径匹配。
16. `.github/workflows/docker.yaml` — 完整分析（reusable workflow，4 jobs: safety-checklist → build → merge → attest）。四种构建模式（build/bake/ko/apko），多平台构建 + manifest merge，GCP OIDC 认证，fork PR ttl.sh 隔离，SLSA 溯源。
17. `README.md` — 完整分析。确认 images.json 契约、tag 格式、集成测试框架。
...
> **Round 2 改进**: Factory 仓库 (`ethereum-optimism/factory`) 已在本轮直接分析，不再是 gap。详见 src-3 源文件清单。
...
| 2 (修订) | 修订 | 根据 draft review 的两项 major finding 修订：(1) **Factory 源码完整性**——直接分析 `ethereum-optimism/factory` 仓库 commit `25fa2ff`，读取 `actions/plan/action.yml`、`.github/workflows/docker.yaml`、`README.md`，替换 round-1 中的推断式描述为直接源码引用。(2) **构建流程事实纠正**——修正 item-1 和 item-3 中的构建触发逻辑：PR 事件→仅受影响镜像，非 PR 非 tag 事件→全部镜像，tag push→解析 tag 名称的单镜像发布。更新 src-3 源覆盖从推断升级为直接分析，移除 factory 未直接访问的 gap。 |
