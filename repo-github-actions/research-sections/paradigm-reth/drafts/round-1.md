...
artifact_paths:
  ...
  draft: "github-actions-optimization/research-sections/paradigm-reth/drafts/round-1.md"
...
3. **安全供应链** 实践一流：所有 Action 均 SHA 全量钉选、`permissions: {}` 默认拒绝、`persist-credentials: false` 普遍应用、GPG 签名发布制品。
...
| 属性 | 值 |
|------|-----|
| **触发器** | `issue_comment` (created), `workflow_dispatch` (13 个输入参数) |
| **关键 Job** | `bench-ack` (参数解析+队列管理), `bench-txgen` (构建+调优+执行+报告) |
| **Runner** | `[self-hosted, Linux, X64, available]` |
| **超时** | 180 分钟 |
| **并发** | `group: reth-bench, cancel-in-progress: false` (队列模式) |

**输入参数** (13 个):
- `blocks`: 重放区块数量
- `big_blocks`: 大区块模式 (gas 目标如 100M/2G)
- `bal`: Block Access List 重放
- `warmup`: 预热区块数
- `baseline` / `feature`: 比较的 git ref
- `baseline_args` / `feature_args`: 每侧额外 CLI 参数
- `wait_time`: 等待时间
- `samply`: samply CPU profiling 开关
- `repetitions`: 重复次数 (用于 ABBA)

**ABBA 交叉对比**:
当 `repetitions=2` 时，执行顺序为 baseline-1 → feature-1 → feature-2 → baseline-2 (BFFB)，交错排列控制热漂移和缓存效应。这是一种科学基准方法论，在开源区块链项目中极为罕见。
...
**注意**: `tempoxyz/ci` 引用 `@main` 而非 SHA 钉选，这是唯一未 SHA 钉选的 workflow 引用。
...
| 5 | **安全与供应链** | 成熟 | 全部 Action SHA 钉选, `permissions: {}` 默认拒绝, `persist-credentials: false` 普遍应用, GPG 签名, Cyclops 审计, 最小 OIDC 范围 |
...
| **P1** | 全部 Action SHA 钉选 + `persist-credentials: false` | 低 | 高 | 安全基线 |
