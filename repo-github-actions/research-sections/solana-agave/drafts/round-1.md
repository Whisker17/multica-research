...
revision_metadata:
  ...
  last_modified_at: "2026-06-10T15:30:00Z"
---
...
**来源**: [.github/workflows/publish-windows-tarball.yml](https://github.com/anza-xyz/agave/blob/3477fa2534a7848446ada6b0e126c525a57d735b/.github/workflows/publish-windows-tarball.yml) @ `3477fa25`
...
flowchart TD
    ...
    F --> G[维护者手动 push tag]
    G --> H{release.yml 触发}
    H --> I[Buildkite secondary pipeline<br/>构建 Linux/macOS 产物]
    H --> J[创建 GitHub prerelease<br/>body: 🚧]
    I --> K[Linux/macOS tarball<br/>上传到 GCS + Release]
    G --> L[维护者手动触发<br/>publish-windows-tarball.yml]
    L --> M[Windows 构建]
    M --> N[上传到 GCS]
    M --> O[附加到 GitHub Release]
    J --> P[维护者编辑 release notes]
```

**Agave 独特模式**: Windows 构建与 Linux/macOS 构建完全分离——后者由 Buildkite 处理（不在 GitHub Actions 中），而 Windows 使用独立的手动触发 workflow。这反映了 Buildkite 不原生支持 Windows runner 的限制。
...
| `ci/publish-tarball.sh` | 构建 release tarball | publish-windows-tarball.yml |
...
flowchart TB
    ...
    subgraph BK["Buildkite"]
        ...
        BK2[agave-secondary pipeline<br/>Release 构建]
    end

    subgraph Bridge["桥接 Workflow"]
        T[trigger-buildkite-pipeline.yml]
        R[release.yml]
    end
...
║                          ║          ║ CI 结果通知等高级功能                     ║
╠══════════════════════════╬══════════╬═══════════════════════════════════════════╣
║ External CI Integration  ║ 成熟     ║ 完整的 GHA→Buildkite 桥接；              ║
║                          ║          ║ 双路径授权（member/label）；              ║
║                          ║          ║ 安全的 pull_request_target 模式           ║
╚══════════════════════════╩══════════╩═══════════════════════════════════════════╝
...
| 5 | **双路径 Buildkite 授权** | `trigger-buildkite-pipeline.yml` | member 自动触发 + CI label 手动触发的模式兼顾安全性和外部贡献者体验 | 如果 Mantle 使用外部 CI 系统，此授权模型直接可用 |
...
#### 5. Windows 构建隔离
Windows 二进制通过独立的手动触发 workflow 构建，与 Linux/macOS 的 Buildkite 构建完全分离。这反映了：
- Buildkite 生态对 Windows runner 的有限支持
- Windows 用户在 Solana 验证器生态中的相对小众地位
- 通过隔离降低 release 流程的复杂度
