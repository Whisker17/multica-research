...
artifact_paths:
  ...
  draft: "github-action-opt/research-sections/tempo-github-actions/drafts/round-1.md"
...
draft_metadata:
  ...
  source_coverage:
    ...
    src-5: "0 (GitHub API not attempted — best effort, noted as inaccessible)"
...
在 10 个改进维度评估中，Tempo 在 Upstream Auto-Sync、Benchmark/Performance Regression、Release Pipeline 三个维度达到"成熟"评级，在 AI Code Review、PR Audit、CI/Testing、Security & Supply Chain、Interactive Agent 五个维度达到"基础"评级，PR Governance 和 Documentation & Infrastructure 维度也达到"基础"水平。没有任何维度评为"缺失"——这在同类项目中极为罕见。
...
两个 repo 均无 `PULL_REQUEST_TEMPLATE.md`。以下配置需 GitHub API 访问，未调查：
- Branch protection rules / repo rulesets
- GitHub Environments 配置
- 已配置的 GitHub Apps（已知 sync-from-upstream 使用 GitHub App）
- Secrets 名称列表
...
| **Release Pipeline** | 成熟 | release.yml series (5 files) | GPG + SLSA v1 + SBOM (SPDX-JSON) + cosign Docker 签名。3 平台 matrix、确定性构建(reproducible-build.yml)、keyless OIDC crate 发布、R2 分发、自动 changelog PR | 极高——完整安全链 |
...
| **Security & Supply Chain** | 基础 | dependabot.yml, release.yml, docker.yml | 三生态 dependabot (7 天 cooldown)、SLSA attestation、SBOM、cosign。但无 CodeQL/SAST、无 secret scanning workflow、token 隔离靠约定非强制 | 中——签名链完备但缺 SAST |
...
| **PR Governance** | 基础 | label-pr.yml, stale.yml, semver-check.yml, sync-from-upstream.yml | 自动标签、7+3 stale、semver check、fork sync。但无 PR template、无 required reviewers workflow、无 merge queue 配置 workflow | 低——标准实践 |
...
4 个"成熟" + 6 个"基础" + 0 个"缺失"。Tempo 的 CI/CD 体系在所有评估维度都至少达到基础水平，在 AI 驱动和性能测试领域处于行业领先。
...
| src-5: GitHub API (best effort) | ⚠️ 0 | 未尝试——Branch protection, Environments, Apps, Secrets 不可访问，标注为"不可访问 (权限受限)" |
...
1. **GitHub API 配置不可访问**：Branch protection rules、GitHub Environments 详细配置、GitHub Apps 列表、Secrets 名称列表均未调查。这些信息有助于更完整地理解安全策略，但不影响 workflow 分析的核心结论。
